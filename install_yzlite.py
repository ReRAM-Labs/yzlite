...
if _version[0] != 3 or _version[1] < 7 or _version[1] >= 11 or _architecture_bits != '64bit':
    sys.stdout.write(
        f'Cannot install YZLITE, Python 64-bit, version 3.7, 3.8, 3.9, 3.10 is required (current version is: Python {_architecture_bits}  v{_version[0]}.{_version[1]})\n')
    sys.stdout
    sys.exit(-1)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Utility to install the yzlite Python package for local development')
    parser.add_argument('--python',
                        help='Path to python executable used to install the yzlite package. If omitted, use the current python executable',
                        default=None
                        )
    parser.add_argument('--repo-path',
                        help='Path to yzlite git repo. This is used by the --dev option. If omitted, use same directory as this script',
                        default=None
                        )
    ...
    parser.add_argument('--extras',
                        help='The additional YZLITE dependencies to install',
                        default='full',
                        choices=['full', 'dev', 'none']
                        )

    args = parser.parse_args()
    install_yzlite_for_local_dev(
        python=args.python,
...
def install_yzlite_for_local_dev(
    python: str = None,
    repo_path: str = None,
    verbose: bool = False,
    extras: str = 'full'
):
    """Install the yzlite for local development

    Args:
        python: Path to python executable used to install the yzlite package. If omitted, use the current python executable
        repo_path: Path to yzlite git repo. If omitted, use same directory as this script
    ...
    logging.info('Installing yzlite for local development')
    logging.info(f'  Python: {python}')
    logging.info(f'  yzlite git repo: {repo_path}')

    repo_setup_py_path = f'{repo_path}/setup.py'
    if not os.path.exists(repo_setup_py_path):
        logging.error(
            'yzlite git repo does not have a setup.py script. Is this a valid yzlite repo?')
        sys.exit(-1)
    ...
    try:
        ...
        raise Exception(
            f'{err_msg}{additional_msg}')  # pylint: disable=raise-missing-from
    ...
    logging.info(
        'Installing the "wheel" Python package into the virtual environment')
    issue_shell_command(python_venv_exe, '-m', 'pip',
                        'install', 'wheel', '--upgrade')

    if install_dev_requirements:
        logging.info('Installing development requirements')
        issue_shell_command(python_venv_exe, '-m', 'pip', 'install',
                            '-r', f'{repo_path}/cpp/tools/utils/dev_requirements.txt')

    logging.info(f'Installing YZLITE into {venv_dir} ...')
    logging.info('(Please be patient, this may take awhile)')
    try:
        env = os.environ.copy()
        cmd = [python_venv_exe, '-m', 'pip', 'install']
        if verbose:
            env['YZLITE_VERBOSE_INSTALL'] = '1'
            cmd.append('-v')
        if 'PYTHONHOME' in env:
            del env['PYTHONHOME']
        env['PATH'] = os.path.dirname(
            python_venv_exe) + os.pathsep + env['PATH']
    ...
    logging.info('Done\n\n')
    logging.info('The YZLITE has successfully been installed!\n')
    logging.info(
        'Issue the following command to activate the YZLITE Python virtual environment:')
    if os.name == 'nt':
        if shutil.which('source'):
            logging.info(f'source {venv_dir}/Scripts/activate\n')
        else:
            logging.info(venv_dir.replace('/', '\\') +
...
def get_python_version(python: str) -> str:
    if os.name == 'nt':
        python = python.replace('/', '\\')
    python_version_raw_str = subprocess.check_output(
        [python, '--version'], text=True)
    match = re.match(r'.*\s(\d+).(\d+).(\d+)', python_version_raw_str.strip())
    if not match:
        logging.error(
            f'Failed to get Python version from {python_version_raw_str}')
        sys.exit(-1)
...
def issue_shell_command(*args, env=None, cwd=None):
    ...
    try:
        p = subprocess.Popen(
            ...
            shell=False,
            # convert the shell output to a string (instead of bytes)
            text=True,
            close_fds=True,
            env=env,
            cwd=cwd
