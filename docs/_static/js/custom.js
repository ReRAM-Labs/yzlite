...
function initializeSurvey() {
    ...
    if (
        localStorage.ignoredSurveyTimestamp &&
        now - parseFloat(localStorage.ignoredSurveyTimestamp) > window.IGNORED_SURVEY_TIMEOUT
    ) {
...
function updateSurveyActivity() {
    ...
    if (elapsed > 5 * 60) {
        // If more than 5min elapsed, then assume the user walked away so ignore this activity
        return
    }
