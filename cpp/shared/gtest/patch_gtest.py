...
def _patch_line(line, new_value):
    new_line = '#ifdef __arm__   // Patched by YZLITE\n'
    new_line += new_value + ' // Patched by YZLITE\n'
    new_line += '#else  // Patched by YZLITE\n'
    new_line += line
    new_line += '#endif // Patched by YZLITE\n'
    return new_line


def _check_if_already_patched(line):
    if '// Patched by YZLITE' in line:
        return None
    else:
        return '// Patched by YZLITE\n' + line
...
def process_internal_gtest_port_h(lineno: int, line: str, context: dict) -> str:
    ...
    if line.strip().startswith('inline int FileNo(FILE* file)'):
        ...
        line = _patch_line(
            line, 'inline char* StrDup(const char* src){return nullptr;}')

    elif line.strip().startswith('inline FILE* FDOpen(int fd, const char* mode)'):
        line = _patch_line(
            line, 'inline FILE* FDOpen(int fd, const char* mode){return nullptr;}')
...
def process_gtest_internal_inl_h(lineno: int, line: str, context: dict) -> str:
    ...
    if context['state'] == 0 and line.strip() == 'if (original_working_dir_.IsEmpty()) {':
        context['state'] = 1
        line = '#ifndef __arm__ // Patched by YZLITE\n' + line
...
def process_gtest_cc(lineno: int, line: str, context: dict) -> str:
    if lineno == 0:
        context['state'] = {'force_color': 0, 'color': 0, 'flags': 0,
                            'config': 0, 'output': 0, 'unstatic printf': 0, 'update_color': 0}
        return _check_if_already_patched(line)
    ...
    if state['force_color'] == 0 and line.strip() == 'static const bool in_color_mode =':
        state['force_color'] = 1
        return 'static const bool in_color_mode = true; // Patched by YZLITE\n'
    ...
    if state['unstatic printf'] == 0 and line.strip() == 'static void ColoredPrintf(GTestColor color, const char *fmt, ...) {':
        ...
        return 'void ColoredPrintf(GTestColor color, const char *fmt, ...) { // Patched by YZLITE\n'

    if state['color'] == 0 and line.strip() == 'if (!use_color) {':
        state['color'] = 1
        return '(void)use_color;\n#ifndef __arm__ // Patched by YZLITE\n' + line
    ...
    if state['flags'] == 0 and line.strip() == 'ParseGoogleTestFlagsOnly(argc, argv);':
        state['flags'] = 1
        return '#ifndef __arm__ // Patched by YZLITE\n' + line
    ...
    if state['config'] == 0 and line.strip() == 'void UnitTestImpl::ConfigureXmlOutput() {':
        state['config'] = 1
        return line + '#ifndef __arm__ // Patched by YZLITE\n'
    ...
    if state['update_color'] == 0 and line.strip() == 'namespace internal {':
        ...
        if line.strip() == 'namespace {':
            ...
            return '// namespace { // Patched by YZLITE\n'
        else:
