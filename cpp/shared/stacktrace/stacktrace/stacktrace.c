...
static int addr2line(const char *addr, const char *program_path) {
  ...
  snprintf(&cmd_buffer[cmd_buffer_offset],
           sizeof(cmd_buffer) - cmd_buffer_offset, " " ADDR2LINE_ARGS,
           program_path, addr);
  ...
  char *unused = fgets(out_buffer, sizeof(out_buffer), fp);
  (void)unused;
  pclose(fp);
  ...
  if ((strstr(out_buffer,
    ...
    if (strstr(out_buffer, "is not recognized") != NULL) {
      printf("HINT: Add the compiler's \"bin\" directory to the environment "
             "\"PATH\" to view the full stacktrace of this exception\n");
    }
...
static void windows_print_stacktrace(CONTEXT *context) {
  ...
  while (StackWalk(IMAGE_FILE_MACHINE, GetCurrentProcess(), GetCurrentThread(),
                   &frame, context, 0, SymFunctionTableAccess, SymGetModuleBase,
                   0)) {
...
static int os_init() {
  ...
  if (stat(addr2line_path, &buffer) != 0) {
    ...
    if (USERPROFILE_DIR != NULL) {
      snprintf(addr2line_path, sizeof(addr2line_path),
               "%s\\.yzlite\\tools\\toolchains\\gcc\\windows\\8.4."
               "0\\mingw64\\bin\\addr2line.exe",
               USERPROFILE_DIR);
      if (stat(addr2line_path, &buffer) != 0) {
  ...
  if (GetModuleHandleEx(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS |
    ...
    if (GetModuleFileName(hModule, stacktrace_program_path,
                          sizeof(stacktrace_program_path))) {
...
static int os_init() {
  ...
  /* register our signal handlers */
  {
    ...
    if (sigaction(SIGIOT, &sig_action, NULL) != 0) {
      err(1, "sigaction");
    }
    if (sigaction(SIGTRAP, &sig_action, NULL) != 0) {
      err(1, "sigaction");
    }
  ...
  int result = readlink("/proc/self/exe", stacktrace_program_path,
                        sizeof(stacktrace_program_path));
  (void)result;
...
#define MAX_STACK_FRAMES 64
  ...
  for (i = 0; i < trace_size;
       ++i) // we'll use this for now so you can see what's going on
  {
