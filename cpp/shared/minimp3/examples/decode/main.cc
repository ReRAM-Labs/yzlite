...
#define SPEAKER_FRAMES_PER_DMA                                                 \
  ((MAX_XFR_PER_DMA / MPEG2_FRAME_LENGTH) * MPEG2_FRAME_LENGTH)
...
static void convert_pcm_to_vdac(const int16_t *pcm_samples, int n_samples,
                                int channels);
...
extern "C" int main(void) {
  ...
  speaker.vdac_buffer_end =
      &speaker.vdac_buffer_ptr[SPEAKER_DMA_COUNT * SPEAKER_FRAMES_PER_DMA];
  ...
  int n_samples =
      mp3dec_decode_frame(&mp3d, mp3_ptr, data_remaining, pcm_samples, &info);
  ...
  while (data_remaining > 0) {
    while (speaker.n_frames_pending >=
...
/*************************************************************************************************/
static bool vdac_dma_callback(unsigned int channel, unsigned int sequenceNo,
                              void *userParam) {
