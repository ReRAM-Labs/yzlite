...
from yzlite.core.tflite_model_parameters import schema
from yzlite.utils.shell_cmd import run_shell_cmd

curdir = os.path.dirname(os.path.abspath(__file__))


tflite_model_parameters_schema_dir = os.path.dirname(
...
retcode, retmsg = run_shell_cmd(
    [flatc, '--cpp', '--scoped-enums', '-o', output_header_dir, fbs_path])
if retcode != 0:
...
with open(output_header_path, 'r') as fp:
    for line in fp:
        if line.strip().startswith('FLATBUFFERS_VERSION_REVISION == 8'):
            # Tensorflow-Lite Micro currently expects Flatbuffers 2.0.6
            line = line.replace('8', '6')
        ...
        if line.startswith('#endif  // FLATBUFFERS_GENERATED_DICTIONARY_H_'):
            output_header_modified += '} // namespace schema\n'
            output_header_modified += '} // namespace yzlite\n\n'
            output_header_modified += line
            continue
