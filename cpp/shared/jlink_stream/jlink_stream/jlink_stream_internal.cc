...
/*************************************************************************************************/
void jlink_stream_internal_init(uint32_t *trigger_address_ptr,
                                uint32_t *trigger_value_ptr,
...
void jlink_stream_set_interrupt_enabled(bool enabled) {
  if (enabled) {
    ...
    __DSB(); // Ensure all data is synchronized before continuing with
             // interrupts disabled
  }
}

/*************************************************************************************************
 * This interrupt is triggered from an external script. The script uses the ARM
 * NVIC software interrupt (NVIC->STIR) register to trigger this interrupt.
 */
