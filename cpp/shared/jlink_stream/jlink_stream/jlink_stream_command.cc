...
/*************************************************************************************************/
JlinkStreamCommand::JlinkStreamCommand(const char *cmd_stream,
                                       const char *res_stream,
                                       logging::Logger *logger)
    : cmd_stream(cmd_stream), res_stream(res_stream), read_size(0),
      connected(false), logger(logger) {}

/*************************************************************************************************/
bool JlinkStreamCommand::enable(void) {
  jlink_stream::initialize();

  if (!jlink_stream::register_stream(cmd_stream, StreamDirection::Read, nullptr,
                                     connection_callback, (void *)&connected)) {
    LOG_ERROR("Failed to register stream: %s", cmd_stream);
    return false;
  }
  if (!jlink_stream::register_stream(res_stream, StreamDirection::Write,
...
bool JlinkStreamCommand::receive_command(uint8_t *&data, uint32_t &length) {
  ...
  if (read_size == 0) {
    ...
    if (!jlink_stream::read(cmd_stream, &read_size, sizeof(uint32_t),
                            (uint32_t *)&bytes_read)) {
      LOG_ERROR("Failed to read cmd length bytes");
    }

    if (bytes_read != sizeof(uint32_t) || read_size == 0 ||
        read_size > STREAM_BUFFER_LENGTH) {
      read_size = 0;
      return false;
    }
  ...
  if (!jlink_stream::read(cmd_stream, data, read_size,
                          (uint32_t *)&bytes_read)) {
...
/*************************************************************************************************/
void JlinkStreamCommand::connection_callback(const char *name, bool connected,
                                             void *arg) {
