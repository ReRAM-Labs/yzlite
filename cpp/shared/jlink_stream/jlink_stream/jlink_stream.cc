...
bool register_stream(const char *name, StreamDirection direction,
  ...
  const uint32_t alloc_size = ALIGN_4(sizeof(StreamBuffer)) +
                              ...
                              STREAM_BUFFER_LENGTH + strlen(name) + 1;
...
bool unregister_stream(const char *name) {
  ...
  for (StreamBuffer *stream = context.streams; stream != nullptr;
       prev = stream, stream = stream->next) {
...
/*************************************************************************************************/
bool write(const char *name, const void *data, uint32_t length,
           uint32_t *bytes_written_ptr) {
  StreamBuffer *stream;
  uint32_t bytes_written_buffer;

  bytes_written_ptr = (bytes_written_ptr == nullptr) ? &bytes_written_buffer
                                                     : bytes_written_ptr;
...
bool write_all(const char *name, const void *data, uint32_t length) {
  ...
  while (total_remaining_length > 0) {
    ...
    const volatile uint32_t header_length = header->length;
    const uint32_t available_space = STREAM_BUFFER_LENGTH - header_length;
    const uint32_t write_chunk_length =
        MIN(available_space, total_remaining_length);
    total_remaining_length -= write_chunk_length;

    if (write_chunk_length > 0) {
      const uint32_t length_to_end = header->end - header->tail;
      const uint32_t chunk_length_to_end =
          MIN(length_to_end, write_chunk_length);
      const uint32_t chunk_remaining_length =
          write_chunk_length - chunk_length_to_end;
...
/*************************************************************************************************/
bool read(const char *name, void *data, uint32_t length,
          uint32_t *bytes_read_ptr) {
  StreamBuffer *stream;
  uint32_t bytes_read_buffer;

  bytes_read_ptr =
      (bytes_read_ptr == nullptr) ? &bytes_read_buffer : bytes_read_ptr;
...
static StreamCommandResult
find_stream_for_command_name(StreamBuffer **stream_ptr);
static StreamCommandResult
find_stream_for_command_id(uint8_t id, StreamBuffer **stream_ptr);
...
void process_command(void) {
  ...
  if (context.header.status == StreamStatus::CommandReady) {
    ...
    case StreamCommand::OpenForWrite:
      result = process_open_command(cmd == StreamCommand::OpenForRead
...
static StreamCommandResult process_open_command(StreamDirection direction) {
  ...
  // If the device opened the stream for reading, then the remote side should
  // open for writing
  else if ((stream->direction == StreamDirection::Read) &&
           (direction != StreamDirection::Write)) {
    return StreamCommandResult::ReadOnly;
  }
  // If the device opened the stream for writing, then the remote side should
  // open for reading
  else if ((stream->direction == StreamDirection::Write) &&
...
/*************************************************************************************************/
static StreamCommandResult process_read_buffer_status_mask_command(void) {
  memcpy(&context.command_buffer[0], (void *)&context.buffer_status_mask,
         sizeof(uint32_t));
...
static StreamCommandResult process_read_buffer_command(void) {
  ...
  // then the remote side can only write it
    ...
    if (stream->data_callback != nullptr) {
      stream->data_callback(stream->name, STREAM_BUFFER_LENGTH - header->length,
                            stream->arg);
    }
  }
...
static StreamCommandResult
find_stream_for_command_name(StreamBuffer **stream_ptr) {
  if (context.header.command_length == 0 ||
      context.header.command_length >= COMMAND_BUFFER_LENGTH) {
    return StreamCommandResult::BadArgs;
  }
