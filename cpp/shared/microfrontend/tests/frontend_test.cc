...
public:
  FrontendTestConfig() {
    ...
    config_.noise_reduction.enable_noise_reduction = true;
...
TEST(Frontend, CheckOutputValues) {
...
TEST(Frontend, CheckConsecutiveWindow) {
  FrontendTestConfig config;
  struct FrontendState state;
  EXPECT_TRUE(FrontendPopulateState(&config.config_, &state, kSampleRate));
  size_t num_samples_read;
...
TEST(Frontend, CheckNotEnoughSamples) {
  FrontendTestConfig config;
  struct FrontendState state;
  EXPECT_TRUE(FrontendPopulateState(&config.config_, &state, kSampleRate));
  size_t num_samples_read;
