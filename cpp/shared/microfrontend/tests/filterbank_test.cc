...
TEST(Filterbank, CheckStartIndex) {
...
TEST(Filterbank, CheckChannelFrequencyStarts) {
  FilterbankTestConfig config;
  struct FilterbankState state;
  EXPECT_TRUE(FilterbankPopulateState(&config.config_, &state, kSampleRate,
                                      kSpectrumSize));
  ...
  EXPECT_EQ(state.num_channels + 1, sizeof(expected) / sizeof(expected[0]));
  int i;
...
TEST(Filterbank, CheckChannelWeightStarts) {
  FilterbankTestConfig config;
  struct FilterbankState state;
  EXPECT_TRUE(FilterbankPopulateState(&config.config_, &state, kSampleRate,
                                      kSpectrumSize));
...
TEST(Filterbank, CheckChannelWidths) {
  FilterbankTestConfig config;
  struct FilterbankState state;
  EXPECT_TRUE(FilterbankPopulateState(&config.config_, &state, kSampleRate,
                                      kSpectrumSize));
...
TEST(Filterbank, CheckUnweights) {
  FilterbankTestConfig config;
  struct FilterbankState state;
  EXPECT_TRUE(FilterbankPopulateState(&config.config_, &state, kSampleRate,
                                      kSpectrumSize));
...
TEST(Filterbank, CheckConvertFftComplexToEnergy) {
  struct FilterbankState state;
...
TEST(Filterbank, CheckAccumulateChannels) {
  FilterbankTestConfig config;
  struct FilterbankState state;
  EXPECT_TRUE(FilterbankPopulateState(&config.config_, &state, kSampleRate,
                                      kSpectrumSize));
  ...
  EXPECT_EQ(state.num_channels + 1, sizeof(kWork) / sizeof(kWork[0]));
  int i;
  for (i = 0; i <= state.num_channels; ++i) {
...
TEST(Filterbank, CheckSqrt) {
  ...
  EXPECT_EQ(state.num_channels, sizeof(expected) / sizeof(expected[0]));
  int i;
  for (i = 0; i < state.num_channels; ++i) {
