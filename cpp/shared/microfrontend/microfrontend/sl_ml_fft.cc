...
static void generate_real_coeff_tables(int n) {
  ...
  for (int i = 0; i < n; i++) {
    real_coeff_table_A[2 * i] = (q15_t)roundf(
        (0.5 * (1.0 - sinf(2 * PI / (float)(2 * n) * (float)i))) * powf(2, 15));
    real_coeff_table_A[2 * i + 1] = (q15_t)roundf(
        (0.5 * (-1.0 * cosf(2 * PI / (float)(2 * n) * (float)i))) *
        powf(2, 15));

    real_coeff_table_B[2 * i] = (q15_t)roundf(
        (0.5 * (1.0 + sinf(2 * PI / (float)(2 * n) * (float)i))) * powf(2, 15));
    real_coeff_table_B[2 * i + 1] = (q15_t)roundf(
        (0.5 * (1.0 * cosf(2 * PI / (float)(2 * n) * (float)i))) * powf(2, 15));
    ;
  }
}
...
sl_status_t sli_ml_fft_init(struct sli_ml_fft_state *state, size_t input_size) {
  ...
  state->output = static_cast<complex_int16_t *>(
      malloc(rfft_size * sizeof(complex_int16_t)));
  if (state->output == nullptr) {
    return SL_STATUS_ALLOCATION_FAILED;
  }

  real_coeff_table_A =
      static_cast<int16_t *>(malloc(rfft_size * sizeof(int16_t)));
