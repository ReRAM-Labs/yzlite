...
#define MAX_PACKET_LENGTH                                                      \
  (sizeof(r503_packet_header_t) + R503_PACKET_MAX_LENGTH + sizeof(uint16_t))
...
static sl_status_t read_packet_header(r503_packet_header_t *header,
                                      uint16_t *checksum);
static uint16_t calculate_packet_checksum(const uint8_t *packet,
                                          uint16_t data_length);
static bool validate_packet_checksum(const r503_packet_t *packet,
                                     uint16_t checksum);
static uint16_t calculate_checksum(const uint8_t *data, uint16_t data_length,
...
sl_status_t r503_init(void) {
  ...
  LDMA_TransferCfg_t rx_cfg = LDMA_TRANSFER_CFG_PERIPHERAL(
      USART_RX_DMA_SIGNAL(FINGERPRINT_READER_USART_PERIPHERAL_NO));
  LDMA_Descriptor_t rx_desc = LDMA_DESCRIPTOR_LINKREL_P2M_BYTE(
      &FINGERPRINT_READER_USART->RXDATA, rx_buffer.data, MAX_PACKET_LENGTH,
  ...
  status = r503_send_command(R503_COMMAND_CHECK_SENSOR, NULL, 0,
                             &command_status, NULL, 0);
  if (status != SL_STATUS_OK) {
    return status;
  }
...
sl_status_t r503_read_system_parameters(r503_system_parameters_t *params) {
  ...
  status =
      r503_send_command(R503_COMMAND_GET_SYSTEM_PARAMETERS, NULL, 0, NULL,
                        (uint8_t *)params, sizeof(r503_system_parameters_t));
  if (status == SL_STATUS_OK) {
...
sl_status_t r503_read_product_info(r503_product_information_t *info) {
  ...
  status =
      r503_send_command(R503_COMMAND_READ_PRODUCT_INFO, NULL, 0, NULL,
                        (uint8_t *)info, sizeof(r503_product_information_t));
  if (status == SL_STATUS_OK) {
...
sl_status_t r503_update_led(const r503_led_config_t *config) {
  return r503_send_command(R503_COMMAND_UPDATE_LED, (uint8_t *)config,
                           sizeof(r503_led_config_t), NULL, NULL, 0);
}

/*************************************************************************************************/
sl_status_t r503_capture_image(r503_status_t *status) {
  sl_status_t sl_status = r503_send_command(R503_COMMAND_CAPTURE_FINGERPRINT,
                                            NULL, 0, status, NULL, 0);
...
static sl_status_t read_image_chunk(uint8_t **dst_ptr, uint16_t dst_length) {
  ...
  if (status != SL_STATUS_OK) {
    ...
    return status;
  } else if (!(packet_header->type == R503_PACKET_TYPE_DATA ||
               packet_header->type == R503_PACKET_TYPE_END_OF_DATA)) {
  ...
  if (!validate_packet_checksum((r503_packet_t *)packet_buffer,
                                header_checksum)) {
  ...
  uint8_t *dst = *dst_ptr;
  for (uint16_t packet_remaining = packet_header->length - sizeof(uint16_t);
       packet_remaining > 0; --packet_remaining, ++src) {
    if (dst_length < 2) {
    ...
    *dst++ =
        (*src &
         0xF0); // NOTE: We keep the 4-bits in the upper part of the dst bytes
    *dst++ = (*src << 4); //       to make the image appear brighter (this is
                          //       the same as multiplying each pixel by 16)
    dst_length -= 2;
  }
  *dst_ptr = dst;
...
sl_status_t r503_read_image(uint8_t *image_buffer) {
  ...
  uint8_t *image_buffer_end =
      ptr + FINGERPRINT_READER_IMAGE_WIDTH * FINGERPRINT_READER_IMAGE_HEIGHT;
...
sl_status_t r503_cancel() {
  for (int i = 0; i < 10; ++i) {
    r503_status_t command_status;
    sl_status_t status = r503_send_command(R503_COMMAND_CANCEL, NULL, 0,
                                           &command_status, NULL, 0);
...
/*************************************************************************************************/
sl_status_t r503_send_command(r503_command_t command, const uint8_t *data,
                              uint16_t data_length,
  ...
  write_data(packet_buffer,
             sizeof(r503_packet_header_t) + 1 + data_length + sizeof(uint16_t));
...
static sl_status_t read_packet_header(r503_packet_header_t *header,
                                      uint16_t *checksum) {
  sl_status_t status =
      read_data((uint8_t *)header, sizeof(r503_packet_header_t));
  if (status != SL_STATUS_OK) {
    return status;
  }
...
static uint16_t calculate_packet_checksum(const uint8_t *packet,
                                          uint16_t data_length) {
  return calculate_checksum(packet + sizeof(uint16_t) + sizeof(uint32_t),
                            3 + data_length, NULL);
}
...
static bool validate_packet_checksum(const r503_packet_t *packet,
  ...
  const uint16_t checksum =
      calculate_checksum(packet->data, data_length, &header_checksum);
...
#define current_dma_ptr()                                                      \
  (rx_buffer.data + (MAX_PACKET_LENGTH -                                       \
                     LDMA_TransferRemainingCount(rx_buffer.dma_channel_id)))
  uint8_t *dst = buffer;
...
static sl_status_t configure_baud() {
  ...
  if (status != SL_STATUS_OK) {
    ...
    status = r503_send_command(R503_COMMAND_SET_SYSTEM_PARAMETER, data,
                               sizeof(data), NULL, NULL, 0);
    if (status != SL_STATUS_OK) {
      return status;
    }
...
static sl_status_t configure_data_length() {
  ...
  if (params.data_packet_size != R503_DATA_LENGTH_256) {
    ...
    status = r503_send_command(R503_COMMAND_SET_SYSTEM_PARAMETER, data, 2, NULL,
                               NULL, 0);
    if (status != SL_STATUS_OK) {
      return status;
    }
...
/*************************************************************************************************/
static uint16_t swap_endianess16(uint16_t v) { return (v >> 8) | (v << 8); }
