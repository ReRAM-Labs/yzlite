...
sl_status_t fingerprint_reader_init(const fingerprint_reader_config_t *config) {
  ...
  // Configure GPIO mode
  GPIO_PinModeSet(FINGERPRINT_READER_USART_TX_PORT,
                  FINGERPRINT_READER_USART_TX_PIN, gpioModePushPull, 1);
  GPIO_PinModeSet(FINGERPRINT_READER_USART_RX_PORT,
                  FINGERPRINT_READER_USART_RX_PIN, gpioModeInput, 1);
  ...
  GPIO->USARTROUTE[FINGERPRINT_READER_USART_PERIPHERAL_NO].ROUTEEN =
      GPIO_USART_ROUTEEN_TXPEN | GPIO_USART_ROUTEEN_RXPEN |
      GPIO_USART_ROUTEEN_CLKPEN;

  GPIO->USARTROUTE[FINGERPRINT_READER_USART_PERIPHERAL_NO].TXROUTE =
      ((uint32_t)FINGERPRINT_READER_USART_TX_PORT
       << _GPIO_USART_TXROUTE_PORT_SHIFT) |
      ((uint32_t)FINGERPRINT_READER_USART_TX_PIN
       << _GPIO_USART_TXROUTE_PIN_SHIFT);

  GPIO->USARTROUTE[FINGERPRINT_READER_USART_PERIPHERAL_NO].RXROUTE =
      ((uint32_t)FINGERPRINT_READER_USART_RX_PORT
  ...
  FINGERPRINT_READER_USART->ROUTEPEN =
      USART_ROUTEPEN_TXPEN | USART_ROUTEPEN_RXPEN;
#endif
  ...
  reader_context.finger_detected_irq_callback =
      config->finger_detected_irq_callback;
  GPIO_PinModeSet(FINGERPRINT_READER_ACTIVITY_PORT,
                  FINGERPRINT_READER_ACTIVITY_PIN, gpioModeInput, 1);
  GPIO_ExtIntConfig(FINGERPRINT_READER_ACTIVITY_PORT,
                    FINGERPRINT_READER_ACTIVITY_PIN,
                    FINGERPRINT_READER_ACTIVITY_PIN, false, true, true);
  GPIOINT_Init();
  GPIOINT_CallbackRegister(FINGERPRINT_READER_ACTIVITY_PIN,
                           on_finger_detected_irq_handler);
...
/*************************************************************************************************/
sl_status_t
fingerprint_reader_update_led(const fingerprint_reader_led_config_t *config) {
  CHECK_INITIALIZED();
...
/*************************************************************************************************/
sl_status_t
fingerprint_reader_get_image(fingerprint_reader_image_t image_buffer) {
  CHECK_INITIALIZED();
