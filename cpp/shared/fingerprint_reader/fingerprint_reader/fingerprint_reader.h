...
typedef struct {
  ...
  uint8_t speed; //!< Mode speed, 0-256, minimum o 5s cycles
                 //!< This is the effective breakthing/flashing/gradual
                 //!< on/gradual off duration
  uint8_t count; //!< Number of time mode should repeat. 0 = infinite, 1-255
} fingerprint_reader_led_config_t;

typedef uint8_t fingerprint_reader_image_t[FINGERPRINT_READER_IMAGE_WIDTH *
                                           FINGERPRINT_READER_IMAGE_HEIGHT];
