 ...
 */
template <class T> struct _initializer_list_with_square_brackets {
  const std::initializer_list<T> &list;

  _initializer_list_with_square_brackets(const std::initializer_list<T> &_list)
      : list(_list) {}
 ...
 */
template <class T>
const _initializer_list_with_square_brackets<T>
initializer_list_accessor(const std::initializer_list<T> &list) {
...
#define CREATE_STATIC_OBJECT_BUFFER(T, name)                                   \
  alignas(T) static uint8_t name[sizeof(T)]

#ifndef STRINGIFY
// Note: The C preprocessor stringify operator ('#') makes a string from its
// argument, without macro expansion e.g. If "version" is #define'd to be "4",
// then STRINGIFY_AWE(version) will return the string "version", not "4" To
// expand "version" to its value before making the string, use
// STRINGIFY(version) instead
#define STRINGIFY_ARGUMENT_WITHOUT_EXPANSION(s) #s
#define STRINGIFY(s) STRINGIFY_ARGUMENT_WITHOUT_EXPANSION(s)
#endif

#if defined(YZLITE_DLL_EXPORT) && !defined(DLL_EXPORT)
