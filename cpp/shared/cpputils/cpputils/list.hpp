...
struct ListIterator {
  ...
  bool operator!=(const ListIterator &rhs) { return ptr != rhs.ptr; }

  void *operator*() { return static_cast<void *>(ptr); }

  void operator++() { ptr += entry_size; }
};
...
public:
  ...
  void initialize(uint32_t entry_size = sizeof(void *),
  ...
  template <typename T> T *get(int i) const { return static_cast<T *>(get(i)); }

  void clear(void);
  bool contains(const void *val) const;

  constexpr void *list(void) const { return static_cast<void *>(head_); }

  void *operator[](int i) const { return get(i); }

  template <typename T> T *operator[](int i) const { return get<T>(i); }

  constexpr uint32_t size(void) const { return count_; }

  constexpr uint32_t capacity(void) const { return capacity_; }

  constexpr bool empty(void) const { return count_ == 0; }

  constexpr bool full(void) const { return count_ == capacity_; }

  ListIterator begin(void) const { return ListIterator(head_, entry_size_); }

  ListIterator end() const {
    return ListIterator((head_ == nullptr) ? nullptr
                                           : &head_[count_ * entry_size_],
                        entry_size_);
  }

  bool has_malloc_error() const { return has_malloc_error_; }
