...
template <typename T> struct TypedLinkedListIterator {
  ...
  T *operator*() { return reinterpret_cast<T *>(current); }
...
template <typename T> struct TypedLinkedList : public LinkedList {
  bool remove(T *item) {
  ...
  T *get(int i) const { return LinkedList::get<T>(i); }
  ...
  constexpr T *list(void) const { return LinkedList::list<T>(); }

  T *operator[](int i) const { return LinkedList::get<T>(i); }
