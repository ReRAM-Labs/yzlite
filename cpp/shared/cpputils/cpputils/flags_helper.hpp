 ...
 */
template <typename FlagsType> struct FlagsHelper {
  FlagsType value;

  FlagsHelper(FlagsType value = (FlagsType)0) : value(value) {}

  void write(FlagsType v) { value = v; }

  constexpr FlagsType read() const { return value; }

  constexpr FlagsType read(FlagsType mask) const { return value & mask; }

  void set(FlagsType flags) { value |= flags; }

  void set(FlagsType flags, FlagsType mask) { value = (value & ~mask) | flags; }

  void clear(FlagsType flags) { value &= ~flags; }
  ...
  FlagsHelper operator|(const FlagsType type) const { return value | type; }
  ...
  FlagsHelper operator~() const { return ~value; }
