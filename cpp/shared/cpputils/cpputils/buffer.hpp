...
public:
  ...
  template <typename T> T *data_as_type(unsigned offset = 0) {
    return static_cast<T *>(data(offset));
  }

  template <typename T> T *next_as_type(unsigned length) {
    return static_cast<T *>(next(length));
  }

  constexpr unsigned size() const { return _size; }

  constexpr unsigned offset() const { return _offset; }

  constexpr unsigned remaining() const { return _size - _offset; }

  void reset_offset() { _offset = 0; }
