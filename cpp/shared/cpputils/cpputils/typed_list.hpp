 ...
 */
template <typename T> struct TypedListIterator {
  uint8_t *ptr;
  ...
  bool operator!=(const TypedListIterator &rhs) { return ptr != rhs.ptr; }
 ...
 * @brief Typed List
 */
template <typename T> class TypedList : public List {
public:
  TypedList(uint32_t initial_capacity = 4)
      : List(sizeof(T), initial_capacity) {}

  TypedList(uint32_t max_capacity, void *buffer)
      : List(sizeof(T), max_capacity, buffer) {}
  ...
  bool remove(int i) { return List::remove(i); }

  T &get(int i) const { return *reinterpret_cast<T *>(List::get(i)); }

  bool contains(const T &val) const { return List::contains((void *)&val); }
  ...
  constexpr T *list(void) const { return (T *)(head_); }
  ...
  TypedListIterator<T> end() const {
    return TypedListIterator<T>(
        (head_ == nullptr) ? nullptr : &head_[count_ * entry_size_],
        entry_size_);
  }
};
