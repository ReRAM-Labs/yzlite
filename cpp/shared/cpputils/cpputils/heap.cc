...
static void debug_malloc_printf(const char *msg, const void *ptr,
                                unsigned length);
#else
#define MDEBUG(...)
#endif
...
extern "C" void heap_set_buffer(void *buffer, uint32_t length) {
  if (length != 0) {
    ...
    auto start_alignment_required =
        sizeof(header_t); // required by _free(start[1]) invocation
    ...
    assert(unaligned_amount ==
           0); // since now aligning based on header_t, just check that this old
               // alignment requirement is already met
    uint8_t align_amount =
        (unaligned_amount > 0) ? sizeof(uintptr_t) - unaligned_amount : 0;
...
static void *_malloc(uint32_t length, uint32_t alignment) {
  ...
  for (p = prevp->ptr;; prevp = p, p = p->ptr) {
    ...
    if (p->size >= num_blocks) /* If p is large enough? */
    {
      ...
      retval =
          (void *)&p[1]; /* return but at the address of p + sizeof(header_t),
                          since p points to the header. */
      break;
    }

    if (p ==
        memory_pool->freep) /* the list does not contain a block big enough */
    {
      break;
    }
  }
...
void _free(void *ap) {
  ...
  bp = (header_t *)(((uint8_t *)ap) -
                    sizeof(header_t)); /* Here is the header of the block */
  ...
  for (p = memory_pool->freep; !(bp > p && bp < p->ptr);
       p = p->ptr, --max_scans) {
    if (p >= p->ptr && (bp > p || bp < p->ptr)) {
      break; /* Bp is present block with the smallest or behind  Largest block
                address */
    }
