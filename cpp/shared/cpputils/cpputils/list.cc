...
/*************************************************************************************************/
void List::initialize(uint32_t entry_size, uint32_t max_capacity,
                      void *buffer) {
  is_mutable_ = false;
  owns_buffer_ = false;
  has_malloc_error_ = false;
...
bool List::prepend(const void *val, bool unique) {
  ...
  memmove(&head_[1 * entry_size_], &head_[0 * entry_size_],
          count_ * entry_size_);
...
bool List::remove(const void *val) {
  ...
  for (int i = 0; i < count_; ++i) {
    ...
    if (memcmp(val, other_val, entry_size_) == 0) {
      ...
      memmove(&head_[i * entry_size_], &head_[((i + 1) * entry_size_)],
              (count_ - i - 1) * entry_size_);
      count_--;
      break;
    }
  }
...
bool List::increase_size(uint32_t new_size) {
  ...
  // If we're trying to malloc more than 1k, then just start incrementing rather
  // than doubling
