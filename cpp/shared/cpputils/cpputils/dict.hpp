...
struct DictItem {
  ...
  template <typename T> T *as_type(void) const {
...
public:
  ...
  bool put(const char *key, const void *value, uint32_t length,
           bool unique = true);
  ...
  template <typename T> bool update(const char *key, const T *value) {
  ...
  template <typename T> bool set(const char *key, const T *value) {
  ...
  void *operator[](const char *key) const { return get(key); }

  template <typename T> T *operator[](const char *key) const {
    return get<T>(key);
  }

  constexpr bool empty(void) const { return (count_ == 0); }

  constexpr uint32_t size(void) const { return count_; }

  constexpr DictItem *base(void) const { return items_; }

  DictValueIterator begin(void) const { return DictValueIterator(items_); }

  DictValueIterator end() const { return DictValueIterator(nullptr); }

  DictItemIterator items(void) const { return DictItemIterator(items_); }

  DictItemIterator enditems() const { return DictItemIterator(nullptr); }

  DictKeyIterator keys(void) const { return DictKeyIterator(items_); }

  DictKeyIterator endkeys() const { return DictKeyIterator(nullptr); }
