...
char *format_units(uint32_t number, uint8_t precision, char *buffer) {
  ...
  if (divisor == 1) {
    ...
    sprintf(buffer, fmt, scaled / precision_factor, scaled % precision_factor,
            unit);
  }
...
/*************************************************************************************************/
const char *format_microseconds_to_milliseconds(uint32_t time_us,
                                                char *buffer) {
  static char default_buffer[16];
  buffer = (buffer == nullptr) ? default_buffer : buffer;

  sprintf(buffer, "%3u.%03u ms", (unsigned int)(time_us / 1000),
          (unsigned int)(time_us % 1000));

  return buffer;
}

/*************************************************************************************************/
const char *format_rate(uint32_t total, uint32_t elapsed_time_us,
                        uint8_t precision, char *buffer) {
...
extern "C" const char *_strcasestr(const char *str1, const char *str2) {
  ...
  while (*cp) {
    ...
    while (*s1 && *s2 && (IS_ALPHA(*s1) && IS_ALPHA(*s2))
