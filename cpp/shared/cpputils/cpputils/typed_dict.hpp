 ...
 */
template <typename T> struct TypedDictItem {
...
template <typename T> struct TypedDictValueIterator {
  TypedDictItem<T> *current;
  TypedDictValueIterator(TypedDictItem<T> *head) : current(head) {}
  bool operator!=(const TypedDictValueIterator &rhs) {
 ...
 */
template <typename T> struct TypedDictKeyIterator {
  TypedDictItem<T> *current;
  TypedDictKeyIterator(TypedDictItem<T> *head) : current(head) {}
  bool operator!=(const TypedDictKeyIterator &rhs) {
    return current != rhs.current;
  }
  const char *operator*() {
 ...
 */
template <typename T> struct TypedDictItemIterator {
  TypedDictItem<T> *current;
  TypedDictItemIterator(TypedDictItem<T> *head) : current(head) {}
  bool operator!=(const TypedDictItemIterator &rhs) {
    return current != rhs.current;
  }
 ...
 */
template <typename T> class TypedDict : public Dict {
public:
  ...
  T *get(const char *key) const { return static_cast<T *>(Dict::get(key)); }
  ...
  T *operator[](const char *key) const { return get(key); }
