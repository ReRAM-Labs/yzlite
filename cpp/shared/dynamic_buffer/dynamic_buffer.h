 ...
 *
 * @param[in] buffer @ref dynamic_buffer_t object to allocate a larger buffer
 * (if necessary)
 ...
 * Copy dynamic buffer
 *
 * This copies the contents of the `src` @ref dynamic_buffer_t into the `dst`
 * buffer.
 *
 ...
 * write is larger than the buffer's current size then more buffer will be
 ...
 *
 * This moves the `append` pointer forwards or backwards by
 ...
 *
 *
 * This moves the `prepend` pointer forwards or backwards by
 * `(signed)adjust_amount`
 *
 * @param[in] buffer @ref dynamic_buffer_t
...
dynamic_buffer_get_data_start(const dynamic_buffer_t *buffer) {
