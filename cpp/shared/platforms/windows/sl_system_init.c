...
void sl_system_process_action(void) {
  ...
  if (elapsed <= periodic_wakeup_interval_ms) {
    ...
    if (periodic_wakeup_overflow_count >= 5) {
      ...
      printf("\n*** Simulated latency is %dms, but app loop took %dms",
             periodic_wakeup_interval_ms, elapsed);
      printf("This likely means the model is taking too long to execute on "
             "your PC\n");
    }
  }

  periodic_wakeup_timestamp =
