...
extern void __wrap__assert(const char *message, const char *filename,
                           unsigned line);
extern void __wrap__wassert(const wchar_t *message, const wchar_t *filename,
                            unsigned line);
...
#define assert(_Expression)                                                    \
  (void)((!!(_Expression)) || (__wrap__wassert(_CRT_WIDE(#_Expression),        \
                                               _CRT_WIDE(__FILE__), __LINE__), \
                               0))
#else /* not unicode */
#define assert(_Expression)                                                    \
  (void)((!!(_Expression)) ||                                                  \
         (__wrap__assert(#_Expression, __FILE__, __LINE__), 0))
