...
#define SL_IOSTREAM_USART_CONCAT_PASTER(first, second, third)                  \
  first##second##third

#define SL_IOSTREAM_USART_TX_IRQ_NUMBER(periph_nbr)                            \
  SL_IOSTREAM_USART_CONCAT_PASTER(USART, periph_nbr, _TX_IRQn)
#define SL_IOSTREAM_USART_RX_IRQ_NUMBER(periph_nbr)                            \
  SL_IOSTREAM_USART_CONCAT_PASTER(USART, periph_nbr, _RX_IRQn)
#define SL_IOSTREAM_USART_TX_IRQ_HANDLER(periph_nbr)                           \
  SL_IOSTREAM_USART_CONCAT_PASTER(USART, periph_nbr, _TX_IRQHandler)
#define SL_IOSTREAM_USART_RX_IRQ_HANDLER(periph_nbr)                           \
  SL_IOSTREAM_USART_CONCAT_PASTER(USART, periph_nbr, _RX_IRQHandler)

#define SL_IOSTREAM_USART_RX_DMA_SIGNAL(periph_nbr)                            \
  SL_IOSTREAM_USART_CONCAT_PASTER(dmadrvPeripheralSignal_USART, periph_nbr,    \
                                  _RXDATAV)

#define SL_IOSTREAM_USART_CLOCK_REF(periph_nbr)                                \
  SL_IOSTREAM_USART_CONCAT_PASTER(cmuClock_, USART, periph_nbr)
// EM Events
#if defined(SL_CATALOG_POWER_MANAGER_PRESENT)
...
sl_status_t sl_iostream_usart_init_vcom(void) {
  ...
  sl_iostream_dma_config_t dma_config_vcom = {
      ...
      .peripheral_signal = SL_IOSTREAM_USART_RX_DMA_SIGNAL(
          SL_IOSTREAM_USART_VCOM_PERIPHERAL_NO)};
  ...
  sl_iostream_uart_config_t uart_config_vcom = {
      ...
      .tx_irq_number =
          SL_IOSTREAM_USART_TX_IRQ_NUMBER(SL_IOSTREAM_USART_VCOM_PERIPHERAL_NO),
      .rx_irq_number =
          SL_IOSTREAM_USART_RX_IRQ_NUMBER(SL_IOSTREAM_USART_VCOM_PERIPHERAL_NO),
...
// VCOM IRQ Handler
void SL_IOSTREAM_USART_TX_IRQ_HANDLER(SL_IOSTREAM_USART_VCOM_PERIPHERAL_NO)(
    void) {
  sl_iostream_usart_irq_handler(&sl_iostream_vcom);
}

void SL_IOSTREAM_USART_RX_IRQ_HANDLER(SL_IOSTREAM_USART_VCOM_PERIPHERAL_NO)(
    void) {
  sl_iostream_usart_irq_handler(&sl_iostream_vcom);
}
...
static void events_handler(sl_power_manager_em_t from,
  ...
  if (((from == SL_POWER_MANAGER_EM2) || (from == SL_POWER_MANAGER_EM3)) &&
    ...
    out = GPIO_PinOutGet(SL_IOSTREAM_USART_VCOM_TX_PORT,
                         SL_IOSTREAM_USART_VCOM_TX_PIN);
    GPIO_PinModeSet(SL_IOSTREAM_USART_VCOM_TX_PORT,
                    SL_IOSTREAM_USART_VCOM_TX_PIN, gpioModePushPull, out);
    ...
    out = GPIO_PinOutGet(SL_IOSTREAM_USART_VCOM_TX_PORT,
                         SL_IOSTREAM_USART_VCOM_TX_PIN);
    GPIO_PinModeSet(SL_IOSTREAM_USART_VCOM_TX_PORT,
                    SL_IOSTREAM_USART_VCOM_TX_PIN, gpioModeDisabled, out);
  }
#endif // _SILICON_LABS_32B_SERIES_2
