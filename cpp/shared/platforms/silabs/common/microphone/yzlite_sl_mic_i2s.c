/***************************************************************************/ /**
                                                                               ...
                                                                               *******************************************************************************
                                                                               * # License
                                                                               * <b>Copyright 2020 Yizhu Technology Inc. www.yizhu.com</b>
                                                                               *******************************************************************************
                                                                               *
...
#define MIC_DMA_LEFT_SIGNAL(N)                                                 \
  SL_CONCAT(ldmaPeripheralSignal_USART, N, _RXDATAV)
#define MIC_DMA_RIGHT_SIGNAL(N)                                                \
  SL_CONCAT(ldmaPeripheralSignal_USART, N, _RXDATAVRIGHT)
...
static bool dma_complete(unsigned int channel, unsigned int sequenceNo,
                         void *userParam);
static void init_dma_descriptors(uint32_t sample_count, uint32_t sample_length,
                                 bool have_callback);

/***************************************************************************/ /**
                                                                               *    Initializes the microphone
                                                                               ******************************************************************************/
sl_status_t yzlite_sl_mic_init(uint32_t sample_rate, uint8_t n_channels) {
  ...
  GPIO->USARTROUTE->ROUTEEN = GPIO_USART_ROUTEEN_RXPEN |
                              GPIO_USART_ROUTEEN_CLKPEN |
                              GPIO_USART_ROUTEEN_CSPEN;
  GPIO->USARTROUTE->RXROUTE =
      (SL_MIC_I2S_RX_PORT << _GPIO_USART_RXROUTE_PORT_SHIFT) |
      (SL_MIC_I2S_RX_PIN << _GPIO_USART_RXROUTE_PIN_SHIFT);
  GPIO->USARTROUTE->CLKROUTE =
      (SL_MIC_I2S_CLK_PORT << _GPIO_USART_CLKROUTE_PORT_SHIFT) |
      (SL_MIC_I2S_CLK_PIN << _GPIO_USART_CLKROUTE_PIN_SHIFT);
  GPIO->USARTROUTE->CSROUTE =
      (SL_MIC_I2S_CS_PORT << _GPIO_USART_CSROUTE_PORT_SHIFT) |
      (SL_MIC_I2S_CS_PIN << _GPIO_USART_CSROUTE_PIN_SHIFT);
#else
  SL_MIC_I2S_PERIPHERAL->ROUTELOC0 =
      (SL_MIC_I2S_RX_LOC << _USART_ROUTELOC0_RXLOC_SHIFT |
       SL_MIC_I2S_CLK_LOC << _USART_ROUTELOC0_CLKLOC_SHIFT |
       SL_MIC_I2S_CS_LOC << _USART_ROUTELOC0_CSLOC_SHIFT);
  SL_MIC_I2S_PERIPHERAL->ROUTEPEN =
      (USART_ROUTEPEN_RXPEN | USART_ROUTEPEN_CLKPEN | USART_ROUTEPEN_CSPEN);
#endif
...
/***************************************************************************/ /**
                                                                               ...
                                                                               ******************************************************************************/
sl_status_t yzlite_sl_mic_deinit(void) {
  /* Stop sampling */
  yzlite_sl_mic_stop();
...
/***************************************************************************/ /**
                                                                               ...
                                                                               ******************************************************************************/
sl_status_t
yzlite_sl_mic_start_streaming(void *buffer, uint32_t n_frames,
                              sl_mic_buffer_ready_callback_t callback) {
  if (!mic_context.initialized) {
  ...
  for (ms_per_callback = MIN(25, ms_per_sample);
       ms_per_callback < ms_per_sample; ++ms_per_callback) {
    const int n_frames_per_callback =
        (ms_per_callback * mic_context.sample_rate) / 1000;
  ...
  mic_context.n_frames_per_callback =
      (ms_per_callback * mic_context.sample_rate) / 1000;

  const uint32_t sample_length = n_frames * mic_context.n_channels;
  const uint32_t callback_sample_length =
      mic_context.n_frames_per_callback * mic_context.n_channels;
  ...
  const uint32_t desc_per_callback_sample =
      DIV_ROUND_UP(callback_sample_length, MAX_DMA_LENGTH);
  const uint32_t alloc_size = sizeof(LDMA_Descriptor_t) *
                              desc_per_callback_sample * callback_sample_count;
  ...
  mic_context.buffer.end =
      mic_context.buffer.ptr + callback_sample_length * callback_sample_count;
  ...
  LDMA_TransferCfg_t dma_config = LDMA_TRANSFER_CFG_PERIPHERAL(
      MIC_DMA_LEFT_SIGNAL(SL_MIC_I2S_PERIPHERAL_NO));
  dma_config.ldmaDbgHalt = false;

  DMADRV_LdmaStartTransfer(mic_context.buffer.dma_ch, &dma_config,
                           mic_context.buffer.dma_desc, dma_complete, NULL);

  if (mic_context.n_channels == 1) {
    LDMA_TransferCfg_t drop_dma_config = LDMA_TRANSFER_CFG_PERIPHERAL(
        MIC_DMA_RIGHT_SIGNAL(SL_MIC_I2S_PERIPHERAL_NO));
    drop_dma_config.ldmaDbgHalt = false;
    DMADRV_LdmaStartTransfer(mic_context.drop_channel.dma_ch, &drop_dma_config,
                             &mic_context.drop_channel.dma_desc, NULL, NULL);
  }
...
/***************************************************************************/ /**
                                                                               ...
                                                                               ******************************************************************************/
sl_status_t yzlite_sl_mic_stop(void) {
  /* Stop sampling */
...
static void init_dma_descriptors(uint32_t sample_count, uint32_t sample_length,
  ...
  for (uint32_t sample_id = 0; sample_id < sample_count; ++sample_id) {
    ...
    for (uint32_t length_remaining = sample_length; length_remaining > 0;) {
      ...
      *desc_ptr = desc_template;
      ...
      desc_ptr->xfer.doneIfs =
          (have_callback && (length_remaining == 0)) ? 1 : 0;
  ...
  if (mic_context.n_channels == 1) {
    ...
    mic_context.drop_channel.dma_desc.xfer.dstAddr =
        (uint32_t)&mic_context.drop_channel.buffer;
