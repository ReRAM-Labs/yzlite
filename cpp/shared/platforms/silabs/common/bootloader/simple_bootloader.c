...
#define PLATFORM_IS_FLASH_ADDRESS(addr)                                        \
  ((((uint32_t)addr) >= PLATFORM_FLASH_START_ADDRESS) &&                       \
   (((uint32_t)addr) < PLATFORM_FLASH_END_ADDRESS))

__STATIC_INLINE void _jumpto(uint32_t addr) {
  addr |= 0x00000001; /* Last bit of jump address indicates whether destination
                         is Thumb or ARM code */
 ...
 */
void __NO_RETURN __attribute__((used, section(".bootloader")))
