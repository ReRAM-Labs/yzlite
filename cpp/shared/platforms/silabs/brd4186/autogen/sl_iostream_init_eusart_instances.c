...
#define SL_IOSTREAM_EUSART_CONCAT_PASTER(first, second, third)                 \
  first##second##third
#if defined(EUART_COUNT) && (EUART_COUNT > 0)
#define SL_IOSTREAM_EUSART_TX_IRQ_NUMBER(periph_nbr)                           \
  SL_IOSTREAM_EUSART_CONCAT_PASTER(EUART, periph_nbr, _TX_IRQn)
#define SL_IOSTREAM_EUSART_RX_IRQ_NUMBER(periph_nbr)                           \
  SL_IOSTREAM_EUSART_CONCAT_PASTER(EUART, periph_nbr, _RX_IRQn)
#define SL_IOSTREAM_EUSART_TX_IRQ_HANDLER(periph_nbr)                          \
  SL_IOSTREAM_EUSART_CONCAT_PASTER(EUART, periph_nbr, _TX_IRQHandler)
#define SL_IOSTREAM_EUSART_RX_IRQ_HANDLER(periph_nbr)                          \
  SL_IOSTREAM_EUSART_CONCAT_PASTER(EUART, periph_nbr, _RX_IRQHandler)
#else
#define SL_IOSTREAM_EUSART_TX_IRQ_NUMBER(periph_nbr)                           \
  SL_IOSTREAM_EUSART_CONCAT_PASTER(EUSART, periph_nbr, _TX_IRQn)
#define SL_IOSTREAM_EUSART_RX_IRQ_NUMBER(periph_nbr)                           \
  SL_IOSTREAM_EUSART_CONCAT_PASTER(EUSART, periph_nbr, _RX_IRQn)
#define SL_IOSTREAM_EUSART_TX_IRQ_HANDLER(periph_nbr)                          \
  SL_IOSTREAM_EUSART_CONCAT_PASTER(EUSART, periph_nbr, _TX_IRQHandler)
#define SL_IOSTREAM_EUSART_RX_IRQ_HANDLER(periph_nbr)                          \
  SL_IOSTREAM_EUSART_CONCAT_PASTER(EUSART, periph_nbr, _RX_IRQHandler)
#endif
#define SL_IOSTREAM_EUSART_RX_DMA_SIGNAL(periph_nbr)                           \
  SL_IOSTREAM_EUSART_CONCAT_PASTER(dmadrvPeripheralSignal_EUSART, periph_nbr,  \
                                   _RXDATAV)
#if defined(LDMAXBAR_CH_REQSEL_SIGSEL_EUART0RXFL)
#define SL_IOSTREAM_EUART_RX_DMA_SIGNAL                                        \
  SL_IOSTREAM_EUSART_CONCAT_PASTER(dmadrvPeripheralSignal_EUART, 0, _RXDATAV)
#endif

#if defined(EUART_COUNT) && (EUART_COUNT > 0)
#define SL_IOSTREAM_EUSART_CLOCK_REF(periph_nbr)                               \
  SL_IOSTREAM_EUSART_CONCAT_PASTER(cmuClock_, EUART, periph_nbr)
#else
#define SL_IOSTREAM_EUSART_CLOCK_REF(periph_nbr)                               \
  SL_IOSTREAM_EUSART_CONCAT_PASTER(cmuClock_, EUSART, periph_nbr)
#endif
...
sl_status_t sl_iostream_eusart_init_vcom(void) {
  ...
  sl_iostream_eusart_config_t config_vcom = {
    ...
    .clock =
        SL_IOSTREAM_EUSART_CLOCK_REF(SL_IOSTREAM_EUSART_VCOM_PERIPHERAL_NO),
  ...
  config_vcom.flow_control =
      SL_IOSTREAM_EUSART_VCOM_FLOW_CONTROL_TYPE != uartFlowControlSoftware
          ? SL_IOSTREAM_EUSART_VCOM_FLOW_CONTROL_TYPE
          : eusartHwFlowControlNone;
#if defined(LDMAXBAR_CH_REQSEL_SIGSEL_EUART0RXFL) &&                           \
    (SL_IOSTREAM_EUSART_VCOM_PERIPHERAL_NO == 0)
  sl_iostream_dma_config_t dma_config_vcom = {
      .src = (uint8_t *)&SL_IOSTREAM_EUSART_VCOM_PERIPHERAL->RXDATA,
      .peripheral_signal = SL_IOSTREAM_EUART_RX_DMA_SIGNAL};
#else
  sl_iostream_dma_config_t dma_config_vcom = {
      .src = (uint8_t *)&SL_IOSTREAM_EUSART_VCOM_PERIPHERAL->RXDATA,
      .peripheral_signal = SL_IOSTREAM_EUSART_RX_DMA_SIGNAL(
  ...
  sl_iostream_uart_config_t uart_config_vcom = {
      ...
      .tx_irq_number = SL_IOSTREAM_EUSART_TX_IRQ_NUMBER(
          SL_IOSTREAM_EUSART_VCOM_PERIPHERAL_NO),
      .rx_irq_number = SL_IOSTREAM_EUSART_RX_IRQ_NUMBER(
          SL_IOSTREAM_EUSART_VCOM_PERIPHERAL_NO),
      .lf_to_crlf = SL_IOSTREAM_EUSART_VCOM_CONVERT_BY_DEFAULT_LF_TO_CRLF,
      .rx_when_sleeping =
          SL_IOSTREAM_EUSART_VCOM_RESTRICT_ENERGY_MODE_TO_ALLOW_RECEPTION,
  };
  uart_config_vcom.sw_flow_control =
      SL_IOSTREAM_EUSART_VCOM_FLOW_CONTROL_TYPE == uartFlowControlSoftware;
  // Instantiate eusart instance
  status = sl_iostream_eusart_init(&sl_iostream_vcom, &uart_config_vcom,
                                   &init_vcom, &config_vcom, &context_vcom);
...
void SL_IOSTREAM_EUSART_TX_IRQ_HANDLER(SL_IOSTREAM_EUSART_VCOM_PERIPHERAL_NO)(
    void) {
  sl_iostream_eusart_irq_handler(&sl_iostream_vcom);
}

void SL_IOSTREAM_EUSART_RX_IRQ_HANDLER(SL_IOSTREAM_EUSART_VCOM_PERIPHERAL_NO)(
    void) {
  sl_iostream_eusart_irq_handler(&sl_iostream_vcom);
}
...
static void events_handler(sl_power_manager_em_t from,
  ...
  if (to < SL_POWER_MANAGER_EM3) {
    ...
    context_vcom.context.set_next_byte_detect(sl_iostream_vcom_handle->context,
                                              true);
  }
}
#endif // SL_CATALOG_POWER_MANAGER_PRESENT
