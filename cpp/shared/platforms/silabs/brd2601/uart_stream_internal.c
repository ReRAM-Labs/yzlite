...
void uart_stream_internal_get_dma_config(UartStreamDmaConfig *config) {
  ...
  config->rx_cfg = cfg;
  config->rx_cfg.ldmaReqSel =
      RX_DMA_SIGNAL(SL_IOSTREAM_EUSART_VCOM_PERIPHERAL_NO);
  config->rx_address = (uint32_t)&peripheral->RXDATA;

  config->tx_cfg = cfg;
  config->tx_cfg.ldmaReqSel =
      TX_DMA_SIGNAL(SL_IOSTREAM_EUSART_VCOM_PERIPHERAL_NO);
