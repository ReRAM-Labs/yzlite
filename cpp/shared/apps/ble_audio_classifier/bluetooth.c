...
void sl_bt_on_event(sl_bt_msg_t *evt) {
  ...
  case sl_bt_evt_gatt_server_characteristic_status_id:
    // if (gattdb_report_button ==
    // evt->data.evt_gatt_server_characteristic_status.characteristic) {
    //   // A local Client Characteristic Configuration descriptor was changed
    //   in
...
static void notify_keyword_detected(uint8_t class_id, uint8_t confidence) {
  ...
  sc = sl_bt_gatt_server_write_attribute_value(gattdb_command, 0, len, data);
  if (sc != SL_STATUS_OK) {
    printf("Failed to write GATT DB, err:%d\n", sc);
    return;
  }

  sc = sl_bt_gatt_server_notify_all(gattdb_command, len, data);
  if (sc != SL_STATUS_OK) {
