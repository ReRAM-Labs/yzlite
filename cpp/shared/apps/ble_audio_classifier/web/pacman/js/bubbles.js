        ...
        for (var x = BUBBLES_X_START, xmax = BUBBLES_X_END, bubble = 1; x < xmax; bubble++, x += BUBBLES_GAP) {
            if (canAddBubble(line, bubble)) {
                ...
                if (isSuperBubble(line, bubble)) {
                    ...
                    SUPER_BUBBLES[s] =
                        line + ';' + bubble + ';' + parseInt(correctionX(x, bubble)) + ',' + parseInt(y) + ';0'
                    s++
                } else {
                    type = 'b'
                    size = BUBBLES_SIZE
                ...
                BUBBLES_ARRAY.push(
                    parseInt(correctionX(x, bubble)) + ',' + parseInt(y) + ';' + line + ';' + bubble + ';' + type + ';0'
                )
                i++
            }
        }
...
function canAddBubble(line, bubble) {
    if (
        ...
        line === 8 &&
        ((bubble >= 7 && bubble <= 8) || (bubble >= 13 && bubble <= 14) || (bubble >= 19 && bubble <= 20))
    ) {
        return false
