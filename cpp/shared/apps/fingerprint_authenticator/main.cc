...
extern "C" int main(void) {
  ...
  YZLITE_INFO("Fingerprint Authenticator app starting\n");

  // First check if a new .tflite was programmed to the end of flash
  // (This will happen when this app is executed from the command-line: "yzlite
  // classify_image my_model")
  if (!yzlite::get_tflite_flatbuffer_from_end_of_flash(
          &model_flatbuffer, nullptr, &linker_nvm_begin)) {
  ...
  if (status != SL_STATUS_OK) {
    YZLITE_ERROR("Failed to initialize the finerprint reader, err: %u", status);
    app_controller.update_state(AppController::State::fatalError);
    return -1;
  }
  ...
  logger.info("  The LED will be solid red, blue, or purple to signify the "
              "current user:");
  logger.info("  - RED    -> user 0");
  logger.info("  - BLUE   -> user 1");
  logger.info("  - PURPLE -> user 2\n");

  logger.info("* Press button 2 for 10s then release to erase the current "
              "user's signatures.");
  logger.info(
  ...
  logger.info("  The LED will flash blue when you should place your finger on "
              "the reader.");
  ...
  logger.info("  i.e. The SAME finger will be captured %d times.",
              SAVED_FINGERPRINT_COUNT);
  ...
  logger.info("  The LED will be solid red, blue, or purple for the "
              "authenticated user.");
  logger.info("  The LED will flash purple for an unknown fingerprint\n");

  logger.info("\n\nHINT: Run the following command to view the captured "
              "fingerprints:\n");
  logger.info("yzlite fingerprint_reader");

  if (fingerprint_authenticator.is_disabled()) {
    YZLITE_WARN(
        "Inference disabled, not generating signature from fingerprint");
  }
...
static void process_normal_mode() {
  ...
  YZLITE_INFO("Reading fingerprint from reader");
  ...
  sl_status_t status =
      fingerprint_reader_get_image(app_controller.image_buffer);
  jlink_stream_set_interrupt_enabled(true);
  if (status != SL_STATUS_OK) {
    if (status == SL_STATUS_EMPTY) {
      YZLITE_WARN("No finger detected, please try again");
      app_controller.update_state(AppController::State::readError);
    } else if (status == SL_STATUS_INVALID_STATE) {
      YZLITE_WARN("Bad fingerprint quality, please try again");
      app_controller.update_state(AppController::State::readError);
    } else {
      YZLITE_WARN("Failed to read sensor, err: %d", status);
  ...
  jlink_stream::write_all("raw", app_controller.image_buffer,
                          sizeof(fingerprint_reader_image_t));

  if (fingerprint_authenticator.is_disabled()) {
    return;
  }

  YZLITE_INFO("Generating signature from fingerprint");

  // Use ML to generate a signature from the fingerprint image
  bool fingerprint_image_valid;
  if (!fingerprint_authenticator.generate_signature(
          app_controller.image_buffer, signature, fingerprint_image_valid)) {
    YZLITE_WARN("Error while generating fingerprint signature");
    app_controller.update_state(AppController::State::fatalError);
    return;
  } else if (!fingerprint_image_valid) {
    YZLITE_WARN("The captured fingerprint has poor quality, please try again");
    app_controller.update_state(AppController::State::readError);
    return;
  }

  YZLITE_INFO("Authenticating signature: %s",
              FingerprintAuthenticator::signature_to_str(signature));

  // Authenticate the signature
  if (!fingerprint_authenticator.authenticate_signature(signature, user_id)) {
    YZLITE_WARN("Failed to authenticate signature");
    app_controller.update_state(AppController::State::fatalError, false);
    return;
  }

  if (user_id == -1) {
    YZLITE_INFO("Unknown fingerprint");
    app_controller.update_state(AppController::State::signatureInvalid);
  } else {
    YZLITE_INFO("Fingerprint belongs to user_id: %d", user_id);
...
static void process_save_fingerprint_mode() {
  ...
  YZLITE_INFO("Collecting %d fingerprints for user %d", SAVED_FINGERPRINT_COUNT,
              current_user_id);

  // Display the current user's LED pattern for a moment
  app_controller.display_user(current_user_id);

  YZLITE_INFO("Place finger on reader ...");
  ...
  for (int sample_count = 0;;) {
    ...
    for (;;) {
      ...
      if ((app_controller.current_timestamp() - start_time) > 10000) {
        YZLITE_WARN("Timed-out waiting for user to place finger on reader");
        return;
      }
    ...
    if (!fingerprint_authenticator.generate_signature(
      ...
      YZLITE_WARN(
          "The captured fingerprint has poor quality, ensure your finger "
          "is clean and please try again");
      app_controller.update_state(AppController::State::readError);
      // Continue to the beginning of the loop
      continue;
    }

    YZLITE_DEBUG("Signature: %s",
                 FingerprintAuthenticator::signature_to_str(signature));
    YZLITE_INFO("Saving signature %d of %d", sample_count + 1,
                SAVED_FINGERPRINT_COUNT);

    // Save the signature for the current user
    if (!fingerprint_authenticator.save_signature(signature, current_user_id)) {
      YZLITE_WARN("Failed to save signature for current user");
      app_controller.update_state(AppController::State::fatalError);
      return;
    }
    ...
    if (sample_count >= SAVED_FINGERPRINT_COUNT) {
      YZLITE_INFO("Finished collecting fingerprints");
      break;
    }
    YZLITE_INFO("Place your SAME finger on the reader ...");
    app_controller.update_state(AppController::State::placeFinger, false);
  }
}
...
static void process_iterate_users_mode() {
  ...
  do {
    ...
    YZLITE_INFO("Current user: %d", current_user_id);
...
static void process_erase_signature_mode() {
  ...
  YZLITE_INFO("Continue to press button 2 for 10s then release to erase user "
              "%d's signatures",
              current_user_id);
  ...
  do {
    button_pressed =
        app_controller.ensure_should_erase_user_signatures(should_erase);
    if (should_erase && !printed_msg) {
      printed_msg = true;
      YZLITE_INFO("Release button 2 to erase user %d's signatures",
                  current_user_id);
    }
  } while (button_pressed);

  if (!should_erase) {
    YZLITE_INFO("Aborting erasing user signatures");
    return;
  }

  YZLITE_INFO("Erasing signatures for user %d", current_user_id);
  ...
  if (!fingerprint_authenticator.remove_signatures(current_user_id)) {
    YZLITE_WARN("Failed to erase user signatures");
    app_controller.update_state(AppController::State::fatalError);
    return;
  }
  app_controller.update_state(AppController::State::erasedSignatures, true);

  YZLITE_INFO("Signatures erased");
}
