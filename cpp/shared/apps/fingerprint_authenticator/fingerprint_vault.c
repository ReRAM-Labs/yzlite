...
#define MAKE_NVM_KEY(user_id, index)                                           \
  ((((uint32_t)user_id) << 16) | ((uint32_t)index))

static uint8_t signature_length = 0;

/*************************************************************************************************/
static int retrieve_user_signature_keys(int32_t user_id,
                                        nvm3_ObjectKey_t *buffer) {
  return nvm3_enumObjects(nvm3_defaultHandle, buffer, MAX_SIGNATURES_PER_USER,
                          MAKE_NVM_KEY(user_id, 0),
                          MAKE_NVM_KEY(user_id, MAX_SIGNATURES_PER_USER - 1));
}
...
bool fingerprint_vault_iterate_user(
    int32_t user_id,
    void (*callback)(int32_t uid, int32_t fpid, const FingerprintSignature *,
                     void *arg),
    void *arg) {
  ...
  for (; uid < uid_end; ++uid) {
    ...
    for (int fpip = 0; fpip < sig_count; ++fpip) {
      ...
      Ecode_t err = nvm3_readData(nvm3_defaultHandle, signatures_keys[fpip],
                                  signature_buffer, signature_length);
      if (err != ECODE_NVM3_OK) {
        continue;
      }

      const FingerprintSignature found_sig = {signature_buffer,
                                              signature_length};
...
/*************************************************************************************************/
bool fingerprint_vault_save_user_signature(
  ...
  Ecode_t result =
      nvm3_writeData(nvm3_defaultHandle, MAKE_NVM_KEY(user_id, signature_count),
...
int fingerprint_vault_count_user_signatures(int32_t user_id) {
  ...
  return nvm3_enumObjects(nvm3_defaultHandle, signatures_keys,
                          MAX_SIGNATURES_PER_USER, MAKE_NVM_KEY(user_id, 0),
                          MAKE_NVM_KEY(user_id, MAX_SIGNATURES_PER_USER - 1));
}
