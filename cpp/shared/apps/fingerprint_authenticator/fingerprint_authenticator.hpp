...
public:
  ...
  bool generate_signature(const fingerprint_reader_image_t fingerprint_image,
                          FingerprintSignature &signature,
                          bool &fingerprint_image_valid);

  bool authenticate_signature(const FingerprintSignature &signature,
                              int32_t &user_id);
  bool save_signature(const FingerprintSignature &signature, int32_t user_id);
  bool remove_signatures(int32_t user_id);

  bool is_disabled() const { return _disabled; }

  static const char *signature_to_str(const FingerprintSignature &signature,
                                      char *buffer = nullptr);
