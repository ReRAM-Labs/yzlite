...
/*************************************************************************************************/
bool DataPreprocessor::load(const TfliteModelParameters &params, uint16_t width,
                            uint16_t height) {
...
bool DataPreprocessor::verify_sample(const uint8_t *sample) const {
  ...
  YZLITE_DEBUG("\ndark_full=%d\nlight_full=%d\nabs(dark_full-light_full)=%d\n("
               ...
               dark_full, light_full, std::abs(dark_full - light_full),
               ((dark_full + light_full) / _verify_full_threshold));
  YZLITE_DEBUG(
      "\ndark_center=%d\nlight_center=%d\nabs(dark_center-light_center)="
      "%d\n(dark_center+light_center)/_verify_center_threshold=%d\n",
      dark_full, light_full, std::abs(dark_center - light_center),
      (dark_center + light_center) / _verify_center_threshold);

  if (std::abs(dark_full - light_full) >
      ((dark_full + light_full) / _verify_full_threshold)) {
    return false;
  }
  if (std::abs(dark_center - light_center) >
      ((dark_center + light_center) / _verify_center_threshold)) {
    return false;
  }
...
/*************************************************************************************************/
bool DataPreprocessor::preprocess_sample(const uint8_t *unprocessed_sample,
                                         uint8_t *processed_sample) const {
