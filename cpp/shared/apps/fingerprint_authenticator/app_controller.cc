...
bool AppController::process() {
  ...
  if (button_is_pressed(&sl_button_btn0, _button0_press_context, 75, 1000)) {
    YZLITE_DEBUG("Button 1 clicked");
    update_mode(Mode::saveFingerprints);
    retval = true;
  } else if (button_is_pressed(&sl_button_btn1, _button1_click_context, 75,
                               1000)) {
    YZLITE_DEBUG("Button 2 clicked");
    update_mode(Mode::iterateUsers);
    retval = true;
  } else if (button_is_pressed(&sl_button_btn1, _button1_press_context, 7000,
                               10000, &elapsed_ms) ||
             elapsed_ms > 1500) {
    if (elapsed_ms > 1500) {
      update_mode(Mode::eraseSignatures);
      YZLITE_DEBUG("Button 2 pressed");
      retval = true;
    }
  }
  ...
  } else if (_button0_press_context != 0 || _button1_click_context != 0 ||
             _button1_press_context != 0) {
    retval = true;
  }
...
/*************************************************************************************************/
int32_t AppController::current_user_id() { return _current_user_id; }

/*************************************************************************************************/
void AppController::update_mode(Mode mode) {
  if (_current_mode != mode) {
    YZLITE_DEBUG("New mode: %s -> %s", to_str(_current_mode), to_str(mode));
    _current_mode = mode;
  }
}
...
void AppController::update_state(State state, bool wait) {
  ...
  if (_current_state != state) {
    YZLITE_DEBUG("New state: %s -> %s", to_str(_current_state), to_str(state));

    for (int i = 10; i > 0; ++i) {
      if (fingerprint_reader_update_led(&LED_STATES[(int)state].config) ==
          SL_STATUS_OK) {
...
bool AppController::display_user(int32_t user_id, bool check_button) {
  ...
  led_config.color = (user_id == 0)   ? fprRedLed
                     : (user_id == 1) ? fprBlueLed
                                      : fprPurpleLed;
  ...
  if (check_button) {
    ...
    YZLITE_INFO("  %d saved signatures", signature_count);

    const uint32_t start_time = current_timestamp();
    while ((current_timestamp() - start_time) < 2500) {
      if (button_is_pressed(&sl_button_btn1, _button1_click_context, 75,
                            1000)) {
        YZLITE_DEBUG("Button 2 clicked");
        return true;
      }
...
/*************************************************************************************************/
bool AppController::button_is_pressed(const sl_button_t *handle,
  ...
  if (active_timestamp == 0 && is_active) {
    ...
    if (!is_active) {
      ...
      if (elapsed_time >= min_time_ms && elapsed_time <= max_time_ms) {
        YZLITE_DEBUG("elapsed: %d", elapsed_time);
        return true;
      }
    }
  }
...
const char *to_str(AppController::State state) {
  switch (state) {
  case AppController::State::init:
    return "init";
  case AppController::State::idle:
    return "idle";
  case AppController::State::reading:
    return "reading";
  case AppController::State::readError:
    return "readError";
  case AppController::State::signatureInvalid:
    return "signatureInvalid";
  case AppController::State::placeFinger:
    return "placeFinger";
  case AppController::State::displayCurrentUser:
    return "displayCurrentUser";
  case AppController::State::eraseSignaturesInit:
    return "eraseSignaturesInit";
  case AppController::State::erasedSignatures:
    return "erasedSignatures";
  case AppController::State::fatalError:
    return "fatalError";
  default:
    return "unknown";
  }
