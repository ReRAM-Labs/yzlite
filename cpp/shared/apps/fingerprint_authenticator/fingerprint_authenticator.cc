...
/*************************************************************************************************/
bool FingerprintAuthenticator::init() { return true; }
...
bool FingerprintAuthenticator::load_model(const void *flatbuffer) {
  ...
  if (!model.load(flatbuffer, _op_resolver)) {
    YZLITE_ERROR("Failed to load .tflite");
    return false;
  }
  ...
  YZLITE_INFO("Signature threshold: %.3f", _auth_threshold);

  if (!_data_preprocessor.load(model.parameters, input_width, input_height)) {
    YZLITE_ERROR("Failed to load data preprocessor");
    return false;
  }

  const fingerprint_vault_config_t vault_config = {_signature_length};

  if (!fingerprint_vault_init(&vault_config)) {
    YZLITE_ERROR("Failed to initialize the fingerprint vault");
    return false;
  }
...
bool FingerprintAuthenticator::generate_signature(
    ...
    FingerprintSignature &signature, bool &fingerprint_image_valid) {
  ...
  if (!_data_preprocessor.preprocess_sample(
          (const uint8_t *)fingerprint_image,
          (uint8_t *)processed_image_buffer)) {
    YZLITE_ERROR("Failed to preprocess sample");
    return false;
  }

  if (!_data_preprocessor.verify_sample((uint8_t *)processed_image_buffer)) {
    YZLITE_WARN("Image not valid");
    fingerprint_image_valid = false;
    return true;
  }
  ...
  if (!model.invoke()) {
    YZLITE_ERROR("Failed to invoke ML model");
    return false;
  }
...
bool FingerprintAuthenticator::authenticate_signature(
    const FingerprintSignature &signature, int32_t &user_id) {
  struct CompareContext {
    ...
    CompareContext(const FingerprintSignature &sig,
                   const TfLiteQuantizationParams &q_params)
        : signature(sig), q_params(q_params), best_score(1e6f),
          best_user_id(-1) {}
  };

  auto compare_callback = [](int32_t uid, int32_t fpid,
                             const FingerprintSignature *sig, void *ctx) {
    ...
    for (int i = 0; i < sig_length; ++i) {
      const float s1 =
          dequantized_value(context.q_params, context.signature.data[i]);
    ...
    YZLITE_INFO("User: %d-%d similarity score: %.3f", uid, fpid, score);
...
/*************************************************************************************************/
const char *FingerprintAuthenticator::signature_to_str(
    const FingerprintSignature &signature, char *buffer) {
