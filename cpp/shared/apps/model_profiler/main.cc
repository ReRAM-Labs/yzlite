...
static void dump_recorded_data(TfliteMicroModel &model,
...
#if __has_include("yzlite_model_profiler_generated_model_op_resolver.hpp")
// Check if yzlite_model_profiler_generated_model_op_resolver.hpp was
// automatically generated. If so, the header contains only the TFLM kernels
// that are required by the given model. This can *greatly* reduce the app's
// flash space. To enable this feature, add the CMake variable:
// MODEL_PROFILER_GENERATE_OP_RESOLVER_HEADER
#include "yzlite_model_profiler_generated_model_op_resolver.hpp"
...
tflite::AllOpsResolver op_resolver;
    ...
    MODEL_PROFILER_RUNTIME_MEMORY_SIZE > 0
...
uint8_t __attribute__((section(MODEL_PROFILER_RUNTIME_MEMORY_SECTION)))
runtime_memory_buffer[MODEL_PROFILER_RUNTIME_MEMORY_SIZE];
#else
...
extern "C" int main(void) {
  ...
  if (!load_model(model, logger, cli_opts.model_flatbuffer,
...
static bool load_model(TfliteMicroModel &model, logging::Logger &logger,
  ...
  // profiler my_model --device")
  else if (!get_tflite_flatbuffer_from_end_of_flash(
               &tflite_flatbuffer, &tflite_flatbuffer_length)) {
...
#ifdef YZLITE_RUN_MODEL_FROM_RAM
  uint8_t *tflite_ram_buffer = (uint8_t *)malloc(tflite_flatbuffer_length);
  if (tflite_ram_buffer == nullptr) {
    logger.error(
        "Cannot load .tflite into RAM. Failed to allocate %d bytes for buffer",
        tflite_flatbuffer_length);
    return -1;
  }
...
#endif // YZLITE_RUN_MODEL_FROM_RAM
  ...
  if (!model.load(tflite_flatbuffer, op_resolver, runtime_memory_buffer,
                  runtime_memory_size)) {
...
static void dump_recorded_data(TfliteMicroModel &model,
  ...
  if (msgpack_deserialize_with_buffer(&root_obj, buffer, buffer_length,
  ...
  auto msgpack_iterator = [](const msgpack_object_t *key,
                             const msgpack_object_t *value, void *arg) -> int {
    ...
    if (context.current_index == context.n_values) {
      ...
      context.logger.info("  %s:",
                          msgpack_to_str(key, key_str, sizeof(key_str)));
      ...
      context.logger.info("    %d: %d elements", context.current_index,
                          MSGPACK_BIN_LENGTH(value));
      ...
      context.logger.info("    %s: %s",
                          msgpack_to_str(key, key_str, sizeof(key_str)),
                          msgpack_to_str(value, val_str, sizeof(val_str)));
      context.current_index += 1;
    }
