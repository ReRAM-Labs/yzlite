...
yzlite::StringList SL_TFLITE_MODEL_CLASSES;
yzlite::Int32List SL_TFLITE_DETECTION_THRESHOLD_LIST;
int SL_TFLITE_MODEL_BAUD_RATE = 0;

bool yzlite_app_settings_load_parameters(const void *tflite_flatbuffer) {
  ...
  if (!cli_opts.average_window_duration_ms_provided) {
    model_parameters.get("average_window_duration_ms",
                         SL_TFLITE_MODEL_AVERAGE_WINDOW_DURATION_MS);
  }

  SL_TFLITE_MODEL_DETECTION_THRESHOLD = cli_opts.detection_threshold;
  if (!cli_opts.detection_threshold_provided) {
    model_parameters.get("detection_threshold",
                         SL_TFLITE_MODEL_DETECTION_THRESHOLD);
  }

  if (model_parameters.get("detection_threshold_list",
                           SL_TFLITE_DETECTION_THRESHOLD_LIST)) {
    if (SL_TFLITE_DETECTION_THRESHOLD_LIST.size() !=
        SL_TFLITE_MODEL_CLASSES.size()) {
      printf("The number of entries in the model parameter: "
             "'detection_threshold_list' must match the number of classes\n");
      return false;
    }
  }
  ...
  if (!cli_opts.verbose_provided) {
    model_parameters.get("verbose_model_output_logs",
  ...
  if (cli_opts.dump_audio) {
    ...
    model_parameters.get("dump_audio",
  ...
  if (cli_opts.dump_raw_spectrograms) {
    ...
    model_parameters.get("dump_raw_spectrograms",
  ...
  if (!cli_opts.volume_gain_provided) {
    model_parameters.get("volume_gain",
