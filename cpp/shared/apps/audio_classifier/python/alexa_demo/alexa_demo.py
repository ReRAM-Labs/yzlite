...
def application_loop(
    ...
    with DevBoard(baud=baud) as board:
        ...
        while True:
            logger.info(
                'Waiting for an "Alexa" command. (speak into the dev board microphone)')
            ...
            for directive in directives:
                if directive.name == 'ExpectSpeech':
                    ...
                    logger.info(
                        f'Received response from AVS cloud: {len(directive.audio_attachment)} bytes')
...
class DevBoard:
    ...
    def play_audio(self, mp3_data: bytes):
        ...
        self._uart.write_command(
            self.CMD_PLAY_AUDIO, struct.pack('<l', len(mp3_data)))
    ...
    def __enter__(self):
        logger.info(
            f'Opening UART connection to development board (BAUD={self._uart.baud}) ...')
        self.open()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--baud', default=115200,
                        help='Specify the UART BAUD rate', type=int, metavar='<rate>')
