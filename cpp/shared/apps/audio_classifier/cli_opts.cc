...
void parse_cli_opts() {
  cxxopts::Options options(
      "Audio Classifier",
      "Classify streaming audio from a microphone using the given ML model");
  options.add_options()("v,verbose", "Enable verbose logging")(
      "l,latency", "Number of ms to simulate per execution loo",
      cxxopts::value<uint32_t>())(
      "m,model",
      "Path to .tflite model file. Use built-in, default model if omitted",
      cxxopts::value<std::string>())(
      "w,window_duration",
      "Controls the smoothing. Longer durations (in milliseconds) will give a "
      "higher confidence that the results are correct, but may miss some "
      "commands",
      cxxopts::value<uint32_t>())(
      "c,count",
      "The minimum number of inference results to average when calculating the "
      "detection value",
      cxxopts::value<uint32_t>())(
      "t,threshold",
      "Minumum model output threshold for a class to be considered detected, "
      "0-255. Higher values increase precision at the cost of recall",
      cxxopts::value<uint32_t>())(
      "s,suppression",
      "Amount of milliseconds to wait after a keyword is detected before "
      "detecting new keywords",
      cxxopts::value<uint32_t>())("d,volume",
                                  "Increase/decrease microphone audio gain. 0 "
                                  "= no change, <0 decrease, >0 increase",
                                  cxxopts::value<int>())(
      "x,dump_audio", "Dump the raw audio samples to the given directory",
      cxxopts::value<std::string>())(
      "r,dump_raw_spectrograms",
      "Dump the raw (i.e. unquantized) generated spectorgrams to the given "
      "directory",
      cxxopts::value<std::string>())(
      "z,dump_spectrograms",
      "Dump the quantized generated spectorgrams to the given directory",
      cxxopts::value<std::string>())(
      "i,sensitivity", "Sensitivity of the activity indicator",
  ...
  try {
    ...
    if (result.count("model")) {
      ...
      if (fp == nullptr) {
        YZLITE_ERROR("Failed to open model file: %s", path);
        exit(-1);
      }
      ...
      fclose(fp);
      if (result != file_size) {
        YZLITE_ERROR("Failed to read model file: %s", path);
        exit(-1);
      }
    ...
    if (result.count("window_duration")) {
      cli_opts.average_window_duration_ms =
          result["window_duration"].as<uint32_t>();
    ...
    if (result.count("dump_raw_spectrograms")) {
      ...
      dump_raw_spectrograms_dir =
          result["dump_raw_spectrograms"].as<std::string>();
    }
