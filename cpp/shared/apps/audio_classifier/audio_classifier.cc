...
static void handle_results(int32_t current_time, int result, uint8_t score,
                           bool is_new_command);
...
void audio_classifier_init(void) {
  ...
  // classify_audio my_model --device")
  ...
  if (!yzlite_sl_ml_audio_feature_generation_load_parameters(
          cli_opts.model_flatbuffer)) {
    printf("ERROR: Failed to load audio feature generator parameters from "
           ".tflite model\n");
    while (1)
      ;
  }
  ...
  if (!yzlite_app_settings_load_parameters(cli_opts.model_flatbuffer)) {
  ...
  static RecognizeCommands static_recognizer(
      SMOOTHING_WINDOW_DURATION_MS, DETECTION_THRESHOLD, SUPPRESION_TIME_MS,
      MINIMUM_DETECTION_COUNT, IGNORE_UNDERSCORE_LABELS);
  ...
  if (category_count != category_label_count) {
    printf("WARNING: Number of categories(%d) is not equal to the number of "
           "labels(%d).\n"
           "Make sure that CATEGORY_LABELS is configured correctly for the "
           "model in use.\n",
           category_count, category_label_count);
  }

  if (!(input->type == kTfLiteInt8 || input->type == kTfLiteUInt16 ||
  ...
  OSTaskCreate(&tcb, task_name, audio_classifier_task, DEF_NULL, TASK_PRIORITY,
  ...
  status = sl_sleeptimer_start_periodic_timer_ms(
      &inference_timer, INFERENCE_INTERVAL_MS, nullptr, nullptr, 0, 0);
  if (status != SL_STATUS_OK) {
...
extern "C" void app_process_action() {
  ...
  if ((current_timestamp - prev_loop_timestamp) >= INFERENCE_INTERVAL_MS) {
    ...
    // If the activity detection block is enabled, then ensure there is activity
...
static sl_status_t run_inference() {
  ...
  sl_status_t status =
      sl_ml_audio_feature_generation_fill_tensor(model.input());
  if (status != SL_STATUS_OK) {
...
static sl_status_t process_output(const bool did_run_inference) {
  ...
  if (did_run_inference)
    process_status = command_recognizer->ProcessLatestResults(
        model.output(), current_timestamp, &result, &score, &is_new_command);
...
static void handle_results(int32_t current_time, int result, uint8_t score,
  ...
  if (is_new_command) {
    ...
    // NOTE: Alternatively, the "suppression" setting can be increased to add a
    // delay
    //       until processing states again (this effectively clears the audio
    //       buffer)
