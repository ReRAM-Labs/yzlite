...
#define SPEAKER_FRAMES_PER_DMA                                                 \
  ALIGN_DOWN(LDMA_MAX_XFER_LENGTH, MPEG2_FRAME_LENGTH)
...
#define MICROPHONE_OPUS_FRAME_SIZE                                             \
  ((MICROPHONE_SAMPLE_RATE * MICROPHONE_OPUS_LENGTH_MS) / 1000)

#define MICROPHONE_CHUNK_LENGTH_MS (MICROPHONE_OPUS_LENGTH_MS * 10)
#define MICROPHONE_CHUNK_LENGTH                                                \
  ((MICROPHONE_CHUNK_LENGTH_MS * MICROPHONE_SAMPLE_RATE) / 1000)
#define MICROPHONE_CHUNK_PCM_BYTE_LENGTH                                       \
  (MICROPHONE_CHUNK_LENGTH * sizeof(int16_t))
#define MICROPHONE_OPUS_BYTE_LENGTH 80 // 80 bytes per AVS spec
#define MICROPHONE_CHUNK_OPUS_BYTE_LENGTH                                      \
  (MICROPHONE_OPUS_BYTE_LENGTH *                                               \
   (MICROPHONE_CHUNK_LENGTH_MS / MICROPHONE_OPUS_LENGTH_MS))
...
static struct {
  ...
  uint32_t n_processed_pcm_samples;
  uint8_t
      opus_encoder_buffer[24544]; // this comes from opus_encoder_get_size(1))
  uint8_t opus_buffer[MICROPHONE_CHUNK_OPUS_BYTE_LENGTH];
  uint8_t vad_context[752]; // this comes from
                            // voice_activity_detector_context_size()
  bool vad_detected;
...
/*************************************************************************************************/
bool audio_io_init() {
  if (!uart_stream::initialize(speaker.uart_rx_buffer,
...
void audio_io_set_microphone_streaming_enabled(bool enabled) {
  if (!enabled) {
    ...
    CORE_ATOMIC_SECTION(tail = microphone.tail;
                        microphone.n_buffered_pcm_samples = 0;)
    ...
    // Ensure that the microphone.head pointer is aligned to an
    // MICROPHONE_OPUS_FRAME_SIZE this way we can cleanly wrap to the beginning
    // of the buffer
    uintptr_t offset = ALIGN_DOWN((uintptr_t)(tail - microphone.start),
                                  MICROPHONE_OPUS_FRAME_SIZE);
...
static bool microphone_init() {
  ...
  assert(sizeof(microphone.opus_encoder_buffer) == opus_encoder_size);
  (void)opus_encoder_size;
  assert(SL_TFLITE_MODEL_FE_SAMPLE_RATE_HZ == MICROPHONE_SAMPLE_RATE);
  assert(SL_ML_AUDIO_FEATURE_GENERATION_AUDIO_BUFFER_SIZE %
             MICROPHONE_OPUS_FRAME_SIZE ==
         0);
  ...
  auto microphone_data_callback = [](const int16_t *buffer, uint32_t n_frames) {
    ...
    microphone.n_buffered_pcm_samples =
        std::min((int)(microphone.n_buffered_pcm_samples + n_frames),
  ...
  microphone.end =
      microphone.start + SL_ML_AUDIO_FEATURE_GENERATION_AUDIO_BUFFER_SIZE;
  microphone.tail = microphone.start;
  microphone.head = nullptr;

  auto opcoder_encoder = (OpusEncoder *)microphone.opus_encoder_buffer;
  // NOTE: Using OPUS_APPLICATION_RESTRICTED_LOWDELAY is critical for running on
  // Bobcat
  //       AVS does not seem to mind the slightly degraded audio quality
  int error = opus_encoder_init(opcoder_encoder, MICROPHONE_SAMPLE_RATE, 1,
  ...
  opus_encoder_ctl(opcoder_encoder,
  ...
  assert(sizeof(microphone.vad_context) == vad_context_size);
  (void)vad_context_size;
 ...
 *
 * This also uses Voice Activity Detection (VAD) to determine when the user
 * finishes speaking a command
 */
static void microphone_process_outgoing_data() {
  // Check if we should start the microphone at the end of the current speaker
  // audio stream
  if (microphone.start_at_end_of_speaker_audio &&
      speaker.n_buffered_pcm_samples <= SPEAKER_FRAMES_PER_DMA) {
    audio_io_set_microphone_streaming_enabled(true);
  }

  // Check if the microphone is enabled and we have enough buffer microphone
  // audio
  const uint32_t n_buffered_pcm_samples = microphone.n_buffered_pcm_samples;
  if (microphone.head == nullptr ||
      n_buffered_pcm_samples < MICROPHONE_CHUNK_LENGTH) {
  ...
  // Determine the amount of contiguous microphone audio we can read, compress,
  // and write to UART stream
  const int16_t *microphone_src = microphone.head;
  const uintptr_t length_to_end = microphone.end - microphone_src;
  const int32_t pcm_chunk_length =
      std::min((int32_t)length_to_end, (int32_t)MICROPHONE_CHUNK_LENGTH);
  assert(pcm_chunk_length != 0 &&
         pcm_chunk_length % MICROPHONE_OPUS_FRAME_SIZE == 0);
  ...
  CORE_ATOMIC_SECTION(microphone.n_buffered_pcm_samples -= pcm_chunk_length;);
  ...
  for (int i = pcm_chunk_length; i > 0; --i) {
    const int32_t v = *p;
    *p++ = std::max(std::min(v * MICROPHONE_GAIN, (int32_t)32768),
                    (int32_t)-32768);
  }
#endif
  ...
  for (int32_t remaining = pcm_chunk_length; remaining > 0;
       remaining -= MICROPHONE_OPUS_FRAME_SIZE) {
    int enc_len = opus_encode(opcoder_encoder, src, MICROPHONE_OPUS_FRAME_SIZE,
  ...
  // We do this by first detecting when the user is activity speaking the
  // command AND THEN when there is no voice activity for a short period of
  // time. Once these conditions are met (or timed-out), then we stop the
  // microphone step to indicate to the Python script that the command has
  // ended.
  ...
  if (!microphone.vad_detected) {
    ...
    if (voice_activity_detector_process(microphone.vad_context, microphone_src,
                                        pcm_chunk_length, 0.8, 350) == 1) {
      // At this point, the user is actively speaking the command
      microphone.vad_detected = 1;
      voice_activity_detector_reset(microphone.vad_context);
    }
    // Otherwise, if there has only been silence for 5s, then stop the
    // microphone
    ...
    if (voice_activity_detector_process(microphone.vad_context, microphone_src,
                                        pcm_chunk_length, 0.3, 750) == 0) {
      // Silence has been detected for ~750ms, we consider this the end of the
      // command So stop the microphone and notify the Python script
      audio_io_set_microphone_streaming_enabled(false);
    }
    // Otherwise, if there has been continuous non-silence for 15s then just
    // stop the microphone
    else if (microphone.n_processed_pcm_samples >=
             MICROPHONE_SAMPLE_RATE * 15) {
      audio_io_set_microphone_streaming_enabled(false);
    }
  }
}
...
static bool speaker_init() {
  ...
  //
  {
    ...
    speaker.vdac_buffer_end =
        speaker.vdac_buffer_ptr + SPEAKER_FRAMES_PER_DMA * SPEAKER_DMA_COUNT;
...

  {
    ...
    // Since the minimum load requirement for high capacitance mode is 25 nF,
    // turn this mode off
  ...
  //
  {
    static const LDMA_Descriptor_t desc_template = {
        .xfer = {.structType = ldmaCtrlStructTypeXfer,
                 .structReq = 0,
    ...
    // This is called by the DMADRV each time SPEAKER_FRAMES_PER_DMA samples are
    // written to the VDAC
    auto vdac_dma_callback = [](unsigned int channel, unsigned int sequenceNo,
                                void *userParam) {
      CORE_ATOMIC_SECTION(
          speaker.n_buffered_pcm_samples -= SPEAKER_FRAMES_PER_DMA;
          assert(speaker.n_buffered_pcm_samples >= 0);
          // NOTE: We need to stop the TIMER when then less than OR EQUAL to 1
          // DMA buffer,
    ...
    DMADRV_LdmaStartTransfer(ch, &transferConfig, speaker.dma_descriptors,
                             vdac_dma_callback, nullptr);
  }
...
/*************************************************************************************************/
static void speaker_stop_playback() { TIMER_Enable(SPEAKER_TIMER, false); }
...
static void speaker_process_incoming_data() {
  ...
  int32_t mp3_length = uart_stream::read(mp3_ptr, SPEAKER_MP3_BUFFER_LENGTH -
                                                      speaker.mp3_buffer_used);
  if (mp3_length < 0) {
    return;
  }
  ...
  while (mp3_buffer_remaining > 0 &&
         speaker.n_buffered_pcm_samples <
             SPEAKER_FRAMES_PER_DMA * (SPEAKER_DMA_COUNT - 1)) {
    // Decode the next MPEG2_FRAME_LENGTH
    mp3dec_frame_info_t mp3_info;
    int16_t pcm_samples[MINIMP3_MAX_SAMPLES_PER_FRAME];
    int n_samples =
        mp3dec_decode_frame(&speaker.mp3_decoder, mp3_ptr, mp3_buffer_remaining,
                            pcm_samples, &mp3_info);
    ...
    // Also write an extra buffer of silence to we can properly stop the DMA
    // circular buffer before it's too late.
    if (speaker.n_mp3_bytes_remaining <= 0) {
      const int rounded_samples_length =
          ALIGN_UP(speaker.n_processed_pcm_samples, SPEAKER_FRAMES_PER_DMA);
      const int padding_samples =
          (rounded_samples_length - speaker.n_processed_pcm_samples) +
          SPEAKER_FRAMES_PER_DMA;
    ...
    CORE_ATOMIC_SECTION(speaker.n_buffered_pcm_samples += n_frames_processed;);
  }
  ...
  if (mp3_buffer_remaining > 0 &&
      mp3_buffer_remaining != speaker.mp3_buffer_used) {
