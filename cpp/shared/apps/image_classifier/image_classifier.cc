...
static yzlite::TfliteMicroModel model;
...
static yzlite::StringList category_labels;
int category_count;
...
static void retrieve_next_camera_image(uint8_t **image_data,
                                       uint32_t *image_size);
static void standardize_image_data(uint8_t *image_data, uint32_t image_size);
static void process_inference_output();
static void handle_result(int32_t current_time, int result, uint8_t score,
                          bool is_new_command);
...
void image_classifier_init(void) {
  ...
  if (!yzlite::get_tflite_flatbuffer_from_end_of_flash(&model_flatbuffer)) {
  ...
  yzlite::yzlite_tflite_micro_register_accelerator();
  ...
  static RecognizeCommands static_recognizer(
      app_settings.average_window_duration_ms, app_settings.detection_threshold,
      app_settings.suppression_count, app_settings.minimum_count);
  ...
  OSTaskCreate(&tcb, task_name, image_classifier_task, DEF_NULL, TASK_PRIORITY,
               &stack[0], (TASK_STACK_SIZE / 10u), TASK_STACK_SIZE, 0u, 0u,
  ...
  // This timer will wake it up to execute another loop every
  // app_settings.latency_ms
  sl_status_t status = sl_sleeptimer_start_periodic_timer_ms(
      &inference_timer, std::max(app_settings.latency_ms, (uint32_t)1), nullptr,
      nullptr, 0, 0);
...
static void image_classifier_task(void *arg) {
  ...
  while (1) {
    ...
    OSTimeDlyHMSM(0, 0, 0, std::max(app_settings.latency_ms, (uint32_t)1),
...
static bool load_model_parameters() {
  ...
  model.parameters.get("samplewise_norm.rescale",
                       app_settings.samplewise_norm_rescale);
  if (app_settings.samplewise_norm_rescale != 0) {
    printf("Image data scaler = %f\n", app_settings.samplewise_norm_rescale);
  }

  model.parameters.get("samplewise_norm.mean_and_std",
                       app_settings.samplewise_norm_mean_and_std);
  ...
  if (app_settings.samplewise_norm_rescale != 0 ||
    ...
    if (input_tensor->type != kTfLiteFloat32) {
      printf("ERROR: If using image scaling or samplewise mean/STD "
             "normalization, then the model input type must be float32\n");
      return false;
    }
  ...
  model.parameters.get("activity_sensitivity",
...
static bool initialize_camera() {
  ...
  cam_config.data_format = input_shape[3] == 1 ? ARDUCAM_DATA_FORMAT_GRAYSCALE
  ...
  const uint32_t length_per_image = arducam_calculate_image_buffer_length(
      cam_config.data_format, cam_config.image_resolution.width,
...
static void retrieve_next_camera_image(uint8_t **image_data,
  ...
  for (;;) {
    ...
    if (status == SL_STATUS_IN_PROGRESS) {
      // NOTE: Unfortunately, the camera doesn't not have a way to interrupt the
      // MCU
      //       once an image is ready. So we must periodically poll the camera
      //       to check its status. This can be done by continuously calling
      //       arducam_get_next_image() OR arducam_poll() from a timer interrupt
      //       handler.
...
static void standardize_image_data(uint8_t *image_data, uint32_t image_size) {
  ...
  if (app_settings.samplewise_norm_rescale != 0) {
    ...
    yzlite::scale_tensor(app_settings.samplewise_norm_rescale, image_data,
                         input_data.f, image_size);
  } else if (app_settings.samplewise_norm_mean_and_std) {
    // input_tensor = (image_data - mean(image_data)) / std(image_data)
    yzlite::samplewise_mean_std_tensor(image_data, input_data.f, image_size);
  } else if (input_tensor->type == kTfLiteInt8) {
...
static void handle_result(int32_t current_time, int result, uint8_t score,
  ...
  if (detected_timeout == 0) {
    ...
    if (diff >= app_settings.activity_sensitivity ||
        (previous_result != result)) {
      previous_result = result;
