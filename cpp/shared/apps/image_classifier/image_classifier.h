...
struct AppSettings {
  ...
  bool samplewise_norm_mean_and_std =
      false; // norm_img = (img - mean(img)) / std(img)
  ...
  uint32_t average_window_duration_ms =
      1000; // Drop all inference results older than this value
  uint32_t minimum_count =
      1; // The minimum number of inference results to average
  uint8_t detection_threshold =
      160; // Minimum averaged model output threshold for a class to be
           // considered detected, 0-255
  uint32_t suppression_count =
      1; // The number of samples that are different than the last detected
         // sample for a new detection to occur
  uint32_t latency_ms =
      0; // This the amount of time in milliseconds between processing loop
