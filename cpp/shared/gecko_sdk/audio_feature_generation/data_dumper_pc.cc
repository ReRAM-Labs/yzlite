...
void dump_raw_spectrogram(const uint16_t *buffer, int length) {
  ...
  char path[1024];
  snprintf(path, sizeof(path), "%s/%d.uint16.npy.txt", spectrogram_dir,
           spectrogram_counter++);
...
void dump_int8_spectrogram(const int8_t *buffer, int length) {
  ...
  char path[1024];
  snprintf(path, sizeof(path), "%s/%d.int8.npy.txt", spectrogram_dir,
           spectrogram_counter++);
  auto spectrogram_fp = fopen(path, "wb");
  if (spectrogram_fp != nullptr) {
...
void dump_float_spectrogram(const float *buffer, int length) {
  ...
  char path[1024];
  snprintf(path, sizeof(path), "%s/%d.float32.npy.txt", spectrogram_dir,
           spectrogram_counter++);
  auto spectrogram_fp = fopen(path, "wb");
  if (spectrogram_fp != nullptr) {
