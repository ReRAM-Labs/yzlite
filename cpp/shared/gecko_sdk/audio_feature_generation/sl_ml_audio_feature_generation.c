/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief Yizhu Technology Feature Generator Core Functionality
                                                                               *******************************************************************************
                                                                               * # License
...
#define FEATURE_BUFFER_SLICE_COUNT                                             \
  (1 + ((SL_ML_FRONTEND_SAMPLE_LENGTH_MS - SL_ML_FRONTEND_WINDOW_SIZE_MS) /    \
        SL_ML_FRONTEND_WINDOW_STEP_MS))
#define FEATURE_BUFFER_SIZE                                                    \
  (SL_ML_FRONTEND_FILTERBANK_N_CHANNELS * FEATURE_BUFFER_SLICE_COUNT)
...
// Audio buffer to store long term audio data, needs to be big enough to store
// 100ms of data
...
static sl_status_t process_audio_buffer_chunk(const int16_t *audio_data,
                                              size_t num_samples,
                                              size_t *slices_updated);
...
sl_status_t sl_ml_audio_feature_generation_frontend_init() {
  ...
  config.filterbank.lower_band_limit =
      SL_ML_FRONTEND_FILTERBANK_LOWER_BAND_LIMIT;
  config.filterbank.upper_band_limit =
      SL_ML_FRONTEND_FILTERBANK_UPPER_BAND_LIMIT;

  // This is unused
  config.filterbank.output_scale_shift = 0;

  // Noise reduction settings
  config.noise_reduction.enable_noise_reduction =
      SL_ML_FRONTEND_NOISE_REDUCTION_ENABLE;
  config.noise_reduction.smoothing_bits =
      SL_ML_FRONTEND_NOISE_REDUCTION_SMOOTHING_BITS;
  config.noise_reduction.even_smoothing =
      SL_ML_FRONTEND_NOISE_REDUCTION_EVEN_SMOOTHING;
  config.noise_reduction.odd_smoothing =
      SL_ML_FRONTEND_NOISE_REDUCTION_ODD_SMOOTHING;
  config.noise_reduction.min_signal_remaining =
      SL_ML_FRONTEND_NOISE_REDUCTION_MIN_SIGNAL_REMAINING;
  ...
  config.activity_detection.enable_activation_detection =
  ...
  config.activity_detection.arm_threshold =
      SL_ML_FRONTEND_ACTIVITY_DETECTION_ARM_THRESHOLD;
  config.activity_detection.trip_threshold =
      SL_ML_FRONTEND_ACTIVITY_DETECTION_TRIP_THRESHOLD;

  // DC notch filter settings
  config.dc_notch_filter.enable_dc_notch_filter =
      SL_ML_FRONTEND_DC_NOTCH_FILTER_ENABLE;
  config.dc_notch_filter.coefficient =
      SL_ML_FRONTEND_DC_NOTCH_FILTER_COEFFICIENT;

  if (SL_ML_AUDIO_FEATURE_GENERATION_QUANTIZE_DYNAMIC_SCALE_ENABLE) {
// dynamic_range =
// SL_ML_AUDIO_FEATURE_GENERATION_QUANTIZE_DYNAMIC_SCALE_RANGE_DB*(2^log_scale_shift)*ln(10)/20
#define LN10_DIV_20 0.11512925465
    quantize_dynamic_scale_range =
        (int)(SL_ML_AUDIO_FEATURE_GENERATION_QUANTIZE_DYNAMIC_SCALE_RANGE_DB *
              (float)(1 << SL_ML_FRONTEND_LOG_SCALE_SHIFT) * LN10_DIV_20);
  }

  if (!FrontendPopulateState(&config, &frontend_state,
                             SL_ML_FRONTEND_SAMPLE_RATE_HZ)) {
...
sl_status_t sli_ml_audio_feature_generation_audio_buffer_write_chunk(
  ...
  // TODO: We should signal an overflow here when write index passes the read
  // index
  ...
  audio_buffer_write_index =
      (audio_buffer_write_index + num_samples) % AUDIO_BUFFER_SIZE;
  return SL_STATUS_OK;
}
...
sl_status_t sl_ml_audio_feature_generation_update_features() {
  ...
  int new_data_length =
      (audio_buffer_read_index < audio_buffer_write_index)
          ? (audio_buffer_write_index - audio_buffer_read_index)
          : ((AUDIO_BUFFER_SIZE - audio_buffer_read_index) +
             audio_buffer_write_index);

  if (new_data_length > 0) {
    const int length_to_end = AUDIO_BUFFER_SIZE - audio_buffer_read_index;
    const int chunk_length =
        (new_data_length < length_to_end) ? new_data_length : length_to_end;

    dump_audio(&audio_buffer[audio_buffer_read_index], chunk_length);
    process_audio_buffer_chunk(&audio_buffer[audio_buffer_read_index],
                               chunk_length, &num_slices_updated);

    // Update read index
    audio_buffer_read_index =
        (audio_buffer_read_index + chunk_length) % (AUDIO_BUFFER_SIZE);
    ...
    if (new_data_length > 0) {
      ...
      process_audio_buffer_chunk(&audio_buffer[0], new_data_length,
                                 &num_slices_updated);
...
sl_ml_audio_feature_generation_get_features_raw(uint16_t *buffer,
  ...
  for (int i = 0; i < FEATURE_BUFFER_SIZE; i++) {
    const uint32_t capture_index =
        (feature_buffer_start + i) % FEATURE_BUFFER_SIZE;
...
sl_status_t sli_ml_audio_feature_generation_get_features_quantized(
  ...
  for (int i = 0; i < FEATURE_BUFFER_SIZE; i++) {
    ...
    int32_t value =
        (((feature_buffer[capture_index] - range_min) * value_scale) +
         (value_div / 2)) /
        value_div;
    value -= 128;
    if (value < -128) {
...
sl_ml_audio_feature_generation_fill_tensor(TfLiteTensor *input_tensor) {
  ...
  if (input_tensor->type == kTfLiteInt8) {
    ...
    status = sl_ml_audio_feature_generation_get_features_raw(
        input_tensor->data.ui16, input_tensor->bytes / sizeof(uint16_t));
    ...
    if (SL_ML_AUDIO_FEATURE_GENERATION_SAMPLEWISE_NORM_RESCALE != 0) {
      status = sl_ml_audio_feature_generation_get_features_scaled(
          input_tensor->data.f, input_tensor->bytes / sizeof(float),
    ...
    dump_float_spectrogram(input_tensor->data.f,
...
sl_status_t sl_ml_audio_feature_generation_activity_detected() {
  ...
  return ActivityDetectionTripped(&frontend_state.activity_detection)
             ? SL_STATUS_OK
...
static sl_status_t process_audio_buffer_chunk(const int16_t *audio_data,
  ...
  while (num_samples > 0) {
    ...
    if (output.values != NULL) {
      ...
      feature_buffer_start =
          (feature_buffer_start + output.size) % (FEATURE_BUFFER_SIZE);
      *slices_updated += 1;
    } else {
      // No feature slice was generated, but data is stored internally in the
      // frontend for next iteration.
    }
  }
