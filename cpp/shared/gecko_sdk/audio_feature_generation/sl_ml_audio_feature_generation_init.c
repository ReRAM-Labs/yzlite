/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief Yizhu Technology Feature Generation Initialization with audio source
                                                                               *******************************************************************************
                                                                               * # License
...
/***************************************************************************/ /**
                                                                               ...
                                                                               ******************************************************************************/
static void mic_buffer_ready_callback(const int16_t *buffer,
                                      uint32_t n_frames) {
...
sl_status_t sl_ml_audio_feature_generation_init() {
  ...
  mic_status = yzlite_sl_mic_init(SL_ML_FRONTEND_SAMPLE_RATE_HZ, 1);
  if (mic_status != SL_STATUS_OK) {
    return mic_status;
  }

  sl_ml_audio_feature_generation_audio_buffer = (int16_t *)malloc(
      SL_ML_AUDIO_FEATURE_GENERATION_AUDIO_BUFFER_SIZE * sizeof(int16_t));
  if (sl_ml_audio_feature_generation_audio_buffer == NULL) {
    return SL_STATUS_ALLOCATION_FAILED;
  }

  // The sl_mic library uses "ping-ponging" hence we divide by 2
  mic_status = yzlite_sl_mic_start_streaming(
