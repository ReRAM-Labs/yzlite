...
template <typename T>
bool get_param(const yzlite::TfliteModelParameters &model_parameters,
...
template <typename T>
bool get_param_default(const yzlite::TfliteModelParameters &model_parameters,
                       const char *key, T &val, T default_val) {
  if (!model_parameters.get(key, val)) {
    val = default_val;
  }
  return false;
...
#define GET_INT(key, val)                                                      \
  missing_params |= get_param(model_parameters, key, val);                     \
  printf("%s=%d\n", key, val)
#define GET_INT_DEFAULT(key, val, default_val)                                 \
  get_param_default(model_parameters, key, val, default_val);                  \
  printf("%s=%d\n", key, val)
#define GET_FLOAT(key, val)                                                    \
  missing_params |= get_param(model_parameters, key, val);                     \
  printf("%s=%5.3f\n", key, val)
#define GET_FLOAT_DEFAULT(key, val, default_val)                               \
  get_param_default(model_parameters, key, val, default_val);                  \
  printf("%s=%5.3f\n", key, val)
...
bool yzlite_sl_ml_audio_feature_generation_load_parameters(
  ...
  yzlite::TfliteModelParameters model_parameters;
  bool missing_params = false;

  if (!yzlite::TfliteModelParameters::load_from_tflite_flatbuffer(
          tflite_flatbuffer, model_parameters)) {
    return false;
  }
  ...
  GET_FLOAT("fe.filterbank_lower_band_limit",
            SL_TFLITE_MODEL_FE_FILTERBANK_LOWER_BAND_LIMIT);
  GET_FLOAT("fe.filterbank_upper_band_limit",
            SL_TFLITE_MODEL_FE_FILTERBANK_UPPER_BAND_LIMIT);
  GET_INT("fe.noise_reduction_enable",
          SL_TFLITE_MODEL_FE_NOISE_REDUCTION_ENABLE);
  GET_INT("fe.noise_reduction_smoothing_bits",
          SL_TFLITE_MODEL_FE_NOISE_REDUCTION_SMOOTHING_BITS);
  GET_FLOAT("fe.noise_reduction_even_smoothing",
            SL_TFLITE_MODEL_FE_NOISE_REDUCTION_EVEN_SMOOTHING);
  GET_FLOAT("fe.noise_reduction_odd_smoothing",
            SL_TFLITE_MODEL_FE_NOISE_REDUCTION_ODD_SMOOTHING);
  GET_FLOAT("fe.noise_reduction_min_signal_remaining",
  ...
  GET_INT_DEFAULT("fe.activity_detection_enable",
                  SL_TFLITE_MODEL_FE_ACTIVITY_DETECTION_ENABLE, 0);
  GET_FLOAT_DEFAULT("fe.activity_detection_alpha_a",
                    SL_TFLITE_MODEL_FE_ACTIVITY_DETECTION_ALPHA_A, 0.f);
  GET_FLOAT_DEFAULT("fe.activity_detection_alpha_b",
                    SL_TFLITE_MODEL_FE_ACTIVITY_DETECTION_ALPHA_B, 0.f);
  GET_FLOAT_DEFAULT("fe.activity_detection_arm_threshold",
                    SL_TFLITE_MODEL_FE_ACTIVITY_DETECTION_ARM_THRESHOLD, 0.f);
  GET_FLOAT_DEFAULT("fe.activity_detection_trip_threshold",
                    SL_TFLITE_MODEL_FE_ACTIVITY_DETECTION_TRIP_THRESHOLD, 0.f);
  GET_INT_DEFAULT("fe.dc_notch_filter_enable",
                  SL_TFLITE_MODEL_FE_DC_NOTCH_FILTER_ENABLE, 0);
  GET_FLOAT_DEFAULT("fe.dc_notch_filter_coefficient",
                    SL_TFLITE_MODEL_FE_DC_NOTCH_FILTER_COEFFICIENT, 0.f);

  GET_INT_DEFAULT("fe.quantize_dynamic_scale_enable",
                  SL_TFLITE_MODEL_FE_QUANTIZE_DYNAMIC_SCALE_ENABLE, 0);
  GET_FLOAT_DEFAULT("fe.quantize_dynamic_scale_range_db",
                    SL_TFLITE_MODEL_FE_QUANTIZE_DYNAMIC_SCALE_RANGE_DB, 0.f);
  ...
  SL_ML_AUDIO_FEATURE_GENERATION_AUDIO_BUFFER_SIZE =
      SL_TFLITE_MODEL_FE_SAMPLE_RATE_HZ * 1;
  printf("fe.audio_buffer_size = %d\n",
         SL_ML_AUDIO_FEATURE_GENERATION_AUDIO_BUFFER_SIZE);

  GET_FLOAT_DEFAULT("samplewise_norm.rescale",
                    SL_TFLITE_MODEL_SAMPLEWISE_NORM_RESCALE, 0.f);
  GET_INT_DEFAULT("samplewise_norm.mean_and_std",
                  SL_TFLITE_MODEL_SAMPLEWISE_NORM_MEAN_AND_STD, 0);
