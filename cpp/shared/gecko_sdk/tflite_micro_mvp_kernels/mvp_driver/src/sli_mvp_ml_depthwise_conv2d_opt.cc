...
extern "C" sl_status_t sli_mvp_ml_depthwise_conv2d_s8_gen_opt(
  ...
  if ((((uintptr_t)bias & 0x1U) != 0U) ||
      (((uintptr_t)output_scaler & 0x1U) != 0U)) {
    return SL_STATUS_INVALID_PARAMETER;
  }

  bool use_parallel = false;
  if (((output_depth & 0x1U) == 0U) && (((uintptr_t)input & 0x1U) == 0U) &&
      (((uintptr_t)output & 0x1U) == 0U) &&
      (((uintptr_t)filter & 0x1U) == 0U) && (((uintptr_t)bias & 0x3U) == 0U) &&
  ...
  if (((uint32_t)filter_width > SLI_MVP_MAX_VECTOR_COUNT) ||
      ((uint32_t)filter_height > SLI_MVP_MAX_ROW_LENGTH)) {
  ...
  for (int batch = 0; batch < batches; ++batch) {
    for (int out_y = 0; out_y < output_height; ++out_y) {
      for (int out_x = 0; out_x < output_width; ++out_x) {
        ...
        // Will be negative when padding=same and filter goes outside of input
        // on the top/left
        ...
        // Compute effective size of convolution. Reduced from full size if
        // filter goes outside of input on the bottom/right
        const int x_size = (in_x_origin + filter_width >= input_width
                                ? input_width - in_x_origin
                                : filter_width) -
                           filter_x_begin;
        const int y_size = (in_y_origin + filter_height >= input_height
        ...
        // R3 = activation_min
        // R4 = activation_max
        sli_mvp_prog_set_reg_f16c(p, SLI_MVP_R0,
        ...
        sli_mvp_prog_set_reg_f16c(
            ...
            (float16_t)(input_offset * SLI_MVP_ACCUMULATOR_SCALER));
        sli_mvp_prog_set_reg_f16c(p, SLI_MVP_R2, (float16_t)output_offset,
                                  (float16_t)output_offset);
        bool do_activation =
            (activation_min != -128) || (activation_max != 127);
        if (do_activation) {
          sli_mvp_prog_set_reg_f16c(p, SLI_MVP_R3, (float16_t)activation_min,
                                    (float16_t)activation_min);
          sli_mvp_prog_set_reg_f16c(p, SLI_MVP_R4, (float16_t)activation_max,
        ...
        sli_mvp_prog_set_array_full(
            p, SLI_MVP_ARRAY(0),
            (void *)&input[batch * input_height * input_width * input_depth +
                           in_y_begin * input_width * input_depth +
                           in_x_begin * input_depth],
            use_parallel ? SLI_MVP_DATATYPE_COMPLEX_INT8
                         : SLI_MVP_DATATYPE_INT8,
            effective_depth, y_size, x_size, 1, input_width * effective_depth,
            effective_depth);
        ...
        sli_mvp_prog_set_array_full(
            p, SLI_MVP_ARRAY(1),
            (void
                 *)&filter[batch * filter_height * filter_width * filter_depth +
                           filter_y_begin * filter_width * filter_depth +
                           filter_x_begin * filter_depth],
            use_parallel ? SLI_MVP_DATATYPE_COMPLEX_INT8
                         : SLI_MVP_DATATYPE_INT8,
            effective_depth, y_size, x_size, 1, filter_width * effective_depth,
            effective_depth);

        // ARRAY2 = bias
        sli_mvp_prog_set_vector(p, SLI_MVP_ARRAY(2),
                                bias != NULL ? (void *)bias : (void *)zero,
                                use_parallel ? SLI_MVP_DATATYPE_COMPLEX_BINARY16
                                             : SLI_MVP_DATATYPE_BINARY16,
                                bias != NULL ? effective_depth : 1); // length

        // ARRAY3 = output_scaler
        sli_mvp_prog_set_vector(p, SLI_MVP_ARRAY(3),
        ...
        sli_mvp_prog_set_array_full(
            p, SLI_MVP_ARRAY(4),
            (void
                 *)&output[batch * output_height * output_width * output_depth +
                           out_y * output_width * output_depth +
                           out_x * output_depth],
            use_parallel ? SLI_MVP_DATATYPE_COMPLEX_INT8
                         : SLI_MVP_DATATYPE_INT8,
            effective_depth, 1, 1, 1, effective_depth, effective_depth);

        // R5 = LOAD(A2) bias_i = load bias_array
        sli_mvp_prog_set_instr(
            p, SLI_MVP_INSTR(0), SLI_MVP_OP(NOOP), SLI_MVP_NONE,
        ...
        // R6 = MAC(R6, R0, R1) input_i = input_i * ACCUMULATOR_SCALER +
        // input_offset_scaled;
        sli_mvp_prog_set_instr(
            p, SLI_MVP_INSTR(1), SLI_MVP_OP(MACR2A),
        ...
        sli_mvp_prog_set_instr(
            p, SLI_MVP_INSTR(2), SLI_MVP_OP(MACR2A),
            SLI_MVP_ALU_Z(SLI_MVP_R5) | SLI_MVP_ALU_X(SLI_MVP_R6) |
                SLI_MVP_ALU_Y(SLI_MVP_R7) | SLI_MVP_ALU_A(SLI_MVP_R5),
            SLI_MVP_NONE, SLI_MVP_NONE, false);
        ...
        if (do_activation) {
          ...
          // A4 = STORE(R7) output_array = store output_i
          sli_mvp_prog_set_instr(
              p, SLI_MVP_INSTR(4), SLI_MVP_OP(CLIP2A),
              SLI_MVP_ALU_Z(SLI_MVP_R7) | SLI_MVP_ALU_X(SLI_MVP_R3) |
                  SLI_MVP_ALU_Y(SLI_MVP_R4) | SLI_MVP_ALU_A(SLI_MVP_R7),
              SLI_MVP_NONE,
              SLI_MVP_STORE(SLI_MVP_R7, SLI_MVP_ARRAY(4), SLI_MVP_INCRDIM0),
              true);
        }
        ...
        sli_mvp_prog_set_loop(
            p, SLI_MVP_LOOP(0), effective_depth, SLI_MVP_INSTR(0),
            do_activation ? SLI_MVP_INSTR(4) : SLI_MVP_INSTR(3),
            SLI_MVP_NOINCR);

        // LOOP(y): increment depth afterwards
        sli_mvp_prog_set_loop(
            p, SLI_MVP_LOOP(1), y_size, SLI_MVP_INSTR(1), SLI_MVP_INSTR(2),
            SLI_MVP_LOOP_INCRDIM(SLI_MVP_ARRAY(0),
                                 SLI_MVP_INCRDIM0) // incr d/2 end of every loop
                | SLI_MVP_LOOP_INCRDIM(SLI_MVP_ARRAY(1), SLI_MVP_INCRDIM0));

        // LOOP(x): increment y afterwards
        sli_mvp_prog_set_loop(
            p, SLI_MVP_LOOP(2), x_size, SLI_MVP_INSTR(1), SLI_MVP_INSTR(2),
            SLI_MVP_LOOP_INCRDIM(SLI_MVP_ARRAY(0),
                                 SLI_MVP_INCRDIM1) // incr y/1 end of every loop
                | SLI_MVP_LOOP_INCRDIM(SLI_MVP_ARRAY(1), SLI_MVP_INCRDIM1));

        YZLITE_PROFILER_INCREMENT_PARALLEL_PROG_COUNT(use_parallel ? 1 : 0);
