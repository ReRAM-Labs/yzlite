...
static sl_status_t
transpose_conv(const sli_mvp_ml_transpose_conv2d_s8_params_t *params,
               bool execute);
...
/***************************************************************************/ /**
                                                                               ...
                                                                               *
                                                                               ******************************************************************************/
sl_status_t sli_mvp_ml_transpose_conv2d_s8(
    const sli_mvp_ml_transpose_conv2d_s8_params_t *params) {
...
/***************************************************************************/ /**
                                                                               ...
                                                                               *
                                                                               ******************************************************************************/
bool sli_mvp_ml_transpose_conv2d_s8_is_supported(
    const sli_mvp_ml_transpose_conv2d_s8_params_t *params) {
  if (transpose_conv(params, false) != SL_STATUS_OK) {
...
transpose_conv(const sli_mvp_ml_transpose_conv2d_s8_params_t *params,
  ...
  static const float16_t zero[2] = {float16_t(0), float16_t(0)};
  ...
  if (execute) {
    if ((bias && ((uintptr_t)bias & 0x1U) != 0U) ||
        (((uintptr_t)output_scaler & 0x1U) != 0U) ||
        (((uintptr_t)tmp_buf & 0x3U) != 0U) ||
  ...
  if (execute) {
    ...
    status = sli_mvp_util_memclr_f16(p, tmp_buf, batches, output_depth,
    ...
    for (int in_y = 0; in_y < input_height; ++in_y) {
      for (int in_x = 0; in_x < input_width; ++in_x) {
        ...
        const int filter_y_end =
            SL_MIN(filter_height, output_height - out_y_origin);
        const int filter_x_start = SL_MAX(0, -out_x_origin);
        const int filter_x_end =
            SL_MIN(filter_width, output_width - out_x_origin);

        const int input_base =
            sli_mvp_util_offset_nhwc(input_height, input_width, input_depth,
                                     batch, in_y, in_x, 0 /* input channel */);
        int tmpbuf_base = sli_mvp_util_offset_nhwc(
            output_height, output_width, output_depth, batch,
            out_y_origin + filter_y_start, out_x_origin + filter_x_start,
            0 /* output channel */);
        int filter_base = sli_mvp_util_offset_nhwc(
            filter_height, filter_width, input_depth, batch, filter_y_start,
            filter_x_start, 0 /* output channel */);
        ...
        for (int out_channel = 0; out_channel < output_depth; ++out_channel) {
          ...
          sli_mvp_pb_config_array_nhwc(p->p, input_array,
                                       (void *)&input[input_base],
                                       SLI_MVP_DATATYPE_INT8, input_height,
                                       input_width, input_depth, &status);

          sli_mvp_pb_config_array_nhwc(p->p, filter_array,
                                       (void *)&filter[filter_base],
                                       SLI_MVP_DATATYPE_INT8, filter_height,
                                       filter_width, input_depth, &status);

          sli_mvp_pb_config_array_nhwc(p->p, tmp_buf_array,
                                       (void *)&tmp_buf[tmpbuf_base],
                                       SLI_MVP_DATATYPE_BINARY16, output_height,
                                       output_width, output_depth, &status);
          ...
          sli_mvp_pb_begin_loop(p, input_depth, &status);
          { // Loop over input channels.

            // Calculate: tmp_buf += (input + input_offset) * ACCUMULATOR_SCALER
            // * filter
            //      =>    tmp_buf += ((input * ACCUMULATOR_SCALER) +
            //      input_offset_scaled) * filter

            // R2 = Input
            // R5 = (R2 * R0) + R1   scratch = (input * ACCUMULATOR_SCALER) +
            // input_offset_scaled
            sli_mvp_pb_compute(
                p, SLI_MVP_OP(MACR2A),
                SLI_MVP_ALU_Z(SLI_MVP_R5) | SLI_MVP_ALU_X(SLI_MVP_R2) |
            ...
            { // Loop over filter height.
              ...
              { // Loop over filter width.
                ...
                // R4 = tmp_buf
                // R3 = Filter
                // R7 = (R5 * R3) + R4   acc += ((input * ACCUMULATOR_SCALER) +
                // input_offset_scaled) * filter output = R7
                sli_mvp_pb_compute(
                    p, SLI_MVP_OP(MACR2A),
                    SLI_MVP_ALU_Z(SLI_MVP_R7) | SLI_MVP_ALU_X(SLI_MVP_R5) |
                        SLI_MVP_ALU_Y(SLI_MVP_R3) | SLI_MVP_ALU_A(SLI_MVP_R4),
                    SLI_MVP_LOAD(0, SLI_MVP_R4, tmp_buf_array, SLI_MVP_NOINCR) |
                        SLI_MVP_LOAD(1, SLI_MVP_R3, filter_array,
                                     SLI_MVP_INCRDIM_WIDTH),
                    SLI_MVP_STORE(SLI_MVP_R7, tmp_buf_array,
                                  SLI_MVP_INCRDIM_WIDTH),
                    &status);
              }
              sli_mvp_pb_end_loop(p); // Filter width.
              sli_mvp_pb_postloop_reset_dim(p, tmp_buf_array,
                                            SLI_MVP_RESETDIM_WIDTH);
              sli_mvp_pb_postloop_reset_dim(p, filter_array,
                                            SLI_MVP_RESETDIM_WIDTH);
              sli_mvp_pb_postloop_incr_dim(p, tmp_buf_array,
                                           SLI_MVP_INCRDIM_HEIGHT);
              sli_mvp_pb_postloop_incr_dim(p, filter_array,
                                           SLI_MVP_INCRDIM_HEIGHT);
            }
            sli_mvp_pb_end_loop(p); // Filter height.
            sli_mvp_pb_postloop_reset_dim(p, tmp_buf_array,
                                          SLI_MVP_RESETDIM_HEIGHT);
            sli_mvp_pb_postloop_reset_dim(p, filter_array,
                                          SLI_MVP_RESETDIM_HEIGHT);
            sli_mvp_pb_postloop_incr_dim(p, input_array, SLI_MVP_INCRDIM_DEPTH);
            sli_mvp_pb_postloop_incr_dim(p, filter_array,
                                         SLI_MVP_INCRDIM_DEPTH);
          }
  ...
  bool parallel = false;
  if ((((uintptr_t)bias & 0x3U) == 0U) && (((uintptr_t)tmp_buf & 0x3U) == 0U) &&
      (((uintptr_t)output_scaler & 0x3U) == 0U) &&
      (((uintptr_t)output & 0x1U) == 0U) && ((output_depth % 2) == 0)) {
    parallel = true;
    output_depth /= 2;
  }
  ...
  for (int batch = 0; batch < batches; ++batch) {
    ...
    const int output_base = sli_mvp_util_offset_nhwc(
        output_height, output_width, output_depth, batch, 0, 0, 0);
    sli_mvp_pb_config_array_nhwc(
        p->p, tmp_buf_array, (void *)&tmp_buf[output_base],
        parallel ? SLI_MVP_DATATYPE_COMPLEX_BINARY16
                 : SLI_MVP_DATATYPE_BINARY16,
        output_height, output_width, output_depth, &status);

    sli_mvp_pb_config_array_nhwc(
        p->p, output_array, (void *)&output[output_base],
        parallel ? SLI_MVP_DATATYPE_COMPLEX_INT8 : SLI_MVP_DATATYPE_INT8,
        output_height, output_width, output_depth, &status);

    sli_mvp_pb_config_vector(p->p, bias_array,
                             bias != NULL ? (void *)bias : (void *)zero,
                             parallel ? SLI_MVP_DATATYPE_COMPLEX_BINARY16
                                      : SLI_MVP_DATATYPE_BINARY16,
                             bias != NULL ? output_depth : 1, &status);

    sli_mvp_pb_config_vector(p->p, output_scaler_array, (void *)output_scaler,
                             parallel ? SLI_MVP_DATATYPE_COMPLEX_BINARY16
                                      : SLI_MVP_DATATYPE_BINARY16,
                             output_depth, &status);

    sli_mvp_prog_set_reg_f16c(p->p, SLI_MVP_R0, (float16_t)output_offset,
                              (float16_t)output_offset);
    bool do_activation = (activation_min != -128) || (activation_max != 127);
    if (do_activation) {
      sli_mvp_prog_set_reg_f16c(p->p, SLI_MVP_R1, (float16_t)activation_min,
                                (float16_t)activation_min);
      sli_mvp_prog_set_reg_f16c(p->p, SLI_MVP_R2, (float16_t)activation_max,
                                (float16_t)activation_max);
    }

    // R4 = bias
    sli_mvp_pb_compute(
        p, SLI_MVP_OP(NOOP), SLI_MVP_NONE,
    ...
    sli_mvp_pb_begin_loop(p, output_height, &status);
    {
      sli_mvp_pb_begin_loop(p, output_width, &status);
      {
        ...
        sli_mvp_pb_begin_loop(p, output_depth, &status);
        {
          ...
          sli_mvp_pb_compute(p, SLI_MVP_OP(ADDC),
                             SLI_MVP_ALU_Z(SLI_MVP_R7) |
                                 SLI_MVP_ALU_X(SLI_MVP_R3) |
                                 SLI_MVP_ALU_A(SLI_MVP_R4),
                             SLI_MVP_LOAD(0, SLI_MVP_R5, output_scaler_array,
                                          SLI_MVP_INCRDIM_WIDTH),
                             SLI_MVP_NONE, &status);

          // R3 = tmp_buf          (load for use in next loop iteration)
          // R4 = bias             (load for use in next loop iteration)
          // R7 = (R7 * R5) + R0   acc = ((tmp_buf + bias) * scaler) +
          // output_offset output = R7
          sli_mvp_pb_compute(
              p, SLI_MVP_OP(MACR2A),
              SLI_MVP_ALU_Z(SLI_MVP_R7) | SLI_MVP_ALU_X(SLI_MVP_R7) |
                  SLI_MVP_ALU_Y(SLI_MVP_R5) | SLI_MVP_ALU_A(SLI_MVP_R0),
              SLI_MVP_LOAD(0, SLI_MVP_R3, tmp_buf_array,
                           SLI_MVP_INCRDIM_DEPTH) |
                  SLI_MVP_LOAD(1, SLI_MVP_R4, bias_array,
                               SLI_MVP_INCRDIM_WIDTH),
              do_activation ? SLI_MVP_NONE
                            : SLI_MVP_STORE(SLI_MVP_R7, output_array,
                                            SLI_MVP_INCRDIM_DEPTH),
              &status);
          ...
          if (do_activation) {
            ...
            sli_mvp_pb_compute(
                p, SLI_MVP_OP(CLIP2A),
                SLI_MVP_ALU_Z(SLI_MVP_R7) | SLI_MVP_ALU_X(SLI_MVP_R1) |
                    SLI_MVP_ALU_Y(SLI_MVP_R2) | SLI_MVP_ALU_A(SLI_MVP_R7),
                SLI_MVP_NONE,
