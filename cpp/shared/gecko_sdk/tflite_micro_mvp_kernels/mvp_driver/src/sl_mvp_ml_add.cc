...
sl_status_t sli_mvp_ml_add_s8(const sli_mvp_ml_add_s8_params_t *params) {
  ...
  float input1_offset_scaled =
      params->input1_offset * params->input1_multiplier;
  float input2_offset_scaled =
      params->input2_offset * params->input2_multiplier;

  // *INDENT-OFF*
  /*
     Software Reference:

     This is the reference algorithm for the add operation. Note that in the 8
     bit version of add there are 3 different quantization parameters. This is
     why the add operation is more complex than a simple addition. We first have
     to convert each of the input by de-quantization before we can perform the
     add operation. After the add operation we have to quantize the output using
     the output specific quantization parameters.

     for (size_t i = 0; i < params->length; ++i) {
       float input1_val = (input1[i] + input1_offset) * input1_scale;
       float input2_val = (input2[i] + input2_offset) * input2_scale;
       float output_val = (input1_val + input2_val) / output_scale +
     output_zero_point; output_val = round(output_val); output[i] =
     clamp(output_val, activation_min, activation_max);
     }

     We can get the core loop without the clamp operation down to 4 instructions
     on the MVP hardware by implementing the following algorithm.
  ...
  sli_mvp_prog_set_reg_f32c(prog, SLI_MVP_R0, input1_offset_scaled,
                            input1_offset_scaled);
  sli_mvp_prog_set_reg_f32c(prog, SLI_MVP_R1, params->input1_multiplier,
                            params->input1_multiplier);
  sli_mvp_prog_set_reg_f32c(prog, SLI_MVP_R2, input2_offset_scaled,
                            input2_offset_scaled);
  sli_mvp_prog_set_reg_f32c(prog, SLI_MVP_R3, params->input2_multiplier,
                            params->input2_multiplier);
  sli_mvp_prog_set_reg_s32c(prog, SLI_MVP_R4, params->output_offset,
                            params->output_offset);
  sli_mvp_prog_set_reg_f32c(prog, SLI_MVP_R5, params->output_multiplier,
  ...
  sli_mvp_prog_set_instr(
      prog, SLI_MVP_INSTR(0), SLI_MVP_OP(MACR2A),
      SLI_MVP_ALU_X(SLI_MVP_R6) | SLI_MVP_ALU_Y(SLI_MVP_R1) |
          SLI_MVP_ALU_A(SLI_MVP_R0) | SLI_MVP_ALU_Z(SLI_MVP_R6),
      SLI_MVP_LOAD(0, SLI_MVP_R6, SLI_MVP_ARRAY(0), SLI_MVP_INCRDIM_COL), 0, 0);
  ...
  // R7 = MACR2A(R7,R3,R2)
  sli_mvp_prog_set_instr(
      prog, SLI_MVP_INSTR(1), SLI_MVP_OP(MACR2A),
      SLI_MVP_ALU_X(SLI_MVP_R7) | SLI_MVP_ALU_Y(SLI_MVP_R3) |
          SLI_MVP_ALU_A(SLI_MVP_R2) | SLI_MVP_ALU_Z(SLI_MVP_R7),
      SLI_MVP_LOAD(0, SLI_MVP_R7, SLI_MVP_ARRAY(1), SLI_MVP_INCRDIM_COL), 0, 0);

  // Instruction 2: Add input1 and input2
  // R6 = ADDC(R6,R7)
  sli_mvp_prog_set_instr(
      prog, SLI_MVP_INSTR(2), SLI_MVP_OP(ADDC),
  ...
  // INC(Array2, Dim0)
  sli_mvp_prog_set_instr(
      prog, SLI_MVP_INSTR(3), SLI_MVP_OP(MACR2A),
      ...
      0, SLI_MVP_STORE(SLI_MVP_R7, SLI_MVP_ARRAY(2), SLI_MVP_INCRDIM_COL),
      SLI_MVP_ENDPROG);
  ...
  while (remaining >= 2) {
    ...
    sli_mvp_prog_set_vector(prog, SLI_MVP_ARRAY(0), (int8_t *)input1_data,
                            SLI_MVP_DATATYPE_COMPLEX_INT8, batch_size);
    sli_mvp_prog_set_vector(prog, SLI_MVP_ARRAY(1), (int8_t *)input2_data,
                            SLI_MVP_DATATYPE_COMPLEX_INT8, batch_size);
    sli_mvp_prog_set_vector(prog, SLI_MVP_ARRAY(2), output_data,
                            SLI_MVP_DATATYPE_COMPLEX_INT8, batch_size);

    // Loop 0 is iterating over elements
    sli_mvp_prog_set_loop(prog, SLI_MVP_LOOP(0), batch_size, SLI_MVP_INSTR(0),
                          SLI_MVP_INSTR(3), 0);
  ...
  if (remaining == 1) {
    ...
    float sum =
        input1 * params->input1_multiplier + input2 * params->input2_multiplier;
  ...
  sli_mvp_math_clamp_i8(params->output, params->length, params->activation_min,
...
bool sli_mvp_ml_add_s8_is_supported(const sli_mvp_ml_add_s8_params_t *params) {
  if (!params) {
    YZLITE_KERNEL_UNSUPPORTED_MSG("Invalid parameters");
    return false;
  } else {
