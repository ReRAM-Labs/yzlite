...
static sl_status_t avg_pool(const sli_mvp_ml_pooling_s8_params_t *params,
                            bool execute);
static sl_status_t avg_pool_opt(const sli_mvp_ml_pooling_s8_params_t *params,
                                bool execute);
static sl_status_t max_pool(const sli_mvp_ml_pooling_s8_params_t *params,
                            bool execute);
...
/***************************************************************************/ /**
                                                                               ...
                                                                               ******************************************************************************/
sl_status_t
sli_mvp_ml_average_pooling_s8(const sli_mvp_ml_pooling_s8_params_t *params) {
  ...
  if (((params->output_width * params->output_height) >
       (((9 * params->filter_width * params->filter_height * params->channels) +
         5) /
        10)) ||
      !opt_supported) {
...
/***************************************************************************/ /**
                                                                               ...
                                                                               ******************************************************************************/
sl_status_t
sli_mvp_ml_average_pooling_s8_test(const sli_mvp_ml_pooling_s8_params_t *params,
...
/***************************************************************************/ /**
                                                                               ...
                                                                               ******************************************************************************/
sl_status_t
sli_mvp_ml_max_pooling_s8(const sli_mvp_ml_pooling_s8_params_t *params) {
...
/***************************************************************************/ /**
                                                                               ...
                                                                               *
                                                                               ******************************************************************************/
bool sli_mvp_ml_average_pooling_s8_is_supported(
    const sli_mvp_ml_pooling_s8_params_t *params) {
  // Check if optimized algorithm can be used.
  if (avg_pool_opt(params, false) == SL_STATUS_OK) {
    return true;
  }
  // Resort to the generic algorithm.
  bool retval = avg_pool(params, false) == SL_STATUS_OK;
  if (!retval) {
...
/***************************************************************************/ /**
                                                                               ...
                                                                               *
                                                                               ******************************************************************************/
bool sli_mvp_ml_max_pooling_s8_is_supported(
    const sli_mvp_ml_pooling_s8_params_t *params) {
  if (max_pool(params, false) != SL_STATUS_OK) {
    YZLITE_KERNEL_UNSUPPORTED_MSG("Hardware limits exceeded");
    return false;
  }
  return true;
...
static sl_status_t avg_pool_opt(const sli_mvp_ml_pooling_s8_params_t *params,
  ...
  if ((parallel == 2) && execute) {
    ...
    if ((((uintptr_t)input & 0x1) != 0U) || (((uintptr_t)output & 0x1) != 0U)) {
      status = SL_STATUS_INVALID_PARAMETER;
    }
  }
    ...
    for (int out_y = 0; out_y < output_height; ++out_y) {
      for (int out_x = 0; out_x < output_width; ++out_x) {
        ...
        const int filter_x_end =
            SL_MIN(filter_width, input_width - in_x_origin);
        const int filter_y_start = SL_MAX(0, -in_y_origin);
        const int filter_y_end =
            SL_MIN(filter_height, input_height - in_y_origin);

        const int input_offset = sli_mvp_util_offset_nhwc(
            input_height, input_width, depth, batch,
            in_y_origin + filter_y_start, in_x_origin + filter_x_start,
            0 /* channel */);
        const int output_offset =
            sli_mvp_util_offset_nhwc(output_height, output_width, depth, batch,
                                     out_y, out_x, 0 /* channel */);
        ...
        sli_mvp_pb_config_array_full(
            ...
            (parallel == 2) ? SLI_MVP_DATATYPE_COMPLEX_INT8 // datatype
        ...
        sli_mvp_prog_set_reg_f16c(p->p, SLI_MVP_R0, final_scale_factor,
                                  final_scale_factor);
        sli_mvp_prog_set_reg_f16c(p->p, SLI_MVP_R1,
                                  (float16_t)SLI_MVP_ACCUMULATOR_SCALER,
                                  (float16_t)SLI_MVP_ACCUMULATOR_SCALER);
        bool do_activation =
            (activation_min != -128) || (activation_max != 127);
        if (do_activation) {
          sli_mvp_prog_set_reg_f16c(p->p, SLI_MVP_R4, (float16_t)activation_min,
                                    (float16_t)activation_min);
          sli_mvp_prog_set_reg_f16c(p->p, SLI_MVP_R5, (float16_t)activation_max,
                                    (float16_t)activation_max);
        }

        sli_mvp_pb_begin_loop(p, depth / parallel, &status);
        { // Loop over channels.

          // R2 = CLEAR() r_acc = 0
          sli_mvp_pb_compute(p, SLI_MVP_OP(CLEAR), SLI_MVP_ALU_Z(SLI_MVP_R2),
                             SLI_MVP_NONE, SLI_MVP_NONE, &status);

          sli_mvp_pb_begin_loop(p, filter_y_count, &status);
          { // Loop over filter height
            sli_mvp_pb_begin_loop(p, filter_x_count, &status);
            { // Loop over filter width

              // Accumulate input tensor values covered by filter.
              // LOAD(ARRAY0, R3)
              // R2 = MACR2A(R3, R1, R2) r_acc = (r_input_i *
              // c_accumulator_scaler) + r_acc
              sli_mvp_pb_compute(
                  p, SLI_MVP_OP(MACR2A),
                  SLI_MVP_ALU_Z(SLI_MVP_R2) | SLI_MVP_ALU_X(SLI_MVP_R3) |
                      SLI_MVP_ALU_Y(SLI_MVP_R1) | SLI_MVP_ALU_A(SLI_MVP_R2),
                  SLI_MVP_LOAD(0, SLI_MVP_R3, SLI_MVP_ARRAY(0),
                               SLI_MVP_INCRDIM_COL),
                  SLI_MVP_NONE, &status);
            }
            ...
            sli_mvp_pb_postloop_incr_dim(p, SLI_MVP_ARRAY(0),
                                         SLI_MVP_INCRDIM_ROW);
            sli_mvp_pb_postloop_reset_dim(p, SLI_MVP_ARRAY(0),
                                          SLI_MVP_RESETDIM_COL);
          }
          sli_mvp_pb_end_loop(p); // Filter height
          sli_mvp_pb_postloop_incr_dim(p, SLI_MVP_ARRAY(0),
                                       SLI_MVP_INCRDIM_VEC);
          sli_mvp_pb_postloop_reset_dim(p, SLI_MVP_ARRAY(0),
                                        SLI_MVP_RESETDIM_ROW);
          ...
          sli_mvp_pb_compute(
              p, SLI_MVP_OP(MULR2A),
              SLI_MVP_ALU_Z(SLI_MVP_R2) | SLI_MVP_ALU_X(SLI_MVP_R2) |
                  SLI_MVP_ALU_Y(SLI_MVP_R0),
              SLI_MVP_NONE,
              do_activation ? SLI_MVP_NONE
                            : SLI_MVP_STORE(SLI_MVP_R2, SLI_MVP_ARRAY(1),
                                            SLI_MVP_INCRDIM_VEC),
              &status);
          if (do_activation) {
            ...
            sli_mvp_pb_compute(
                p, SLI_MVP_OP(CLIP2A),
                SLI_MVP_ALU_Z(SLI_MVP_R6) | SLI_MVP_ALU_X(SLI_MVP_R4) |
                    SLI_MVP_ALU_Y(SLI_MVP_R5) | SLI_MVP_ALU_A(SLI_MVP_R2),
                SLI_MVP_NONE,
                SLI_MVP_STORE(SLI_MVP_R6, SLI_MVP_ARRAY(1),
                              SLI_MVP_INCRDIM_VEC),
                &status);
          }
        }
        ...
        if (execute) {
          ...
          YZLITE_PROFILER_INCREMENT_PARALLEL_PROG_COUNT(parallel == 2 ? 1 : 0);
          YZLITE_PROFILER_INCREMENT_OPT_PROG_COUNT(1);
        }
      }
    }
  }
...
static sl_status_t avg_pool(const sli_mvp_ml_pooling_s8_params_t *params,
  ...
  for (int out_x_min = 0, out_x_max; out_x_min < output_width;
    ...
    // increment across it. Instead, can use that dimension for row striding and
    // handle entire column in single program.
    bool single_out_x = output_width_truncated <= 1;
    // However, this mechanism leads to longer strides and so disabling here in
    // those cases where later stride computation would be too large.
    SLI_MVP_CHECK((stride_width * depth) <= (int)SLI_MVP_MAX_VECTOR_STRIDE);
    if ((stride_height * input_width * depth) >
    ...
    for (int out_y_min = 0, out_y_max; out_y_min < output_height;
      ...
      // the results will start to become very innaccurate due to accumulator
      // becoming much larger than inputs.
      ...
      for (int out_y_offset = 0; out_y_offset < out_y_num_offsets;
        ...
        if (single_out_x) {
          ...
          // Set in_y_size to exact value needed for rollover, to save MVP code
          // space by using a Reset
          in_y_size =
        ...
        for (int batch = 0; batch < batches; ++batch) {
          // Not enough input dimensions for including channel loop in the
          // program If input stored with channel as first dimension then would
          // help
          ...
          for (int channel = 0; channel < depth; channel += parallelization) {
            ...
            int output_index_base = sli_mvp_util_offset_nhwc(
                output_height, output_width, depth, batch,
                out_y_min + out_y_offset, out_x_min, channel);
            ...
            if (output_stride_vec > (int)SLI_MVP_MAX_VECTOR_STRIDE) {
              // If fails, should be split or run in software
              // EFM_ASSERT(false);
            ...
            int input_index_base = sli_mvp_util_offset_nhwc(
                ...
                (out_y_min + out_y_offset) * stride_height - pad_height +
                    filter_y_start,
                out_x_min * stride_width - pad_width + filter_x_start, channel);
            ...
            // If there is at least one more channel left, then group it with
            // this one and do 2 at a time in parallel In general, the
            // paralellization works if depth is even, but the full expression
            // for
            ...
            // Hardware strides are in terms of base unit, so must update
            // strides to reflect base unit scaling by parallelization.
            ...
            sli_mvp_pb_config_array_full(
                ...
                parallelize_channels ? SLI_MVP_DATATYPE_COMPLEX_INT8 // datatype
                                     : SLI_MVP_DATATYPE_INT8,
                input_size_vec,   // vecs
                input_size_row,   // rows
                input_size_col,   // cols
            ...
            bool do_activation =
                (activation_min != -128) || (activation_max != 127);
            if (do_activation) {
              sli_mvp_prog_set_reg_f16c(p->p, SLI_MVP_R4,
                                        (float16_t)activation_min,
                                        (float16_t)activation_min);
              sli_mvp_prog_set_reg_f16c(p->p, SLI_MVP_R5,
                                        (float16_t)activation_max,
                                        (float16_t)activation_max);
            }
            ...
            sli_mvp_pb_begin_loop(p, output_width_truncated, &status);
            {
              ...
              {
                // R2 = CLEAR()
                sli_mvp_pb_compute(p, SLI_MVP_OP(CLEAR),
                                   SLI_MVP_ALU_Z(SLI_MVP_R2), SLI_MVP_NONE,
                                   SLI_MVP_NONE, &status);
                ...
                if (input_row_incr_left-- >
                    0) { // use one of the available load streams of next op
                  // LOAD(ARRAY0, R3)
                  // R2 = MULR2A(R2, R0)
                  // STORE(ARRAY1, R2)
                  sli_mvp_pb_compute(
            ...
            // Execute the program
            if (execute) {
              YZLITE_PROFILER_INCREMENT_PARALLEL_PROG_COUNT(
                  parallelize_channels ? 1 : 0)
              sli_mvp_pb_execute_program(p);
            }
          } // channels
        }   // batches
...
static sl_status_t max_pool(const sli_mvp_ml_pooling_s8_params_t *params,
  ...
  for (int out_x_min = 0, out_x_max; out_x_min < output_width;
    ...
    if (in_x_origin_min < 0) { // Case 1.  Each out_x leads to 1 filter_x_start
      ...
      if (input_width - in_x_origin_min >=
          filter_width) { // Case 2a. true for multiple out_x
        out_x_max = out_x_center_max;
      } else { // Case 2b. each out_x leads to 1 filter_x_end
        out_x_max = out_x_min;
      }
    ...
    for (int out_y_min = 0, out_y_max; out_y_min < output_height;
      ...
      for (int out_y_offset = 0; out_y_offset < out_y_num_offsets;
        ...
        // Ideally, at this point (or in above loops) would determine:
        //  * If all dimensions fit within MVP. Otherwise split into chunks that
        //  do fit
        //  * If problem is too small for MVP, i.e., there is more CPU overhead
        ...
        for (int batch = 0; batch < batches; ++batch) {
          ...
          for (int channel = 0; channel < depth; channel += parallelization) {
            // These array indexing computations should be moved up to highest
            // loop level possible so operation is more efficient.
            int output_index_base = sli_mvp_util_offset_nhwc(
                output_height, output_width, depth, batch,
                out_y_min + out_y_offset, out_x_min, channel);
            int output_stride_col = 0;
            ...
            // this one and do 2 at a time in parallel In general, the
            // paralellization works if depth is even, but the full expression
            // for testing is included below for completeness. Could just check
            // for depth being even at top of function and replace all the
            // complex checks with assertions.
            const bool parallelize_channels =
                (channel + 1 < depth) && (output_index_base % 2 == 0) &&
                (input_index_base % 2 == 0) && (input_stride_col % 2 == 0) &&
                (input_stride_row % 2 == 0) && (input_stride_vec % 2 == 0) &&
                (output_stride_col % 2 == 0) && (output_stride_row % 2 == 0) &&
            ...
            sli_mvp_prog_set_reg_f16c(p->p, SLI_MVP_R0, (float16_t)INT8_MIN,
                                      (float16_t)INT8_MIN);
            sli_mvp_prog_set_reg_f16c(p->p, SLI_MVP_R3,
                                      (float16_t)activation_min,
                                      (float16_t)activation_min);
            sli_mvp_prog_set_reg_f16c(p->p, SLI_MVP_R4,
                                      (float16_t)activation_max,
              ...
              sli_mvp_pb_begin_loop(p, out_y_size, &status);
              {
                ...
                sli_mvp_pb_begin_loop(p, filter_height_truncated, &status);
                {
                  sli_mvp_pb_begin_loop(p, input_size_col, &status);
                  {
                    ...
                    sli_mvp_pb_compute(
                        p, SLI_MVP_OP(MAX2A),
                        SLI_MVP_ALU_Z(SLI_MVP_R1) | SLI_MVP_ALU_X(SLI_MVP_R2) |
                            SLI_MVP_ALU_A(SLI_MVP_R1),
                        SLI_MVP_LOAD(0, SLI_MVP_R2, SLI_MVP_ARRAY(0),
                                     SLI_MVP_INCRDIM_COL),
                        SLI_MVP_NONE, &status);
                  }
                  sli_mvp_pb_end_loop(p); // input_size_col
                ...
                if (input_row_incr_left-- >
                  ...
                  sli_mvp_pb_compute(
                      p, SLI_MVP_OP(CLIP2A),
                      SLI_MVP_ALU_Z(SLI_MVP_R5) | SLI_MVP_ALU_X(SLI_MVP_R3) |
                          SLI_MVP_ALU_Y(SLI_MVP_R4) | SLI_MVP_ALU_A(SLI_MVP_R1),
                      SLI_MVP_LOAD(0, SLI_MVP_R2, SLI_MVP_ARRAY(0),
                                   SLI_MVP_INCRDIM_ROW),
                      SLI_MVP_STORE(SLI_MVP_R5, SLI_MVP_ARRAY(1),
                                    SLI_MVP_INCRDIM_VEC),
                      &status);
                } else {
                  // R5 = CLIP2A(R3, R4, R1)
