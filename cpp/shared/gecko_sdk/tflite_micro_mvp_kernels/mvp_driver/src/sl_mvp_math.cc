...
void sli_mvp_math_vector_add_i8(const int8_t *input_a, const int8_t *input_b,
                                int8_t *output, size_t len) {
  sli_mvp_program_t *program = sli_mvp_get_program_area_single();

  // Configure 3 Arrays: Array0 = input_a Array1 = input_b Array2 = output
  sli_mvp_prog_set_vector(program, SLI_MVP_ARRAY(0), (void *)input_a,
                          SLI_MVP_DATATYPE_INT8, len);
  sli_mvp_prog_set_vector(program, SLI_MVP_ARRAY(1), (void *)input_b,
                          SLI_MVP_DATATYPE_INT8, len);
  sli_mvp_prog_set_vector(program, SLI_MVP_ARRAY(2), output,
                          SLI_MVP_DATATYPE_INT8, len);
  ...
  sli_mvp_prog_set_instr(
      program, SLI_MVP_INSTR(0), SLI_MVP_OP(ADDC),
      SLI_MVP_ALU_X(SLI_MVP_R1) | SLI_MVP_ALU_A(SLI_MVP_R2) |
  ...
  sli_mvp_prog_set_loop(program, SLI_MVP_LOOP(0),
...
void sli_mvp_math_clamp_i8(int8_t *data, size_t len, int8_t min, int8_t max) {
  ...
  const size_t threshold =
      160; // non-mvp algorithm is faster bellow this threshold
  ...
  // Small datasets are faster to handle by cpu due to overhead of hw
  // accelerator
  if (len <= threshold) {
  ...
  // R2 = Load(A0), A0.Dim0++
  sli_mvp_prog_set_instr(
      prog, SLI_MVP_INSTR(0), SLI_MVP_OP(NOOP), 0,
      SLI_MVP_LOAD(0, SLI_MVP_R2, SLI_MVP_ARRAY(0), SLI_MVP_INCRDIM_COL), 0, 0);

  // R3 = Load(A0), A0.Dim0++
  // R2 = CLIP2A(R0,R1,R2)
  // Store(A1,R2), A1.Dim0++
  sli_mvp_prog_set_instr(
      prog, SLI_MVP_INSTR(1), SLI_MVP_OP(CLIP2A),
      ...
      SLI_MVP_STORE(SLI_MVP_R2, SLI_MVP_ARRAY(1), SLI_MVP_INCRDIM_COL), 0);

  // R2 = Load(A0), A0.Dim0++
  // R3 = CLIP2A(R0,R1,R3)
  // Store(A1,R3), A1.Dim0++
  sli_mvp_prog_set_instr(
      prog, SLI_MVP_INSTR(2), SLI_MVP_OP(CLIP2A),
      SLI_MVP_ALU_X(SLI_MVP_R0)        // X (R0) - Min value
          | SLI_MVP_ALU_Y(SLI_MVP_R1)  // Y (R1) - Max value
  ...
  while (remaining >= threshold) {
    ...
    sli_mvp_prog_set_vector(prog, SLI_MVP_ARRAY(0), data,
                            SLI_MVP_DATATYPE_COMPLEX_INT8, array_size);
    sli_mvp_prog_set_vector(prog, SLI_MVP_ARRAY(1), data,
                            SLI_MVP_DATATYPE_COMPLEX_INT8, array_size);

    // Loop 0 is iterating over the batch size, handling 4 elements at a time
    sli_mvp_prog_set_loop(prog, SLI_MVP_LOOP(0), batch_size, SLI_MVP_INSTR(1),
                          SLI_MVP_INSTR(2), SLI_MVP_NONE);
