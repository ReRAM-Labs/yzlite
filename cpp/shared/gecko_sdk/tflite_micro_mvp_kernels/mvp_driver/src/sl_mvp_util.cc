...
/***************************************************************************/ /**
                                                                               ...
                                                                               *
                                                                               ******************************************************************************/
sl_status_t sli_mvp_util_memclr_f16(sli_mvp_program_context_t *p, void *dst,
                                    unsigned short batches, unsigned short vecs,
  ...
  for (int batch = 0; batch < batches; ++batch) {
    ...
    sli_mvp_prog_set_reg_f16c(p->p, SLI_MVP_R0, (float16_t)0, (float16_t)0);
    sli_mvp_pb_config_array_nhwc(p->p, SLI_MVP_ARRAY(0), dst,
                                 parallel == true
                                     ? SLI_MVP_DATATYPE_COMPLEX_BINARY16
                                     : SLI_MVP_DATATYPE_BINARY16,
                                 height, width, depth, &status);
    sli_mvp_pb_begin_loop(p, depth, NULL);
    sli_mvp_pb_begin_loop(p, width, NULL);
    sli_mvp_pb_begin_loop(p, height, NULL);
    sli_mvp_pb_compute(
        p, SLI_MVP_OP(NOOP), SLI_MVP_NONE, SLI_MVP_NONE,
