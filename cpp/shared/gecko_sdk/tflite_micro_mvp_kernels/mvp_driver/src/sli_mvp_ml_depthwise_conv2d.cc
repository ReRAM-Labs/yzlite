...
struct DwConv2DDims {
  ...
  int extra_incr_map[kDimCount][kDimCount] = {
      {0}}; // Extra increments after final iteration to help with flattening.
  int mvp_dims[kDimCount] = {0};         // MVP dimension mapping.
  int reverse_mvp_dims[SLI_MVP_MAX_DIM]; // Reverse mapping from mvp dim to this
                                         // dim.
  int array_no;
  ...
  /* Return the new size of dim2 in the case that dims are flattened, but don't
   * actually flatten */
  int dry_run_flatten_dim_into_another(int dim1, int dim2) {
    auto stride_ratio = std::div(strides[dim1], strides[dim2]);
    SLI_MVP_CHECK(stride_ratio.rem == 0, 0); // check divides evenly
    int extra_incr = stride_ratio.quot - sizes[dim2];
    SLI_MVP_CHECK(extra_incr >= 0, 0);
  ...
  void unflatten_dim_from_another(int dim1, int dim2, int group_size) {
    ...
    SLI_MVP_CHECK(sizes[dim1] ==
                  1); // Dim1 should have been flattened in order to unflatten
  ...
  void mvp_incr_index_stream(sli_mvp_program_context_t *p, int dim,
                             int stream_no, int reg_no) {
    if (p->last_instr >= 8) {
      status = SL_STATUS_INVALID_PARAMETER;
      YZLITE_KERNEL_UNSUPPORTED_MSG("Instruction overflow");
      return;
    }

    if (isActive(dim)) {
      uint32_t cfg1 =
          (reg_no << _MVP_INSTRCFG1_ISTREAM0REGID_SHIFT) |
  ...
  void mvp_incr_index_loop(sli_mvp_program_context_t *p, int dim, int reg_no) {
    if ((p->loop_level + 1) >= 8) {
      YZLITE_KERNEL_UNSUPPORTED_MSG("Loop overflow");
      status = SL_STATUS_INVALID_PARAMETER;
      return;
    }
    ...
    if (isActive(dim)) {
      ...
      p->p->LOOP[p->loop_level + 1].CFG |=
          (MVP_LOOPCFG_ARRAY0INCRDIM0 << (4 * array_no)) << mvp_dims[dim];
    }
...
private:
  void mvp_incr_check_other_dimensions(sli_mvp_program_context_t *p, int dim,
                                       int reg_no, int loop_no) {
    for (int dim2 = 0; dim2 < kDimCount; ++dim2) {
      ...
      if (cnt != 0) {
        ...
        if (loop_no >= 0) {
          uint32_t loopcfg_arrayincr_mask =
              (MVP_LOOPCFG_ARRAY0INCRDIM0 | MVP_LOOPCFG_ARRAY0INCRDIM1 |
          ...
          if ((p->p->LOOP[loop_no].CFG & loopcfg_arrayincr_mask) == 0U) {
            // Set DIMx increment bit.
            loopcfg_arrayincr_mask =
                (MVP_LOOPCFG_ARRAY0INCRDIM0 << (4 * array_no)) << dim_id;
...
sl_status_t sli_mvp_ml_depthwise_conv2d_s8_gen(
    const sli_mvp_ml_depthwise_conv2d_s8_params_t *params, bool execute) {
  ...
  // In some cases, pack two reals to use both FMACs in MVP and double
  // throughput NOTE: The TFLM tensor allocator should ensure tensor buffers are
  // aligned to 8-bytes
  ...
  if (execute) {
    ...
    if (parallelize_channels) {
      if ((((uintptr_t)output & 0x1) != 0U) ||
          (((uintptr_t)filter & 0x1) != 0U) ||
          (((uintptr_t)bias & 0x3) != 0U) ||
          (((uintptr_t)output_scaler & 0x3) != 0U)) {
        status = SL_STATUS_INVALID_PARAMETER;
      }
    } else {
      // Some tensor buffers require specific alignment when processing two
      // values at a time with the complex datatypes.
      if ((((uintptr_t)bias & 0x3) != 0U) ||
          (((uintptr_t)output_scaler & 0x3) != 0U)) {
        status = SL_STATUS_INVALID_PARAMETER;
      }
    }
  ...
  calc_strides(original_output_strides, output_height, output_width,
               output_depth);
  calc_strides(original_filter_strides, filter_height, filter_width,
               filter_depth);
  ...
  input_dims.strides[kDimOutYOffset] =
      original_input_strides[2] * stride_height;
  ...
  input_dims.strides[kDimOutYGroup] =
      input_dims.strides[kDimOutYOffset] * input_dims.sizes[kDimOutYOffset];
  input_dims.sizes[kDimFilterY] = filter_height;
  input_dims.strides[kDimFilterY] =
      original_input_strides[2] * dilation_height_factor;
  input_dims.sizes[kDimFilterX] = filter_width;
  input_dims.strides[kDimFilterX] =
      original_input_strides[1] * dilation_width_factor;
  ...
  output_dims.strides[kDimInChannel] =
      original_output_strides[0] * depth_multiplier;
  ...
  for (int dim = 0; dim < kDimCount; ++dim) {
    if (input_dims.isActive(dim)) {
      if ((unsigned)input_dims.sizes[dim] > SLI_MVP_MAX_VECTOR_COUNT) {
        YZLITE_KERNEL_UNSUPPORTED_MSG("input_shape[%d] (%d) > %d", dim,
                                      input_dims.sizes[dim],
                                      SLI_MVP_MAX_VECTOR_STRIDE);
        return SL_STATUS_INVALID_PARAMETER;
      }
    }
    if (output_dims.isActive(dim)) {
      if ((unsigned)output_dims.sizes[dim] > SLI_MVP_MAX_VECTOR_COUNT) {
        YZLITE_KERNEL_UNSUPPORTED_MSG("output_shape[%d] (%d) > %d", dim,
                                      output_dims.sizes[dim],
                                      SLI_MVP_MAX_VECTOR_COUNT);
        return SL_STATUS_INVALID_PARAMETER;
      }
      if ((unsigned)output_dims.strides[dim] > SLI_MVP_MAX_VECTOR_STRIDE) {
        YZLITE_KERNEL_UNSUPPORTED_MSG("output_stride[%d] (%d) > %d", dim,
                                      output_dims.strides[dim],
                                      SLI_MVP_MAX_VECTOR_STRIDE);
        return SL_STATUS_INVALID_PARAMETER;
      }
    }
  ...
  const int out_y_center_max =
      div_floor(in_y_origin_center_max + pad_height, stride_height);
  const int in_x_origin_center_max = input_width - filter_width;
  const int out_x_center_max =
      div_floor(in_x_origin_center_max + pad_width, stride_width);
  ...
  for (int out_x_min = 0, out_x_max; out_x_min < output_width;
    ...
    const int filter_x_end =
        std::min(filter_width, input_width - in_x_origin_min);
    ...
    SLI_MVP_CHECK(output_width_truncated > 0, SL_STATUS_INVALID_PARAMETER);
    ...
    for (int out_y_min = 0, out_y_max; out_y_min < output_height;
      ...
      const int filter_y_end =
          std::min(filter_height, input_height - in_y_origin_min);
      ...
      SLI_MVP_CHECK(output_height_truncated > 0, SL_STATUS_INVALID_PARAMETER);
      ...
      if (((unsigned)in_dims.numActiveDims() > SLI_MVP_MAX_DIM) ||
          ((unsigned)in_dims.strides[kDimOutYOffset] >
           SLI_MVP_MAX_VECTOR_STRIDE)) {
        if (in_dims.isActive(kDimOutYOffset) && in_dims.isActive(kDimOutX)) {
          int new_size = in_dims.dry_run_flatten_dim_into_another(
              kDimOutYOffset, kDimOutX);

          // Pre-emptively split into groups if will be required after
          // flattening.
          if ((unsigned)new_size > SLI_MVP_MAX_VECTOR_COUNT) {
            ...
            // out_dims also has to be split in the same way since loops are not
            // adjacent and the iteration order won't match the original
            // flattened order.
            in_dims.unflatten_dim_from_another(kDimOutYGroup, kDimOutYOffset,
                                               group_size);
            out_dims.unflatten_dim_from_another(kDimOutYGroup, kDimOutYOffset,
                                                group_size);
          ...
          SLI_MVP_CHECK((unsigned)in_dims.sizes[kDimOutX] <=
                            SLI_MVP_MAX_VECTOR_COUNT,
      ...
      if (((unsigned)in_dims.numActiveDims() > SLI_MVP_MAX_DIM) ||
          ((unsigned)in_dims.strides[kDimOutYGroup] >
           SLI_MVP_MAX_VECTOR_STRIDE)) {
      ...
      SLI_MVP_CHECK(in_dims.numActiveDims() <= 4, SL_STATUS_INVALID_PARAMETER);
      ...
      if (((unsigned)out_dims.numActiveDims() > SLI_MVP_MAX_DIM) ||
          ((unsigned)out_dims.strides[kDimOutYOffset] >
           SLI_MVP_MAX_VECTOR_STRIDE)) {
      ...
      if (out_dims.isActive(kDimOutYGroup) &&
          (unsigned)out_dims.strides[kDimOutYGroup] >
              SLI_MVP_MAX_VECTOR_STRIDE) {
        ...
        while ((unsigned)out_dims.strides[kDimOutYGroup] >
               SLI_MVP_MAX_VECTOR_STRIDE) {
        ...
        // Not using the extra_incr_map because not set up for doing extra
        // incr's on same dim yet
      ...
      if ((output_width_truncated == 1) &&
          ((output_height_truncated == 1) || (filter_height_truncated == 1) ||
           (filter_width_truncated == 1))) {
        SLI_MVP_CHECK(!in_channel_outside_program, SL_STATUS_INVALID_PARAMETER);
      }
      ...
      // Could work to include batch loop in program, but it is generally just 1
      // anyway
      for (int batch = 0; batch < batches; ++batch) {
        ...
        int num_in_channels_per_program =
            in_channel_outside_program ? 1 : input_depth;
        for (int in_channel_group = 0; in_channel_group < in_channel_groups;
             ++in_channel_group) {
          in_dims.offsets[kDimInChannel] =
              in_channel_group; /* * num_in_channels_per_program; */
          out_dims.offsets[kDimInChannel] =
              in_channel_group; /* * num_in_channels_per_program; */
          SLI_MVP_CHECK((in_channel_group == 0) ||
                            (num_in_channels_per_program == 1),
                        SL_STATUS_INVALID_PARAMETER); // check assumption above

          sli_mvp_pb_begin_program(p);

          void *v_ptr;
          const int in_channel_start =
              in_channel_group * num_in_channels_per_program;
          ...
          if (parallelize_channels) {
            sli_mvp_prog_set_reg_f16c(p->p, SLI_MVP_R0,
                                      (float16_t)SLI_MVP_ACCUMULATOR_SCALER,
                                      (float16_t)SLI_MVP_ACCUMULATOR_SCALER);
            sli_mvp_prog_set_reg_f16c(
                p->p, SLI_MVP_R1,
                (float16_t)(input_offset * SLI_MVP_ACCUMULATOR_SCALER),
                (float16_t)(input_offset * SLI_MVP_ACCUMULATOR_SCALER));
            sli_mvp_prog_set_reg_f16c(p->p, SLI_MVP_R2,
                                      (float16_t)output_offset,
                                      (float16_t)output_offset);
          } else {
            sli_mvp_prog_set_reg_f16(p->p, SLI_MVP_R0,
                                     (float16_t)SLI_MVP_ACCUMULATOR_SCALER);
            sli_mvp_prog_set_reg_f16(
                p->p, SLI_MVP_R1,
                (float16_t)(input_offset * SLI_MVP_ACCUMULATOR_SCALER));
            sli_mvp_prog_set_reg_f16(p->p, SLI_MVP_R2,
                                     (float16_t)output_offset);
          }
          ...
          // Set up output array
          const int nonparallel_output_offset =
              out_dims.flat_offset() * parallelization;
          ...
          // Set up filter array
          const int nonparallel_filter_offset = sli_mvp_util_offset_nhwc(
              filter_height, filter_width, filter_depth, 0, filter_y_start,
              filter_x_start, out_channel_start);
          ...
          if (bias != nullptr) {
            bias_data_ptr =
                const_cast<float16_t *>(&bias[nonparallel_bias_offset]);
          } else {
          ...
          sli_mvp_pb_config_vector(
              ...
              bias != nullptr ? output_depth / parallelization : 1, &status);

          // Set up output_scaler array
          const int nonparallel_output_scaler_offset = out_channel_start;
          void *output_scaler_data_ptr;
          output_scaler_data_ptr = const_cast<float16_t *>(
              &output_scaler[nonparallel_output_scaler_offset]);
          sli_mvp_pb_config_vector(
              p->p, SLI_MVP_ARRAY(4),
              ...
              parallelize_channels == true
                  ? SLI_MVP_DATATYPE_COMPLEX_BINARY16 // datatype
                  : SLI_MVP_DATATYPE_BINARY16,
              output_depth / parallelization, &status);
          ...
          sli_mvp_pb_compute(p, SLI_MVP_OP(COPY),
                             ...
                             SLI_MVP_LOAD(0, SLI_MVP_R4, SLI_MVP_ARRAY(3),
                                          SLI_MVP_INCRDIM_COL) |
                                 SLI_MVP_LOAD(1, SLI_MVP_R3, SLI_MVP_ARRAY(4),
                                              SLI_MVP_INCRDIM_COL),
                             SLI_MVP_NONE, &status);
              ...
              sli_mvp_pb_begin_loop(p, out_y_groups, &status);
              {
                sli_mvp_pb_begin_loop(p, filter_height_truncated, &status);
                {
                  sli_mvp_pb_begin_loop(p, filter_width_truncated, &status);
                  {
                    ...
                    // INCR(ARRAY2, DIM1)
                    // R6 = MAC(R6, R0, R1) Compute(r_input_i, MACC, r_input_i,
                    ...
                    sli_mvp_pb_compute(
                        ...
                        SLI_MVP_LOAD(0, SLI_MVP_R6, SLI_MVP_ARRAY(0),
                                     SLI_MVP_NOINCR) |
                            SLI_MVP_LOAD(1, SLI_MVP_R7, SLI_MVP_ARRAY(2),
                                         SLI_MVP_INCRDIM_ROW),
                        SLI_MVP_NONE, &status);
                    in_dims.mvp_incr_index_stream(p, kDimFilterX, kIStream0,
                                                  SLI_MVP_R6);
                ...
                // R7 = MAC2RA(R5, R3, R2) Compute(r_output_i, MACR2A, r_acc,
                // r_output_scaler_i, c_output_offset) STORE(ARRAY1, R7)
                sli_mvp_pb_compute(
                    p, SLI_MVP_OP(MACR2A),
                ...
                out_dims.mvp_incr_index_stream(p, kDimOutYGroup, kOStream,
                                               SLI_MVP_R6);
                ...
                if (out_dim_out_y_group_extra_incr == 0) {
                  ...
                  sli_mvp_pb_begin_loop(p, out_dim_out_y_group_extra_incr,
                                        &status);
                  {
                    // R5 = COPY(R4)       Compute(r_acc, COPY, r_bias_i)
                    sli_mvp_pb_compute(
                        ...
                        SLI_MVP_LOAD(0, SLI_MVP_R7, SLI_MVP_ARRAY(1),
                                     1 << out_dims.mvp_dims[kDimOutYGroup]),
                        SLI_MVP_NONE, &status);
                  }
                  sli_mvp_pb_end_loop(p);
                }
  ...
  if (execute) {
    ...
    int computations = filter_height * filter_width * output_height *
