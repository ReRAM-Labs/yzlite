...
static uint32_t mvp_reg_cfg =
    MVP_CFG_PERF0CNTSEL_RUN | MVP_CFG_PERF1CNTSEL_STALL | MVP_CFG_PERFCNTEN;
...
void sli_mvp_power_down(void) {
  if (((CMU->CLKEN1 & CMU_CLKEN1_MVP) != 0) && ((MVP->EN & MVP_EN_EN) != 0)) {
    mvp_reg_cfg = MVP->CFG;
...
static void sli_mvp_on_em_transition(sl_power_manager_em_t from,
...
static void sli_mvp_add_transition_callback(void) {
  sl_power_manager_subscribe_em_transition_event(&mvp_event_handle,
                                                 &mvp_event_info);
}
...
void sli_mvp_power_program_wait(void) { mvp_wait(); }
...
void sli_mvp_power_program_prepare(void) {
  ...
  sl_power_manager_add_em_requirement(SL_POWER_MANAGER_EM1);
#endif
  uint32_t ret = osEventFlagsWait(mvp_event_id, SL_MVP_EVENT_FLAG_IDLE,
