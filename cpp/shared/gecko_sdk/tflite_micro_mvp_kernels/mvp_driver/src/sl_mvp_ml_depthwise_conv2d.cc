...
extern "C" sl_status_t sli_mvp_ml_depthwise_conv2d_s8_gen(
...
/***************************************************************************/ /**
                                                                               ...
                                                                               *
                                                                               ******************************************************************************/
sl_status_t sli_mvp_ml_depthwise_conv2d_s8(
    const sli_mvp_ml_depthwise_conv2d_s8_params_t *params) {
  sl_status_t status = SL_STATUS_FAIL;
  bool opt_supported = false;

  // Check if the optimized algorithm can be used.
  // Does not support dilation or depth multiplier.
  if ((params->dilation_width == 1) && (params->dilation_height == 1) &&
  ...
  if (((params->input_width * params->input_height) <=
       (4 * params->filter_width * params->filter_height *
        params->out_channels)) &&
      opt_supported) {
  ...
  if (status != SL_STATUS_OK) {
    ...
    if ((status != SL_STATUS_OK) && opt_supported) {
      // Generic algorithm failed. Optimized algorithm is not optimal, but can
      // be used.
      status = sli_mvp_ml_depthwise_conv2d_s8_gen_opt(params, true);
    }
  }
...
/***************************************************************************/ /**
                                                                               ...
                                                                               *
                                                                               ******************************************************************************/
bool sli_mvp_ml_depthwise_conv2d_s8_is_supported(
    const sli_mvp_ml_depthwise_conv2d_s8_params_t *params) {
  ...
  if (!supported) {
    supported =
        sli_mvp_ml_depthwise_conv2d_s8_gen(params, false) == SL_STATUS_OK;
  }
