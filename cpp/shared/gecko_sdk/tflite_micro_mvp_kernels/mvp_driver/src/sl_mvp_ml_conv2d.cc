...
#define SLI_MVP_CHECK(x)                                                       \
  do {                                                                         \
    if (!(x)) {                                                                \
      YZLITE_KERNEL_UNSUPPORTED_MSG("Assertion failed: " STRINGIFY(x));        \
      status = SL_STATUS_INVALID_RANGE;                                        \
    }                                                                          \
  } while (0)

static sl_status_t conv2d(const sli_mvp_ml_conv2d_s8_params_t *params,
                          bool execute);
static sl_status_t conv1d(const sli_mvp_ml_conv2d_s8_params_t *params,
                          bool execute);
static sl_status_t
conv1d_one_column(const sli_mvp_ml_conv2d_s8_params_t *params, int out_x);

__STATIC_FORCEINLINE int sli_div_floor_int(const int dividend,
                                           const int divisor) {
  return dividend / divisor;
}

__STATIC_FORCEINLINE int sli_div_ceil_int(const int dividend,
                                          const int divisor) {
...
sl_status_t sli_mvp_ml_conv2d_s8(const sli_mvp_ml_conv2d_s8_params_t *params) {
  ...
  if ((((params->input_width * params->in_channels) >
        (int)SLI_MVP_MAX_VECTOR_STRIDE) ||
       ((params->output_width * params->out_channels) >
        (int)SLI_MVP_MAX_VECTOR_STRIDE)) &&
      (params->input_height == 1) && (params->filter_height == 1) &&
      (params->output_height == 1)) {
    status = conv1d(params, true);
  }
...
/***************************************************************************/ /**
                                                                               ...
                                                                               *
                                                                               ******************************************************************************/
bool sli_mvp_ml_conv2d_s8_is_supported(
    const sli_mvp_ml_conv2d_s8_params_t *params) {
  if ((((params->input_width * params->in_channels) >
        (int)SLI_MVP_MAX_VECTOR_STRIDE) ||
       ((params->output_width * params->out_channels) >
        (int)SLI_MVP_MAX_VECTOR_STRIDE)) &&
      (params->input_height == 1) && (params->filter_height == 1) &&
      (params->output_height == 1)) {
    if (conv1d(params, false) != SL_STATUS_OK) {
      YZLITE_KERNEL_UNSUPPORTED_MSG("Hardware limits exceeded");
      return false;
    }
  } else if (conv2d(params, false) != SL_STATUS_OK) {
    YZLITE_KERNEL_UNSUPPORTED_MSG("Hardware limits exceeded");
    return false;
  }
  return true;
...
/***************************************************************************/ /**
                                                                               ...
                                                                               *
                                                                               ******************************************************************************/
int sli_mvp_ml_conv2d_s8_get_scratch_buffer_size(
    const sli_mvp_ml_conv2d_s8_params_t *params) {
  ...
  scratch_buffer_size =
      (input_width * input_height * input_depth) * sizeof(float16_t);
#else
  (void)params;
#endif
...
static sl_status_t conv1d(const sli_mvp_ml_conv2d_s8_params_t *params,
  ...
  if (padding) { // True if "SAME" padding
    ...
    pad_along_width =
        SL_MAX((out_width - 1) * stride_width + filter_width - input_width, 0);
  ...
  while ((remaining_input_width >= filter_width) && (status == SL_STATUS_OK)) {
    if (input_width_max <= output_width_max) {
      ...
      chunk_input_width =
          sli_div_floor_int(chunk_input_width - filter_width, stride_width);
      chunk_input_width = (chunk_input_width * stride_width) + filter_width;
      chunk_output_width =
          sli_div_floor_int(chunk_input_width - filter_width, stride_width) + 1;
    } else {
      ...
      chunk_input_width =
          ((chunk_output_width - 1) * stride_width) + filter_width;
    }
    par.input_width = chunk_input_width;
    par.output_width = chunk_output_width;

    // Do one Conv1D.
    status = conv2d(&par, execute);
...
static sl_status_t conv2d(const sli_mvp_ml_conv2d_s8_params_t *params,
  ...
  const float16_t input_offset_scaled(params->input_offset *
                                      SLI_MVP_ACCUMULATOR_SCALER);
  ...
  if ((dilation_width_factor != 1) || (dilation_height_factor != 1)) {
    status = SL_STATUS_INVALID_PARAMETER;
    YZLITE_KERNEL_UNSUPPORTED_MSG("Dilation not supported");
  }
  ...
  if (execute) {
    ...
    // itself dictates it.
    if ((((uintptr_t)input & 0x1) != 0U) || (((uintptr_t)filter & 0x1) != 0U) ||
        (((uintptr_t)bias & 0x1) != 0U) ||
        (((uintptr_t)output_scaler & 0x1) != 0U)) {
      status = SL_STATUS_INVALID_PARAMETER;
    }
  }
  ...
  for (int batch = 0; batch < batches; ++batch) {
    ...
    int input_index =
        sli_mvp_util_offset_nhwc(input_height, input_width, input_depth,
                                 batch /* out_channel_start */, 0, 0, 0);

    // Condition to pack two reals to use both FMACs in MVP and double
    // throughput.
    const bool use_parallel_mac_input_scaling =
        (input_size_dim2 % 2 == 0) && (input_stride_dim1 % 2 == 0) &&
        (input_stride_dim0 % 2 == 0) && (input_index % 2 == 0);
    ...
    sli_mvp_pb_config_array_full(
        p->p, SLI_MVP_ARRAY(0), (void *)&input[input_index],
        use_parallel_mac_input_scaling == true ? SLI_MVP_DATATYPE_COMPLEX_INT8
                                               : SLI_MVP_DATATYPE_INT8,
        input_size_dim0, input_size_dim1, input_size_dim2, input_stride_dim0,
        input_stride_dim1, input_stride_dim2, &status);
    ...
    sli_mvp_prog_set_reg_f16(p->p, SLI_MVP_R0,
                             (float16_t)SLI_MVP_ACCUMULATOR_SCALER);
    if (use_parallel_mac_input_scaling) {
      sli_mvp_prog_set_reg_f16c(p->p, SLI_MVP_R1, input_offset_scaled,
                                input_offset_scaled);
    } else {
    ...
    // R6 = MACC(R5, R0, R1) Compute(r_input_i, MACC, r_input_i,
    // c_accumulator_scaler, c_input_offset_scaled)
    sli_mvp_pb_compute(
        p, SLI_MVP_OP(MACC),
        SLI_MVP_ALU_Z(SLI_MVP_R6) | SLI_MVP_ALU_X(SLI_MVP_R5) |
            SLI_MVP_ALU_Y(SLI_MVP_R0) | SLI_MVP_ALU_A(SLI_MVP_R1),
        SLI_MVP_LOAD(0, SLI_MVP_R5, SLI_MVP_ARRAY(0), SLI_MVP_INCRDIM2),
        SLI_MVP_STORE(SLI_MVP_R6, SLI_MVP_ARRAY(1), SLI_MVP_INCRDIM2), &status);
  ...
  const int out_y_center_max =
      sli_div_floor_int(in_y_origin_center_max + pad_height, stride_height);
  const int in_x_origin_center_max = input_width - filter_width;
  const int out_x_center_max =
      sli_div_floor_int(in_x_origin_center_max + pad_width, stride_width);

  for (int out_x_min = 0, out_x_max; out_x_min < output_width;
       out_x_min = out_x_max + 1) {
    /* Truncate filter width to actual filter width when filter starts outside
    of valid input area, i.e. padded area */
    const int in_x_origin_min = (out_x_min * stride_width) - pad_width;
    const int filter_x_start = SL_MAX(0, -in_x_origin_min);
    const int filter_x_end =
        SL_MIN(filter_width, input_width - in_x_origin_min);
    ...
    for (int out_y_min = 0, out_y_max; out_y_min < output_height;
      ...
      if (output_height_truncated == 1 || single_out_x) {
        ...
        in_y_extra_incr = 0;
        out_y_incr = 1;
      } else if (stride_height ==
                 filter_height_truncated) { // Case 1: no extra work required
        in_y_extra_incr = 0;
        out_y_incr = 1;
      } else if (stride_height >
                 filter_height_truncated) { // Case 2: requires an extra
                                            // counting loop, but still one pass
        ...
        if (div_rem == 0) { // Case 3a: filter_height_truncated
          ...
          out_y_incr = div_quot +
                       1; // Ceiling             // next stride height multiple
      ...
      for (int out_y_offset = 0; out_y_offset < out_y_num_offsets;
           ++out_y_offset) {
        // Could compute out_y_size for offset 0 and use for all but it is
        // possible that some more programs may be possible with a tighter bound
        // on array size.
        SLI_MVP_CHECK(output_height_truncated >= out_y_offset);
        int out_y_size = sli_div_ceil_int(
            output_height_truncated - out_y_offset, out_y_incr);
        // Don't need to worry about the increments if will never go to next
        // output.
        ...
        if (single_out_x) {
          ...
          in_y_size =
              (filter_height_truncated + in_y_extra_incr_adjusted) * out_y_size;
        }

        // Ideally, at this point (or in above loops) would determine:
        //  * if all dimensions fit within MVP.  otherwise split into chunks
        //  that do fit
        //  * if problem is too small for MVP, i.e., there is more CPU overhead
        //  *   launching MVP than just computing outright, then skip mvp.

        // No more dimensions to include batch loop in program, but it is
        // generally just 1 anyway.
        for (int batch = 0; batch < batches; ++batch) {
          // These array indexing computations should be moved up to highest
          // loop level possible so uncompiled operation is more efficient.
          int output_index_base = sli_mvp_util_offset_nhwc(
              output_height, output_width, output_depth, batch,
              out_y_min + out_y_offset, out_x_min, 0 /* out_channel_start */);
          ...
          if (output_stride_vec > (int)SLI_MVP_MAX_VECTOR_STRIDE) {
            YZLITE_KERNEL_UNSUPPORTED_MSG(
                "Output vector stride (%d) exceeded (max=%d)",
                output_stride_vec, SLI_MVP_MAX_VECTOR_STRIDE)
          ...
          int filter_index_base = sli_mvp_util_offset_nhwc(
              filter_height, filter_width, input_depth,
              0 /* out_channel_start */, filter_y_start, filter_x_start,
              0 /* in_channel_start */);
          ...
          int input_index_base = sli_mvp_util_offset_nhwc(
              input_height, input_width, input_depth, batch,
              (out_y_min + out_y_offset) * stride_height - pad_height +
                  dilation_height_factor * filter_y_start,
              out_x_min * stride_width - pad_width +
          ...
          // Condition to pack two reals to use both FMACs in MVP and double
          // throughput.
          const bool use_parallel_mac =
              (input_index_base % 2 == 0) && (input_size_col % 2 == 0) &&
              (input_stride_row % 2 == 0) && (input_stride_vec % 2 == 0) &&
              (filter_index_base % 2 == 0) && (filter_size_col % 2 == 0) &&
              (filter_stride_row % 2 == 0) && (filter_stride_vec % 2 == 0);
          if (use_parallel_mac) {
            input_size_col /= 2;
            input_stride_vec /= 2;
            input_stride_row /= 2;
...
//   Array4  output
          ...
          sli_mvp_pb_config_array_full(
              p->p, SLI_MVP_ARRAY(0), (void *)&scaled_input[input_index_base],
              use_parallel_mac == true ? SLI_MVP_DATATYPE_COMPLEX_BINARY16
                                       : SLI_MVP_DATATYPE_BINARY16,
              input_size_vec, input_size_row, input_size_col, input_stride_vec,
              input_stride_row, input_stride_col, &status);
#else
          // Use unscaled input values
          sli_mvp_pb_config_array_full(
              p->p, SLI_MVP_ARRAY(0), (void *)&input[input_index_base],
              use_parallel_mac == true ? SLI_MVP_DATATYPE_COMPLEX_INT8
                                       : SLI_MVP_DATATYPE_INT8,
              input_size_vec, input_size_row, input_size_col, input_stride_vec,
              input_stride_row, input_stride_col, &status);
#endif
          ...
          sli_mvp_pb_config_array_full(
              p->p, SLI_MVP_ARRAY(1), (void *)&filter[filter_index_base],
              use_parallel_mac == true ? SLI_MVP_DATATYPE_COMPLEX_INT8
                                       : SLI_MVP_DATATYPE_INT8,
              filter_size_vec, filter_size_row, filter_size_col,
              filter_stride_vec, filter_stride_row, filter_stride_col, &status);

          sli_mvp_pb_config_vector(p->p, SLI_MVP_ARRAY(2),
                                   bias != NULL ? (void *)bias : (void *)&zero,
                                   SLI_MVP_DATATYPE_BINARY16,
                                   bias != NULL ? output_depth : 1, &status);

          sli_mvp_pb_config_vector(
              p->p, SLI_MVP_ARRAY(3), (void *)output_scaler,
              SLI_MVP_DATATYPE_BINARY16, output_depth, &status);

          sli_mvp_pb_config_array_full(
              p->p, SLI_MVP_ARRAY(4), &output[output_index_base],
              SLI_MVP_DATATYPE_INT8, output_size_vec, output_size_row,
              output_size_col, output_stride_vec, output_stride_row,
              output_stride_col, &status);
          ...
          sli_mvp_pb_begin_loop(p, output_depth, &status);
          {
            ...
            sli_mvp_pb_compute(
                p, SLI_MVP_OP(COPY),
                SLI_MVP_ALU_Z(SLI_MVP_R5) | SLI_MVP_ALU_A(SLI_MVP_R4),
                SLI_MVP_LOAD(0, SLI_MVP_R4, SLI_MVP_ARRAY(2), SLI_MVP_NOINCR) |
                    SLI_MVP_LOAD(1, SLI_MVP_R3, SLI_MVP_ARRAY(3),
                                 SLI_MVP_NOINCR),
                SLI_MVP_NONE, &status);
            ...
            sli_mvp_pb_begin_loop(p, output_width_truncated, &status);
            {
              sli_mvp_pb_begin_loop(p, out_y_size, &status);
              {
                sli_mvp_pb_begin_loop(p, filter_height_truncated, &status);
                {
                  sli_mvp_pb_begin_loop(p, input_size_col, &status);
                  {
                    ...
                    // R5 = MAC(R6, R7, R5) Compute(r_acc, MACR2A, r_input_i,
                    // r_filter_i, r_acc)
                    sli_mvp_pb_compute(
                        p, SLI_MVP_OP(MACR2A),
                        SLI_MVP_ALU_Z(SLI_MVP_R5) | SLI_MVP_ALU_X(SLI_MVP_R6) |
                            SLI_MVP_ALU_Y(SLI_MVP_R7) |
                            SLI_MVP_ALU_A(SLI_MVP_R5),
                        SLI_MVP_LOAD(0, SLI_MVP_R6, SLI_MVP_ARRAY(0),
                                     SLI_MVP_INCRDIM_COL) |
                            SLI_MVP_LOAD(1, SLI_MVP_R7, SLI_MVP_ARRAY(1),
                                         SLI_MVP_INCRDIM_COL),
                        SLI_MVP_NONE, &status);
                    ...
                    // LOAD(ARRAY1, R7)      Filter
                    // R6 = MACC(R6, R0, R1) Compute(r_input_i, MACC, r_input_i,
                    // c_accumulator_scaler, c_input_offset_scaled)
                    sli_mvp_pb_compute(
                        p, SLI_MVP_OP(MACC),
                        SLI_MVP_ALU_Z(SLI_MVP_R6) | SLI_MVP_ALU_X(SLI_MVP_R6) |
                    ...
                    // R5 = MAC(R6, R7, R5) Compute(r_acc, MACR2A, r_input_i,
                    // r_filter_i, r_acc)
                    sli_mvp_pb_compute(p, SLI_MVP_OP(MACR2A),
                                       SLI_MVP_ALU_Z(SLI_MVP_R5) |
                                           SLI_MVP_ALU_X(SLI_MVP_R6) |
                                           SLI_MVP_ALU_Y(SLI_MVP_R7) |
                                           SLI_MVP_ALU_A(SLI_MVP_R5),
                                       SLI_MVP_NONE, SLI_MVP_NONE, &status);
#endif
                  }
                  sli_mvp_pb_end_loop(p); // input_size_col
                  sli_mvp_pb_postloop_incr_dim(p, SLI_MVP_ARRAY(0),
                                               SLI_MVP_INCRDIM_ROW);
                  sli_mvp_pb_postloop_incr_dim(p, SLI_MVP_ARRAY(1),
                                               SLI_MVP_INCRDIM_ROW);
                }
                ...
                if (use_parallel_mac) {
                  ...
                  sli_mvp_pb_compute(
                      p, SLI_MVP_OP(ADDR),
                      SLI_MVP_ALU_Z(SLI_MVP_R5) | SLI_MVP_ALU_A(SLI_MVP_R5),
                ...
                // R7 = MAC(R5, R3, R2) Compute(r_output_i, MACC, r_acc,
                // r_output_scaler_i, c_output_offset) STORE(ARRAY4, R7)  Output
                sli_mvp_pb_compute(
                    p, SLI_MVP_OP(MACC),
                    SLI_MVP_ALU_Z(SLI_MVP_R7) | SLI_MVP_ALU_X(SLI_MVP_R5) |
                        SLI_MVP_ALU_Y(SLI_MVP_R3) | SLI_MVP_ALU_A(SLI_MVP_R2),
                    input_row_incr_left-- > 0
                        ? SLI_MVP_LOAD(0, SLI_MVP_R6, SLI_MVP_ARRAY(0),
                                       SLI_MVP_INCRDIM_ROW)
                        : SLI_MVP_NONE,
                    SLI_MVP_STORE(SLI_MVP_R7, SLI_MVP_ARRAY(4),
                                  SLI_MVP_INCRDIM_VEC),
                    &status);
                ...
                if (out_y_extra_incr > 0) {
                  ...
                  {
                    // NOOP used for loop incrementing output VEC array index by
                    // dummy load of R7.
                    sli_mvp_pb_compute(p, SLI_MVP_OP(NOOP), SLI_MVP_NONE,
                                       SLI_MVP_LOAD(0, SLI_MVP_R7,
                                                    SLI_MVP_ARRAY(4),
                                                    SLI_MVP_INCRDIM_VEC),
                                       SLI_MVP_NONE, &status);
                  }
                ...
                // R5 = COPY(R4)       Compute(r_acc, COPY, r_bias_i)
                sli_mvp_pb_compute(p, SLI_MVP_OP(COPY),
                                   SLI_MVP_ALU_Z(SLI_MVP_R5) |
              ...
              sli_mvp_pb_postloop_incr_dim(p, SLI_MVP_ARRAY(4),
                                           SLI_MVP_INCRDIM_ROW);
              if (!single_out_x) {
                sli_mvp_pb_postloop_incr_dim(p, SLI_MVP_ARRAY(0),
                                             SLI_MVP_INCRDIM_VEC);
            ...
            sli_mvp_pb_postloop_incr_dim(p, SLI_MVP_ARRAY(1),
                                         SLI_MVP_INCRDIM_VEC);
            sli_mvp_pb_postloop_incr_dim(p, SLI_MVP_ARRAY(2),
                                         SLI_MVP_INCRDIM_COL);
            sli_mvp_pb_postloop_incr_dim(p, SLI_MVP_ARRAY(3),
                                         SLI_MVP_INCRDIM_COL);
          }
          ...
          if (execute) {
            YZLITE_PROFILER_INCREMENT_PARALLEL_PROG_COUNT(use_parallel_mac ? 1
                                                                           : 0)
            sli_mvp_pb_execute_program(p);
          }
        } // batches
  ...
  if (execute) {
    ...
    sli_mvp_math_clamp_i8(
        params->output, batches * output_height * output_width * output_depth,
    ...
    for (int out_channel = 0; out_channel < output_depth; ++out_channel) {
      ...
      for (int filter_x = filter_x_start; filter_x < filter_x_end; ++filter_x) {
        ...
        for (int in_channel = 0; in_channel < input_depth; ++in_channel) {
          input_index = sli_mvp_util_offset_nhwc(1, input_width, input_depth,
                                                 batch, 0, in_x, in_channel);
          const int8_t input_value = input[input_index];
          filter_index =
              sli_mvp_util_offset_nhwc(1, filter_width, input_depth,
                                       out_channel, 0, filter_x, in_channel);
          const int8_t filter_value = filter[filter_index];
          acc += (input_value + input_offset) * filter_value *
                 SLI_MVP_ACCUMULATOR_SCALER;
        }
      }
      ...
      output_index = sli_mvp_util_offset_nhwc(1, 1, output_depth, batch, 0,
                                              out_x, out_channel);
