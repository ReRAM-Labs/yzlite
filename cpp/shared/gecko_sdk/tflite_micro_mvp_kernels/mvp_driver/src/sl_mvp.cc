...
static LDMA_Descriptor_t ldma_descriptor = LDMA_DESCRIPTOR_SINGLE_M2M_WORD(
    0, &(MVP->ALU[0]), sizeof(sli_mvp_program_t) / sizeof(uint32_t));
static LDMA_TransferCfg_t ldma_config = LDMA_TRANSFER_CFG_MEMORY();
static volatile uint32_t perfcnt[NUM_PERF_CNT] = {0};

/* Abstraction of interaction with the MVP hardware. This represents the
 * functionality needed for loading and executing an MVP program and makes it
 * possible to use this driver with a simulated model of the MVP hardware. */
typedef struct {
...
void sli_mvp_prog_set_reg_s8(sli_mvp_program_t *prog, uint8_t reg,
                             int8_t value) {
  sli_mvp_prog_set_reg_f16c(prog, reg, value, 0);
}

void sli_mvp_prog_set_reg_s8c(sli_mvp_program_t *prog, uint8_t reg, int8_t real,
                              int8_t imag) {
  sli_mvp_prog_set_reg_f16c(prog, reg, real, imag);
}

void sli_mvp_prog_set_reg_s16(sli_mvp_program_t *prog, uint8_t reg,
                              int16_t value) {
  sli_mvp_prog_set_reg_f16c(prog, reg, value, 0);
}

void sli_mvp_prog_set_reg_s16c(sli_mvp_program_t *prog, uint8_t reg,
                               int16_t real, int16_t imag) {
  sli_mvp_prog_set_reg_f16c(prog, reg, real, imag);
}

void sli_mvp_prog_set_reg_s32(sli_mvp_program_t *prog, uint8_t reg,
                              int32_t value) {
  sli_mvp_prog_set_reg_f16c(prog, reg, value, 0);
}

void sli_mvp_prog_set_reg_s32c(sli_mvp_program_t *prog, uint8_t reg,
                               int32_t real, int32_t imag) {
  sli_mvp_prog_set_reg_f16c(prog, reg, real, imag);
}

void sli_mvp_prog_set_reg_f16(sli_mvp_program_t *prog, uint8_t reg,
                              float16_t value) {
  sli_mvp_prog_set_reg_f16c(prog, reg, value, 0);
}

void sli_mvp_prog_set_reg_f16c(sli_mvp_program_t *prog, uint8_t reg,
                               float16_t real, float16_t imag) {
...
void sli_mvp_prog_set_reg_f32(sli_mvp_program_t *prog, uint8_t reg,
                              float value) {
  sli_mvp_prog_set_reg_f16c(prog, reg, value, 0);
}

void sli_mvp_prog_set_reg_f32c(sli_mvp_program_t *prog, uint8_t reg, float real,
                               float imag) {
  sli_mvp_prog_set_reg_f16c(prog, reg, real, imag);
}

void sli_mvp_prog_set_array_full(sli_mvp_program_t *prog, uint8_t index,
                                 void *addr, sli_mvp_datatype_t type,
                                 unsigned short vecs, unsigned short rows,
                                 unsigned short cols, int vecstride,
  ...
  EFM_ASSERT(vecstride >= MIN_ARRAY_STRIDE_SIZE &&
             vecstride <= MAX_ARRAY_STRIDE_SIZE);
  EFM_ASSERT(rowstride >= MIN_ARRAY_STRIDE_SIZE &&
             rowstride <= MAX_ARRAY_STRIDE_SIZE);
  EFM_ASSERT(colstride >= MIN_ARRAY_STRIDE_SIZE &&
             colstride <= MAX_ARRAY_STRIDE_SIZE);

  uint32_t datatype = (uint32_t)type;
  prog->ARRAY[index].DIM0CFG = (((vecs - 1) << _MVP_ARRAYDIM0CFG_SIZE_SHIFT) &
                                _MVP_ARRAYDIM0CFG_SIZE_MASK);
  prog->ARRAY[index].DIM1CFG = (((rows - 1) << _MVP_ARRAYDIM1CFG_SIZE_SHIFT) &
                                _MVP_ARRAYDIM1CFG_SIZE_MASK);
  prog->ARRAY[index].DIM2CFG = (((cols - 1) << _MVP_ARRAYDIM2CFG_SIZE_SHIFT) &
                                _MVP_ARRAYDIM2CFG_SIZE_MASK);

  // Set up Array Strides This is a 10-bit signed integer between -512 and +511
  prog->ARRAY[index].DIM0CFG |= ((vecstride << _MVP_ARRAYDIM0CFG_STRIDE_SHIFT) &
                                 _MVP_ARRAYDIM0CFG_STRIDE_MASK);
  prog->ARRAY[index].DIM1CFG |= ((rowstride << _MVP_ARRAYDIM1CFG_STRIDE_SHIFT) &
                                 _MVP_ARRAYDIM1CFG_STRIDE_MASK);
  prog->ARRAY[index].DIM2CFG |= ((colstride << _MVP_ARRAYDIM2CFG_STRIDE_SHIFT) &
                                 _MVP_ARRAYDIM2CFG_STRIDE_MASK);

  // Set up datatype
  prog->ARRAY[index].DIM0CFG |=
      ((datatype << _MVP_ARRAYDIM0CFG_BASETYPE_SHIFT) &
       _MVP_ARRAYDIM0CFG_BASETYPE_MASK);
  if ((datatype & 0x4) == 0x4) {
    prog->ARRAY[index].DIM0CFG |=
        (_MVP_ARRAYDIM0CFG_COMPLEX_COMPLEX << _MVP_ARRAYDIM0CFG_COMPLEX_SHIFT);
  }
...
void sli_mvp_prog_set_array(sli_mvp_program_t *prog, uint8_t index, void *addr,
                            sli_mvp_datatype_t type, unsigned short vecs,
                            unsigned short rows, unsigned short cols) {
  int colstride = 1;
  int rowstride = cols;
  int vecstride = rows * cols;
  sli_mvp_prog_set_array_full(prog, index, addr, type, vecs, rows, cols,
                              vecstride, rowstride, colstride);
}

void sli_mvp_prog_set_vector(sli_mvp_program_t *prog, uint8_t index, void *addr,
                             sli_mvp_datatype_t type, unsigned short len) {
  int colstride = 1;
  sli_mvp_prog_set_array_full(prog, index, addr, type, 1, 1, len, 0, 0,
                              colstride);
}

void sli_mvp_prog_set_matrix(sli_mvp_program_t *prog, uint8_t index, void *addr,
                             sli_mvp_datatype_t type, unsigned short rows,
                             unsigned short cols) {
  int colstride = 1;
  int rowstride = cols;
  sli_mvp_prog_set_array_full(prog, index, addr, type, 1, rows, cols, 0,
                              rowstride, colstride);
}

void sli_mvp_prog_set_instr(sli_mvp_program_t *prog, uint8_t index,
                            uint32_t opcode, uint32_t alu_cfg,
                            uint32_t load_cfg, uint32_t store_cfg, bool end) {
...
void sli_mvp_prog_set_loop(sli_mvp_program_t *prog, uint8_t index,
                           unsigned short count, uint8_t istart, uint8_t iend,
                           uint32_t incrarray) {
  ...
  // This is done with a read-modify-write since it overlays fields set by the
  // instruction call
  if (istart == iend) {
...
void sli_mvp_progcnt_reset(void) { mvp.program_count = 0; }

uint32_t sli_mvp_progcnt_get(void) { return mvp.program_count; }
...
void sli_mvp_pb_compute(sli_mvp_program_context_t *p, uint32_t opcode,
                        uint32_t alu_cfg, uint32_t load_cfg, uint32_t store_cfg,
                        sl_status_t *status) {
  if (p->last_instr >= 7) {
    if (status != NULL) {
      *status = SL_STATUS_INVALID_PARAMETER;
  ...
  p->last_instr++;
  sli_mvp_prog_set_instr(p->p, p->last_instr, opcode, alu_cfg, load_cfg,
                         store_cfg, false);
}

void sli_mvp_pb_config_array_full(sli_mvp_program_t *prog, uint8_t index,
                                  void *addr, sli_mvp_datatype_t type,
                                  unsigned short vecs, unsigned short rows,
                                  unsigned short cols, int vecstride,
                                  int rowstride, int colstride,
  ...
  if ((vecs > MAX_ARRAY_DIM_SIZE) || (rows > MAX_ARRAY_DIM_SIZE) ||
      (cols > MAX_ARRAY_DIM_SIZE) ||
      (vecstride < MIN_ARRAY_STRIDE_SIZE ||
       vecstride > MAX_ARRAY_STRIDE_SIZE) ||
      (rowstride < MIN_ARRAY_STRIDE_SIZE ||
       rowstride > MAX_ARRAY_STRIDE_SIZE) ||
      (colstride < MIN_ARRAY_STRIDE_SIZE ||
       colstride > MAX_ARRAY_STRIDE_SIZE)) {
    if (status != NULL) {
...
void sli_mvp_pb_config_array_nhwc(sli_mvp_program_t *prog, uint8_t index,
                                  void *addr, sli_mvp_datatype_t type,
                                  unsigned short h, unsigned short w,
                                  unsigned short c, sl_status_t *status) {
...
void sli_mvp_pb_config_matrix(sli_mvp_program_t *prog, uint8_t index,
                              void *addr, sli_mvp_datatype_t type,
                              unsigned short rows, unsigned short cols,
                              sl_status_t *status) {
  int colstride = 1;
  int rowstride = cols;
  sli_mvp_pb_config_array_full(prog, index, addr, type, 1, rows, cols, 0,
                               rowstride, colstride, status);
}

void sli_mvp_pb_config_vector(sli_mvp_program_t *prog, uint8_t index,
                              void *addr, sli_mvp_datatype_t type,
                              unsigned short len, sl_status_t *status) {
  int colstride = 1;
  sli_mvp_pb_config_array_full(prog, index, addr, type, 1, 1, len, 0, 0,
                               colstride, status);
}
...
void sli_mvp_pb_execute_program(sli_mvp_program_context_t *p) {
  ...
  for (int i = 0; i <= p->last_instr; i++) {
    p->p->INSTR[i].CFG2 =
        (p->p->INSTR[i].CFG2 & 0xFFFF0000U) | p->loop_begin_end[i];
  }
  // Mark end of program.
...
void sli_mvp_pb_postloop_incr_dim(sli_mvp_program_context_t *p,
                                  uint8_t array_index, uint8_t dimension) {
