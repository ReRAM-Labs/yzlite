...
#ifdef __arm__
#define YZLITE_PROFILER_INCREMENT_PARALLEL_PROG_COUNT(amount) ;
#define YZLITE_PROFILER_INCREMENT_OPT_PROG_COUNT(amount) ;
#else

extern void sli_mvp_increment_profiling_stat(const char *name, int32_t amount);
#define YZLITE_PROFILER_INCREMENT_PARALLEL_PROG_COUNT(amount)                  \
  sli_mvp_increment_profiling_stat("accelerator_parallel_loads", amount);
#define YZLITE_PROFILER_INCREMENT_OPT_PROG_COUNT(amount)                       \
  sli_mvp_increment_profiling_stat("accelerator_optimized_loads", amount);
#endif
