...
#define SLI_MVP_MAX_VECTOR_COUNT                                               \
  ((_MVP_ARRAYDIM0CFG_SIZE_MASK >> _MVP_ARRAYDIM0CFG_SIZE_SHIFT) + 1U)
#define SLI_MVP_MAX_ROW_LENGTH                                                 \
  ((_MVP_ARRAYDIM1CFG_SIZE_MASK >> _MVP_ARRAYDIM1CFG_SIZE_SHIFT) + 1U)
#define SLI_MVP_MAX_COLUMN_LENGTH                                              \
  ((_MVP_ARRAYDIM2CFG_SIZE_MASK >> _MVP_ARRAYDIM2CFG_SIZE_SHIFT) + 1U)
#define SLI_MVP_MAX_VECTOR_STRIDE                                              \
  ((_MVP_ARRAYDIM0CFG_STRIDE_MASK >> (_MVP_ARRAYDIM0CFG_STRIDE_SHIFT + 1)))
#define SLI_MVP_MIN_VECTOR_STRIDE (-MAX_VECTOR_STRIDE - 1)
#define SLI_MVP_MAX_ROW_STRIDE                                                 \
  ((_MVP_ARRAYDIM1CFG_STRIDE_MASK >> (_MVP_ARRAYDIM1CFG_STRIDE_SHIFT + 1)))
#define SLI_MVP_MIN_ROW_STRIDE (-MAX_ROW_STRIDE - 1)
#define SLI_MVP_MAX_COLUMN_STRIDE                                              \
  ((_MVP_ARRAYDIM2CFG_STRIDE_MASK >> (_MVP_ARRAYDIM2CFG_STRIDE_SHIFT + 1)))
...
// be used instead when building up the ALU configuration when no flags are
// used. a - 0=ALUIN0, 1=ALUIN1, 2=ALUIN2 r - 0-7=R0-R7 f - flags, combination
// of any of these:
...
typedef struct {
  ...
  int prog_index; ///< Index used to select one of two possible MVP program
                  ///< buffers.
...
/**
 * @brief
 *   Configure a specific array in a program, with possibility to specify
 * strides.
 *
 ...
 * @param[in] vecstride Number of elements to advance when moving to the next
 * vector (can be negative).
 * @param[in] rowstride Number of elements to advance when moving to the next
 * row (can be negative).
 * @param[in] colstride Number of elements to advance when moving to the next
 ...
 * @param[in] alu_cfg  ALU configuration specifying input/output registers to
 * use.
 ...
 * @param[in] end True if this is the last instruction of the program, false
 * otherwise.
 */
 ...
 *
 * @details
 *   Each of the performance counters can track different events. What event to
 * count is configurable using the type parameter.
 *
 * @param[in] id
 ...
 * @param[out] status Returns SL_STATUS_INVALID_PARAMETER when loop count is
 * exhausted. status is only updated when this error occur.
 */
 ...
 * @param[out] status Returns SL_STATUS_INVALID_PARAMETER when instruction count
 * is exhausted. status is only updated when this error occur.
 */
