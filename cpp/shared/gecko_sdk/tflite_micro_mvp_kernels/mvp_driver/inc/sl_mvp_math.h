 ...
 *   following operation: Z = A + B. The add operation is performing a
 * saturation add, which means that the operation will never overflow or
 * underflow. When adding two elements would overflow (>127) then the result
 ...
 *
 * @details
 *   Given a min/max value, this function will make sure that none of the
 * element in the input vector will be < min or > max. If any elements are < min
 * then the value will be modified to min. If any elements are > max then value
 * will be modified to max.
 *
