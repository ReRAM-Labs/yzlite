...
TfLiteStatus Prepare(TfLiteContext *context, TfLiteNode *node) {
  ...
  auto padding = ComputePaddingHeightWidth(
      ...
      1, // dilation rate height/width.
      data->op_params.input_height, data->op_params.input_width,
      params->filter_height, params->filter_width, params->padding, &out_height,
      &out_width);
...
TfLiteStatus AveragePrepare(TfLiteContext *context, TfLiteNode *node) {
  ...
  if (status == kTfLiteOk) {
    if (input->type == kTfLiteInt8) {
      data->supported =
          sli_mvp_ml_average_pooling_s8_is_supported(&data->op_params)
              ? kMvp
              : kCmsisNN;
      if (data->supported == kCmsisNN) {
        const int32_t buffer_size = arm_avgpool_s8_get_buffer_size(
            data->op_params.output_width, data->op_params.channels);
...
TfLiteStatus AverageEval(TfLiteContext *context, TfLiteNode *node) {
  ...
  if (data->supported == kMvp) {
    ...
    TFLITE_DCHECK_EQ(arm_avgpool_s8(&ctx, &pool_params, &input_dims,
                                    ...
                                    &output_dims, data->op_params.output),
