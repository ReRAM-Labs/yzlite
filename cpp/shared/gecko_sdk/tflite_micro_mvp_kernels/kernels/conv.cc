...
TfLiteStatus Prepare(TfLiteContext *context, TfLiteNode *node) {
  ...
  if (input->type == kTfLiteInt8) {
    if (sli_mvp_ml_conv2d_s8_is_supported(&data->op_params)) {
      ...
      scratch_buffer_size =
          sli_mvp_ml_conv2d_s8_get_scratch_buffer_size(&data->op_params);
...
TfLiteStatus eval_mvp_int8(TfLiteContext *context, OpData *data,
  ...
  if (data->scratch_buffer_index > -1) {
    data->op_params.scratch_buffer = (float16_t *)context->GetScratchBuffer(
        context, data->scratch_buffer_index);
  }

  TF_LITE_ENSURE_EQ(context, SL_STATUS_OK,
                    sli_mvp_ml_conv2d_s8(&data->op_params));
...
TfLiteStatus eval_float(TfLiteConvParams *params, const OpData *data,
                        const TfLiteEvalTensor *input,
                        const TfLiteEvalTensor *filter,
                        const TfLiteEvalTensor *bias,
                        TfLiteEvalTensor *output) {
