...
TfLiteStatus Prepare(TfLiteContext *context, TfLiteNode *node) {
  ...
  if (input->type == kTfLiteInt8) {
    ...
    CalculateActivationRange(kTfLiteActNone, &data->activation_min_f32,
...
TfLiteStatus eval_mvp_int8(TfLiteContext *context, OpData *data,
  ...
  if (data->scratch_buffer_index > -1) {
    scratch = reinterpret_cast<float16_t *>(
  ...
  TF_LITE_ENSURE_EQ(context, SL_STATUS_OK,
                    sli_mvp_ml_transpose_conv2d_s8(&data->op_params));

  return kTfLiteOk;
}

TfLiteStatus eval_tflm_int8(TfLiteContext *context, OpData *data,
                            const TfLiteEvalTensor *input,
                            const TfLiteEvalTensor *filter,
  ...
  if (data->scratch_buffer_index > -1) {
    scratch = reinterpret_cast<int32_t *>(
        context->GetScratchBuffer(context, data->scratch_buffer_index));
  } else {
    return kTfLiteError;
  }
  ...
  reference_integer_ops::TransposeConv(
      ...
      tflite::micro::GetTensorData<int32_t>(
          const_cast<TfLiteEvalTensor *>(bias)),
...
TfLiteStatus eval_float(TfLiteTransposeConvParams *params, const OpData *data,
                        const TfLiteEvalTensor *input,
                        const TfLiteEvalTensor *filter,
                        const TfLiteEvalTensor *bias,
                        TfLiteEvalTensor *output) {
