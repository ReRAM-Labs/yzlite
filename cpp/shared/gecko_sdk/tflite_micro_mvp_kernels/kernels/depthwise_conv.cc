...
TfLiteStatus PopulateConvolutionQuantizationParams(
    ...
    const TfLiteTensor *filter, TfLiteTensor *output,
    const TfLiteFusedActivation &activation, int32_t *output_activation_min,
    int32_t *output_activation_max, float16_t *per_channel_scalers,
    int num_channels, float accumulator_multipler) {
  auto affine_quantization = reinterpret_cast<const TfLiteAffineQuantization *>(
  ...
  for (int i = 0; i < num_channels; ++i) {
    ...
    const float effective_output_scale =
        (input_scale * filter_scale) / output_scale;
    const float acc_output_scale =
        effective_output_scale * accumulator_multipler;
...
TfLiteStatus Prepare(TfLiteContext *context, TfLiteNode *node) {
  ...
  const auto padding = ComputePaddingHeightWidth(
      ...
      params->padding, &dummy_height, &dummy_width);
  ...
  if (input->type == kTfLiteInt8) {
    if (sli_mvp_ml_depthwise_conv2d_s8_is_supported(&data->op_params)) {
      ...
      float16_t *bias_data =
          static_cast<float16_t *>(context->AllocatePersistentBuffer(
      ...
      dw_conv_params.ch_mult =
          data->op_params.out_channels / data->op_params.in_channels;
...
TfLiteStatus eval_mvp_int8(TfLiteContext *context, OpData *data,
  ...
  TF_LITE_ENSURE_EQ(context, SL_STATUS_OK,
                    sli_mvp_ml_depthwise_conv2d_s8(&data->op_params));

  return kTfLiteOk;
}

#ifdef __arm__
TfLiteStatus eval_cmsis_int8(TfLiteContext *context, OpData *data,
                             const TfLiteEvalTensor *input,
                             const TfLiteEvalTensor *filter,
  ...
  TFLITE_DCHECK_EQ(
      ...
      arm_depthwise_conv_wrapper_s8(
          ...
          bias == nullptr ? NULL : tflite::micro::GetTensorData<int32_t>(bias),
...
TfLiteStatus eval_tflm_int8(OpData *data, const TfLiteEvalTensor *input,
                            const TfLiteEvalTensor *filter,
                            const TfLiteEvalTensor *bias,
                            TfLiteEvalTensor *output) {
  ...
  dw_op_params.depth_multiplier =
      data->op_params.out_channels / data->op_params.in_channels;

  reference_integer_ops::DepthwiseConvPerChannel(
      dw_op_params, data->per_channel_output_multiplier,
...
TfLiteStatus eval_float(TfLiteDepthwiseConvParams *params, const OpData *data,
                        const TfLiteEvalTensor *input,
                        const TfLiteEvalTensor *filter,
                        const TfLiteEvalTensor *bias,
                        TfLiteEvalTensor *output) {
  ...
  reference_ops::DepthwiseConv(
      dw_op_params, tflite::micro::GetTensorShape(input),
