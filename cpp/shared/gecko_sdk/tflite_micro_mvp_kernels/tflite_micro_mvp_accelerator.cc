...
static const char *perfcnt_names[] = {"run",
                                      "cmd",
                                      "stall",
                                      "noop",
                                      "alu-active",
...
/*************************************************************************************************/
static void deinit_accelerator() { sli_mvp_deinit(); }

/*************************************************************************************************/
static int get_profiler_loop_count() { return 7; }
...
#ifdef TFLITE_MICRO_SIMULATOR_ENABLED
  if (profiler != nullptr)
    sli_mvp_set_current_profiler(profiler);
#endif
...
static void stop_op_profiler(int op_idx, profiling::Profiler *profiler) {
  ...
  if (profiler != nullptr) {
    ...
    profiler->parent()->increment_custom_stat(
        perfcnt_names[current_loop_index * 2], percnt0);
    profiler->parent()->increment_custom_stat(
        perfcnt_names[current_loop_index * 2 + 1], percnt1);
    profiler->increment_custom_stat(perfcnt_names[current_loop_index * 2],
                                    percnt0);
    profiler->increment_custom_stat(perfcnt_names[current_loop_index * 2 + 1],
                                    percnt1);
#else
...
extern "C" void yzlite_tflite_micro_register_accelerator() {
  yzlite_tflite_micro_set_accelerator(&accelerator);
}
