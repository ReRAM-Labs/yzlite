...
#define GetInput(index)                                                        \
  tflite::micro::GetEvalInput(context, &node_and_registration.node, index)
#define GetOutput(index)                                                       \
  tflite::micro::GetEvalOutput(context, &node_and_registration.node, index)

/*************************************************************************************************/
static uint32_t add_activation_ops(TfLiteFusedActivation activation,
                                   int count) {
  uint32_t retval = 0;
...
static void calculate_fully_connected(
  ...
  const int accum_depth =
      weights_shape.Dims(weights_shape.DimensionsCount() - 1);
...
/*************************************************************************************************/
static void
calculate_conv2d(const TfLiteContext *context,
                 const tflite::NodeAndRegistration &node_and_registration,
                 profiling::Metrics &metrics) {
  ...
  metrics.macs = ((filter_height * filter_width * input_depth) * output_width *
  ...
  metrics.ops += add_activation_ops(
      params->activation, output_width * output_height * output_depth);
}
...
static void calculate_transpose_conv(
  ...
  tflite::ComputePaddingHeightWidth(
      params->stride_height, params->stride_width, 1, 1, input_height,
      input_width, filter_height, filter_width, params->padding, &output_height,
      &output_width);

  metrics.macs = ((filter_height * filter_width * output_depth) * input_width *
...
static void calculate_depthwise_conv2d(
  ...
  metrics.macs = (filter_width * filter_height * depth_multiplier *
                  input_depth * output_width * output_height);
  metrics.ops = metrics.macs * 2;
  if (bias != nullptr) {
    metrics.ops +=
        depth_multiplier * input_depth * output_width * output_height;
  }
  metrics.ops +=
      add_activation_ops(params->activation, depth_multiplier * input_depth *
                                                 output_width * output_height);
}

/*************************************************************************************************/
static void
calculate_max_pool2d(const TfLiteContext *context,
                     const tflite::NodeAndRegistration &node_and_registration,
                     profiling::Metrics &metrics) {
  constexpr int kInputTensor = 0;
  constexpr int kOutputTensor = 0;
  ...
  metrics.ops = (output_height * output_width * output_depth *
                 (params->filter_width * params->filter_height));
  metrics.ops += add_activation_ops(
      params->activation, output_height * output_width * output_depth);
}
...
static void calculate_average_pool2d(
  ...
  metrics.ops = (output_height * output_width * output_depth *
                 (params->filter_width * params->filter_height + 1));
  metrics.ops += add_activation_ops(
      params->activation, output_height * output_width * output_depth);
}
...
/*************************************************************************************************/
static void
calculate_softmax(const TfLiteContext *context,
                  const tflite::NodeAndRegistration &node_and_registration,
                  profiling::Metrics &metrics) {
...
/*************************************************************************************************/
static void
calculate_add(const TfLiteContext *context,
              const tflite::NodeAndRegistration &node_and_registration,
              profiling::Metrics &metrics) {
  const auto input = GetInput(0);
...
/*************************************************************************************************/
static void
calculate_quantize(const TfLiteContext *context,
                   const tflite::NodeAndRegistration &node_and_registration,
                   profiling::Metrics &metrics) {
  const auto input = GetInput(0);
  const auto input_shape = tflite::micro::GetTensorShape(input);
...
/*************************************************************************************************/
static void
calculate_dequantize(const TfLiteContext *context,
                     const tflite::NodeAndRegistration &node_and_registration,
                     profiling::Metrics &metrics) {
  const auto input = GetInput(0);
  const auto input_shape = tflite::micro::GetTensorShape(input);
...
/*************************************************************************************************/
static void
calculate_pad(const TfLiteContext *context,
              const tflite::NodeAndRegistration &node_and_registration,
              profiling::Metrics &metrics) {
...
/*************************************************************************************************/
static void
calculate_reshape(const TfLiteContext *context,
                  const tflite::NodeAndRegistration &node_and_registration,
                  profiling::Metrics &metrics) {
  const auto input = GetInput(0);
  const auto output = GetOutput(0);
...
/*************************************************************************************************/
static void
calculate_mean(const TfLiteContext *context,
               const tflite::NodeAndRegistration &node_and_registration,
               profiling::Metrics &metrics) {
  //    const auto input = GetInput(0);
  ...
  metrics.ops =
      0; // (output_depth * input_height * input_width) + output_depth;
}
...
static void calculate_resize_nearest_neighbor(
  ...
  constexpr int GetNearestNeighbor_flops =
      8; // just an approximation for the number of operations in
         // GetNearestNeighbor()
  ...
  metrics.ops = output_height * (GetNearestNeighbor_flops +
                                 output_width * GetNearestNeighbor_flops);
}

/*************************************************************************************************/
static void
calculate_relu(const TfLiteContext *context,
               const tflite::NodeAndRegistration &node_and_registration,
               profiling::Metrics &metrics) {
  constexpr int kInputTensor = 0;
  ...
  metrics.ops = input_shape.FlatSize() *
                2; // 2 ops, one for matching upper and lower limits
}

/*************************************************************************************************/
static void
calculate_multiply(const TfLiteContext *context,
                   const tflite::NodeAndRegistration &node_and_registration,
                   profiling::Metrics &metrics) {
  const auto output = GetOutput(0);
  const auto output_shape = tflite::micro::GetTensorShape(output);
