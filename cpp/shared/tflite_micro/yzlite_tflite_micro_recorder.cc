...
bool get_recorded_data(const uint8_t **data_ptr, uint32_t *length_ptr) {
  ...
  return msgpack_buffered_writer_get_buffer(
             _msgpack_context, (uint8_t **)data_ptr, length_ptr) == 0;
}
...
record_layer_conv_params(const tflite::ConvParams &params,
  ...
  msgpack_write_dict_bin(msgpack, "per_channel_multiplier",
                         per_channel_output_multiplier,
                         sizeof(int32_t) * n_channels);
  msgpack_write_dict_bin(msgpack, "per_channel_shift", per_channel_output_shift,
                         sizeof(int32_t) * n_channels);
  msgpack_write_dict_int(msgpack, "padding_type",
                         padding_to_tflite_schema(params.padding_type));
  msgpack_write_dict_int(msgpack, "padding_width", params.padding_values.width);
  msgpack_write_dict_int(msgpack, "padding_height",
  ...
  msgpack_write_dict_int(msgpack, "dilation_width_factor",
                         params.dilation_width_factor);
  msgpack_write_dict_int(msgpack, "dilation_height_factor",
  ...
  msgpack_write_dict_int(msgpack, "quantized_activation_min",
                         params.quantized_activation_min);
  msgpack_write_dict_int(msgpack, "quantized_activation_max",
...
extern "C" void record_layer_fully_connected_params(
  ...
  msgpack_write_dict_int(msgpack, "weights_offset", params.weights_offset);
  msgpack_write_dict_int(msgpack, "output_offset", params.output_offset);
  msgpack_write_dict_int(msgpack, "output_multiplier",
...
/*************************************************************************************************/
#ifndef YZLITE_DLL_IMPORT
void record_layer_pool_params(const tflite::PoolParams &params) {
  msgpack_context_t *msgpack = get_layer_recording_context();
  msgpack_write_dict_dict(msgpack, "params", -1);
  msgpack_write_dict_int(msgpack, "padding_type",
                         padding_to_tflite_schema(params.padding_type));
