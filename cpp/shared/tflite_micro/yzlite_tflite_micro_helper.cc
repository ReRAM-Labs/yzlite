...
static Logger *yzlite_logger = nullptr;
...
TfLiteStatus allocate_scratch_buffer(TfLiteContext *ctx, unsigned size_bytes,
  ...
  auto status =
      ctx->RequestScratchBufferInArena(ctx, size_bytes, scratch_buffer_index);
  if (status != kTfLiteOk) {
...
extern "C" void issue_unsupported_kernel_message(const char *fmt, ...) {
  ...
  char op_name[92];
  const int l =
      snprintf(buffer, sizeof(buffer), "%s not supported: ",
               op_to_str(_current_kernel_index,
                         (tflite::BuiltinOperator)_current_kernel_op_code));
...
/*************************************************************************************************/
extern "C" void
yzlite_tflite_micro_get_current_layer_opcode_and_index(int *opcode,
                                                       int *index) {
  *opcode = yzlite::_current_kernel_op_code;
  *index = yzlite::_current_kernel_index;
}

#endif // YZLITE_DLL_IMPORT

/*************************************************************************************************/
Logger &get_logger() {
  if (yzlite_logger == nullptr) {
    yzlite_logger = logging::get("YZLITE");
    if (yzlite_logger == nullptr) {
      yzlite_logger = logging::create("YZLITE", LogLevel::Info);
      assert(yzlite_logger != nullptr);
    }
  }

  return *yzlite_logger;
}

/*************************************************************************************************/
bool set_log_level(LogLevel level) { return get_logger().level(level); }
...
/*************************************************************************************************/
#ifndef YZLITE_DLL_IMPORT
extern "C" const TfliteMicroAccelerator *
...
extern "C" void yzlite_tflite_micro_register_accelerator() {
  // This is just a placeholder if no accelerator is built into the binary /
  // shared library (i.e. each accelerator also defines this API and internally
  // calls yzlite_tflite_micro_set_accelerator())
  yzlite_tflite_micro_set_accelerator(nullptr);
}
#endif
...
/*************************************************************************************************/
bool get_tflite_flatbuffer_from_end_of_flash(const uint8_t **flatbuffer,
                                             uint32_t *length,
  ...
  *flatbuffer = nullptr;
  ...
  flash_end_addr = (flash_end_addr == nullptr)
                       ? (const uint32_t *)(FLASH_BASE + FLASH_SIZE)
                       : flash_end_addr;
  ...
  const uint8_t *tflite_flatbuffer =
      (const uint8_t *)flash_end_addr - sizeof(uint32_t) - tflite_length;
  if (verify_model_flatbuffer(tflite_flatbuffer, tflite_length)) {
    *flatbuffer = tflite_flatbuffer;
    if (length != nullptr) {
      *length = tflite_length;
    }
    YZLITE_INFO("Using .tflite in flash at 0x%08X", tflite_flatbuffer);
    return true;
  }
#endif
...
bool verify_model_flatbuffer(const void *flatbuffer, int flatbuffer_length) {
  flatbuffers::Verifier verifier((const uint8_t *)flatbuffer,
                                 flatbuffer_length);
