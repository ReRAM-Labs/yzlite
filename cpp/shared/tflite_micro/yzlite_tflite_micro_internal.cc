...
void allocate_profilers(int subgraph_index, int op_count) {
  ...
  _inference_profiler->flags(profiling::Flag::ReportTotalChildrenCycles |
                             profiling::Flag::ReportsFreeRunningCpuCycles);
  _kernel_profilers = static_cast<profiling::Profiler **>(
      malloc(sizeof(profiling::Profiler *) * op_count));
...
void register_profiler(
  ...
  profiling::register_profiler(op_to_str(op_idx, op_type), profiler,
                               _inference_profiler);
  if (profiler == nullptr) {
    return;
  }
...
const char *op_to_str(int op_idx, tflite::BuiltinOperator op_type) {
  ...
  snprintf(op_name_buffer, sizeof(op_name_buffer), "Op%d-%s", op_idx,
           to_str(op_type));
