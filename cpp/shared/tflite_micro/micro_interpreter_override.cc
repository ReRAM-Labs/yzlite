...
TfLiteStatus MicroInterpreter::PrepareNodeAndRegistrationDataFromFlatbuffer() {
  ...
  for (int subgraph_idx = 0; subgraph_idx < graph_.NumSubgraphs();
    ...
    for (size_t i = 0; i < operators_size; ++i) {
      ...
      if (status != kTfLiteOk) {
        retval = kTfLiteError;
        logger.error("Op%d-%s not supported: Unknown layer type", i,
      ...
      if (registration == nullptr) {
        retval = kTfLiteError;
        logger.error("Op%d-%s not supported: Failed to get registration for "
                     "opcode_index %d",
                     i, EnumNameBuiltinOperator(GetBuiltinCode(opcode)), index);
        continue;
      }
      ...
      if (op_type == BuiltinOperator_CUSTOM) {
        ...
        if (op->custom_options() != nullptr) {
          retval = kTfLiteError;
          logger.error("Op%d-%s not supported: Found builtin operator %s with "
                       "custom options",
        ...
        if (parser == nullptr) {
          retval = kTfLiteError;
          logger.error("Op%d-%s not supported: Did not find a parser", i,
  ...
  if (retval != kTfLiteOk) {
    yzlite::model_has_unsupported_layers = true;
  }
  return retval;
}
