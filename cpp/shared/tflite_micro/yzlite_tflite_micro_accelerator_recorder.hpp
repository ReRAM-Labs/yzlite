...
#define TFLITE_MICRO_ACCELERATOR_RECORDER_CLEAR()                              \
  ::yzlite::TfliteMicroAcceleratorRecorder::instance().clear()
#define TFLITE_MICRO_ACCELERATOR_RECORDER_START_LAYER()                        \
  ::yzlite::TfliteMicroAcceleratorRecorder::instance().start_layer()
#define TFLITE_MICRO_ACCELERATOR_RECORDER_END_LAYER()                          \
  ::yzlite::TfliteMicroAcceleratorRecorder::instance().end_layer()
#define TFLITE_MICRO_ACCELERATOR_RECORD_PROGRAM(data, length)                  \
  ::yzlite::TfliteMicroAcceleratorRecorder::instance().record_program(         \
      (const void *)data, length)
#define TFLITE_MICRO_ACCELERATOR_RECORD_DATA(key, data, ...)                   \
  ::yzlite::TfliteMicroAcceleratorRecorder::instance().record_data(            \
      key, data, ##__VA_ARGS__)
...
struct TfliteMicroAcceleratorRecorder {
  ...
  void start_layer() { layer_started = true; }
  ...
  template <typename T> bool record_data(const char *key, T value) {
    if (!data_recording_enabled || !layer_started) {
      return false;
    }
    auto msgpack = get_layer_recording_context(true);
