...
MicroGraph::MicroGraph(TfLiteContext *context, const Model *model,
    ...
    : context_(context), model_(model), allocator_(allocator),
      current_subgraph_index_(0), resource_variables_(resource_variables) {
  if (model != nullptr) {
    subgraphs_ = model->subgraphs();
  }
}

MicroGraph::~MicroGraph() { FREE_PROFILERS(); }
    ...
    for (size_t i = 0; i < operators_size; ++i) {
      ...
      if (registration->prepare != nullptr) {
        ...
        if (prepare_status != kTfLiteOk) {
          logger.error(
              "Op%d-%s not supported: Failed to prepare with status %d", i,
      ...
      REGISTER_PROFILER(
          subgraph_idx, i, registration->builtin_code, context_,
...
// only defined for builds with the error strings.
    ...
    if (invoke_status == kTfLiteError) {
      logger.error("Op%d-%s not supported: Failed to invoke with status %d", i,
                   OpNameFromRegistration(registration), invoke_status);
      return kTfLiteError;
