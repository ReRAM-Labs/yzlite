...
#define _MSGPACK_GET_OBJ_VALUE_(obj, obj_type, name)                           \
  ((const obj_type *)(obj))->data.name
#define _MSGPACK_IS_(_obj, _type)                                              \
  ((_obj != NULL) &&                                                           \
   (((const msgpack_object_t *)_obj)->type == MSGPACK_TYPE_##_type))

/**
 * Get MessagePack object as boolean
 */
#define MSGPACK_BOOL(obj)                                                      \
  _MSGPACK_GET_OBJ_VALUE_(obj, msgpack_object8_t, boolean)
/**
 ...
 */
#define MSGPACK_DOUBLE(obj)                                                    \
  _MSGPACK_GET_OBJ_VALUE_(obj, msgpack_object64_t, dbl)
/**
 * Get MessagePack object with specific key in dictionary (aka map)
 */
#define MSGPACK_DICT(obj, key)                                                 \
  msgpack_get_dict_object((msgpack_object_dict_t *)obj, key, MSGPACK_TYPE_ANY)
/**
 ...
 */
#define MSGPACK_DICT_TYPE(obj, key, type)                                      \
  msgpack_get_dict_object((msgpack_object_dict_t *)obj, key,                   \
                          MSGPACK_TYPE_##type)
/**
 * Get MessagePack dictionary entry with a STR type,
 * return NULL if the entry is found but the wrong type
 */
#define MSGPACK_DICT_STR(obj, key)                                             \
  (msgpack_object_str_t *)MSGPACK_DICT_TYPE(obj, key, STR)
/**
 * Get MessagePack dictionary entry with a BIN type,
 * return NULL if the entry is found but the wrong type
 */
#define MSGPACK_DICT_BIN(obj, key)                                             \
  (msgpack_object_bin_t *)MSGPACK_DICT_TYPE(obj, key, BIN)
/**
 * Get MessagePack dictionary entry with a DICT type,
 * return NULL if the entry is found but the wrong type
 */
#define MSGPACK_DICT_DICT(obj, key)                                            \
  (msgpack_object_dict_t *)MSGPACK_DICT_TYPE(obj, key, DICT)
/**
 * Get MessagePack dictionary entry with a ARRAY type,
 * return NULL if the entry is found but the wrong type
 */
#define MSGPACK_DICT_ARRAY(obj, key)                                           \
  (msgpack_object_array_t *)MSGPACK_DICT_TYPE(obj, key, ARRAY)
/**
 * Get MessagePack dictionary entry with an signed integer (unsigned and bool
 * included) type. Use MSGPACK_DICT_TYPE(obj, key, INT) if you specifically need
 * a signed integer. return NULL if the entry is found but the wrong type
 */
#define MSGPACK_DICT_INT(obj, key) MSGPACK_DICT_TYPE(obj, key, INT_OR_UINT)
/**
 * Get MessagePack dictionary entry with a unsigned integer (signed and bool
 * included) type, Use MSGPACK_DICT_TYPE(obj, key, UINT) if you specifically
 * need an unsigned integer. return NULL if the entry is found but the wrong
 * type
 */
 ...
 */
#define MSGPACK_ARRAY(obj, index)                                              \
  msgpack_get_array_object((msgpack_object_array_t *)obj, index,               \
                           MSGPACK_TYPE_ANY)
/**
 * Get MessagePack array at index with a specific type,
 * return NULL if the entry is found but the wrong type
 */
#define MSGPACK_ARRAY_TYPE(obj, index, type)                                   \
  msgpack_get_array_object((msgpack_object_array_t *)obj, index,               \
                           MSGPACK_TYPE_##type)
/**
 ...
 * return NULL if the entry is found but the wrong type
 */
#define MSGPACK_ARRAY_DICT(obj, index)                                         \
  (msgpack_object_dict_t *)MSGPACK_ARRAY_TYPE(obj, index, DICT)
/**
 * Get MessagePack array at index with a ARRAY type,
 * return NULL if the entry is found but the wrong type
 */
#define MSGPACK_ARRAY_ARRAY(obj, index)                                        \
  (msgpack_object_array_t *)MSGPACK_ARRAY_TYPE(obj, index, ARRAY)
/**
 ...
 * return NULL if the entry is found but the wrong type
 */
#define MSGPACK_ARRAY_STR(obj, index)                                          \
  (msgpack_object_str_t *)MSGPACK_ARRAY_TYPE(obj, index, STR)
/**
 * Get MessagePack array at index with a BIN type,
 * return NULL if the entry is found but the wrong type
 */
#define MSGPACK_ARRAY_BIN(obj, index)                                          \
  (msgpack_object_bin_t *)MSGPACK_ARRAY_TYPE(obj, index, BIN)
/**
 * Get MessagePack array at index with a signed integer (unsigned and bool
 * included) type, return NULL if the entry is found but the wrong type
 */
#define MSGPACK_ARRAY_INT(obj, index) MSGPACK_ARRAY_TYPE(obj, index, INT)
/**
 * Get MessagePack array at index with a unsigned integer (signed and bool
 * included) type, return NULL if the entry is found but the wrong type
 */
 ...
 */
#define MSGPACK_BIN_CMP(obj, mem)                                              \
 ...
 */
#define MSGPACK_STR(obj, buffer, max_length)                                   \
  msgpack_get_str((msgpack_object_t *)obj, buffer, max_length)
/**
 * Copy BIN obj contents to buffer
 */
#define MSGPACK_BIN(obj, mem, max_length)                                      \
 ...
 */
#define MSGPACK_TO_STR(obj, buffer, max_length)                                \
  msgpack_to_str((msgpack_object_t *)obj, buffer, max_length)
 ...
 */
#define MSGPACK_IS_UINT(obj)                                                   \
  msgpack_object_is_type((msgpack_object_t *)obj, MSGPACK_TYPE_UINT)
/**
 * Return true if MessagePack object is an signed integer
 */
#define MSGPACK_IS_INT(obj)                                                    \
  msgpack_object_is_type((msgpack_object_t *)obj, MSGPACK_TYPE_INT)
/**
...
/**
 * Return the number of data buffer bytes currently used by a @ref
 * msgpack_context_t
 *
 * @param ctx @ref msgpack_context_t initialized with a data buffer
 */
#define MSGPACK_BUFFER_USED(ctx)                                               \
  (uintptr_t)((ctx)->buffer.ptr - (ctx)->buffer.buffer)
/**
 * Return the number of data buffer bytes unused (i.e. available) by a @ref
 * msgpack_context_t
 * @param ctx @ref msgpack_context_t initialized with a data buffer
 */
#define MSGPACK_BUFFER_REMAINING(ctx)                                          \
  (uintptr_t)((ctx)->buffer.end - (ctx)->buffer.ptr)

/**
 * Return an object for the corresponding key in a dictionary
 * @see msgpack_get_dict_object for more info
 */
#define MSGPACK_DICT_GET_OBJECT(dict, key)                                     \
 ...
 */
#define MSGPACK_ARRAY_GET_OBJECT(array, index)                                 \
  msgpack_get_array_object((msgpack_object_array_t *)array, index,             \
                           MSGPACK_TYPE_ANY)

/**
 * Non-recursively iterate the key/value pairs of a dictionary or entries in
 * array
 * @see msgpack_foreach() for more info
 */
#define MSGPACK_FOREACH(dict_or_array, callback, arg)                          \
  msgpack_foreach((const msgpack_object_t *)dict_or_array, callback, arg, 0)

/**
 * Recursively iterate the key/value pairs of a dictionary or entries in array
 * @see msgpack_foreach() for more info
 */
#define MSGPACK_FOREACH_RECURSIVE(dict_or_array, callback, arg)                \
  msgpack_foreach((const msgpack_object_t *)dict_or_array, callback, arg,      \
                  UINT32_MAX)
...
enum msgpack_object_flag_enum {
  ...
  MSGPACK_OBJECT_FLAG_WAS_ALLOCATED =
      (1 << 0), //!< The object as allocated from the heap
  MSGPACK_OBJECT_FLAG_AUTO_FREE_USER =
      (1 << 1) //!< When the object is freed with @ref msgpack_free_objects(),
               //!< also free the `user context` set with @ref
               //!< msgpack_set_user_context() (if applicable)
};
 ...
 * or each entry of a @ref msgpack_object_array_t supplied to @ref
 * msgpack_foreach().
 *
 * This callback is invoked in the context of the caller of @ref
 * msgpack_foreach(). If this callback returns:
 * - 0 - then continue to the next key/value pair or entry
 * - else - abort the iteration and return the given result to the caller of
 * @ref msgpack_foreach().
 *
 * @param[in] key Dictionary key if iterating a directory, NULL if iterating an
 * array
 * @param[in] value Dictionary entry value or array entry
 * @param[in] arg Custom argument passed to @ref msgpack_foreach()
 * @return @ref 0 to keep iterating, else abort iteration and return result to
 * caller
 */
typedef int (*msgpack_iterator_t)(const msgpack_object_t *key,
                                  const msgpack_object_t *value, void *arg);

/**
 * MessagePack data writer
 *
 * When using the msgpack_write_xxx APIs this callback will be invoked to write
 * the 'packed' message data IF the @ref msgpack_context_t is initialized with a
 * writer
 *
 * @param user Optional user object specified when initializing the @ref
...
enum msgpack_flag_enum {
  ...
  MSGPACK_DESERIALIZE_WITH_PERSISTENT_STRINGS =
      (1 << 0), //!< If specified, strings within the de-serialize objects will
                //!< persist after the provided buffer is freed If NOT
                //!< specified, then strings will NOT be valid after the
                //!< provided buffer is freed
  MSGPACK_PACK_16BIT_DICTS =
      (1 << 1) //!< If specified, the dictionary length is always 16bits
               //!< If NOT specified, the dictionary length is variable based on
               //!< the provided element count
};
 ...
 *
 * @see @ref msgpack_init_with_buffer() and @ref msgpack_init_with_writer()
 * helper macros
 */
typedef struct {
  struct {
    ...
    uint8_t
        *end; ///< Address of end of buffer ( end = buffer + sizeof(buffer) )
  } buffer;
  ...
  struct {
    int32_t count; ///< Either the number of elements remaining to be written to
                   ///< the current container or the number of elements that
                   ///< have been written to a dynamically sized container
    uint32_t
        marker_offset; ///< If using a dynamically sized container, this points
                       ///< to the buffer offset of the container's marker
  } containers[MSGPACK_MAX_NESTED_CONTAINERS];
#endif
  int32_t
      container_index;  ///< The current nested container (dict or array) index
 ...
 */
static inline msgpack_context_t msgpack_init_with_buffer(uint8_t *buffer,
                                                         uint32_t length) {
  msgpack_context_t context = {
      .buffer =
          {
...
/**
 * Declare and initialize a @ref msgpack_context_t with the supplied @ref
 * msgpack_writer_t
 *
 * @param writer MessagePack data writer, see @ref msgpack_writer_t
 * @param user Optional user argument given to the data writer
 */
static inline msgpack_context_t
msgpack_init_with_writer(msgpack_writer_t writer, void *user) {
  msgpack_context_t context = {
      .buffer =
          {
              .buffer = NULL,
 ...
 *
 * @param context Pointer to previously declared/allocated @ref
 * msgpack_context_t
 * @param writer MessagePack data writer, see @ref msgpack_writer_t
 * @param user Optional user argument given to the data writer
 */
static inline void msgpack_set_writer(msgpack_context_t *context,
                                      msgpack_writer_t writer, void *user) {
  context->writer = writer;
  context->user = user;
}

/**
 * Convert a 'packed' MessagePack binary string to a linked list of @ref
 msgpack_object_t
 *
 * This will parse the binary MessagePack data into a linked list of @ref
 msgpack_object_t.
 * Each @ref msgpack_object_t is dynamically allocated.
 *
 * Use @ref msgpack_free_objects() to release the allocated memory.
 *
 * By default, this function does NOT allocate additional memory for binary
 strings and strings.
 * Thus the given `buffer` MUST persist for as long as the return objects are
 referenced.
 * If the given `buffer` is released, the memory a @ref MSGPACK_TYPE_STR or @ref
 MSGPACK_TYPE_BIN
 * object references will be invalid.
 * Use the flag @ref MSGPACK_DESERIALIZE_WITH_PERSISTENT_STRINGS to ensure that
 the object data persists after the given `buffer` is de-allocated.

 * @note The returned root object will be a @ref msgpack_object_array_t OR @ref
 msgpack_object_dict_t
 *
 * @param root_ptr Pointer to hold allocated linked list of MessagePack object
 * @param buffer Buffer containing 'packed' MessagePack data, this buffer MUST
 persist for as long as the return object are referenced IF binary strings or
 strings are used
 * @param length Length of buffer to parse
 * @param flags @ref msgpack_flag_t
 * @return 0 on success, else failure
 */
int msgpack_deserialize_with_buffer(msgpack_object_t **root_ptr,
                                    const void *buffer, uint32_t length,
                                    msgpack_flag_t flags);

/**
 * Release all @ref msgpack_object_t the supplied object references
 *
 * This should be used to release the memory that's allocated with @ref
 * msgpack_deserialize_with_buffer()
 ...
 *
 * Optionally, if `auto_free=true`, when the `obj` is cleaned up, the given
 * `user` context will also be cleaned via @ref free()
 *
 * One use-case of this is to set the user context pointer as the `buffer`
 * argument that was given to
 * @ref msgpack_deserialize_with_buffer(). This way when the root object is
 * cleaned the associated `buffer` is also automatically cleaned.
 *
 ...
 * @param auto_free If true, the `user` is automatically freed via @ref free()
 * when the `obj` is cleaned
 * @return @ref result_t of API call
 */
int msgpack_set_user_context(msgpack_object_t *obj, void *context,
                             bool auto_free);

/**
 * Retrieve user context pointer lined to object
 *
 * This returns the user context pointer that was set via @ref
 * msgpack_set_user_context()
 *
 ...
 * @param type The expected type of object to return, see @ref msgpack_type_t,
 * set to @ref MSGPACK_TYPE_ANY to return any type
 * @return The dictionary value IF found, NULL else
 */
msgpack_object_t *msgpack_get_dict_object(const msgpack_object_dict_t *dict,
                                          const char *key, msgpack_type_t type);
...
msgpack_object_t *msgpack_get_array_object(const msgpack_object_array_t *array,
                                           uint32_t index, msgpack_type_t type);

/**
 * Iterate the entries in a dictionary or array
 *
 * This invokes the supplied `iterator_callback` for each entry in the supplied
 * `dict_or_array`. See @ref msgpack_iterator_t for more details on how the
 * callback works.
 *
 * The `arg` argument will be supplied to the `iterator_callback` callback.
 *
 * If the `depth` argument is greater than zero then dictionary/array entries
 * will be recursively iterated.
 *
 *
 * @param dict_or_array A @ref msgpack_object_dict_t or @ref
 * msgpack_object_array_t object to iterate
 * @param iterator_callback @ref msgpack_iterator_t callback to be invoked for
 * each entry in the dictionary or array
 * @param arg Custom argument to pass to the `iterator_callback`
 * @param depth Recursion depth, 0 will only iterate the entries in the given
 * `dict_or_array`, greater than 0 will iterate sub array/dictionaries
 * @return 0 if all all entries iterated, else failure
 */
int msgpack_foreach(const msgpack_object_t *dict_or_array,
                    msgpack_iterator_t iterator_callback, void *arg,
 ...
 */
char *msgpack_to_str(const msgpack_object_t *obj, char *buffer,
                     uint32_t max_length);
 ...
 */
bool msgpack_object_is_type(const msgpack_object_t *object,
                            msgpack_type_t type);
 ...
 * copies a string object's string into the given buffer AND null-terminates the
 * string.
 *
 * @param object MessagPacket @ref MSGPACK_TYPE_STR object
 * @param buffer Buffer to hold string data, this buffer should contain an
 * additional byte to hold the null-terminator
 * @param max_length The maximum length of the supplied `buffer`
 * @return Pointer to populated `buffer`, this is the same pointer as the
 * supplied `buffer`
 */
char *msgpack_get_str(const msgpack_object_t *object, char *buffer,
                      uint16_t max_length);

/**
 * Compare a string object to a string
 *
 * This has the same functionality as `strcmp()` but accepts a @ref
 * msgpack_object_str_t as the first argument.
 *
 * @param object String msgpack object
 * @param str String to compare
 * @return <0 the first character that does not match has a lower value in
 * `object` than in `str` 0 the contents of both strings are equal >0 the first
 * character that does not match has a greater value in `object` than in `str`
 */
 ...
 * @return 0 on success
 */
int msgpack_write_bin(msgpack_context_t *context, const void *data,
                      uint32_t length);

/**
 * Write a previously written @ref msgpack_context_t
 *
 * This writes the contents of a previously written @ref msgpack_context_t to
 * the supplied `context`. The previously written context MUST have been
 * initialized with a buffer (not a writer)
 *
 * The @ref msgpack_context_t should have been previously initialized.
 *
 * @param context Previously initialize @ref msgpack_context_t
 ...
 * @return 0 on success
 */
int msgpack_write_context(msgpack_context_t *context,
 ...
 * The binary string value MUST be immediately written after calling this
 * function. This function is useful if the binary string needs to be written in
 * chunks.
 *
 * The @ref msgpack_context_t should have been previously initialized.
 *
 ...
 *
 * This writes the number of key/values that will go into the dictionary to the
 * given @ref msgpack_context_t. The dictionary key/values MUST be immediately
 * written after calling this function.
 *
 * The @ref msgpack_context_t should have been previously initialized.
 *
 * @note The msgpack_write_dict_xxx APIs may be used to write the dictionary
 * key/values.
 *
 * @note Set the `size` to `-1` to automatically count the number of elements.
 * In this case, @ref msgpack_finalize_dynamic() must be called once the
 ...
 * @ref msgpack_context_t. The array values MUST be immediately written after
 ...
 * @ref msgpack_context_t. The @ref msgpack_context_t should have been
 * previously initialized.
 *
 * @ref msgpack_write_dict_marker() MUST have been previously called to specify
 * the dictionary.
 *
 * @param context Previously initialize @ref msgpack_context_t
 ...
 *
 * This packs a boolean value with the corresponding dictionary key into the
 * given @ref msgpack_context_t. The @ref msgpack_context_t should have been
 * previously initialized.
 *
 * @ref msgpack_write_dict_marker() MUST have been previously called to specify
 * the dictionary.
 ...
 * @return 0 on success
 */
int msgpack_write_dict_bool(msgpack_context_t *context, const char *key,
                            bool value);

/**
 * Pack a signed integer (8-32bits) to the current dictionary
 *
 * This packs a signed integer value with the corresponding dictionary key into
 * the given @ref msgpack_context_t. The @ref msgpack_context_t should have been
 * previously initialized.
 *
 * @ref msgpack_write_dict_marker() MUST have been previously called to specify
 * the dictionary.
 ...
 * @return 0 on success
 */
int msgpack_write_dict_int(msgpack_context_t *context, const char *key,
                           int32_t value);

/**
 * Pack an unsigned integer (8-32bits) to the current dictionary
 *
 * This packs an unsigned integer value with the corresponding dictionary key
 * into the given @ref msgpack_context_t. The @ref msgpack_context_t should have
 ...
 * @return 0 on success
 */
int msgpack_write_dict_uint(msgpack_context_t *context, const char *key,
                            uint32_t value);
 ...
 * @return 0 on success
 */
int msgpack_write_dict_long(msgpack_context_t *context, const char *key,
                            int64_t value);
 ...
 * @return 0 on success
 */
int msgpack_write_dict_ulong(msgpack_context_t *context, const char *key,
                             uint64_t value);
 ...
 * @return 0 on success
 */
int msgpack_write_dict_float(msgpack_context_t *context, const char *key,
                             float value);

/**
 * Pack a double into to the current dictionary
 *
 * This packs a double value with the corresponding dictionary key into the
 * given @ref msgpack_context_t. The @ref msgpack_context_t should have been
 * previously initialized.
 *
 * @ref msgpack_write_dict_marker() MUST have been previously called to specify
 ...
 * @return 0 on success
 */
int msgpack_write_dict_double(msgpack_context_t *context, const char *key,
                              double value);

/**
 * Pack a string into to the current dictionary
 *
 * This packs a string value with the corresponding dictionary key into the
 * given @ref msgpack_context_t. The @ref msgpack_context_t should have been
 * previously initialized.
 *
 * @ref msgpack_write_dict_marker() MUST have been previously called to specify
 ...
 * @return 0 on success
 */
int msgpack_write_dict_str(msgpack_context_t *context, const char *key,
                           const char *value);

/**
 * Pack a binary string into to the current dictionary
 *
 * This packs a binary string value with the corresponding dictionary key into
 * the given @ref msgpack_context_t. The @ref msgpack_context_t should have been
 * previously initialized.
 *
 * @ref msgpack_write_dict_marker() MUST have been previously called to specify
 ...
 * @return 0 on success
 */
int msgpack_write_dict_bin(msgpack_context_t *context, const char *key,
                           const void *value, uint32_t length);

/**
 * Pack a dictionary marker into to the current dictionary
 *
 * This packs a new dictionary marker with the corresponding dictionary key into
 * the given @ref msgpack_context_t. The @ref msgpack_context_t should have been
 * previously initialized.
 *
 * @ref msgpack_write_dict_marker() MUST have been previously called to specify
 * the dictionary.
 *
 * @note Set the `dict_count` to `-1` to automatically count the number of
 * elements. In this case, @ref msgpack_finalize_dynamic() must be called once
 * the elements are written.
 *
 * @param context Previously initialize @ref msgpack_context_t
 * @param key Dictionary key of written value
 * @param dict_count Number of key/value pairs that will go in new dictionary
 * within the current dictionary
 * @return 0 on success
 */
int msgpack_write_dict_dict(msgpack_context_t *context, const char *key,
                            int32_t dict_count);

/**
 * Pack a array marker into to the current dictionary
 *
 * This packs a new array marker with the corresponding dictionary key into the
 * given @ref msgpack_context_t. The @ref msgpack_context_t should have been
 * previously initialized.
 *
 * @ref msgpack_write_dict_marker() MUST have been previously called to specify
 * the dictionary.
 *
 * @note Set the `array_count` to `-1` to automatically count the number of
 * elements. In this case, @ref msgpack_finalize_dynamic() must be called once
 * the elements are written.
 *
 * @param context Previously initialize @ref msgpack_context_t
 * @param key Dictionary key of written value
 * @param array_count Number of entries that will go in new array within the
 * current dictionary
 * @return 0 on success
 */
int msgpack_write_dict_array(msgpack_context_t *context, const char *key,
                             int32_t array_count);

/**
 * Pack a previous written context into to the current dictionary
 *
 * This packs a previously written @ref msgpack_context_t with the corresponding
 * dictionary key into the given @ref msgpack_context_t. The @ref
 * msgpack_context_t should have been previously initialized. The previously
 * written context MUST have been initialized with a buffer (not a writer)
 *
 * @ref msgpack_write_dict_marker() MUST have been previously called to specify
 * the dictionary.
 *
 * @param context Previously initialize @ref msgpack_context_t
 * @param key Dictionary key of written value
 * @param value_context Previously written context to write to supplied
 * `context`
 * @return 0 on success
 */
int msgpack_write_dict_context(msgpack_context_t *context, const char *key,
                               const msgpack_context_t *value_context);

/**
 * Finalize a dynamic dict or array
 *
 * When calling @ref msgpack_write_dict_marker(), @ref
 * msgpack_write_dict_array_marker(),
 * @ref msgpack_write_dict_dict(), or @ref msgpack_write_dict_array() with the
 * "count" argument set to `-1` then the number of elements written will be
 * automatically counted as they're written. Once all of the desired elements
 * are written, this API should be called to finalize th actual count.
 *
 * @note This feature does have an overhead in that the element count always
 * uses 16-bits. There is also an upper limit of 65535 elements that may be
 * stored.
 *
 * @param context Previously initialize @ref msgpack_context_t
 * @return 0 on success
 ...
 * While writing data, the internal buffer will automatically be increased as
 * necessary.
 *
 * Once all the data is populated, use @ref msgpack_buffered_writer_get_buffer()
 * to retrieve the populated buffer.
 *
 * Use @ref msgpack_buffered_writer_deinit() to cleanup the allocated @ref
 * msgpack_context_t.
 *
 * @param[out] context_ptr Pointer to hold allocated @ref msgpack_context_t
 * @param[in] initial_length Initial length of internal buffer, this will
 * increase as needed
 * @return 0 on success
 */
int msgpack_buffered_writer_init(msgpack_context_t **context_ptr,
 ...
 * then @ref msgpack_buffered_writer_get_buffer() should FIRST be used to
 * retrieve a reference to the internal buffer.
 *
 * Then at a later point, @ref free() should be used to de-allocate the returned
 * `buffer.data`.
 *
 * @param[in] context Buffered msgpack writer context to de-allocated
 * @param[in] free_buffer If true also de-allocate internal buffer, if false
 * internal buffer is NOT de-allocated.
 * @return 0 on success
 */
int msgpack_buffered_writer_deinit(msgpack_context_t *context,
                                   bool free_buffer);
 ...
 */
int msgpack_buffered_writer_get_buffer(const msgpack_context_t *context,
                                       uint8_t **buffer_ptr,
                                       uint32_t *length_ptr);
