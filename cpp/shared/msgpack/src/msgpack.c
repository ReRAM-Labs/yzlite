...
int msgpack_remove_dict_object(msgpack_object_dict_t *dict_obj, void *obj) {
  ...
  for (int i = 0; i < dict_obj->count; ++i) {
    ...
    if ((entry->key != NULL) &&
        (entry->key->flags & MSGPACK_OBJECT_FLAG_WAS_ALLOCATED)) {
      free((void *)entry->key);
    }
...
void msgpack_free_objects(void *obj) {
  ...
  if (object != NULL) {
    ...
    if (is_dict_or_array &&
        (object->flags & MSGPACK_OBJECT_FLAG_AUTO_FREE_USER)) {
...
msgpack_object_t *msgpack_get_array_object(const msgpack_object_array_t *array,
  ...
  if (array == NULL || !MSGPACK_IS_ARRAY(array)) {
    ...
    return msgpack_object_is_type(entry, type) ? (msgpack_object_t *)entry
                                               : NULL;
  }
}
  ...
  if (dict_or_array == NULL) {
    ...
    for (int i = 0; i < dict->count; ++i) {
      ...
      if (CHECK_FAILURE(result,
                        iterator_callback(entry->key, entry->value, arg))) {
        break;
      }
      ...
      if ((recursive_depth > 0) &&
        ...
        if (CHECK_FAILURE(result,
                          msgpack_foreach(entry->value, iterator_callback, arg,
                                          recursive_depth - 1))) {
          break;
        }
      }
    ...
    const msgpack_object_array_t *array_obj =
        (const msgpack_object_array_t *)dict_or_array;
    ...
    for (uint32_t i = 0; i < MSGPACK_ARRAY_LENGTH(array_obj); ++i) {
      ...
      if ((recursive_depth > 0) &&
        ...
        if (CHECK_FAILURE(result,
                          msgpack_foreach(array_entry, iterator_callback, arg,
                                          recursive_depth - 1))) {
          break;
        }
      }
...
char *msgpack_to_str(const msgpack_object_t *obj, char *buffer,
  ...
  if (MSGPACK_IS_DICT(obj)) {
    ...
    snprintf(buffer, max_length, "<dict: %u entries>",
             (unsigned int)dict->count);
  } else if (MSGPACK_IS_ARRAY(obj)) {
    const msgpack_object_array_t *array_obj =
        (const msgpack_object_array_t *)obj;
    snprintf(buffer, max_length, "<array: %u entries>",
             (unsigned int)array_obj->count);
  } else if (MSGPACK_IS_STR(obj)) {
    MSGPACK_STR(obj, buffer, max_length);
  } else if (MSGPACK_IS_BIN(obj)) {
    str_binary_to_hex_buffer(buffer, max_length, MSGPACK_BIN_VALUE(obj),
                             MSGPACK_BIN_LENGTH(obj));
...
bool msgpack_object_is_type(const msgpack_object_t *object,
  ...
  if (object == NULL) {
    ...
    const msgpack_type_t obj_type = object->type;
    retval =
        (obj_type == MSGPACK_TYPE_UINT8 || obj_type == MSGPACK_TYPE_UINT16 ||
...
uint64_t *msgpack_get_ulong(const msgpack_object_t *object, uint64_t *buffer) {
  ...
  if (type == MSGPACK_TYPE_STR) {
    ...
    *buffer = str_to_uint64(str_buffer);
  } else if (type == MSGPACK_TYPE_INT64 || type == MSGPACK_TYPE_UINT64 ||
    ...
    const uint8_t sign_extend =
        ((val & 0x80000000UL) &&
         ((type == MSGPACK_TYPE_INT8) || (type == MSGPACK_TYPE_INT16) ||
          (type == MSGPACK_TYPE_INT32)))
            ? 0xFF
            : 0x00;
...
static msgpack_user_context_t *get_user_context(const msgpack_object_t *obj) {
  ...
  if (obj->type == MSGPACK_TYPE_DICT) {
    ...
    retval = (msgpack_user_context_t *)((uint8_t *)dict_obj +
                                        sizeof(msgpack_object_dict_t) +
                                        sizeof(msgpack_dict_entry_t) *
                                            dict_obj->count);
  } else if (obj->type == MSGPACK_TYPE_ARRAY) {
    const msgpack_object_array_t *array_obj = (msgpack_object_array_t *)obj;
    retval =
        (msgpack_user_context_t *)((uint8_t *)array_obj +
                                   sizeof(msgpack_object_array_t) +
                                   sizeof(msgpack_object_t) * array_obj->count);
  }
