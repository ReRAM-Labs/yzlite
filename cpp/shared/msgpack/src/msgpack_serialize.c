...
static int write_marker_and_bytes(msgpack_context_t *context,
                                  msgpack_marker_t marker,
                                  const uint8_t *buffer, uint32_t length);
static int write_bytes(msgpack_context_t *context, const uint8_t *buffer,
                       uint32_t length);
...
int msgpack_write_int(msgpack_context_t *context, int32_t value) {
  ...
  return write_marker_and_bytes(context, marker, (const uint8_t *)&value,
                                length);
}
...
int msgpack_write_long(msgpack_context_t *context, int64_t value) {
  ...
  if (value >= 0) {
    ...
    result = write_marker_and_bytes(context, S64_MARKER,
                                    (const uint8_t *)&value, sizeof(uint64_t));
  }
...
int msgpack_write_ulong(msgpack_context_t *context, uint64_t value) {
  ...
  if (value <= UINT32_MAX) {
    ...
    result = write_marker_and_bytes(context, U64_MARKER,
                                    (const uint8_t *)&value, sizeof(uint64_t));
  }
...
int msgpack_write_float(msgpack_context_t *context, float value) {
  return write_marker_and_bytes(context, FLOAT_MARKER, (const uint8_t *)&value,
                                sizeof(uint32_t));
}

/*************************************************************************************************/
int msgpack_write_double(msgpack_context_t *context, double value) {
  return write_marker_and_bytes(context, DOUBLE_MARKER, (const uint8_t *)&value,
                                sizeof(uint64_t));
}
...
int msgpack_write_str_marker(msgpack_context_t *context, uint32_t size) {
  if (size <= FIXSTR_SIZE) {
    ...
    const uint8_t marker = (size <= UINT8_MAX)    ? STR8_MARKER
                           : (size <= UINT16_MAX) ? STR16_MARKER
                                                  : STR32_MARKER;
    const uint8_t length = (size <= UINT8_MAX)    ? sizeof(uint8_t)
                           : (size <= UINT16_MAX) ? sizeof(uint16_t)
                                                  : sizeof(uint32_t);
    return write_marker_and_bytes(context, marker, (const uint8_t *)&size,
                                  length);
  }
}
...
int msgpack_write_str(msgpack_context_t *context, const char *str) {
  ...
  } else if (CHECK_FAILURE(result,
                           write_bytes(context, (const uint8_t *)str, len))) {
  }
  return result;
}
...
int msgpack_write_bin(msgpack_context_t *context, const void *data,
  ...
  } else if (CHECK_FAILURE(
                 result, write_bytes(context, (const uint8_t *)data, length))) {
  }
  return result;
}
...
int msgpack_write_context(msgpack_context_t *context,
  ...
  if (value_context->buffer.ptr == NULL ||
      value_context->buffer.buffer == NULL) {
    return -1;
  }

  return write_bytes(context, value_context->buffer.buffer,
                     MSGPACK_BUFFER_USED(value_context));
}
...
int msgpack_write_dict_marker(msgpack_context_t *context, int32_t size) {
  ...
  if ((size >= 0) && !(context->flags & MSGPACK_PACK_16BIT_DICTS) &&
      (size <= FIXMAP_SIZE)) {
    ...
    const uint8_t marker =
        (normalized_sized <= UINT16_MAX) ? MAP16_MARKER : MAP32_MARKER;
    const uint8_t length =
        (normalized_sized <= UINT16_MAX) ? sizeof(uint16_t) : sizeof(uint32_t);

    RETURN_ON_FAILURE(write_marker_and_bytes(context, marker,
                                             (const uint8_t *)&size, length));
    RETURN_ON_FAILURE(push_container(context, size * 2));
    return 0;
  }
}
...
int msgpack_write_array_marker(msgpack_context_t *context, int32_t size) {
  ...
  if ((size >= 0) && size <= FIXARRAY_SIZE) {
    ...
    const int32_t normalized_sized = (size < 0) ? UINT16_MAX : size;
    const uint8_t marker =
        (normalized_sized <= UINT16_MAX) ? ARRAY16_MARKER : ARRAY32_MARKER;
    const uint8_t length =
        (normalized_sized <= UINT16_MAX) ? sizeof(uint16_t) : sizeof(uint32_t);
...
int msgpack_write_dict_dict(msgpack_context_t *context, const char *key,
  ...
  if (CHECK_FAILURE(result, msgpack_write_str(context, key))) {
                           ...
                           msgpack_write_dict_marker(context, dict_count))) {
  }
...
int msgpack_write_dict_array(msgpack_context_t *context, const char *key,
  ...
  if (CHECK_FAILURE(result, msgpack_write_str(context, key))) {
                           ...
                           msgpack_write_array_marker(context, array_count))) {
  }
...
int msgpack_write_dict_context(msgpack_context_t *context, const char *key,
  ...
  if (CHECK_FAILURE(result, msgpack_write_str(context, key))) {
                           ...
                           msgpack_write_context(context, value_context))) {
  }
...
int msgpack_finalize_dynamic(msgpack_context_t *context) {
  ...
  if (context == NULL || context->container_index < 0 ||
      context->containers[context->container_index].marker_offset ==
          UINT32_MAX ||
...
static int push_container(msgpack_context_t *context, int32_t count) {
  ...
  if (count < 0) {
    if (context->buffer.ptr != NULL && context->buffer.buffer != NULL) {
      context->containers[context->container_index].marker_offset =
          (uintptr_t)(context->buffer.ptr - context->buffer.buffer) - 3;
    } else {
      return -1;
    }
  } else {
...
static int write_marker(msgpack_context_t *context, msgpack_marker_t marker) {
  ...
  if (context->container_index >= 0) {
    if (context->containers[context->container_index].marker_offset ==
        UINT32_MAX) {
