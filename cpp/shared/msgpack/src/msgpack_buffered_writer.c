...
static int buffered_msgpack_writer(void *user, const void *data,
                                   uint32_t length);
...
int msgpack_buffered_writer_init(msgpack_context_t **context_ptr,
  ...
  *context_ptr = NULL;
  ...
  if (CHECK_FAILURE(result, dynamic_buffer_alloc(&context->dynamic_buffer,
                                                 initial_length))) {
  } else {
    context->msgpack =
        msgpack_init_with_writer(buffered_msgpack_writer, context);
  ...
  *length_ptr = 0;
  ...
  if (buf_context == NULL || buf_context->dynamic_buffer.buffer == NULL) {
    result = -1;
                 ...
                 result, dynamic_buffer_get_buffer(&buf_context->dynamic_buffer,
...
static int buffered_msgpack_writer(void *user, const void *data,
  ...
  RETURN_ON_FAILURE(
      dynamic_buffer_write(&context->dynamic_buffer, data, length));
