...
static int deserialize_dict(msgpack_context_t *context,
                            msgpack_object_dict_t *dict);
static int deserialize_array(msgpack_context_t *context,
                             msgpack_object_array_t *array);
static int deserialize_next_object(msgpack_context_t *context,
                                   msgpack_object_t **obj_ptr);
static int read_and_convert_endian(msgpack_context_t *context, uint8_t *data,
                                   uint32_t length);
static int read_bytes(msgpack_context_t *context, uint8_t *data,
                      uint32_t length);
...
int msgpack_deserialize_with_buffer(msgpack_object_t **root_ptr,
  ...
  msgpack_context_t context =
      msgpack_init_with_buffer((uint8_t *)buffer, length);
  context.flags = flags;
  ...
  for (int i = 0; i < dict->count; ++i) {
    ...
    if (CHECK_FAILURE(result, deserialize_next_object(context, &key_obj))) {
      break;
                             ...
                             deserialize_next_object(context, &value_obj))) {
      break;
    } else {
      ...
      if (MSGPACK_IS_DICT(tmp) &&
          CHECK_FAILURE(result, deserialize_dict(
                                    context, (msgpack_object_dict_t *)tmp))) {
        break;
      } else if (MSGPACK_IS_ARRAY(tmp) &&
                 CHECK_FAILURE(result,
                               deserialize_array(
                                   context, (msgpack_object_array_t *)tmp))) {
        break;
      }
    }
  }
...
static int deserialize_array(msgpack_context_t *context,
  ...
  for (int i = 0; i < array->count; ++i) {
    ...
    if (MSGPACK_IS_DICT(tmp) &&
        CHECK_FAILURE(
            result, deserialize_dict(context, (msgpack_object_dict_t *)tmp))) {
      break;
    } else if (MSGPACK_IS_ARRAY(tmp) &&
               CHECK_FAILURE(
                   result,
                   deserialize_array(context, (msgpack_object_array_t *)tmp))) {
      break;
    }
  }
...
static int deserialize_next_object(msgpack_context_t *context,
  ...
  RETURN_ON_FAILURE(
      read_bytes(context, &type_marker, sizeof(msgpack_marker_t)));
  ...
  // --------------------------
    ...
    msgpack_object8_t *obj = (msgpack_object8_t *)temp_obj_buffer;
    obj->obj.type =
        (type_marker <= 0x7F) ? MSGPACK_TYPE_UINT8 : MSGPACK_TYPE_INT8;
  ...
  // 8bit values
  // --------------------------
  else if (type_marker == U8_MARKER || type_marker == S8_MARKER) {
    msgpack_object8_t *obj = (msgpack_object8_t *)temp_obj_buffer;
    RETURN_ON_FAILURE(read_and_convert_endian(context, (uint8_t *)&obj->data.u,
                                              sizeof(uint8_t)));
    obj->obj.type =
        (type_marker == U8_MARKER) ? MSGPACK_TYPE_UINT8 : MSGPACK_TYPE_INT8;
    obj_size = sizeof(msgpack_object8_t);
  }
  // --------------------------
  // 16bit values
  // --------------------------
  else if (type_marker == U16_MARKER || type_marker == S16_MARKER) {
    msgpack_object16_t *obj = (msgpack_object16_t *)temp_obj_buffer;
    RETURN_ON_FAILURE(read_and_convert_endian(context, (uint8_t *)&obj->data.u,
                                              sizeof(uint16_t)));
    obj->obj.type =
        (type_marker == U16_MARKER) ? MSGPACK_TYPE_UINT16 : MSGPACK_TYPE_INT16;
    obj_size = sizeof(msgpack_object16_t);
  }
  // --------------------------
  // 32bit values
  // --------------------------
  else if (type_marker == FLOAT_MARKER || type_marker == U32_MARKER ||
    ...
    obj->obj.type = (type_marker == FLOAT_MARKER) ? MSGPACK_TYPE_FLOAT
                    : (type_marker == U32_MARKER) ? MSGPACK_TYPE_UINT32
                                                  : MSGPACK_TYPE_INT32;
  ...
  // 64bit values
  // --------------------------
  else if (type_marker == DOUBLE_MARKER || type_marker == U64_MARKER ||
    ...
    obj->obj.type = (type_marker == DOUBLE_MARKER) ? MSGPACK_TYPE_DOUBLE
                    : (type_marker == U64_MARKER)  ? MSGPACK_TYPE_UINT64
                                                   : MSGPACK_TYPE_INT64;
  ...
  // BIN (binary string)
  // --------------------------
  else if (type_marker == BIN8_MARKER || type_marker == BIN16_MARKER ||
           type_marker == BIN32_MARKER) {
    const uint8_t read_len = (type_marker == BIN8_MARKER)    ? sizeof(uint8_t)
                             : (type_marker == BIN16_MARKER) ? sizeof(uint16_t)
                                                             : sizeof(uint32_t);

    msgpack_object_bin_t *obj = (msgpack_object_bin_t *)temp_obj_buffer;
    RETURN_ON_FAILURE(
        read_and_convert_endian(context, (uint8_t *)&obj->length, read_len));
  ...
  // DICT
  // --------------------------
  else if (type_marker <= 0x8F || type_marker == MAP16_MARKER ||
    ...
    if (type_marker <= 0x8F) {
      ...
      const uint8_t read_len =
          (type_marker == MAP16_MARKER) ? sizeof(uint16_t) : sizeof(uint32_t);
      RETURN_ON_FAILURE(
          read_and_convert_endian(context, (uint8_t *)&obj->count, read_len));
    }

    is_dict_or_array = true;
    obj->obj.type = MSGPACK_TYPE_DICT;
    obj_size = sizeof(msgpack_object_dict_t) +
               sizeof(msgpack_dict_entry_t) * obj->count;
  }
  // --------------------------
  // ARRAY
  // --------------------------
  else if (type_marker <= 0x9F || type_marker == ARRAY16_MARKER ||
    ...
    if (type_marker <= 0x9F) {
      ...
      const uint8_t read_len =
          (type_marker == ARRAY16_MARKER) ? sizeof(uint16_t) : sizeof(uint32_t);
      RETURN_ON_FAILURE(
          read_and_convert_endian(context, (uint8_t *)&obj->count, read_len));
    }
    ...
    obj_size = sizeof(msgpack_object_array_t) +
               sizeof(msgpack_object_t *) * obj->count;
  }
  // --------------------------
  // STR (UTF8 string)
  // --------------------------
  else if (type_marker <= 0xBF || type_marker == STR8_MARKER ||
           type_marker == STR16_MARKER || type_marker == STR32_MARKER) {
  ...
  // If this is a dictionary or array then allocate space for user context
  // pointer
  if (is_dict_or_array) {
