...
static bool start_next_rx_dma_transfer(unsigned int unused = 0,
                                       unsigned int unused2 = 0,
                                       void *unused3 = nullptr);
static bool start_next_tx_dma_transfer(unsigned int unused = 0,
                                       unsigned int unused2 = 0,
                                       void *unused3 = nullptr);
...
/*************************************************************************************************/
bool initialize(uint8_t *rx_buffer, uint32_t rx_buffer_length,
                uint32_t baud_rate) {
...
bool is_synchronized() {
  ...
  CORE_ATOMIC_SECTION(synced = !context.sync_requested &&
                               context.tx.bytes_available >= 0;);
...
int32_t get_rx_bytes_available() {
  ...
  CORE_ATOMIC_SECTION(
      bytes_available =
          (!context.sync_requested && context.tx.bytes_available >= 0)
              ? context.rx.bytes_available - context.rx.dma_bytes_remaining
              : -1;);
...
int32_t get_tx_bytes_available() {
  ...
  CORE_ATOMIC_SECTION(bytes_available =
                          context.sync_requested ? -1
                          : ((context.tx.active_data_packet_id > 0) ||
                             (context.tx.dma_bytes_remaining > 0))
                              ? 0
...
bool synchronize() {
  ...
  CORE_ATOMIC_SECTION(sync_requested = context.sync_requested;
...
int32_t write(const void *data, int32_t max_length, bool block) {
  ...
  CORE_ATOMIC_SECTION(context.tx.dma_bytes_remaining = tx_length;
                      context.tx.dma_ptr = (const uint8_t *)data;);

  uint8_t tx_packet_id = write_data_packet_header(tx_length, false);
  CORE_ATOMIC_SECTION(context.tx.active_data_packet_id = tx_packet_id;);
...
int32_t read(void *data, int32_t max_length) {
  ...
  if (rx_length > 0) {
    ...
    const uintptr_t length_to_end =
        (uintptr_t)(context.rx.end - context.rx.head);
    ...
    CORE_ATOMIC_SECTION(assert(context.rx.bytes_available >= rx_length);
...
int32_t read_direct(uint8_t **data_ptr, int32_t max_length) {
  ...
  const int32_t rx_length = std::min(std::min(max_length, rx_bytes_available),
                                     (int32_t)length_to_end);
...
extern "C" void uart_stream_internal_rx_irq_callback() {
  ...
  if (!(header.id != 0 &&
        std::memcmp(header.delimiter2, PACKET_DELIMITER2,
                    sizeof(PACKET_DELIMITER2)) == 0 &&
        std::memcmp(header.delimiter1, PACKET_DELIMITER1,
                    sizeof(PACKET_DELIMITER1)) == 0)) {
    std::memmove(context.rx.header_buffer, &context.rx.header_buffer[1],
  ...
  if (header.id < 0) {
    ...
    return;
  } else if (!(header.data.rx_buffer_available >= 0 &&
               header.data.packet_length >= -1)) {
    return;
  }
...
/*************************************************************************************************/
static bool start_next_rx_dma_transfer(unsigned int channel,
  ...
  uint32_t chunk_length =
      std::min(std::min((uint32_t)length_to_end,
                        (uint32_t)context.rx.dma_bytes_remaining),
               (uint32_t)MAX_DMA_LENGTH);
  if (chunk_length == 0) {
  ...
  LDMA_Descriptor_t desc = LDMA_DESCRIPTOR_SINGLE_P2M_BYTE(
      context.dma_config.rx_address, context.rx.tail, chunk_length);
  desc.xfer.doneIfs = 1;
  context.rx.dma_chunk_length = chunk_length;

  uart_stream_internal_set_irq_enabled(false);
  DMADRV_LdmaStartTransfer(context.rx.dma_channel, &context.dma_config.rx_cfg,
                           &desc, start_next_rx_dma_transfer, nullptr);

  return true;
}

/*************************************************************************************************/
static bool start_next_tx_dma_transfer(unsigned int unused,
                                       unsigned int unused2, void *unused3) {
  ...
  const uint32_t chunk_length = std::min(
      (uint32_t)MAX_DMA_LENGTH, (uint32_t)context.tx.dma_bytes_remaining);
  LDMA_Descriptor_t desc = LDMA_DESCRIPTOR_SINGLE_M2P_BYTE(
      context.tx.dma_ptr, context.dma_config.tx_address, chunk_length);
  desc.xfer.doneIfs = 1;
  context.tx.dma_chunk_length = chunk_length;

  DMADRV_LdmaStartTransfer(context.tx.dma_channel, &context.dma_config.tx_cfg,
                           &desc, start_next_tx_dma_transfer, nullptr);
...
static int8_t write_data_packet_header(int32_t tx_length, bool block) {
  ...
  CORE_ATOMIC_SECTION(unused_rx_buffer_length =
                          context.rx.buffer_length - context.rx.bytes_available;
                      packet_id = context.tx.next_packet_id++;
                      ack_id = context.rx.previous_data_packet_id;
                      context.tx.next_packet_id =
                          std::max(context.tx.next_packet_id, (int8_t)1);
                      context.tx.send_ack_after_transfer = false;);

  const PacketHeader header(packet_id, ack_id, tx_length,
                            unused_rx_buffer_length);
  if (block) {
    wait_for_tx_dma_to_complete();
  }
