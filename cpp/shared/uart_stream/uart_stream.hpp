 ...
 * - Data flow control
 * - Python library (see yzlite.utils.uart_stream)
...
/**
 * Return the amount of data that can be immediately read using the @ref read()
 * API.
 *
...
/**
 * Return the amount of data that can be immediately written using the @ref
 * write() API.
 *
 ...
 *  In this case, @ref get_tx_bytes_available() returns 0 until the DMA is
 * complete.
 ...
 * @param max_length Maximum amount of data to read. If -1 then read @ref
 * get_rx_bytes_available() bytes of data.
 ...
 * Only update to @ref get_rx_bytes_available() are guaranteed to be available
 * after the returned pointer. The returned number of bytes is also guaranteed
 * to be contiguous.
 *
 * @param data_ptr Buffer to hold pointer into rx_buffer
 * @param max_length Maximum amount of data to be directly read. If -1 then
 * attempt to read @ref get_rx_bytes_available() bytes of data.
 * @return The number of contiguous bytes read (i.e. immediately available
 * starting at the returned data_ptr). -1 if the link is not synchronized.
 */
 ...
 *
 * @param max_length Maximum amount of data to be dropped. If -1 then drop @ref
 * get_rx_bytes_available() bytes of data.
 ...
 *
 * A "command" consists of an unsigned, 8-bit code and an optional, 6-byte
 * payload. While the command is guaranteed to be sent the other side. Reception
 * at the other side's application-level is not guaranteed.
 *
 ...
 */
bool write_cmd(uint8_t code,
               const uint8_t payload[COMMAND_PAYLOAD_LENGTH] = nullptr);

/**
 * Read a command to the other side
 *
 * A "command" consists of an unsigned, 8-bit code and an optional, 6-byte
 * payload. If the other side sends commands, then the application should
 * periodically call this API to receive the command.
 *
