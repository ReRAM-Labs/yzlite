...
runtime_shape_to_vector(const tflite::RuntimeShape &shape) {
...
py::array tflite_tensor_to_array(const tflite::RuntimeShape &shape,
                                 dtype *data) {
  if (data == nullptr) {
    return py::none();
  }
  ...
  return py::array(py::buffer_info((void *)data, element_size,
...
static inline py::array tflite_tensor_to_array(const TfliteTensorView &tensor) {
  ...
  return py::array(
      py::buffer_info(tensor.data.raw, element_size,
                      tflite_type_to_format_descriptor(tensor.type),
                      shape.length, dims, strides),
      dummy);
}

/*************************************************************************************************/
template <typename dtype> py::array create_1d_array(int length, dtype *data) {
  ...
  return py::array(py::buffer_info((void *)data, sizeof(dtype),
                                   py::format_descriptor<dtype>::format(), 1,
                                   dims, strides),
                   dummy);
}
