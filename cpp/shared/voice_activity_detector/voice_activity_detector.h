 ...
 *
 * @param context The pointer to a buffer of at least @ref
 * voice_activity_detector_context_size() bytes
 *
 ...
 */
int voice_activity_detector_process_frame(VoiceActivityContext context,
 ...
 * and count the number of 20ms frames that contain VAD (see @ref
 * voice_activity_detector_process_frame()). If the number of VAD frames /
 * processed frames > threshold then voice activity is considered to be in the
 * given audio.
 *
 * This may be called multiple times, but any remaining audio that is not
 * aligned to 20ms will be dropped.
 *
 ...
 * @param threshold Ratio of VAD frames to non-VAD frames in previous
 * processing_length_ms of audio for data to be considered to contain voice
 * activity. A larger value means there should be more activity in audio for a
 * detection to triggered
 * @param processing_length_ms The audio length, in milliseconds, to process for
 * activity. This has an upper limit of 64 * 0.020 = 1280 milliseconds
 * @return  1 -> voice activity detected, 0 -> no voice activity detected, -1 ->
 * less than processing_length_ms of audio processed
 */
int voice_activity_detector_process(VoiceActivityContext context,
                                    const int16_t *audio, int length,
                                    float threshold, int processing_length_ms);
