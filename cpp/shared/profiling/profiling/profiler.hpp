...
typedef void(CustomStatsPrinter)(Profiler &profiler, logging::Logger &logger,
...
public:
  ...
  constexpr const cpputils::TypedList<Profiler *> &children() const {
    return _children;
  }
  ...
  virtual void update_stats(bool stop, uint32_t stop_cpu_cycle,
...
public:
  ...
  void update_stats(bool stop, uint32_t stop_cpu_cycle,
                    uint32_t stop_time_us) override;

  uint64_t total_time_us = 0;
  uint64_t total_cpu_cycles = 0;
  uint32_t total_count = 0;

  AveragedProfiler(void *object_buffer, const char *name)
      : Profiler(object_buffer, name) {}
