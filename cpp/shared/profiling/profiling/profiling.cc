...
static void print_profiler_statistics(Profiler *profiler,
                                      logging::Logger &logger, int level,
                                      char *level_str, bool pretty_print);
static void print_profiler_metric(Profiler *profiler, logging::Logger &logger,
                                  int level, char *level_str,
                                  bool pretty_print);
...
/*************************************************************************************************/
void set_profiler_list(ProfilerList *list) { _profilers = list; }

/*************************************************************************************************/
#ifndef YZLITE_DLL_IMPORT
...
/*************************************************************************************************/
template <typename T>
bool register_profiler_internal(const char *name, T *&profiler,
                                Profiler *parent) {
  ...
  if (profiling::get(fullname.value) != nullptr) {
    PROFILER_WARN("Profiler: %s already registered, can't register again",
                  fullname.value);
    return false;
  }
...
/*************************************************************************************************/
bool register_profiler(const char *name, Profiler *&profiler,
                       Profiler *parent) {
  return register_profiler_internal(name, profiler, parent);
}

/*************************************************************************************************/
bool register_profiler(const char *name, AveragedProfiler *&profiler,
                       Profiler *parent) {
  return register_profiler_internal(name, profiler, parent);
}
...
bool unregister(const Profiler *profiler, bool unregister_children) {
  ...
  return (profiler == nullptr)
             ? false
             : unregister(profiler->fullname(fullname), unregister_children);
}
...
/*************************************************************************************************/
bool exists(const char *name) { return (profiling::get(name) != nullptr); }
...
/*************************************************************************************************/
void print_metrics(const char *name, logging::Logger *logger,
                   bool pretty_print) {
  print_metrics(profiling::get(name), logger, pretty_print);
}

/*************************************************************************************************/
void print_metrics(const Profiler *profiler, logging::Logger *optional_logger,
                   bool pretty_print) {
  if (profiler == nullptr) {
    return;
  }
  ...
  logger.info("CPU clock: %sHz",
              cpputils::format_units(CMU_ClockFreqGet(cmuClock_CORE), 1));

  print_profiler_metric(const_cast<Profiler *>(profiler), logger, 0, level_str,
                        pretty_print);
...
/*************************************************************************************************/
void print_stats(const Profiler *profiler, logging::Logger *optional_logger,
                 bool pretty_print) {
  if (profiler == nullptr) {
    return;
  }
  ...
  print_profiler_statistics(const_cast<Profiler *>(profiler), logger, 0,
                            level_str, pretty_print);
...
/*************************************************************************************************/
uint32_t calculate_total_children_cpu_cycles(const Profiler *profiler,
                                             bool is_root) {
  uint32_t total_cycles = 0;

  if (!is_root &&
      !profiler->flags().isSet(Flag::ExcludeFromTotalChildrenCyclesReport)) {
...
/*************************************************************************************************/
uint32_t calculate_total_children_accelerator_cycles(const Profiler *profiler,
                                                     bool is_root) {
  uint32_t total_cycles = 0;
...
static void print_profiler_statistics(Profiler *profiler,
  ...
  const auto print_profiler_name_if_necessary = [logger, profiler,
                                                 &printed_name, level_str]() {
    if (!printed_name) {
      printed_name = true;
  ...
  if (stats.time_us > 0) {
    ...
    logger.info("%s  %9s", level_str,
                format_microseconds_to_milliseconds(stats.time_us));
  }

  if (stats.cpu_cycles != 0) {
    print_profiler_name_if_necessary();
    logger.info(
        "%s %8s CPU cycles%s", level_str, format_units(stats.cpu_cycles, 2),
        flags.isSet(Flag::ReportsFreeRunningCpuCycles) ? " (free running)"
        : flags.isSet(Flag::ExcludeFromTotalChildrenCyclesReport)
            ? " (excluded from child sum)"
            : "");
  }

  if (flags.isSet(Flag::ReportTotalChildrenCycles)) {
    const auto children_cpu_cycles =
        calculate_total_children_cpu_cycles(profiler);
    if (children_cpu_cycles > 0) {
      print_profiler_name_if_necessary();
      logger.info("%s %8s CPU cycles (sum of all child profilers)", level_str,
                  format_units(children_cpu_cycles, 2));
    }

    const auto children_accelerator_cycles =
        calculate_total_children_accelerator_cycles(profiler);
    if (children_accelerator_cycles > 0) {
      print_profiler_name_if_necessary();
      logger.info("%s %8s Accelerator cycles (sum of all child profilers)",
                  level_str, format_units(children_accelerator_cycles, 2));
    }
  }
  ...
  if (!flags.isSet(Flag::ExcludeStatsFromReport)) {
    if (stats.accelerator_cycles != 0) {
      print_profiler_name_if_necessary();
      logger.info("%s %8s Accelerator cycles", level_str,
                  format_units(stats.accelerator_cycles, 2));
    }

    if (stats.time_us > 0) {
      if (metrics.ops != 0) {
        print_profiler_name_if_necessary();
        logger.info("%s %8s OP/s", level_str,
                    format_rate(metrics.ops, stats.time_us));
      }

      if (metrics.macs != 0) {
        print_profiler_name_if_necessary();
        logger.info("%s %8s MAC/s", level_str,
                    format_rate(metrics.macs, stats.time_us));
      }
    }

    for (auto item_it = profiler->custom_stats.items();
         item_it != profiler->custom_stats.enditems(); ++item_it) {
      print_profiler_name_if_necessary();
  ...
  if (profiler->_custom_stats_printer != nullptr) {
    print_profiler_name_if_necessary();
    profiler->_custom_stats_printer(const_cast<Profiler &>(*profiler), logger,
                                    level_str);
  }
  ...
  for (auto child : profiler->_children) {
    ...
    print_profiler_statistics(child, logger, level + 1, level_str,
                              pretty_print);
  }
}
...
static logging::Logger &get_logger(logging::Logger *logger) {
  if (logger == nullptr) {
    logger = logging::get("YZLiteProfiler");
    if (logger == nullptr) {
      logger = logging::create("YZLiteProfiler");
    }
  }
