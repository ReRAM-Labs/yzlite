...
/*************************************************************************************************/
cpputils::LinkedListItem *Profiler::next() { return _linked_list_next; }
...
/*************************************************************************************************/
const char *Profiler::name() const { return _name; }
...
/*************************************************************************************************/
void Profiler::flags(Flags flags) { _flags = flags; }
...
/*************************************************************************************************/
const char *Profiler::msg() const { return (_msg == nullptr) ? "" : _msg; }
...
/*************************************************************************************************/
void Profiler::get_child_metrics(const Profiler *profiler,
 ...
 */
void Profiler::update_stats(bool stop, uint32_t stop_cpu_cycle,
  ...
  if (stop) {
    ...
    // OR the time is measured as the accumlation between start/pause and
    // start/stop
...
/*************************************************************************************************/
void AveragedProfiler::update_stats(bool stop, uint32_t stop_cpu_cycle,
                                    uint32_t stop_time_us) {
  if (_state != State::Stopped) {
    if (_state == State::Started) {
      const uint32_t cpu_diff = stop_cpu_cycle - _cpu_accumulator.start_marker;
