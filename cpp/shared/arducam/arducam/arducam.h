 ...
 * The given image buffer length should be a multiple of the output of @ref
 * arducam_calculate_image_buffer_length(). Providing 2 buffers allows for best
 * performance.
 *
 * @note This will fail if the camera hardware fails to initialize or if the
 * given image buffer is too small.
 *
 * @param config @ref arducam_config_t driver configuration
 * @param image_buffer Image data buffer
 * @param buffer_length Size of the image data buffer in bytes, this should be a
 * multiple of  @ref arducam_calculate_image_buffer_length()
 * @return sl_status_t
 */
sl_status_t arducam_init(const arducam_config_t *config, uint8_t *image_buffer,
 ...
 *
 * This sets a specific camera setting. See @ref arducam_setting_t for the
 * available settings.
...
// sl_status_t arducam_get_setting(arducam_setting_t setting, int32_t*
// value_ptr);
 ...
 * After calling this, periodically call @ref arducam_get_next_image() to
 * retrieve a pointer to the next available image.
 *
 ...
 * If an image is available, the data_ptr argument will point to the image
 * buffer and the length_ptr will contain the length of the image in bytes.
 *
 ...
 * @note @ref arducam_start_capture() must be called before using this API.
 *
 * @param data_ptr Pointer to hold reference to next captured image, NULL if no
 * image is available
 * @param length_ptr Optional pointer to hold length of image in bytes. Leave
 * NULL if unused.
 * @return sl_status_t
 */
 ...
 *
 * @note This value may be different than the value returned by @ref
 * arducam_calculate_image_size() due to internal color space conversion
 * requirements.
 *
 ...
 */
uint32_t arducam_calculate_image_buffer_length(arducam_data_format_t format,
 ...
 */
uint32_t arducam_calculate_image_size(arducam_data_format_t format,
                                      uint32_t width, uint32_t height);
