...
#define GPIO_DIR_SENSOR_RESET                                                  \
  0x01 // Sensor reset IO direction, 0 = input, 1 = output
...
typedef struct {
  ...
  struct {
    ...
    uint8_t *head;  // Next image buffer address to write in the local buffer
                    // (Camera FIFO -> SPI -> head)
    uint8_t *tail;  // Next image buffer to return to app
    volatile uint32_t local_count; // Number of images buffered on the device,
                                   // ready to be used by the firmware
    uint32_t read_length; // The number of image bytes to read from the camera
    uint32_t
        read_buffer_offset; // The offset into the read buffer (this is required
                            // so we can expand RGB88 images in-place)
  ...
  bool add_dummy_byte_to_burst_read; // Older Arducam versions do not require a
                                     // dummy byte between the burst read
                                     // command and RX data
