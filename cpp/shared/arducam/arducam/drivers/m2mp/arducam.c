...
sl_status_t arducam_init(const arducam_config_t *config, uint8_t *image_buffer,
  ...
  arducam_context.image_size_bytes = arducam_calculate_image_size(
      config->data_format, config->image_resolution.width,
  ...
  arducam_context.buffer.max_local_count =
      (image_buffer_length / buffer_bytes_per_image);
  if (arducam_context.buffer.max_local_count == 0) {
    return SL_STATUS_INVALID_PARAMETER;
  }

  arducam_context.buffer.start = image_buffer;
  arducam_context.buffer.end =
      image_buffer +
      arducam_context.buffer.max_local_count * buffer_bytes_per_image;

  // This camera returns either YUV422 or RGB565 images
  // Both formats require 2 bytes per pixel
  // NOTE: grayscale uses RGB565
  arducam_context.buffer.read_length =
      config->image_resolution.width * config->image_resolution.height * 2;
  ...
  if (config->data_format == ARDUCAM_DATA_FORMAT_RGB888) {
    arducam_context.buffer.read_buffer_offset =
        buffer_bytes_per_image - arducam_context.buffer.read_length;
  }
...
sl_status_t arducam_start_capture() {
  ...
  arducam_context.buffer.head = arducam_context.buffer.tail =
      arducam_context.buffer.start;
...
sl_status_t arducam_poll() {
  ...
  if (arducam_context.is_started &&
    ...
    // Then poll the camera to see if the next image has been buffered in the
    // camera's FIFO
