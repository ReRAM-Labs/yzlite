...
#define OV2640_WRITE_VERIFY(addr, val)                                         \
  SL_VERIFY(arducam_driver_i2c_write_reg(addr, val))
...
static sl_status_t set_framesize(arducam_resolution_t resolution,
...
sl_status_t ov2640_init(const arducam_config_t *config) {
  ...
  SL_VERIFY(set_framesize(config->sensor_resolution,
...
/*************************************************************************************************/
sl_status_t ov2640_deinit() { return SL_STATUS_OK; }
...
static sl_status_t set_framesize(arducam_resolution_t resolution,
  ...
  SL_VERIFY(arducam_driver_i2c_write_reg(ZMHH, ZMHH_OUTW_SET(width) |
                                                   ZMHH_OUTH_SET(height)));
...
static sl_status_t set_contrast(int level) {
  ...
  for (int i = 0; i < ARRAY_COUNT(ov2640_contrast_regs[0]); i++) {
    OV2640_WRITE_VERIFY(ov2640_contrast_regs[0][i],
                        ov2640_contrast_regs[val][i]);
  }
...
static sl_status_t set_saturation(int level) {
  ...
  for (int i = 0; i < ARRAY_COUNT(ov2640_saturation_regs[0]); i++) {
    OV2640_WRITE_VERIFY(ov2640_saturation_regs[0][i],
                        ov2640_saturation_regs[val][i]);
  }
...
static sl_status_t set_brightness(int level) {
  ...
  for (int i = 0; i < ARRAY_COUNT(ov2640_brightness_regs[0]); i++) {
    OV2640_WRITE_VERIFY(ov2640_brightness_regs[0][i],
                        ov2640_brightness_regs[val][i]);
  }
...
/*************************************************************************************************/
static sl_status_t set_gainceiling(int gainceiling) {
  if (gainceiling < ARDUCAM_GAINCEILING_MIN ||
      gainceiling > ARDUCAM_GAINCEILING_MAX) {
    return SL_STATUS_INVALID_PARAMETER;
  }
...
/*************************************************************************************************/
static sl_status_t set_specialeffect(int effect) {
  if (effect < ARDUCAM_SPECIALEFFECT_NONE ||
      effect >= ARDUCAM_SPECIALEFFECT_COUNT) {
    return SL_STATUS_INVALID_PARAMETER;
  }
  ...
  for (int i = 0; i < ARRAY_COUNT(ov2640_special_effect_regs[0]); i++) {
    OV2640_WRITE_VERIFY(ov2640_special_effect_regs[0][i],
                        ov2640_special_effect_regs[effect + 1][i]);
  }
