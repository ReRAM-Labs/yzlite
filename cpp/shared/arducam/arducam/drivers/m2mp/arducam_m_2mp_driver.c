...
#define USART_RX_DMA_SIGNAL(N)                                                 \
  SL_CONCAT(dmadrvPeripheralSignal_USART, N, _RXDATAV)
#define USART_TX_DMA_SIGNAL(N) SL_CONCAT(dmadrvPeripheralSignal_USART, N, _TXBL)

#if defined(_SILICON_LABS_32B_SERIES_2)
#define ENABLE_SPI_INTERRUPT()                                                 \
  LDMA->IEN_SET = (1 << arducam_context.dma_rx_channel)
#define DISABLE_SPI_INTERRUPT()                                                \
  LDMA->IEN_CLR = (1 << arducam_context.dma_rx_channel)
#else
#define ENABLE_SPI_INTERRUPT()                                                 \
  LDMA->IEN |= (1 << arducam_context.dma_rx_channel)
#define DISABLE_SPI_INTERRUPT()                                                \
  LDMA->IEN &= ~(1 << arducam_context.dma_rx_channel)
#endif
...
sl_status_t arducam_driver_init(const arducam_config_t *config) {
  ...
  // Configure GPIO mode
  GPIO_PinModeSet(ARDUCAM_USART_CLK_PORT, ARDUCAM_USART_CLK_PIN,
                  gpioModePushPull, 0); // US0_CLK is push pull
  GPIO_PinModeSet(ARDUCAM_USART_CS_PORT, ARDUCAM_USART_CS_PIN, gpioModePushPull,
                  1); // US0_CS is push pull
  GPIO_PinModeSet(ARDUCAM_USART_TX_PORT, ARDUCAM_USART_TX_PIN, gpioModePushPull,
                  1); // US0_TX (MOSI) is push pull
  GPIO_PinModeSet(ARDUCAM_USART_RX_PORT, ARDUCAM_USART_RX_PIN, gpioModeInput,
                  1); // US0_RX (MISO) is input

  USART_InitSync_TypeDef usart_config = USART_INITSYNC_DEFAULT;
  usart_config.master = true; // master mode
  usart_config.baudrate =
      1000000; // CLK freq is 1MHz
               // NOTE: This can technically go up to 8M but can be unreliable
               // depending on the hardware setup
  usart_config.autoCsEnable =
      true; // CS pin controlled by hardware, not firmware
  usart_config.clockMode =
      usartClockMode0;      // clock idle low, sample on rising/first edge
  usart_config.msbf = true; // send MSB first
  usart_config.enable =
      usartDisable; // making sure USART isn't enabled until we set it up
  USART_InitSync(USART_PORT(ARDUCAM_USART_PERIPHERAL_NO), &usart_config);

  // Set USART pin locations
#if defined(_SILICON_LABS_32B_SERIES_2)
  GPIO->USARTROUTE[ARDUCAM_USART_PERIPHERAL_NO].ROUTEEN =
      GPIO_USART_ROUTEEN_TXPEN | GPIO_USART_ROUTEEN_RXPEN |
      GPIO_USART_ROUTEEN_CLKPEN;

  GPIO->USARTROUTE[ARDUCAM_USART_PERIPHERAL_NO].TXROUTE =
      ((uint32_t)ARDUCAM_USART_TX_PORT << _GPIO_USART_TXROUTE_PORT_SHIFT) |
      ((uint32_t)ARDUCAM_USART_TX_PIN << _GPIO_USART_TXROUTE_PIN_SHIFT);

  GPIO->USARTROUTE[ARDUCAM_USART_PERIPHERAL_NO].RXROUTE =
      ((uint32_t)ARDUCAM_USART_RX_PORT << _GPIO_USART_RXROUTE_PORT_SHIFT) |
      ((uint32_t)ARDUCAM_USART_RX_PIN << _GPIO_USART_RXROUTE_PIN_SHIFT);

  GPIO->USARTROUTE[ARDUCAM_USART_PERIPHERAL_NO].CLKROUTE =
      ((uint32_t)ARDUCAM_USART_CLK_PORT << _GPIO_USART_CLKROUTE_PORT_SHIFT) |
      ((uint32_t)ARDUCAM_USART_CLK_PIN << _GPIO_USART_CLKROUTE_PIN_SHIFT);

#else
  // Enable USART pins
  USART_PORT(ARDUCAM_USART_PERIPHERAL_NO)->ROUTEPEN =
      USART_ROUTEPEN_CLKPEN | USART_ROUTEPEN_TXPEN | USART_ROUTEPEN_RXPEN;
  ...
  GPIO_PinModeSet(ARDUCAM_I2C_SCL_PORT, ARDUCAM_I2C_SCL_PIN,
                  gpioModeWiredAndPullUp, 1);
  GPIO_PinModeSet(ARDUCAM_I2C_SDA_PORT, ARDUCAM_I2C_SDA_PIN,
                  gpioModeWiredAndPullUp, 1);
...
#if defined(_SILICON_LABS_32B_SERIES_2)
  GPIO->I2CROUTE[ARDUCAM_I2C_PERIPHERAL_NO].ROUTEEN =
      GPIO_I2C_ROUTEEN_SDAPEN | GPIO_I2C_ROUTEEN_SCLPEN;
  GPIO->I2CROUTE[ARDUCAM_I2C_PERIPHERAL_NO].SCLROUTE =
      (uint32_t)((ARDUCAM_I2C_SCL_PIN << _GPIO_I2C_SCLROUTE_PIN_SHIFT) |
                 (ARDUCAM_I2C_SCL_PORT << _GPIO_I2C_SCLROUTE_PORT_SHIFT));
  GPIO->I2CROUTE[ARDUCAM_I2C_PERIPHERAL_NO].SDAROUTE =
      (uint32_t)((ARDUCAM_I2C_SDA_PIN << _GPIO_I2C_SDAROUTE_PIN_SHIFT) |
                 (ARDUCAM_I2C_SDA_PORT << _GPIO_I2C_SDAROUTE_PORT_SHIFT));

#else
  I2C_PORT(ARDUCAM_I2C_PERIPHERAL_NO)->ROUTEPEN =
      I2C_ROUTEPEN_SDAPEN | I2C_ROUTEPEN_SCLPEN;
  I2C_PORT(ARDUCAM_I2C_PERIPHERAL_NO)->ROUTELOC0 =
      (uint32_t)((ARDUCAM_I2C_SDA_LOC << _I2C_ROUTELOC0_SDALOC_SHIFT) |
...
#define SPI_TEST_VALUE 0xBA
  ...
  if (reg_value != 0x73) {
    // Older Arducam versions do not require a dummy byte between the burst read
    // command and RX data
  ...
  arducam_driver_spi_write_reg(ARDUCHIP_REG_FIFO, FIFO_CLEAR_MASK |
                                                      FIFO_RDPTR_RST_MASK |
                                                      FIFO_WRPTR_RST_MASK);
...
sl_status_t arducam_driver_i2c_write_regs(const reg_addr_value_t *regs,
  ...
  for (const reg_addr_value_t *addr_value = regs;; ++addr_value) {
    if (addr_value->address == REG_ADDR_ACTION) {
      ...
      SL_VERIFY(
          arducam_driver_i2c_write_reg(addr_value->address, addr_value->value));
    }
  }
...
sl_status_t arducam_driver_spi_write_reg(uint8_t addr, uint8_t data) {
  ...
  USART_SpiTransfer(USART_PORT(ARDUCAM_USART_PERIPHERAL_NO),
...
/*************************************************************************************************/
static bool on_dma_rx_complete_irq_handler(unsigned int channel,
...
#define MAX_DMA_LENGTH                                                         \
  ...
  if (userParam == NULL) {
    ...
    arducam_context.dma_length_remaining -=
        MIN(MAX_DMA_LENGTH, arducam_context.dma_length_remaining);
  }
  ...
  LDMA_TransferCfg_t rx_cfg = LDMA_TRANSFER_CFG_PERIPHERAL(
      USART_RX_DMA_SIGNAL(ARDUCAM_USART_PERIPHERAL_NO));
  LDMA_Descriptor_t rx_desc = LDMA_DESCRIPTOR_LINKREL_P2M_BYTE(
      &USART_PORT(ARDUCAM_USART_PERIPHERAL_NO)->RXDATA,
      arducam_context.dma_rx_ptr, chunk_length, 0);
  rx_desc.xfer.link = 0;
  rx_desc.xfer.doneIfs = 1;

  LDMA_TransferCfg_t tx_cfg = LDMA_TRANSFER_CFG_PERIPHERAL(
      USART_TX_DMA_SIGNAL(ARDUCAM_USART_PERIPHERAL_NO));
  LDMA_Descriptor_t tx_desc = LDMA_DESCRIPTOR_LINKREL_M2P_BYTE(
      &dummy_tx_buffer, &USART_PORT(ARDUCAM_USART_PERIPHERAL_NO)->TXDATA,
      chunk_length, 0);
  tx_desc.xfer.link = 0;
  ...
  // Start receive DMA
  DMADRV_LdmaStartTransfer(arducam_context.dma_rx_channel, &rx_cfg, &rx_desc,
                           on_dma_rx_complete_irq_handler, NULL);

  // Start transmit DMA
  DMADRV_LdmaStartTransfer(arducam_context.dma_tx_channel, &tx_cfg, &tx_desc,
                           NULL, NULL);
