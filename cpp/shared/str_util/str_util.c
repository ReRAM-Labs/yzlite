...
/*************************************************************************************************/
int str_isempty(const char *s) { return ((s == NULL) || (*s == 0)); }
...
/*************************************************************************************************/
int char_isprint(int c) { return (c >= 0x20 && c <= 0x7E); }
...
/*************************************************************************************************/
int char_isnum(int c) { return (c >= '0' && c <= '9'); }
...
/*************************************************************************************************/
char *str_lstrip(char *s, const char *chars) { return s + strspn(s, chars); }
...
/*************************************************************************************************/
int str_parse_base(const char *s, int base, intmax_t *result, intmax_t min,
                   intmax_t max) {
  if (str_isempty(s)) {
    return -1;
  }
...
int str_parse_int(const char *s, intmax_t *result, intmax_t min, intmax_t max) {
  ...
  if (s[0] == '0' && (s[1] == 'x' || s[1] == 'X')) {
    ...
    base = 2;
    // The function strtoll cannot deal with the binary prefix in its input, so
    // skip past this.
    s += 2;
  } else {
    base = 10;
...
int str_parse_bool(const char *onoff, bool *var) {
  static const char *const on_vals[4] = {"1", "on", "true", "yes"};
...
/*************************************************************************************************/
char *str_binary_to_hex_buffer(char *hex_str, uint32_t hex_str_max_len,
...
/*************************************************************************************************/
const char *uint32_to_padded_str(uint32_t value, char *output,
                                 uint8_t max_padding) {
  ...
  *ptr-- = 0;
  ...
  for (digits_left = max_padding; (value != 0) && (digits_left > 0);
       --digits_left) {
    *ptr-- = (char)((value % 10) + '0');
    value = value / 10;
  }
...
/*************************************************************************************************/
const char *bool_to_str(bool v) { return (v) ? "true" : "false"; }
