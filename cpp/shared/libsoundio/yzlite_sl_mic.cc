...
static void microphone_read_callback(struct SoundIoInStream *instream,
...
/*************************************************************************************************/
extern "C" sl_status_t yzlite_sl_mic_init(uint32_t sample_rate,
                                          uint8_t channels) {
  int err;
...
/*************************************************************************************************/
extern "C" sl_status_t yzlite_sl_mic_deinit(void) {
  if (context.instream != nullptr) {
...
static bool find_microphone_device() {
  ...
  const char *YZLITE_MICROPHONE = getenv("YZLITE_MICROPHONE");
  int input_count = soundio_input_device_count(context.soundio);

  if (YZLITE_MICROPHONE != nullptr) {
    printf("Environment variable: YZLITE_MICROPHONE=%s\n", YZLITE_MICROPHONE);
  }
      ...
      if (context.sample_rate_hz <= range->max) {
        for (int j = 0; j < device->format_count; j += 1) {
          if (device->formats[j] == SoundIoFormatS16LE) {
            ++compatible_count;
            if (YZLITE_MICROPHONE != nullptr) {
              if (strcasestr(device->name, YZLITE_MICROPHONE) != nullptr) {
...
exit:
    ...
    if (compatible_count > 0) {
      ...
      for (int i = 0; i < input_count; i += 1) {
        struct SoundIoDevice *device =
            soundio_get_input_device(context.soundio, i);
        for (int j = 0; j < device->sample_rate_count; j += 1) {
          struct SoundIoSampleRateRange *range = &device->sample_rates[j];
          if (context.sample_rate_hz <= range->max) {
            for (int j = 0; j < device->format_count; j += 1) {
  ...
  if (YZLITE_MICROPHONE == nullptr) {
    printf("(Hint: Manually specify microphone with ENV variable: "
           "YZLITE_MICROPHONE=<mic-name>)\n\n");
  }
  fflush(stdout);
