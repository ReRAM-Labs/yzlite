...
class Logger {
public:
  Logger(const char *tag, Level level = Level::Info,
  ...
  void dump_hex(const void *address, unsigned length,
                const char *desc = nullptr, ...);
  void vdump_hex(const void *address, unsigned length, const char *desc,
                 va_list args);
  ...
  Flags &flags(void) { return _flags; }

  const Flags &flags(void) const { return _flags; }

  constexpr Level level(void) const { return _level; }

  constexpr const char *tag() const { return _tag; }
