...
void adpcm_decode_mono_data(const uint8_t *in, int16_t *out, int n_frames,
                            adpcm_state_t *state) {
  assert(n_frames % 2 == 0);
...
void adpcm_encode_mono_data(const int16_t *in, uint8_t *out, int n_frames,
                            adpcm_state_t *state) {
  assert(n_frames % 2 == 0);
...
void adpcm_decode_sample(uint8_t sample, adpcm_state_t *state) {
  ...
  state->predicted_sample =
      CLAMP(state->predicted_sample + diff, -32768, 32767);
  state->step_index =
      CLAMP(state->step_index + STEP_INDICES[sample & 7], 0, 88);
}
...
uint8_t adpcm_encode_sample(int16_t sample, adpcm_state_t *state) {
  ...
  state->predicted_sample =
      CLAMP(state->predicted_sample + diff, -32768, 32767);
  state->step_index =
      CLAMP(state->step_index + STEP_INDICES[encoded_sample & 7], 0, 88);
