...
int adjust_required_tensor_arena_bytes_from_64bit_to_32bit(
    const void *flatbuffer, tflite::MicroInterpreter *interpreter,
    int tensor_arena_size);

/*************************************************************************************************/
TfliteMicroModel::~TfliteMicroModel() { unload(); }

/*************************************************************************************************/
bool TfliteMicroModel::load(const void *flatbuffer,
  ...
  if (TFLITE_MICRO_VERSION != nullptr) {
    YZLITE_INFO("Using Tensorflow-Lite Micro version: %s",
                TFLITE_MICRO_VERSION);
  }

  auto accelerator = yzlite_tflite_micro_get_registered_accelerator();
  ...
  if (runtime_buffer == nullptr) {
    ...
    // If the runtime_buffer_size  < 0, then we skip down to finding the optimal
    // run-time buffer size.
    ...
    if (runtime_buffer_size >= 0) {
      ...
      if (runtime_buffer_size == 0) {
        ...
        if (model_runtime_size < 0) {
          ...
          YZLITE_INFO("Runtime memory size from .tflite model: %d",
                      model_runtime_size);
        }
      } else {
      ...
      // If we should attempt to allocate a buffer before searching for the
      // optimal size
      if (model_runtime_size > 0) {
        allocated_buffer_size = model_runtime_size;

#if INTPTR_MAX == INT64_MAX
        // The buffer size embedded into the .tflite is meant for a 32-bit ARM
        // MCU If we're running on a 64-bit system then add 1MB of additional
        // memory to account for 64-bit pointer overhead
        ...
        if (runtime_buffer == nullptr) {
          ...
          YZLITE_WARN("Failed to allocate buffer with size: %d specified in "
                      ".tflite model",
                      allocated_buffer_size);
        }
        // Load the model with the buffer
        else if (load_interpreter(flatbuffer, op_resolver, runtime_buffer,
                                  allocated_buffer_size)) {
          // If we successfully loaded the model with the runtime memory size
          // from the flatbuffer then we're done
          ...
          if (model_has_unsupported_layers) {
            YZLITE_WARN(
                "Model contains one or more unknown layers. You may need "
                "to update your OpResolver");
            unload();
            return false;
          }
          ...
          YZLITE_WARN("Failed to load model with buffer of size %d",
                      model_runtime_size);
        }

        // If a size was specified by the API call and we failed to load the
        // model with it then just return the error NOTE: If a size was
        // specified in the model parameters and it was too small,
    ...
    if (runtime_buffer == nullptr) {
      ...
      if (!find_optimal_buffer_size(flatbuffer, op_resolver,
                                    runtime_buffer_size)) {
        // On failure, just return
        if (model_has_unsupported_layers) {
          YZLITE_WARN("Model contains one or more unknown layers. You may need "
                      "to update your OpResolver");
        } else {
          YZLITE_ERROR(
              "Failed to allocate buffer for model (likely heap memory "
              "overflow)");
        }
        unload();
        return false;
      ...
      // This is only used when allocating temporary tenesors (e.g.
      // context.GetTensor())
      allocated_buffer_size += 1024 * 1024;
#endif
      ...
      if (runtime_buffer == nullptr) {
        ...
        YZLITE_WARN("Failed to allocate buffer with size: %d",
                    allocated_buffer_size);
        unload();
        return false;
      }
    ...
    // At this point, the runtime memory buffer is allocated and the model
    // flatbuffer is loaded. This model now owns the buffer When the model is
    // unloaded, the buffer will be automatically freed
  ...
  // Else if a pre-allocated buffer was given to this API
    ...
    if (runtime_buffer_size <= 0) {
      YZLITE_ERROR("Must specify runtime_buffer_size when providing "
    ...
    // If a runtime buffer was provided, then load the interpreter with it now
      ...
      if (model_has_unsupported_layers) {
        YZLITE_WARN(
            "Model contains one or more unknown layers. You may need to "
            "update your OpResolver");
      } else {
        YZLITE_ERROR("Failed to load model with runtime buffer size: %d",
                     runtime_buffer_size);
      }
      unload();
      return false;
...
#else
  _model_details._runtime_memory_size =
      adjust_required_tensor_arena_bytes_from_64bit_to_32bit(
          flatbuffer, _interpreter, runtime_buffer_size);
#endif

  if (accelerator != nullptr) {
    _model_details._accelerator = accelerator->name;
#ifdef TFLITE_MICRO_SIMULATOR_ENABLED
    accelerator->set_simulator_memory(
        "sram", runtime_buffer,
        allocated_buffer_size); // Be sure to use the actually allocated buffer
                                // size used by this 64-bit program
    accelerator->set_simulator_memory("flash", (void *)flatbuffer,
                                      2 * 1024 * 1024);
#endif
  }
...
/*************************************************************************************************/
bool TfliteMicroModel::is_loaded() const { return _interpreter != nullptr; }
...
bool TfliteMicroModel::invoke() const {
  ...
  yzlite::_processing_callback = this->_processing_callback;
  yzlite::_processing_callback_arg = this->_processing_callback_arg;

#ifdef TFLITE_MICRO_SIMULATOR_ENABLED
  if (accelerator != nullptr) {
    retval = accelerator->invoke_simulator(
        [this]() -> bool { return _interpreter->Invoke() == kTfLiteOk; });
  } else {
  ...
  yzlite::_processing_callback = nullptr;
  yzlite::_processing_callback_arg = nullptr;
...
void TfliteMicroModel::print_summary(logging::Logger *logger) const {
  ...
  l.info("Tensor runtime memory: %s",
         cpputils::format_units(_model_details.runtime_memory_size(), 3,
                                fmt_buffer));
  ;
...
bool TfliteMicroModel::enable_profiler() {
  ...
  if (is_loaded()) {
    YZLITE_ERROR("Model already loaded");
    return false;
  }
  model_profiler_enabled = true;
  return true;
#else
  YZLITE_ERROR("C++ library not build with profiling support");
  return false;
#endif
}
...
bool TfliteMicroModel::enable_tensor_recorder() {
  ...
  return true;
#else
  YZLITE_ERROR("C++ library not build with recording support");
  return false;
#endif
}
...
bool TfliteMicroModel::recorded_data(const uint8_t **buffer_ptr,
                                     uint32_t *length_ptr) const {
  return get_recorded_data(buffer_ptr, length_ptr);
}
#endif

/*************************************************************************************************/
void TfliteMicroModel::set_processing_callback(void (*callback)(void *),
                                               void *arg) {
...
/*************************************************************************************************/
const void *TfliteMicroModel::find_metadata(const char *tag,
...
bool TfliteMicroModel::load_model_parameters(const void *flatbuffer) {
  ...
  if (TfliteModelParameters::load_from_tflite_flatbuffer(flatbuffer,
                                                         this->parameters)) {
...
/*************************************************************************************************/
bool TfliteMicroModel::load_interpreter(const void *flatbuffer,
                                        tflite::MicroOpResolver &op_resolver,
                                        uint8_t *runtime_buffer,
                                        unsigned runtime_buffer_size,
                                        bool disable_logs) {
  ...
  _interpreter = new (_interpreter_buffer) tflite::MicroInterpreter(
      tflite_model, op_resolver, runtime_buffer, runtime_buffer_size);
...
bool TfliteMicroModel::find_optimal_buffer_size(
    const void *flatbuffer, tflite::MicroOpResolver &op_resolver,
    unsigned &runtime_buffer_size) {
#ifdef __arm__
  ...
  YZLITE_INFO("Searching for optimal runtime memory size ...");
  ...
  // Add some additional memory for any padding or invoking the
  // context.GetTensor() APIs.
  last_working_buffer_size += 256;

  YZLITE_INFO("Determined optimal runtime memory size to be %d",
              last_working_buffer_size);
...
/*************************************************************************************************
 * The following code is used to account for the overhead 64-bit builds add to
 * the runtime memory size. Recall that embedded builds use 32-bit. The
 * following reduces the runtime memory size to convert from 64-bit pointers to
 * 32-bit pointers. The values below were experimentally found
 */
int adjust_required_tensor_arena_bytes_from_64bit_to_32bit(
    const void *flatbuffer, tflite::MicroInterpreter *interpreter,
    int tensor_arena_size) {
  ...
  const int scratch_buffer_count =
      interpreter->allocator_.scratch_buffer_request_count_;
