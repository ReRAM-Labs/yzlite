...
public:
   ...
   *
   * @note The provided `flatbuffer` MUST persist for the life of this model
   * object.
   *
   * @note If runtime_buffer is NULL then a buffer will be automatically
   * allocated. In this case,
   * - If runtime_buffer_size < 0, then automatically find the optimal run-time
   * buffer size
   * - If runtime_buffer_size > 0, then allocate the specified size, if the size
   * is too small then return the error
   * - If runtime_buffer_size == 0, then attempt to retrieve the arena size from
   * the .tflite parameters metadata, and allocate the buffer. If the buffer is
   * too small or was not found in the metadata, then automatically find the
   * optimal run-time buffer size.
   *
   ...
   */
  bool load(const void *flatbuffer, tflite::MicroOpResolver &op_resolver,
   ...
   * used by the model
   */
  tflite::MicroInterpreter *interpreter() { return _interpreter; }

  /**
   * Return a pointer to the tflite::MicroOpResolver
   * used by the model
   */
  tflite::MicroOpResolver *ops_resolver() { return _ops_resolver; }
...
private:
  ...
  bool load_interpreter(const void *flatbuffer,
                        tflite::MicroOpResolver &op_resolver,
                        uint8_t *runtime_buffer, unsigned runtime_buffer_size,
                        bool disable_logs = false);
  bool find_optimal_buffer_size(const void *flatbuffer,
                                tflite::MicroOpResolver &op_resolver,
...
extern "C" void yzlite_sl_tflite_micro_init(yzlite::TfliteMicroModel *model);
extern "C" TfLiteStatus yzlite_sl_tflite_micro_invoke();
