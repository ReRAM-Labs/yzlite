...
extern bool yzlite_tflm_force_buffer_overlap;
...
/*************************************************************************************************/
bool TfliteMicroModelWrapper::load(const std::string &flatbuffer_data,
                                   void *accelerator, bool enable_profiler,
  ...
  yzlite_tflm_force_buffer_overlap = force_buffer_overlap;
  ...
  bool retval =
      TfliteMicroModel::load(this->_flatbuffer_data.c_str(), *op_resolver,
                             runtime_buffer, runtime_memory_size);

  yzlite_tflm_force_buffer_overlap = false;
...
bool TfliteMicroModelWrapper::invoke() const {
  if (this->_accelerator_wrapper != nullptr) {
    // Technically, this should not be required as the accelerator should have
    // already been set by the accelerator wrapper when calling
    // yzlite_tflite_micro_register_accelerator() in the load() However, for
    // some reason the pointer is being cleared on Linux in some instances. So,
    // as a failsafe we set the accelerator pointer again before invoking the
    // simulator.
    auto accelerator_wrapper =
        (const TfliteMicroAcceleratorWrapper *)this->_accelerator_wrapper;
    yzlite_tflite_micro_set_accelerator(accelerator_wrapper->accelerator);
  }
...
py::dict TfliteMicroModelWrapper::get_details() const {
  ...
  details_dict["tflite_buffer_addr"] =
      (uint32_t)((uintptr_t)this->_flatbuffer_data.c_str());
...
py::list TfliteMicroModelWrapper::get_profiling_results() const {
  ...
  for (const auto profiler : profiler_list) {
    ...
    result["cpu_cycles"] = 0; // stats.cpu_cycles;  CPU cycles and time are not
                              // currently measured in the simulator
