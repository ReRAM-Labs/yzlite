...
def generate_model_op_resolver_header(
    ...
    """Generate a model op resolver header file from a YZLITE model or .tflite
    ...
    with open(output, 'w') as f:
        ...
        f.write(
            f'class {obj_name} : public tflite::MicroMutableOpResolver<{len(layer_opcodes)}> {{\n')
        f.write('  public:\n')
        f.write(f'    {obj_name}() {{\n')

        for opcode in layer_opcodes:
            func_name = opcode.replace('_', ' ').title().replace(' ',  '').replace(
                '1d', '1D').replace('2d', '2D').replace('3d', '3D')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Generate a model op resolver header file from a YZLITE model or .tflite')
    parser.add_argument(
        'model', help='Name of YZLITE model or path to .tflite')
    parser.add_argument('--output', default='generated_op_resolver.hpp',
                        help='Path to generated output header')
    parser.add_argument('--name', default='MyOpResolver',
                        help='Name of custom op resolver class')
    ...
    try:
        ...
        cli.handle_exception(
            'Failed to generate model op resolver header', _ex)
