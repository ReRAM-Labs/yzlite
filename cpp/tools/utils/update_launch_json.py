...
from yzlite.utils.commander import (download_commander, get_commander_settings,
                                    get_device_from_platform)
from yzlite.utils.path import fullpath, get_user_setting
from yzlite.utils.python import load_json_safe
from yzlite.utils.system import is_windows
...
def update_launch_json(
    ...
    if os.path.exists(launch_json_path):
        try:
            ...
            raise RuntimeError(
                f'Failed to load: {launch_json_path}, err: {json_load_ex}')
    else:
    ...
    if platform in ('windows', 'linux'):
        ...
        commander_dir = os.path.dirname(
            download_commander()).replace('\\', '/')
    ...
    for i, cfg in enumerate(configurations):
        if cfg['name'] == config_name:
            ...
            update_keys = ('program', 'executable', 'serialNumber',
                           'ipAddress', 'armToolchainPath')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Utility to update .vscode/launch.json with a embedded debug configuration')
    parser.add_argument('--name', help='The executable name')
    parser.add_argument('--path', help='Path to the executable')
    parser.add_argument(
        '--toolchain', help='Path to build toolchains binary directory')
    parser.add_argument('--platform', help='Name of build platform')
    parser.add_argument('--workspace', help='Path to workspace directory')
    parser.add_argument(
        '--device', help='Device options to pass to JLink GDB server')
    ...
    parser.add_argument(
        '--interface', help=' JLink debugger interface (jtag, swd, etc)', default='swd')
    parser.add_argument(
        '--speed', help=' JLink debugger speed', default='auto')
