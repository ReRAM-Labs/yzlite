...
def build_docs_command(
    ...
    """Build the YZLITE online documentation

    This uses the sphinx docs build system to convert the
    markdown files in <yzlite repo>/docs into a website
    """
    ...
    env = os.environ.copy()
    env['YZLITE_REPO_PROJECT_NAME'] = repo_project_name

    sphinx_exe = os.path.dirname(sys.executable) + '/sphinx-build'

    docs_dir = f'{YZLITE_ROOT_DIR}/docs/website_builder'
    ...
    if clean:
        ...
        for fn in built_files:
            p = f'{YZLITE_ROOT_DIR}/docs/{fn}'
            if os.path.exists(p):
                os.remove(p)
        for dn in built_dirs:
            remove_directory(f'{YZLITE_ROOT_DIR}/docs/{dn}')
        for dn in website_builder_build_dirs:
            remove_directory(f'{YZLITE_ROOT_DIR}/docs/website_builder/{dn}')
    ...
    retcode, retmsg = run_shell_cmd(
        cmd, outfile=logger, env=env, logger=logger)
    if retcode != 0:
        cli.abort(msg=f'Failed to build docs: {retmsg}')

    # Copy the generated docs from build directory
    # to the docs directory
    # Updating the "yizhutech" to te test repo's name as necessary
    repo_name = get_user_setting('repo_project_name')
    for fn in built_files:
        _copy_file(f'{build_dir}/{fn}',
                   f'{YZLITE_ROOT_DIR}/docs/{fn}', repo_name=repo_name)
    for dn in built_dirs:
        _copy_directory(f'{build_dir}/{dn}',
                        f'{YZLITE_ROOT_DIR}/docs/{dn}', repo_name=repo_name)

    if show:
        webbrowser.open(f'file:///{YZLITE_ROOT_DIR}/docs/index.html')
    ...
    if checklinks:
        ...
        index_path = os.path.abspath(
            fullpath(f'{YZLITE_ROOT_DIR}/docs/index.html'))
...
def _copy_file(src, dst, repo_name=None):
    url_re = re.compile(
        r'.*="(https:\/\/yizhutech\.github\.io\/yzlite\/).*\..*".*', re.I)
    ...
    docs_base_dir = f'{YZLITE_ROOT_DIR}/docs'
    ...
    if repo_name:
        ...
        data = data.replace('YizhuTech.github.io/yzlite',
                            f'{repo_name}.github.io/yzlite')
        data = data.replace('github/yizhutech/yzlite',
                            f'github/{repo_name}/yzlite')
        data = data.replace('github/YizhuTech/yzlite',
                            f'github/{repo_name}/yzlite')
        data = data.replace('github.com/yizhutech/yzlite',
                            f'github.com/{repo_name}/yzlite')
        data = data.replace('github.com/YizhuTech/yzlite',
                            f'github.com/{repo_name}/yzlite')
        data = data.replace('raw.githubusercontent.com/yizhutech/yzlite',
                            f'raw.githubusercontent.com/{repo_name}/yzlite')
        data = data.replace('raw.githubusercontent.com/YizhuTech/yzlite',
                            f'raw.githubusercontent.com/{repo_name}/yzlite')
    ...
    def _clean_dir(path: str):
        run_shell_cmd(
            ['git', 'restore', '--source=HEAD', '--staged',
                '--worktree', '--', f'{YZLITE_ROOT_DIR}/docs/{path}'],
            logger=logger
        )
        run_shell_cmd(
            ['git', 'clean', '-fd', f'{YZLITE_ROOT_DIR}/docs/{path}'],
            logger=logger
        )
    ...
    with open(generate.__file__, 'r') as f:
        for line in f:
            if 'filename = os.path.join(path, filename_map.get(name, name) + suffix)' in line.strip():
                line = line.replace('filename = os.path.join(path, filename_map.get(name, name) + suffix)',
                                    'filename = path + suffix # Patched by the YZLITE # os.path.join(path, filename_map.get(name, name) + suffix)')
                updated = True
            data += line
