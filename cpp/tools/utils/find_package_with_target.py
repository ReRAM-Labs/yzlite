...
YZLITE_CPP_DIR = os.path.normpath(f'{CURDIR}/../..').replace('\\', '/')
YZLITE_PYTHON_DIR = os.path.normpath(
    f'{CURDIR}/../../../yzlite').replace('\\', '/')


def main():
    parser = argparse.ArgumentParser(
        description='Find a CMakeList.txt containing the given target and print its directory')
    ...
    search_dirs = []
    if args.paths:
        search_dirs.extend(x.strip()
                           for x in args.paths.split(';') if x.strip())
    search_dirs.append(YZLITE_CPP_DIR)
    search_dirs.append(YZLITE_PYTHON_DIR)
    ...
    for search_dir in search_dirs:
        ...
        if search_dir in (YZLITE_PYTHON_DIR, YZLITE_CPP_DIR):
            search_depth = 5
        else:
            search_depth = 10
        ...
        for root, _, files in walk_with_depth(search_dir, depth=search_depth):
            ...
            # Do not search the yzlite/cpp/tools directory
            if root.startswith(f'{YZLITE_CPP_DIR}/tools'):
                continue
            ...
            for fn in files:
                if fn.lower() == 'cmakelists.txt':
                    ...
                    _find_target(add_custom_target_re,
                                 root, contents, target_re)
    ...
    search_paths_str = "\n".join(search_dirs)
    print(
        f'Failed to find target: {args.target}, search paths:\n{search_paths_str}')
    sys.exit(-1)
