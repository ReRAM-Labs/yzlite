...
from yzlite.core.model import load_tflite_model
from yzlite.core.tflite_micro import TfliteMicro
from yzlite.core.tflite_model import TfliteModel
from yzlite.utils.bin2header import bin2header
from yzlite.utils.hasher import hash_file
...
def generate_model_header(
    ...
    """Generate a model header file from a YZLITE model or .tflite

    Args:
        model: Name of YZLITE model or path to .tflite
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Generate a model header file from a YZLITE model or .tflite')
    parser.add_argument(
        'model', help='Name of YZLITE model or path to .tflite')
    parser.add_argument('--output', default='generated_model.tflite.h',
                        help='Path to generated output header')
    parser.add_argument('--name', default='MODEL_DATA', help='Name of C array')
    parser.add_argument('--length_name', default='MODEL_DATA_LENGTH',
                        help='Name of C variable to hold length of data in bytes')
    parser.add_argument('--attributes', default=None,
                        help='Attributes to prepend to C array variable')
    parser.add_argument('--accelerator', default=None,
                        help='Specific accelerator for which to generate model header')
    parser.add_argument('--memory-section', default=None,
                        help='The memory section to place the model')
