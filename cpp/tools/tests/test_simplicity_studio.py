...
from yzlite.utils.path import (clean_directory, create_tempdir,
                               get_user_setting, recursive_listdir)
from yzlite.utils.python import import_module_at_path
from yzlite.utils.shell_cmd import run_shell_cmd
from yzlite.utils.test_helper import get_logger, run_yzlite_command

# SPECIFIC_BOARD = 'brd4001a,brd4186c'

if os.name == 'nt':
    SIMPLICITY_STUDIO_PATH = get_user_setting(
        'simplicity_studio_path') or 'C:/YizhuTech/SimplicityStudio/v5'
    ...
    if not os.path.exists(SIMPLICITY_STUDIO_PATH):
        warnings.warn(
            ...
            'Alternatively, create/update the file: ~/.yzlite/user_settings.yaml\n'
    ...
    if not SIMPLICITY_STUDIO_PATH:
        warnings.warn(
            'Simplicity Studio not found.\n'
            'Create/update the file: ~/.yzlite/user_settings.yaml\n'
            'and add the following:\n\n'
            'simplicity_studio_path: <path to Simplicity Studio>\n\n'
...
def test_get_slc_cli() -> str:
    ...
    download_arm_toolchain_module = import_module_at_path(
        f'{YZLITE_ROOT_DIR}/cpp/tools/toolchains/gcc/arm/download_toolchain.py')
    gcc_dir = download_arm_toolchain_module.download_arm_toolchain(
        return_path=True)
    generate_slcp_logger.info(f'ARM GCC dir: {gcc_dir}')

    generate_slcp_logger.info('Installing the YZLITE GSDK extension ...')
    run_yzlite_command('build', 'gsdk_yzlite_extension',
                       '--no-show', logger=generate_slcp_logger)
    ...
    if retcode != 0:
        raise RuntimeError(
            f'Failed to "trust" {gsdk_dir}\ncmd: {" ".join(cmd)}\nerr: {retmsg}')

    cmd = slc_cli + ['signature', 'trust', '-s', gsdk_dir,
                     '-extpath', f'{gsdk_dir}/extension/yzlite']
    generate_slcp_logger.info(" ".join(cmd))
    retcode, retmsg = run_shell_cmd(cmd, outfile=generate_slcp_logger)
    if retcode != 0:
        raise RuntimeError(
            f'Failed to "trust" YZLITE extension\ncmd: {" ".join(cmd)}\nerr: {retmsg}')
...
def test_simplicity_studio_project(app_name, board):
    ...
    app_dir = f'{YZLITE_ROOT_DIR}/cpp/shared/apps/{app_name}'
    build_dir = create_tempdir(
        f'utest/simplicity_studio/{app_name}-{board.replace(",", "_")}')
    ...
    cmd = slc_cli + ['generate',
                     ...
                     '-d', build_dir,
                     '--with', board,
                     '-p', f'{app_dir}/yzlite_{app_name}.slcp'
                     ]
    generate_slcp_logger.info(" ".join(cmd))

    env = os.environ
    env['JAVA11_HOME'] = jre_bin_dir
    # Clear the venv bin dir as that seems to be causing errors in the slc cli
    ...
    if retcode != 0:
        raise RuntimeError(
            f'Failed generate Simplicity Studio project for {app_name} app\ncmd: {" ".join(cmd)}\nerr: {retmsg}')

    env = os.environ
    if os.name == 'nt':
        os.environ["PATH"] = os.path.normpath(
            SIMPLICITY_STUDIO_MYSYS_BIN_PATH) + os.pathsep + os.environ["PATH"]

    env['ARM_GCC_DIR'] = gcc_dir

    cmd = ['make', '-f', f'yzlite_{app_name}.Makefile',
           '-j', str(multiprocessing.cpu_count())]
    generate_slcp_logger.info(" ".join(cmd))
    retcode, retmsg = run_shell_cmd(
        cmd, outfile=generate_slcp_logger, env=env, cwd=build_dir)
    if retcode != 0:
        raise RuntimeError(
            f'Failed build Simplicity Studio project for {app_name} app\ncmd: {" ".join(cmd)}\nerr: {retmsg}')


def _get_slc_exe() -> str:
    """Download the slc_cli stand-alone utility and return the path to the executable"""
    WINDOWS_URL = 'https://www.yizhu.com/documents/login/software/slc_cli_windows.zip'
    LINUX_URL = 'https://www.yizhu.com/documents/login/software/slc_cli_linux.zip'
    ...
    if retcode != 0:
        raise RuntimeError(
            f'Failed to create venv at: {venv_dir}, err: {retmsg}')
    ...
    retcode, retmsg = run_shell_cmd(
        [python_venv_exe, '-m', 'pip', 'install', '-r', f'{slc_dir}/requirements.txt'])
    if retcode != 0:
        raise RuntimeError(
            f'Failed to install slc requirements.txt, err: {retmsg}')
    ...
    if retcode != 0:
        raise RuntimeError(
            f'Failed to verify slc executable: {slc_dir}/slc, err: {retmsg}')
...
def _get_gsdk_dir() -> str:
    ...
    if not sdk_version:
        raise RuntimeError(
            f'Failed to parse {cmakeliststxt_path} for GSDK version')
