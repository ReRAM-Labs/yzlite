...
sections must have ALLOC flag and: for RAM - have WRITE flag, for ROM - not have NOBITS type.
    ...
    printing_group.add_argument('--output',
...
class Symbol:
    ...
    def extract_elf_symbols_info(cls, elf_file, readelf_exe='readelf'):
        ...
        log.info('ignored %d/%d symbols' %
                 (n_ignored, len(symbols) + n_ignored))
...
class SymbolsTreeByPath:
    ...
    def add(self, symbol):
        assert isinstance(
            symbol, Symbol), "Only instances of Symbol can be added!"
    ...
    def sort(self, key, reverse=False):
        ...
        for node in nodes_with_children:
            ...
            def path_key(
                node): return -node.cumulative_size if node.cumulative_size is not None else node.data
            ...
            symbols = sorted(symbols, key=lambda node: key(
                node.data), reverse=reverse)
    ...
    def _add_field_strings(self, protolines, indent, human_readable):
        for line in protolines:
            ...
            if line.node.is_path():
                ...
                fields = [indent_str + line.node.data.name,
                          size_str, percent_str]
            else:
                raise Exception('Wrong symbol type encountered')
    ...
    def _create_header_protolines(self, header_fmt, table_headers, sizes_dict, header):
        ...
        header_protolines = [self.Protoline(string=s) for s in [
            header, table_header, separator]]
...
ERROR: No symbols from given section found or all were ignored!
    ...
    if args.rom:
        print_tree('ROM', filter_symbols(
            lambda sec: sec and sec.occupies_rom()))

    if args.ram:
        print_tree('RAM', filter_symbols(
            lambda sec: sec and sec.occupies_ram()))
