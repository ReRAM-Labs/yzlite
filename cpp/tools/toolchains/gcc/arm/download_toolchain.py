...
def download_arm_toolchain(return_path=False):
    ...
    try:
        ...
        dest_dir = create_user_dir(
            f'tools/toolchains/gcc/arm/{url_info["version"]}')
        ...
        with open(log_file, 'w') as f:
            ...
            f.write(
                f'Error while importing YZLITE package, do you run the install_yzlite.py script first?\nError details: {e}')
    ...
    if return_path:
        if retcode != 0:
            raise RuntimeError(
                f'Failed to download ARM GCC toolchain, err: {retval}')
        return retval
    else:
        if retcode != 0:
            retval = log_file
