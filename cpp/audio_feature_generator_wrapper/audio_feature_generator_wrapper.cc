...
#define GET_SETTING(var, name, dtype)                                          \
  verify_setting(settings, name);                                              \
  var = settings[name].cast<dtype>()
...
typedef void *(*PopulateFunc)(AudioFeatureGeneratorWrapper *self,
                              const struct FrontendOutput &frontend_output,
                              void *output);
static void *populate_int8_slice(AudioFeatureGeneratorWrapper *self,
                                 const struct FrontendOutput &frontend_output,
                                 void *output);
static void *populate_uint16_slice(AudioFeatureGeneratorWrapper *self,
                                   const struct FrontendOutput &frontend_output,
                                   void *output);
static void *populate_float_slice(AudioFeatureGeneratorWrapper *self,
                                  const struct FrontendOutput &frontend_output,
                                  void *output);
static void dynamic_scale_int8_spectrogram(const uint16_t *src, int8_t *dst,
                                           int length,
                                           int dynamic_quantize_range);
...
AudioFeatureGeneratorWrapper::AudioFeatureGeneratorWrapper(
  ...
  GET_FLOAT(config.filterbank.lower_band_limit,
            "fe.filterbank_lower_band_limit");
  GET_FLOAT(config.filterbank.upper_band_limit,
            "fe.filterbank_upper_band_limit");

  GET_INT(config.noise_reduction.enable_noise_reduction,
          "fe.noise_reduction_enable");
  GET_INT(config.noise_reduction.smoothing_bits,
          "fe.noise_reduction_smoothing_bits");
  GET_FLOAT(config.noise_reduction.even_smoothing,
            "fe.noise_reduction_even_smoothing");
  GET_FLOAT(config.noise_reduction.odd_smoothing,
            "fe.noise_reduction_odd_smoothing");
  GET_FLOAT(config.noise_reduction.min_signal_remaining,
  ...
  GET_INT(config.activity_detection.enable_activation_detection,
          "fe.activity_detection_enable");
  ...
  GET_FLOAT(config.activity_detection.arm_threshold,
            "fe.activity_detection_arm_threshold");
  GET_FLOAT(config.activity_detection.trip_threshold,
            "fe.activity_detection_trip_threshold");
  GET_INT(config.dc_notch_filter.enable_dc_notch_filter,
          "fe.dc_notch_filter_enable");
  GET_FLOAT(config.dc_notch_filter.coefficient,
            "fe.dc_notch_filter_coefficient");
  ...
  GET_FLOAT(quantize_dynamic_scale_range_db,
            "fe.quantize_dynamic_scale_range_db");
...
#define LN10_DIV_20 0.11512925465
    _dynamic_quantize_range =
        (int)(quantize_dynamic_scale_range_db *
              (float)(1 << config.log_scale.scale_shift) * LN10_DIV_20);
  } else {
...
/*************************************************************************************************/
void AudioFeatureGeneratorWrapper::process_sample(
    const py::array_t<int16_t> &input, py::array &output) {
  ...
  if (input_buf.size != _sample_length) {
    throw std::invalid_argument("Input sample must contain " +
                                std::to_string(_sample_length) + " elements");
  }
  if (output_buf.ndim != 2) {
    throw std::invalid_argument("Output must be 2D array");
  }
  if (shape[0] != _n_features || shape[1] != _n_channels) {
    throw std::invalid_argument("Output must have shape" +
                                std::to_string(_n_features) + "x" +
                                std::to_string(_n_channels));
  }
  ...
  if (dtype == int8_dtype) {
    ...
    if (_dynamic_quantize_range > 0) {
      ...
      output_ptr = quantize_tmp_buffer =
          malloc(sizeof(uint16_t) * _n_features * _n_channels);
    } else {
    ...
    throw std::invalid_argument(
        "Output data type must be a int8, uint16, or float32");
  }
  ...
  for (int i = _n_features; i > 0; --i) {
    ...
    const auto frontend_output = FrontendProcessSamples(
        &_frontend_state, audio_ptr, _sample_length - samples_processed,
        &num_samples_read);
  ...
  if (quantize_tmp_buffer != nullptr) {
    dynamic_scale_int8_spectrogram(
        (const uint16_t *)quantize_tmp_buffer, (int8_t *)output_buf.ptr,
        _n_features * _n_channels, this->_dynamic_quantize_range);
...
static void verify_setting(const py::dict &settings, const std::string &name) {
  if (!settings.contains(name)) {
    throw std::invalid_argument(
        "Expected AudioFeatureGenerator setting not found: " + name);
  }
}
