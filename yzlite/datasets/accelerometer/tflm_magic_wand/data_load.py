...
# ==============================================================================
# pylint: disable=redefined-builtin,bad-indentation

"""Load data from the specified paths a
nd format them for training."""
...
class DataLoader(object):
    ...
    def format_support_func(self, padded_num, length, data, label) -> Tuple[int, tf.data.Dataset]:
        """Support function for format.(Helps format train, valid and test.)"""
        # Add 2 padding, initialize data and label
        length *= padded_num
        features = np.zeros(
            (length, self.seq_length, self.dim), dtype=np.float32)
