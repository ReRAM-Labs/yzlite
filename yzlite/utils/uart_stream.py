...
class UartStream:
    ...
    - Data flow control
    - C++ library (see <yzlite repo>/cpp/shared/uart_stream)
    ...
    def read(
        ...
        if rx_length > 0:
            with self._lock:
                ...
                logger.debug(
                    f'Read {len(retval)} bytes ({self.rx_bytes_available} still pending)')
    ...
    def write(
        ...
        if tx_length > 0:
            with self._lock:
                ...
                self._tx_active_packet_id = self._write_data_packet_header(
                    tx_length)
                time.sleep(0.001)
    ...
    def write_command(
        ...
        if payload:
            if not isinstance(payload, (bytes, bytearray)) or len(payload) > 6:
                raise ValueError(
                    'payload must be byte string no longer than 6 bytes')
    ...
    def _write_data_packet_header(self, tx_length: int) -> int:
        with self._lock:
            ...
            unused_rx_buffer_length = self._rx_buffer_length - \
                len(self._rx_buffer)
            ...
            self._tx_next_packet_id = max(
                (self._tx_next_packet_id + 1) % 128, 1)
    ...
    def _rx_loop(self):
        try:
            while not self._rx_thread_active.is_set():
                ...
                assert self.rx_bytes_available + \
                    header.data_tx_length <= self._rx_buffer_length, 'RX buffer overflow'
    ...
    def _read_packet_data(self, packet_length: int):
        ...
        while len(packet_data) < packet_length and not self._rx_thread_active.is_set():
            ...
            if max_read_length == 0:
                ...
                logger.warning(
                    f'Timed-out waiting for packet of length {packet_length}')
                return

            chunk_length = min(
                max_read_length, packet_length - len(packet_data))
...
class PacketHeader:
    ...
    def deserialize(data: bytes) -> PacketHeader:
        ...
        if header.is_cmd_packet:
            ...
            header.data_ack_id, header.data_tx_length, header.data_rx_available = struct.unpack(
                '<bhl', data[3:10])
