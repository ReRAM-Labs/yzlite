...
def build_yzlite_target(
    target: str = None,
    yzlite_target: str = None,
    ...
    Args:
        ...
        yzlite_target: Name of YZLITE_TARGET, if omitted use target
        ...
        source_dir: Path to source directory, if omitted use yzlite root directory
        build_dir: Path to build directory, if omitted use <temp dir>/yzlite/build
    ...
    yzlite_target = yzlite_target or target
    ...
    if platform not in PLATFORM_TOOLCHAIN_MAPPING:
        ...
        raise Exception(
            f'Unsupported platform {platform}, supported platforms are: {support_platforms}')
    ...
    cmake_vars = parse_variables([
        ...
        f'CMAKE_TOOLCHAIN_FILE:FILEPATH={YZLITE_ROOT_DIR}/cpp/tools/toolchains/{toolchain_file}',
        f'CMAKE_BUILD_TYPE:STRING={build_type}',
        f'YZLITE_PYTHON_VENV_DIR:FILEPATH={python_dir}',
        f'YZLITE_PLATFORM_NAME:STRING={platform}',
        f'YZLITE_TARGET:STRING={yzlite_target}'
    ])
    if not use_user_options:
        cmake_vars['YZLITE_NO_USER_OPTIONS'] = 'ON'
    if verbose:
        cmake_vars['YZLITE_CMAKE_LOG_LEVEL:STRING'] = 'debug'
    if accelerator:
    ...
    if not build_only:
        ...
        if retcode != 0:
            raise RuntimeError(
                f'Failed to configure CMake project using command:\n{cmd_str}')
    ...
    if not config_only:
        ...
        if retcode != 0:
            raise RuntimeError(
                f'Failed to build CMake project using command:\n{cmd_str}')

    return build_dir


def invoke_yzlite_target(
    target: str,
...
def parse_variables(
    ...
    if isinstance(cmake_variables, (list, str)):
        ...
        for v in cmake_variables:
            ...
            if not match:
                raise ValueError(
                    f'Invalid CMake variable: {v}, must be of form: <name>=<value>')
