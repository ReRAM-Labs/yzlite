...
class ProcessPool:
    ...
    def __init__(
        ...
        if os.environ.get('YZLITE_PROCESS_POOL_SUBPROCESS', ''):
            return
    ...
    def process(
        ...
        **kwargs
        ...
        batch = pool_batch or ProcessPoolBatch(
        ...
        while True:
            ...
            try:
                subprocess: _Subprocess = self._ready_q.get(
                    block=True, timeout=0.100)
                break
            except queue.Empty:
                continue
    ...
    def _process_thread_loop_unsafe(self):
        while True:
            ...
            if retcode is not None:
                if retcode != 0:
                    raise RuntimeError(
                        f'{self.name} terminated with error code: {retcode}')
                return
...
def calculate_n_jobs(n_jobs: Union[float, int]) -> int:
    ...
    if n_jobs == -1:
        ...
        if n_jobs < 0 or n_jobs > 1:
            raise ValueError(
                'Must either be an integer or a float in the range (0, 1.0]')
