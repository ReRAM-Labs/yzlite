...
from typing import Callable, Dict, List, TextIO, Tuple, Union
...
class SerialReader:
    ...
    def __init__(
        ...
        callback_regex: List[Tuple[re.Pattern,
                                   Callable[[re.Match], None]]] = None,
        ...
        self.ignore_chars = ignore_chars or [
            13] + list(range(127, 256))  # Non-ASCII and \r
    ...
    def open(self):
        ...
        def _map_pattern(s):
            ...
            if isinstance(s, re.Pattern):
                return s
            raise ValueError(
                f'Invalid regex: {s}, must be string or re.Pattern')

        self._start_regex = [_map_pattern(r)
                             for r in as_list(self.start_regex)]
        ...
        self._callback_regex = [_CallbackRegexContext(
            _map_pattern(r), cb) for (r, cb) in as_list(self.callback_regex)]
        self._captured_data = ''
