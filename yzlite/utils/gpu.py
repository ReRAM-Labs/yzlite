...
TensorflowCudaVersions = namedtuple('TensorflowCudaVersions', [
                                    'tensorflow', 'cudnn', 'cuda', 'python_min', 'python_max'])
...
def initialize(logger=None):
    ...
    CUDA_VISIBLE_DEVICES = get_user_setting(
        ...
        os.getenv('YZLITE_CUDA_VISIBLE_DEVICES',
            ...
            for gpu_id in gpu_ids:
                ...
                for tf_gpu in tf_gpus:
                    if tf_gpu.name.endswith(f'GPU:{gpu_id}'):
                        ...
                        logger.info(
                            f"Selecting GPU : {gpus[gpu_id].name} (id={gpu_id})")
...
def get_tensorflow_version_with_cudnn_version(cudnn_ver: str) -> TensorflowCudaVersions:
    ...
    # strip of the "patch" part of the version string
    cudnn_ver = '.'.join(toks[:2])
...
def check_tensorflow_cuda_compatibility_error(log_file_path: str) -> str:
    ...
    cuda_error_re = re.compile(
        r'.*Loaded runtime CuDNN library: (\d+\.\d+\.\d+) but source was compiled with: (\d+\.\d+\.\d+).*')
    for line in log_lines:
        match = cuda_error_re.match(line)
        if match:
            installed_cudnn_ver = match.group(1)
            expected_cudnn_ver = match.group(2)
            required_tensorflow_version = get_tensorflow_version_with_cudnn_version(
                installed_cudnn_ver)
    ...
    retval = 'There appears to be a compatibility issue with YZLITE Python venv Tensorflow version and installed GPU driver.\n'
    ...
    if required_tensorflow_version is not None:
        retval += f'{count}. Update the Tensorflow version in the YZLITE Python venv to match the installed GPU driver by running:\n'
        ...
        if not _current_python_version_supported(required_tensorflow_version):
            ...
            retval += '   2. Install the YZLITE Python package\n'
    ...
    retval += f'{count}. Update your GPU driver to match the installed Tensorflow version in the YZLITE venv, see:\n'
