...
from yzlite.utils.audio_dataset_generator import (AudioDatasetGenerator,
...
KEYWORDS = [
    Keyword('alexa',
            # In practice, the max count should be much larger (e.g. 10000)
            max_count=100,
            aliases=('ehlexa', 'eelexa', 'aalexa')
            ),
    Keyword('_unknown_',
            # In practice, the max count should be much larger (e.g. 20000)
            max_count=200,
            aliases=(
        ...
        for keyword, config_list in all_configurations.items():
            with tqdm.tqdm(desc=keyword.value.rjust(10), total=len(config_list), unit='word', position=0) as pb_inner:
                for config in config_list:
                    generator.generate(
                        config,
                        on_finished=lambda _: (
                            pb_inner.update(1), pb_outer.update(1))
                    )
