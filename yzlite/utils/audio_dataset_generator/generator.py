...
from .generator_types import (Augmentation, BackendBase, GenerationConfig,
                              Keyword, Voice, logger)
...
class AudioDatasetGenerator:
    ...
    def __init__(self, out_dir: str, n_jobs: int = 4):
        ...
        self._pending_configs: Dict[BackendBase,
                                    List[Tuple[GenerationConfig, Callable]]] = {}
    ...
    def list_voices(self, language_code: str = None, backend: str = None) -> List[Voice]:
        ...
        retval: List[Voice] = []
        for backend_name in self._get_backend_list(backend):
            retval.extend(self._backends[backend_name].list_voices(
    ...
    def list_configurations(
        ...
        for keyword in keywords:
            ...
            retval[keyword] = sorted(keyword_configs, key=lambda x: (
                x.keyword, x.voice.backend, x.voice.language_code, x.voice.name, x.rate, x.pitch))
    ...
    def count_characters(
        ...
        for keyword, config_list in config.items():
            ...
            for cfg in config_list:
                n_chars = self._backends[cfg.voice.backend].count_characters(
                    cfg)
                stats[cfg.voice.backend] = stats.get(
                    cfg.voice.backend, 0) + n_chars
    ...
    def get_summary(
        ...
        for keyword, stats in retval['characters'].items():
            ...
            for backend, count in stats.items():
                s += f'    {backend:<6s}: {format_units(count, add_space=False, precision=1)}\n'
                backend_totals[backend] = backend_totals.get(
                    backend, 0) + count
    ...
    def _process_once(self):
        ...
        for backend, configs in self._pending_configs.items():
            ...
            self._pool(
                ...
                config=config,
                out_dir=out_dir,
                _on_finished=functools.partial(
                    self._on_finished, callback=callback),
