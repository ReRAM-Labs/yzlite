...
def extract_archive(
    ...
    try:
        ...
        prepend_exception_msg(
            e, f'Failed to extract {archive_path} to {dest_dir}')
        raise
...
def gzip_directory_files(
    ...
    if regex is not None:
        ...
        def regex_func(
            _): return True  # pylint: disable-unnecessary-lambda-assignment
...
def _extractall_patool(archive_path, output_dir, patool_path=None):
    ...
    try:
        patoolib.extract_archive(
            archive_path, interactive=False, outdir=output_dir)
    except patoolib.util.PatoolError as e:
        prepend_exception_msg(
            e, f'Failed to extract archive: {archive_path} to {output_dir}')
            ...
            for member in tar_file.getmembers():
                ...
                if not _is_within_directory(output_dir, member_path):
                    raise RuntimeWarning(
                        f"Attempted path traversal in TAR file: {archive_path}, archive path {member_path} not within {output_dir}")
...
def _extractnested_archive(
    ...
    if not ext:
        raise Exception(
            f'Archive path: {archive_path} does not have a valid file extension')
    ext = '.' + ext

    tmp_dir = path.create_tempdir(
        'tmp_archives/' + os.path.basename(archive_path).replace(ext, ''))
    ...
    if extract_nested:
        ...
        ext = path.extension(archive_path)
        nested_tmp_dir = tmp_dir + '/' + \
            os.path.basename(nested_archive_path).replace(ext, '')
        extract_archive(nested_archive_path, nested_tmp_dir,
        ...
        for fn in os.listdir(tmp_dir):
            ...
            if os.path.isfile(p) or (os.path.isdir(p) and nested_src_dir is not None):
                raise Exception(
                    'Archive does not contain a single root directory')
            nested_src_dir = p
