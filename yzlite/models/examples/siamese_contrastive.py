...
.. code-block:: shell
   ...
   yzlite train siamese_contrastive-test

   # Train the model
   yzlite train siamese_contrastive

   # Evaluate the trained model .tflite model
   yzlite evaluate siamese_contrastive --tflite

   # Profile the model in the MVP hardware accelerator simulator
   yzlite profile siamese_contrastive --accelerator MVP

   # Profile the model on a physical development board
   yzlite profile siamese_contrastive --accelerator MVP --device
...
.. code-block:: shell
    ...
    >  yzlite summarize siamese_contrastive --tflite --build
...
.. code-block:: shell
   ...
   yzlite view siamese_contrastive --tflite --build
...
..  literalinclude:: ../../../../../../../yzlite/models/examples/siamese_contrastive.py
    :language: python
    :lines: 137-
...
from yzlite.core import (EvaluationResults, KerasModel, TfliteModel,
...
class ContrastiveLoss(keras.losses.Loss):
    ...
    def call(self, y_true, y_pred):
        ...
        margin_square = tf.math.square(
            tf.math.maximum(self.margin - (y_pred), 0))
    ...
    if isinstance(built_model, KerasModel):
        ...
        for input_tensor in input_tensors:
            tflite_interpreter.resize_tensor_input(
                input_tensor['index'], new_input_shape)
        tflite_interpreter.resize_tensor_input(
            output_tensor['index'], new_output_shape)
...
def visualize_custom_command(
    ...
    yzlite custom siamese_contrastive visualize --predict
    """
    ...
    if predict:
        ...
        visualize(my_model.x, my_model.y, to_show=count,
                  num_col=col, predictions=predictions, test=True)
...
if __name__ == '__main__':
    ...
    # Train the model
    # This does the same as issuing the command: yzlite train siamese_contrastive-test --clean
    train_results = yzlite_core.train_model(
        my_model, clean=True, test=test_mode_enabled)
    print(train_results)

    # Evaluate the model against the quantized .h5 (i.e. float32) model
    # This does the same as issuing the command: yzlite evaluate siamese_contrastive-test
    tflite_eval_results = yzlite_core.evaluate_model(
        my_model, verbose=True, test=test_mode_enabled)
    print(tflite_eval_results)

    # Profile the model in the simulator
    # This does the same as issuing the command: yzlite profile siamese_contrastive-test
