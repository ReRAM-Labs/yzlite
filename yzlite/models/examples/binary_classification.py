...
.. code-block:: shell
   ...
   yzlite custom binary_classification datagen_dump

   # Do a "dry run" test training of the model
   yzlite train binary_classification-test

   # Train the model
   yzlite train binary_classification

   # Evaluate the trained model .tflite model
   yzlite evaluate binary_classification --tflite

   # Profile the model in the MVP hardware accelerator simulator
   yzlite profile binary_classification --accelerator MVP

   # Profile the model on a physical development board
   yzlite profile binary_classification --accelerator MVP --device
...
.. code-block:: shell
    ...
    yzlite summarize binary_classification --tflite
...
.. code-block:: shell
   ...
   yzlite view binary_classification --tflite
...
..  literalinclude:: ../../../../../../../yzlite/models/examples/binary_classification.py
    :language: python
    :lines: 125-
...
def my_dataset_loader(model: MyModel):
    (cifar10_x_train, cifar10_y_train), (cifar10_x_test,
                                         cifar10_y_test) = cifar10.load_data()
    ...
    for x, y in zip(cifar10_x_train, cifar10_y_train):
        if y == 3:  # cat label, see https://www.cs.toronto.edu/~kriz/cifar.html
            ...
            # cat maps to id 0, see the my_model.classes above
            cats_and_dogs_y_train.append(0)
            n_cat += 1
        elif y == 5:  # dog label
...
def my_model_builder(model: MyModel):
    ...
    # Binary so we only need 1 unit for this layer
    keras_model.add(layers.Dense(1))
    # Binary so we want the activation to be between 0 and 1 which is what the sigmoid function produces
    keras_model.add(layers.Activation('sigmoid'))
...
def datagen_dump_custom_command(
    ...
    yzlite custom binary_classification datagen_dump --count 20
    """
...
if __name__ == '__main__':
    ...
    # Train the model
    # This does the same as issuing the command: yzlite train binary_classification-test --clean
    train_results = yzlite_core.train_model(
        my_model, clean=True, test=test_mode_enabled)
    print(train_results)

    # Evaluate the model against the quantized .h5 (i.e. float32) model
    # This does the same as issuing the command: yzlite evaluate binary_classification-test
    tflite_eval_results = yzlite_core.evaluate_model(
        my_model, verbose=True, test=test_mode_enabled)
    print(tflite_eval_results)

    # Profile the model in the simulator
    # This does the same as issuing the command: yzlite profile binary_classification-test
