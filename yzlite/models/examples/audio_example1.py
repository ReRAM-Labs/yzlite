...
This use the Google speech_commands dataset with the :py:class:`yzlite.core.preprocess.audio.parallel_generator.ParallelAudioDataGenerator`
...
.. code-block:: shell
   ...
   yzlite train audio_example1-test

   # Train the model
   yzlite train audio_example1

   # Evaluate the trained model .tflite model
   yzlite evaluate audio_example1 --tflite

   # Profile the model in the MVP hardware accelerator simulator
   yzlite profile audio_example1 --accelerator MVP

   # Profile the model on a physical development board
   yzlite profile audio_example1 --accelerator MVP --device

   # Run this model in the audio classifier application
   yzlite classify_audio audio_example1 --device --verbose
...
.. code-block:: shell
   ...
   yzlite profile audio_example1 --device --accelerator MVP
...
.. code-block:: shell
   ...
   yzlite view audio_example1 --tflite
...
..  literalinclude:: ../../../../../../../yzlite/models/examples/audio_example1.py
    :language: python
    :lines: 184-
...
my_model.reduce_lr_on_plateau = dict(
    monitor='accuracy',
    factor=0.25,
    # NOTE: In practice, this should be larger (e.g 15) but it will increase training times
    patience=4
)
...
my_model.early_stopping = dict(
    ...
    # NOTE: In practice, this should be larger (e.g. 45) but it will increase training times
    patience=15
)
...
my_model.datagen = ParallelAudioDataGenerator(
    ...
    rescale=None,
    # In practice, this should be larger (e.g. 4.0) but it will increase training times
    unknown_class_percentage=2,
...
def _on_training_complete(results: yzlite_core.TrainingResults):
    ...
    tflite_path = results.yzlite_model.tflite_archive_path
    h5_path = results.yzlite_model.h5_archive_path
...
if __name__ == '__main__':
    ...
    # Train the model
    # This does the same as issuing the command: yzlite train audio_example1-test --clean
    train_results = yzlite_core.train_model(
        my_model, clean=True, test=test_mode_enabled)
    print(train_results)

    # Evaluate the model against the quantized .h5 (i.e. float32) model
    # This does the same as issuing the command: yzlite evaluate audio_example1-test
    tflite_eval_results = yzlite_core.evaluate_model(
        my_model, verbose=True, test=test_mode_enabled)
    print(tflite_eval_results)

    # Profile the model in the simulator
    # This does the same as issuing the command: yzlite profile audio_example1-test
