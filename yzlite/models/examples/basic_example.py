...
.. code-block:: shell
   ...
   yzlite train basic_example-test

   # Train the model
   yzlite train basic_example

   # Evaluate the trained model .tflite model
   yzlite evaluate basic_example --tflite

   # Profile the model in the MVP hardware accelerator simulator
   yzlite profile basic_example --accelerator MVP

   # Profile the model on a physical development board
   yzlite profile basic_example --accelerator MVP --device
...
..  literalinclude:: ../../../../../../../yzlite/models/examples/basic_example.py
    :language: python
    :lines: 44-
...
# NOTE: The following uses the YZLITE mnist dataset,
#       but we could have also used: tf.keras.datasets.mnist
from yzlite.datasets.image import mnist
...
def my_dataset_loader(
    ...
    **kwargs
    ...
    if subset == 'training':
        my_model.class_counts['training'] = {my_model.classes[class_id]: count for (
            class_id, count) in enumerate(np.bincount(y_train))}
    else:
        my_model.class_counts['evaluation'] = {my_model.classes[class_id]: count for (
            class_id, count) in enumerate(np.bincount(y_test))}
...
if __name__ == '__main__':
    ...
    # Train the model
    # This does the same as issuing the command: yzlite train basic_example-test --clean
    train_results = yzlite_core.train_model(
        my_model, clean=True, test=test_mode_enabled)
    print(train_results)

    # Evaluate the model against the quantized .h5 (i.e. float32) model
    # This does the same as issuing the command: yzlite evaluate basic_example-test
    tflite_eval_results = yzlite_core.evaluate_model(
        my_model, verbose=True, test=test_mode_enabled)
    print(tflite_eval_results)

    # Profile the model in the simulator
    # This does the same as issuing the command: yzlite profile basic_example-test
