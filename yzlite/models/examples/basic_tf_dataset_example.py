...
.. code-block:: shell
   ...
   yzlite train basic_tf_dataset_example-test

   # Train the model
   yzlite train basic_tf_dataset_example

   # Evaluate the trained model .tflite model
   yzlite evaluate basic_tf_dataset_example --tflite

   # Profile the model in the MVP hardware accelerator simulator
   yzlite profile basic_tf_dataset_example --accelerator MVP

   # Profile the model on a physical development board
   yzlite profile basic_tf_dataset_example --accelerator MVP --device
...
..  literalinclude:: ../../../../../../../yzlite/models/examples/basic_tf_dataset_example.py
    :language: python
    :lines: 45-
...
from yzlite import core as yzlite_core
# For this example we use the Fashion MNIST dataset.
# NOTE: The following uses the YZLITE Fashion mnist dataset,
#       but we could have also used: tf.keras.datasets.fashion_mnist
from yzlite.datasets.image import fashion_mnist
...
def my_dataset_loader(subset: str, test: bool, **kwargs) -> tf.data.Dataset:
    ...
    Args:
        ...
        test: This is optional, it is used when invoking a training "dryrun", e.g.: yzlite train basic_tf_dataset_example-test
    ...
    if subset == 'training':
        ...
        train_dataset = _create_dataset_subset(
            x_train, y_train, subset='training')
        validation_dataset = _create_dataset_subset(
            x_validation, y_validation, subset='validation')

        return train_dataset, None, validation_dataset

    else:
        evaluation_dataset = _create_dataset_subset(
            x_test, y_test, subset='evaluation')
...
def _create_dataset_subset(x: np.ndarray, y: np.ndarray, subset: str) -> tf.data.Dataset:
    ...
    my_model.class_counts[subset] = {my_model.classes[class_id]: count for (
        class_id, count) in enumerate(np.bincount(y))}
...
##########################################################################
# Build the model
def my_model_builder(my_model: yzlite_core.YZLiteModel) -> tf.keras.Model:
...
##########################################################################
# Create the YZLiteModel instance
...
class MyModel(
    ...
    # While not required, also inherit EvaluateClassifierMixin to help will generating evaluation for our classification model
...
# Automatically generate balanced class weights for training
...
if __name__ == '__main__':
    ...
    # Train the model
    # This does the same as issuing the command: yzlite train basic_tf_dataset_example-test --clean
    train_results = yzlite_core.train_model(
        my_model, clean=True, test=test_mode_enabled)
    print(train_results)

    # Evaluate the model against the quantized .h5 (i.e. float32) model
    # This does the same as issuing the command: yzlite evaluate basic_tf_dataset_example-test
    tflite_eval_results = yzlite_core.evaluate_model(
        my_model, verbose=True, test=test_mode_enabled)
    print(tflite_eval_results)

    # Profile the model in the simulator
    # This does the same as issuing the command: yzlite profile basic_tf_dataset_example-test
