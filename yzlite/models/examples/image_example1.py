...
.. code-block:: shell
   ...
   yzlite evaluate image_example1 --tflite

   # Profile the model in the MVP hardware accelerator simulator
   yzlite profile image_example1 --accelerator MVP

   # Profile the model on a physical development board
   yzlite profile image_example1 --accelerator MVP --device

   # Use the model custom command to dump the augmented samples to ~/.yzlite/models/image_example1/datagen_dump
   yzlite custom image_example1 datagen_dump --count 20
...
.. code-block:: shell
    ...
    yzlite summarize image_example1 --tflite
...
.. code-block:: shell
   ...
   yzlite profile image_example1 --device --accelerator MVP
...
..  literalinclude:: ../../../../../../../yzlite/models/examples/image_example1.py
    :language: python
    :lines: 167-
...
# Import the dataset
from yzlite.datasets.image import rock_paper_scissors_v1
...
# Need to use float32 used using samplewise_center=True and samplewise_std_normalization=True
...
def my_model_builder(model: MyModel):
    ...
    keras_model.add(AveragePooling2D(pool_size=(
        2, 2), strides=(2, 2), padding='valid'))
    keras_model.add(Conv2D(16, strides=(2, 2), kernel_size=3,
                    use_bias=True, padding='valid', activation='relu'))
    keras_model.add(Conv2D(24, strides=(1, 1), kernel_size=3,
...
def datagen_dump_custom_command(
    ...
    my_model.datagen.save_to_dir = my_model.create_log_dir(
        'datagen_dump', delete_existing=True)
...
if __name__ == '__main__':
    ...
    # Train the model
    # This does the same as issuing the command: yzlite train image_example1-test --clean
    train_results = yzlite_core.train_model(
        my_model, clean=True, test=test_mode_enabled)
    print(train_results)

    # Evaluate the model against the quantized .h5 (i.e. float32) model
    # This does the same as issuing the command: yzlite evaluate image_example1-test
    tflite_eval_results = yzlite_core.evaluate_model(
        my_model, verbose=True, test=test_mode_enabled)
    print(tflite_eval_results)

    # Profile the model in the simulator
    # This does the same as issuing the command: yzlite profile image_example1-test
