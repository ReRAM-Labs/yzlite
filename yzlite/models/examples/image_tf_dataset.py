...
.. code-block:: shell
   ...
   yzlite train image_tf_dataset-test

   # Train the model
   yzlite train image_tf_dataset

   # Evaluate the trained model .tflite model
   yzlite evaluate image_tf_dataset --tflite

   # Profile the model in the MVP hardware accelerator simulator
   yzlite profile image_tf_dataset --accelerator MVP

   # Profile the model on a physical development board
   yzlite profile image_tf_dataset --accelerator MVP --device
...
..  literalinclude:: ../../../../../../../yzlite/models/examples/image_tf_dataset.py
    :language: python
    :lines: 46-
...
def image_augmentation(batch: np.ndarray, seed: np.ndarray) -> np.ndarray:
    ...
    for i, image_path in enumerate(batch):
        ...
        if enabled_numpy_behavior:
            x = tf.keras.preprocessing.image.random_channel_shift(
                x, .1, channel_axis=2)
            x = tf.keras.preprocessing.image.random_shear(
                x, 0.1, row_axis=0, col_axis=1, channel_axis=2)
            x = tf.keras.preprocessing.image.random_zoom(
                x, (0.90, 1.10), row_axis=0, col_axis=1, channel_axis=2)
            x = tf.keras.preprocessing.image.random_shift(
                x, .1, .1, row_axis=0, col_axis=1, channel_axis=2)
            x = tf.keras.preprocessing.image.random_rotation(
                x, 10, row_axis=0, col_axis=1, channel_axis=2)
...
class MyDataset(yzlite_core.YZLiteDataset):
    ...
    def load_dataset(
        ...
        **kwargs
        ...
        Args:
            ...
            test: This is optional, it is used when invoking a training "dryrun", e.g.: yzlite train image_tf_dataset-test
        ...
        if not self.dataset_dir:
            self.dataset_dir = download_verify_extract(
                url='https://www.yizhu.com/public/files/github/machine_learning/benchmarks/datasets/vw_coco2014_96.tar.gz',
    ...
    def _load_subset(self, subset: str, test: bool) -> tf.data.Dataset:
        ...
        ds = ds.shuffle(per_job_batch_size,
...
if __name__ == '__main__':
    ...
    # Train the model
    # This does the same as issuing the command: yzlite train image_tf_dataset-test --clean
    train_results = yzlite_core.train_model(
        my_model, clean=True, test=test_mode_enabled)
    print(train_results)

    # Evaluate the model against the quantized .h5 (i.e. float32) model
    # This does the same as issuing the command: yzlite evaluate image_tf_dataset-test
    tflite_eval_results = yzlite_core.evaluate_model(
        my_model, verbose=True, test=test_mode_enabled)
    print(tflite_eval_results)

    # Profile the model in the simulator
    # This does the same as issuing the command: yzlite profile image_tf_dataset-test
