...
.. code-block:: shell
   ...
   yzlite train anomaly_detection-test

   # Train the model
   yzlite train anomaly_detection

   # Evaluate the trained model .tflite model
   yzlite evaluate anomaly_detection --tflite

   # Profile the model in the MVP hardware accelerator simulator
   yzlite profile anomaly_detection --accelerator MVP

   # Profile the model on a physical development board
   yzlite profile anomaly_detection --accelerator MVP --device
...
.. code-block:: shell
    ...
    yzlite summarize anomaly_detection --tflite
...
.. code-block:: shell
   ...
   yzlite profile anomaly_detection --device --accelerator MVP
...
.. code-block:: shell
   ...
   yzlite view anomaly_detection --tflite
...
..  literalinclude:: ../../../../../../../yzlite/models/tinyml/anomaly_detection.py
    :language: python
    :lines: 218-
...
from yzlite.core.model import (EvaluateAutoEncoderMixin, ImageDatasetMixin,
...
from yzlite.models.shared import FullyConnectedAutoEncoder
...
def reshape_input_callback(params: ParallelProcessParams, x: np.ndarray):
    ...
    if params.subset == 'training':
        ...
        right_offset = 50
        start = left_offset + \
            rnd.randrange((x_length - input_height+1) -
...
if __name__ == '__main__':
    ...
    # Train the model
    # This does the same as issuing the command: yzlite train anomaly_detection-test --clean
    train_results = yzlite_core.train_model(
        my_model, clean=True, test=test_mode_enabled)
    print(train_results)

    # Evaluate the model against the quantized .h5 (i.e. float32) model
    # This does the same as issuing the command: yzlite evaluate anomaly_detection-test
    tflite_eval_results = yzlite_core.evaluate_model(
        my_model, verbose=True, test=test_mode_enabled)
    print(tflite_eval_results)

    # Profile the model in the simulator
    # This does the same as issuing the command: yzlite profile anomaly_detection-test
