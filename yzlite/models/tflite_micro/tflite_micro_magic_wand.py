...
.. code-block:: shell
   ...
   yzlite train tflite_micro_magic_wand-test

   # Train the model
   yzlite train tflite_micro_magic_wand

   # Evaluate the trained model .tflite model
   yzlite evaluate tflite_micro_magic_wand --tflite

   # Profile the model in the MVP hardware accelerator simulator
   yzlite profile tflite_micro_magic_wand --accelerator MVP

   # Profile the model on a physical development board
   yzlite profile tflite_micro_magic_wand --accelerator MVP --device
...
.. code-block:: shell
    ...
    yzlite summarize tflite_micro_magic_wand --tflite
...
.. code-block:: shell
   ...
   yzlite profile tflite_micro_magic_wand --device --accelerator MVP
...
.. code-block:: shell
   ...
   yzlite view tflite_micro_magic_wand --tflite
...
..  literalinclude:: ../../../../../../../yzlite/models/tflite_micro/tflite_micro_magic_wand.py
    :language: python
    :lines: 153-
...
from yzlite.core.model import (DatasetMixin, EvaluateClassifierMixin,
                               TrainMixin, YZLiteModel)
from yzlite.datasets.accelerometer import \
    tflm_magic_wand as tflm_magic_wand_dataset
...
class MyModel(
    ...
    def load_dataset(
        ...
        **kwargs
        ...
        self._data_loader = tflm_magic_wand_dataset.load_data(
...
def my_model_builder(model: MyModel):
    keras_model = tf.keras.Sequential([
        ...
        tf.keras.layers.MaxPool2D(
            (3, 1), padding="same"),  # (batch, 14, 1, 16)
        tf.keras.layers.Dropout(0.1),  # (batch, 14, 1, 16)
        tf.keras.layers.Flatten(),  # (batch, 224)
        tf.keras.layers.Dense(16, activation="relu"),  # (batch, 16)
        tf.keras.layers.Dropout(0.1),  # (batch, 16)
        tf.keras.layers.Dense(
            model.n_classes, activation="softmax")  # (batch, 4)
    ], name=model.name)
    keras_model.compile(
        loss=model.loss,
...
if __name__ == '__main__':
    ...
    # Train the model
    # This does the same as issuing the command: yzlite train tflite_micro_magic_wand-test --clean
    train_results = yzlite_core.train_model(
        my_model, clean=True, test=test_mode_enabled)
    print(train_results)

    # Evaluate the model against the quantized .h5 (i.e. float32) model
    # This does the same as issuing the command: yzlite evaluate tflite_micro_magic_wand-test
    tflite_eval_results = yzlite_core.evaluate_model(
        my_model, verbose=True, test=test_mode_enabled)
    print(tflite_eval_results)

    # Profile the model in the simulator
    # This does the same as issuing the command: yzlite profile tflite_micro_magic_wand-test
