...
.. code-block:: shell
    ...
    yzlite train keyword_spotting_on_off_v2
...
This uses the :py:class:`yzlite.datasets.audio.speech_commands.speech_commands_v2` dataset provided by Google.
...
This uses the :py:class:`yzlite.core.preprocess.audio.audio_feature_generator.AudioFeatureGenerator` with the
:py:class:`yzlite.core.preprocess.audio.audio_feature_generator.AudioFeatureGenerator` settings:
...
.. code-block:: shell
   ...
   export TRAIN_TEACHER=1
   yzlite train keyword_spotting_on_off_v2-test
   ...
   yzlite evaluate keyword_spotting_on_off_v2 --tflite

   # Profile the model in the MVP hardware accelerator simulator
   yzlite profile keyword_spotting_on_off_v2 --accelerator MVP

   # Profile the model on a physical development board
   yzlite profile keyword_spotting_on_off_v2  --accelerator MVP --device

   # Run the model in the audio classifier on the local PC
   yzlite classify_audio keyword_spotting_on_off_v2 --verbose

   # Run the model in the audio classifier on the physical device
   yzlite classify_audio keyword_spotting_on_off_v2 --device --verbose
...
.. code-block:: shell
    ...
    yzlite summarize keyword_spotting_on_off_v2 --tflite
...
.. code-block:: shell
   ...
   yzlite profile keyword_spotting_on_off_v2 --device --accelerator MVP
...
.. code-block:: shell
   ...
   yzlite view keyword_spotting_on_off_v2 --tflite
...
..  literalinclude:: ../../../../../../../yzlite/models/yizhutech/keyword_spotting_on_off_v2.py
    :language: python
    :lines: 320-
...
from yzlite.core.keras.models import KnowledgeDistillationModel
...
from yzlite.core.preprocess.utils import split_file_list
...
from yzlite.datasets.audio.speech_commands import speech_commands_v2
...
def my_teacher_model_builder(model: MyModel) -> tf.keras.Model:
    ...
    This is called by the YZLITE just before "teacher" training starts.
    See https://keras.io/examples/vision/knowledge_distillation/ for more details.

    Arguments:
        my_model: The YZLiteModel instance
    ...
    keras_model.add(tf.keras.layers.Conv2D(filters, (3, 3),
    ...
    keras_model.add(tf.keras.layers.Dense(
        model.n_classes, activation='softmax'))
...
def my_teacher_model_saver(
    yzlite_model: yzlite_core.YZLiteModel,
...
def my_student_model_builder(model: MyModel) -> tf.keras.Model:
    ...
    This is called by the YZLITE just before training starts.
    ...
    distiller = KnowledgeDistillationModel(
        student=keras_model, teacher=teacher_model)
    distiller.compile(
        optimizer='adam',
        metrics=['accuracy'],
...
def my_student_model_saver(
    ...
    shutil.copy(teacher_h5_path,
                f'{os.path.dirname(h5_path)}/{os.path.basename(teacher_h5_path)}')
            ...
            audio_augmentations = audiomentations.Compose(
                ...
                transforms=[
                    ...
                    audiomentations.TimeStretch(
                        min_rate=0.90, max_rate=1.1, p=1.0),
...
class MyDataset(yzlite_core.YZLiteDataset):
    ...
    def load_subset(self, subset: str, test: bool) -> tf.data.Dataset:
        ...
        def _add_additional_unknown_samples(
            ...
            **kwargs
            ...
            paths = sorted(os.listdir(
                additional_unknown_dataset_dir + '/unknown'))
            if shuffle:
        ...
        features_ds, labels_ds = tf_dataset_utils.load_audio_directory(
            ...
            # We want to shuffle ALL of the synthetic samples with the speech_commands samples BEFORE splitting the data into subsets
            shuffle_index_directory=f'{my_model.log_dir}/dataset',
...
def prepare_teacher_or_student_model(train_teacher: bool = None):
    ...
    if train_teacher:
        ...
        my_model.ssh_download_files = [
            f'~/.yzlite/models/{my_model.name}/{os.path.basename(my_model.h5_log_dir_path)}|{os.path.basename(teacher_h5_path)}']
...
if __name__ == '__main__':
    ...
    # Train the "teacher" model
    # This does the same as issuing the command: export TRAIN_TEACHER=1 && yzlite train keyword_spotting_on_off_v2-test --clean
    ...
    # Train the "student" model
    # This does the same as issuing the command: export TRAIN_TEACHER=0 && yzlite train keyword_spotting_on_off_v2-test --clean
    ...
    # This does the same as issuing the command: yzlite evaluate keyword_spotting_on_off_v2-test
    tflite_eval_results = yzlite_core.evaluate_model(
        my_model, verbose=True, test=test_mode_enabled)
    print(tflite_eval_results)

    # Profile the model in the simulator
    # This does the same as issuing the command: yzlite profile keyword_spotting_on_off_v2-test
