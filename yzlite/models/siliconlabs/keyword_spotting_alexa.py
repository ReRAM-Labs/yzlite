...
Preprocessing
--------------
This uses the :py:class:`yzlite.core.preprocess.audio.audio_feature_generator.AudioFeatureGenerator` to generate spectrograms with the settings:
...
.. code-block:: shell
   ...
   yzlite train keyword_spotting_alexa-test

   # Train the model
   yzlite train keyword_spotting_alexa

   # Evaluate the trained model .tflite model
   yzlite evaluate keyword_spotting_alexa --tflite

   # Profile the model in the MVP hardware accelerator simulator
   yzlite profile keyword_spotting_alexa --accelerator MVP

   # Profile the model on a physical development board
   yzlite profile keyword_spotting_alexa  --accelerator MVP --device

   # Run the model in the audio classifier on the local PC
   yzlite classify_audio keyword_spotting_alexa --verbose

   # Run the model in the audio classifier on the physical device feature an MVP hardware accelerator
   yzlite classify_audio keyword_spotting_alexa --device  --accelerator MVP --verbose
...
.. code-block:: shell
    ...
    yzlite summarize keyword_spotting_alexa --tflite
...
.. code-block:: shell
   ...
   yzlite profile keyword_spotting_alexa --device --accelerator MVP
...
.. code-block:: shell
   ...
   yzlite view keyword_spotting_alexa --tflite
...
..  literalinclude:: ../../../../../../../yzlite/models/yizhutech/keyword_spotting_alexa.py
    :language: python
    :lines: 399-
...
from yzlite.core.keras.callbacks import SteppedLearnRateScheduler
# Import the AudioFeatureGeneratorSettings which we'll configure
from yzlite.core.preprocess.audio.audio_feature_generator import \
    AudioFeatureGeneratorSettings
from yzlite.core.preprocess.utils import audio as audio_utils
from yzlite.core.preprocess.utils import image as image_utils
from yzlite.core.preprocess.utils import (shuffle_file_list_by_group,
                                          split_file_list)
from yzlite.core.preprocess.utils import tf_dataset as tf_dataset_utils
from yzlite.models.shared import tenet
from yzlite.utils.archive_downloader import (download_url,
                                             download_verify_extract)
from yzlite.utils.path import create_user_dir
from yzlite.utils.python import install_pip_package

##########################################################################################
# Instantiate the YZLiteModel instance
#
...
class MyModel(
    ...
    # We also inherit the TrainMixin since we want to train this model
    yzlite_core.TrainMixin,
    # We also need the DatasetMixin mixin to provide the relevant dataset properties
    yzlite_core.DatasetMixin,
    # While not required, also inherit EvaluateClassifierMixin to help will generating evaluation stats for our classification model
    yzlite_core.EvaluateClassifierMixin,
    yzlite_core.SshMixin,
):
    pass
...
# NOTE: The version is optional and not used directly used by the YZLITE
my_model.version = 1
...
# The YZLITE enables the tf.keras.callbacks.ModelCheckpoint by default.
...
# Use 1.2s audio clips to ensure the full "alexa" keyword is captured
...
# The dev board mic seems to have a lot of noise at lower frequencies
frontend_settings.filterbank_lower_band_limit = 125.0

# Enable the noise reduction block to help ignore background noise in the field
...
# This value is fairly large (which makes the background noise reduction small)
...
# Enable the DC notch filter, to help remove the DC signal from the dev board's mic
...
# This controls how many more "unknown" samples there are relative to the "known" samples
unknown_class_multiplier = 1.5
...
def audio_pipeline_with_augmentations(
    ...
    for i, (audio_path, labels, unknown_sample) in enumerate(zip(path_batch, label_batch, unknown_samples_batch)):
        ...
        if not using_silence_as_unknown:
            ...
            try:
                sample, original_sample_rate = audio_utils.read_audio_file(
                    audio_path, return_numpy=True, return_sample_rate=True)
        ...
        padded_sample_length = int(
            (original_sample_rate * padded_frontend_settings.sample_length_ms) / 1000)
        ...
        if use_cropped_sample_as_unknown:
            ...
            trimmed_sample_length = min(
                len(trimmed_sample), padded_sample_length)
            cropped_sample_percent = np.random.uniform(.2, .5)
            cropped_sample_length = int(
                trimmed_sample_length * cropped_sample_percent)
            ...
            out_length = int(
                (original_sample_rate * frontend_settings.sample_length_ms) / 1000)
            ...
            audio_augmentations = audiomentations.Compose(
                ...
                transforms=[
                    audiomentations.Gain(
                        min_gain_in_db=0.95, max_gain_in_db=1.5, p=1.0),
                    ...
                    audiomentations.AddGaussianSNR(
                        min_snr_in_db=30, max_snr_in_db=60, p=0.25),
                ])
            globals()['audio_augmentations'] = audio_augmentations

        # Apply random augmentations to the audio sample
        augmented_sample = audio_augmentations(
            padded_sample, original_sample_rate)
        ...
        if data_dump_dir:
            ...
            spectrogram_dumped = cv2.resize(
                spectrogram_dumped, (height*3, width*3))

            valid_sample_length = int(
                (frontend_settings.sample_length_ms * frontend_settings.sample_rate_hz) / 1000)
...
##########################################################################################
# Define the YZLiteDataset object
# NOTE: This class is optional but is useful for organizing the code
#
class MyDataset(yzlite_core.YZLiteDataset):
    ...
    def summarize_dataset(self) -> str:
        ...
        s = self.summary
        s += yzlite_core.YZLiteDataset.summarize_class_counts(
    ...
    def load_dataset(
        ...
        **kwargs
        ...
        This is called automatically by the YZLITE before training
        or evaluation.

        Args:
            subset: The dataset subset to return: 'training' or 'evaluation'
            test: This is optional, it is used when invoking a training "dryrun", e.g.: yzlite train audio_tf_dataset-test
    ...
    def load_subset(self, subset: str, test: bool) -> tf.data.Dataset:
        ...
        for url in URLS:
            ...
            if not os.path.exists(dst_path):
                ...
                audio_utils.write_audio_file(
                    dst_path, sample, sample_rate=16000)
        ...
        features_ds, labels_ds = tf_dataset_utils.load_audio_directory(
            ...
            # We manually populate the "known" class in the add_unknown_samples() callback
        ...
        unknown_samples_ds = tf.data.Dataset.from_tensor_slices(
            self.all_unknown_samples)
        unknown_samples_ds = unknown_samples_ds.shuffle(
            max(len(self.all_unknown_samples), 10000), reshuffle_each_iteration=True)
        ...
        if subset:
            ...
            features_ds = features_ds.zip(
                (features_ds, labels_ds, unknown_samples_ds, seed_counter))
            ...
            features_ds = features_ds.batch(
                per_job_batch_size // per_job_batch_multiplier, drop_remainder=True)
            ...
            features_ds, pool = tf_dataset_utils.parallel_process(
                ...
                # These are the settings for a 128 CPU core cloud machine
                n_jobs=72 if subset == 'training' else 32,
    ...
    def list_valid_filenames_in_directory(
        ...
        if os.path.exists(index_path):
            ...
            for root, _, files in os.walk(base_directory, followlinks=follow_links):
                ...
                file_list = shuffle_file_list_by_group(
                    file_list, get_sample_group_id_from_path)

                # Write the file list file
                yzlite_core.get_yzlite_logger().info(
                    f'Generating index for "{search_class}" ({len(file_list)} samples): {index_path}')
    ...
    def add_unknown_samples(
        self,
        directory: str,
        # A dictionary: <class name>, [<sample paths relative to directory>],
        ...
        **kwargs
        ...
        file_list = list([f'_unknown/unknown/{x}' for x in os.listdir(
            unknown_dir) if x.endswith('.wav') and os.path.getsize(f'{unknown_dir}/{x}') > 0])
        ...
        for key, value in sample_paths.items():
            if key != '_unknown_':
                sample_paths['_unknown_'] = [''] * \
                    int(len(value) * unknown_class_multiplier)
                break
...
def get_sample_group_id_from_path(p: str) -> str:
    ...
    # If this sample is from a yizhu synthetic dataset
...
my_model.model_parameters['average_window_duration_ms'] = int(
    my_model.model_parameters['latency_ms']*my_model.model_parameters['minimum_count']*1.1)
...
if __name__ == '__main__':
    ...
    # Train the model
    # This does the same as issuing the command:  yzlite train keyword_spotting_alexa-test --clean)
    train_results = yzlite_core.train_model(
        my_model, clean=True, test=test_mode_enabled)
    print(train_results)

    # Evaluate the model against the quantized .h5 (i.e. float32) model
    # This does the same as issuing the command: yzlite evaluate keyword_spotting_alexa-test
    tflite_eval_results = yzlite_core.evaluate_model(
        my_model, verbose=True, test=test_mode_enabled)
    print(tflite_eval_results)

    # Profile the model in the simulator
    # This does the same as issuing the command: yzlite profile keyword_spotting_alexa-test
    profiling_results = yzlite_core.profile_model(
        my_model, test=test_mode_enabled)
