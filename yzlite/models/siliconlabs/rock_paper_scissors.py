...
.. code-block:: shell
   ...
   yzlite train rock_paper_scissors-test

   # Train the model
   yzlite train rock_paper_scissors

   # Evaluate the trained model .tflite model
   yzlite evaluate rock_paper_scissors --tflite

   # Profile the model in the MVP hardware accelerator simulator
   yzlite profile rock_paper_scissors --accelerator MVP

   # Profile the model on a physical development board
   yzlite profile rock_paper_scissors --accelerator MVP --device

   # Dump some of the augmented images
   yzlite custom rock_paper_scissors dump --count 100

   # Run this model in the image classifier application
   yzlite classify_image rock_paper_scissors --dump-images
...
.. code-block:: shell
    ...
    yzlite summarize rock_paper_scissors --tflite
...
.. code-block:: shell
   ...
   yzlite profile rock_paper_scissors --device --accelerator MVP
...
.. code-block:: shell
   ...
   yzlite view  rock_paper_scissors --tflite
...
..  literalinclude:: ../../../../../../../yzlite/models/yizhutech/rock_paper_scissors.py
    :language: python
    :lines: 173-
...
from yzlite.core.preprocess.image.parallel_generator import \
    ParallelImageDataGenerator
# from keras.preprocessing.image import ImageDataGenerator
# Import the dataset
from yzlite.datasets.image import rock_paper_scissors_v2
...
# yzlite classify_image rock_paper_scissors
...
def my_model_builder(model: MyModel):
    ...
    # This should be the same size at the previous Conv2D layer count
...
def dump_custom_command(
    ...
    yzlite custom rock_paper_scissors dump --count 20
    """

    my_model.datagen.save_to_dir = my_model.create_log_dir(
        'dump', delete_existing=True)
...
if __name__ == '__main__':
    ...
    # Train the model
    # This does the same as issuing the command: yzlite train rock_paper_scissors-test --clean
    train_results = yzlite_core.train_model(
        my_model, clean=True, test=test_mode_enabled)
    print(train_results)

    # Evaluate the model against the quantized .h5 (i.e. float32) model
    # This does the same as issuing the command: yzlite evaluate rock_paper_scissors-test
    tflite_eval_results = yzlite_core.evaluate_model(
        my_model, verbose=True, test=test_mode_enabled)
    print(tflite_eval_results)

    # Profile the model in the simulator
    # This does the same as issuing the command: yzlite profile rock_paper_scissors-test
