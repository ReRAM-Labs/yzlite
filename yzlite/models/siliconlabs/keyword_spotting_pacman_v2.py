...
- Source code: `keyword_spotting_pacman_v2.py <https://github.com/chenxingqiang/yzlite/blob/master/yzlite/models/yizhutech/keyword_spotting_pacman_v2.py>`_
- Pre-trained model: `keyword_spotting_pacman_v2.yzlite.zip <https://github.com/chenxingqiang/yzlite/raw/master/yzlite/models/yizhutech/keyword_spotting_pacman_v2.yzlite.zip>`_
...
.. code-block:: shell
    ...
    yzlite train keyword_spotting_pacman_v2
...
The following changes have been made from the original `keyword_spotting_pacman <https://github.com/chenxingqiang/yzlite/docs/python_api/models/yizhutech/keyword_spotting_pacman.html>`_ model:
...
.. code-block:: shell
   ...
   export TRAIN_TEACHER=1
   yzlite train keyword_spotting_pacman_v2-test
   ...
   yzlite evaluate keyword_spotting_pacman_v2 --tflite

   # Profile the model in the MVP hardware accelerator simulator
   yzlite profile keyword_spotting_pacman_v2 --accelerator MVP

   # Profile the model on a physical development board
   yzlite profile keyword_spotting_pacman_v2  --accelerator MVP --device

   # Run the model in the audio classifier on the local PC
   yzlite classify_audio keyword_spotting_pacman_v2 --verbose

   # Run the model in the audio classifier on the physical device
   yzlite classify_audio keyword_spotting_pacman_v2 --device --verbose
...
.. code-block:: shell
    ...
    yzlite summarize keyword_spotting_pacman_v2 --tflite
...
.. code-block:: shell
   ...
   yzlite profile keyword_spotting_pacman_v2 --device --accelerator MVP
...
.. code-block:: shell
   ...
   yzlite view keyword_spotting_pacman_v2 --tflite
...
..  literalinclude:: ../../../../../../../yzlite/models/yizhutech/keyword_spotting_pacman_v2.py
    :language: python
    :lines: 320-
...
def audio_augmentation_pipeline(
    ...
    for i, (audio_path, labels) in enumerate(zip(path_batch, label_batch)):
        ...
        if use_cropped_sample_as_unknown:
            ...
            padded_sample[cropped_sample_offset:cropped_sample_offset +
                          trimmed_sample_length] += trimmed_sample
        else:
...
class MyDataset(yzlite_core.YZLiteDataset):
    ...
    def load_subset(self, subset: str, test: bool) -> tf.data.Dataset:
        ...
        os.makedirs(os.path.dirname(
            pacman_background_noise_dst_path), exist_ok=True)
        if not os.path.exists(pacman_background_noise_dst_path):
            pacman_background_noise_dir = download_verify_extract(
                url='https://github.com/chenxingqiang/yzlite_assets/raw/master/datasets/recorded_pacman_game_play.7z',
            ...
            shutil.copy(pacman_background_noise_src_path,
                        pacman_background_noise_dst_path)
        ...
        os.makedirs(os.path.dirname(
            brd2601_background_noise_dst_path), exist_ok=True)
        if not os.path.exists(brd2601_background_noise_dst_path):
            ...
            shutil.copy(background_noise_src_path,
                        brd2601_background_noise_dst_path)
...
my_model.model_parameters['detection_threshold_list'] = list(
    map(lambda x: int(x*255), [.95, .95, .94, .94, .99, .99, 1.0]))
...
if __name__ == '__main__':
    ...
    # Train the "teacher" model
    # This does the same as issuing the command: export TRAIN_TEACHER=1 && yzlite train keyword_spotting_pacman_v2-test --clean
    ...
    # Train the "student" model
    # This does the same as issuing the command: export TRAIN_TEACHER=0 && yzlite train keyword_spotting_pacman_v2-test --clean
    ...
    # This does the same as issuing the command: yzlite evaluate keyword_spotting_pacman_v2-test
    tflite_eval_results = yzlite_core.evaluate_model(
        my_model, verbose=True, test=test_mode_enabled)
    print(tflite_eval_results)

    # Profile the model in the simulator
    # This does the same as issuing the command: yzlite profile keyword_spotting_pacman_v2-test
