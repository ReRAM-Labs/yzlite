...
.. code-block:: shell
   ...
   yzlite train keyword_spotting_mobilenetv2-test

   # Train the model
   yzlite train keyword_spotting_mobilenetv2

   # Evaluate the trained model .tflite model
   yzlite evaluate keyword_spotting_mobilenetv2 --tflite

   # Profile the model in the MVP hardware accelerator simulator
   yzlite profile keyword_spotting_mobilenetv2 --accelerator MVP

   # Profile the model on a physical development board
   yzlite profile keyword_spotting_mobilenetv2 --accelerator MVP --device

   # Run the model in the audio classifier on the local PC
   yzlite classify_audio keyword_spotting_mobilenetv2 --verbose

   # Run the model in the audio classifier on the physical device
   yzlite classify_audio keyword_spotting_mobilenetv2 --device --verbose
...
.. code-block:: shell
    ...
    yzlite summarize keyword_spotting_mobilenetv2 --tflite
...
.. code-block:: shell
   ...
   yzlite profile keyword_spotting_mobilenetv2 --device --accelerator MVP
...
.. code-block:: shell
   ...
   yzlite view keyword_spotting_mobilenetv2 --tflite
...
..  literalinclude:: ../../../../../../../yzlite/models/yizhutech/keyword_spotting_mobilenetv2.py
    :language: python
    :lines: 445-
...
my_model.classes = ['left', 'right', 'up', 'down',
...
# We use a 1.2s buffer to ensure we can process a sample multiple times
...
def get_batches_samples(
    ...
    for i, filename in enumerate(filenames):
        ...
        if params.subset != 'validation' or params.audio_data_generator.validation_augmentation_enabled:
            ...
            use_cropped_sample_as_unknown = len(
                non_unknown_class_indices) > 0 and classes[i] == unknown_class_id and random.randint(0, 1) == 1
        ...
        x = params.audio_data_generator.apply_transform(
            x, orignal_sr, transform_params)
        ...
        if use_cropped_sample_as_unknown:
            ...
            bg_noise = random.choice(
                [*params.audio_data_generator.bg_noises.values()])
            bg_noise = params.audio_data_generator.adjust_length(
                bg_noise, params.sample_rate, offset=bg_noise_offset, out_length=x_len)
            x = bg_noise * \
                (transform_params['bg_noise_factor']
                 * transform_params['loudness_factor'])
            ...
            sample_data, _ = librosa.load(
                filepath, sr=params.sample_rate, mono=True, dtype='float32')
        ...
        if params.frontend_enabled:
            ...
            x = params.audio_data_generator.apply_frontend(
                x, dtype=params.dtype)
    ...
    if params.class_mode == 'input':
        ...
        batch_y = np.zeros(
            (len(batch_x), len(params.class_indices)), dtype=params.dtype)
        for i, class_id in enumerate(classes):
...
if __name__ == '__main__':
    ...
    # Train the model
    # This does the same as issuing the command: yzlite train keyword_spotting_mobilenetv2-test --clean
    train_results = yzlite_core.train_model(
        my_model, clean=True, test=test_mode_enabled)
    print(train_results)

    # Evaluate the model against the quantized .h5 (i.e. float32) model
    # This does the same as issuing the command: yzlite evaluate keyword_spotting_mobilenetv2-test
    tflite_eval_results = yzlite_core.evaluate_model(
        my_model, verbose=True, test=test_mode_enabled)
    print(tflite_eval_results)

    # Profile the model in the simulator
    # This does the same as issuing the command: yzlite profile keyword_spotting_mobilenetv2-test
