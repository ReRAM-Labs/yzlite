...
from yzlite.core.training_results import TrainingResults
from yzlite.utils.python import DefaultDict

from ..model_attributes import (CallableType, DictType,
...
class TrainMixin(BaseMixin):
    ...
    def build_model_function(self) -> Callable:
        """Function that builds and returns a compiled :py:attr:`yzlite.core.KerasModel` instance
        ...
        .. code-block:: python
           ...
           # Create a YZLiteModel instance with the 'train' mixin
           class MyModel(
               YZLiteModel, 
               TrainMixin, 
               ImageDatasetMixin, 
           ...
           def my_model_builder(yzlite_model):
               keras_model = Sequential()
               keras_model.add(Conv2D(8, kernel_size=(3,3), padding='valid', input_shape=yzlite_model.input_shape))
               keras_model.add(Flatten())
               keras_model.add(Dense(yzlite_model.n_classes, activation='softmax'))

               keras_model.compile(loss=yzlite_model.loss, optimizer=yzlite_model.optimizer, metrics=yzlite_model.metrics)

               return keras_model

           # Set the YZLiteModel's build_model function
    ...
    def on_save_keras_model(self) -> Callable[[object, KerasModel, logging.Logger], KerasModel]:
        ...
        .. code-block:: python
           ...
           def _on_save_keras_model(yzlite_model:YZLiteModel, keras_model:KerasModel, logger:logging.Logger) -> KerasModel:
               ...
               return keras_model
    ...
    def tensorboard(self) -> dict:
        ...
        * Sampled profiling
        ...
        .. note::
           ...
           * Set to ``None`` to disable this callback
           * Tensorboard logs are saved to :py:attr:`yzlite.core.YZLiteModel.log_dir`/train/tensorboard
    ...
    def checkpoint(self) -> dict:
        ...
        .. note::
           ...
           * Set to ``None`` to disable this callback
           * Tensorboard logs are saved to `YZLiteModel.log_dir`/train/weights
    ...
    def _register_attributes(self):
        self._attributes.register(
            'train.build_model_function', dtype=CallableType)
        ...
        self._attributes.register(
            'train.lr_schedule', dtype=(DictType, CallableType))
        ...
        self._attributes.register(
            'train.on_training_complete', dtype=CallableType)
        self._attributes.register(
            'train.on_save_keras_model', dtype=CallableType)
        self._attributes.register('train.kwargs', dtype=dict)


def _build_model_placeholder(yzlite_model):
    raise NotImplementedError(
        'You model must define the property "yzlite_model.build_model_function" which '
