...
from ..model_attributes import CallableType, YZLiteModelAttributesDecorator
...
class DatasetMixin(BaseMixin):
    ...
    def loaded_subset(self, subset: str):
        if subset is not None:
            ...
            if subset not in ('training', 'validation', 'evaluation'):
                raise Exception(
                    'Dataset subset must be one of: training, validation, evaluation')

        self._attributes['dataset.loaded_subset'] = subset

    @property
    def dataset(self) -> Union[types.ModuleType, Callable, str, YZLiteDataset]:
        """The model dataset
        ...
        - A reference to :py:class:`yzlite.core.YZLiteDataset` instance
        ...
        For instance, the :py:class:`yzlite.core.AudioDatasetMixin` and :py:class:`yzlite.core.ImageDatasetMixin` override this property.
        Refer to their documentation for more details.
        """
        return self._attributes.get_value('dataset.dataset', default=None)

    @dataset.setter
    def dataset(self, v: Union[types.ModuleType, Callable, str, YZLiteDataset]):
    ...
    def load_dataset(
        ...
        **kwargs
        ...
        This is automatically invoked by the YZLITE during the train, quantize and evaluation operations.
        ...
        - Your YZLiteModel class definition
        - Another YZLITE model mixin such as :py:class:`yzlite.core.AudioDatasetMixin` or :py:class:`yzlite.core.ImageDatasetMixin`
        ...
        self.trigger_event(YZLiteModelEvent.BEFORE_LOAD_DATASET,
                           subset=subset, test=test, **kwargs)
        retval = None

        if isinstance(dataset, YZLiteDataset):
            retval = dataset.load_dataset(
                subset=subset, test=test, yzlite_model=self, **kwargs)

        elif callable(dataset):
            retval = dataset(subset=subset, test=test,
                             yzlite_model=self, **kwargs)

        elif isinstance(dataset, (types.ModuleType, object)):
            if not hasattr(dataset, 'load_dataset'):
                raise Exception(
                    'If a module or class is set in yzlite_model.dataset, the the module/class must specify the function: load_dataset(subset:str, test:bool, **kwargs)')
            ...
            try:
                ...
                prepend_exception_msg(
                    e, f'Exception while invoking yzlite_model.dataset.load_dataset(): {dataset}')
                raise

        else:
            raise RuntimeError(
                'yzlite_model.dataset must either be one of:\n'
                '- YZLiteDataset instance\n'
        ...
        self.trigger_event(YZLiteModelEvent.AFTER_LOAD_DATASET,
                           subset=subset, test=test, **kwargs)
    ...
    def unload_dataset(self):
        ...
        dataset = self.dataset
        self.trigger_event(YZLiteModelEvent.BEFORE_UNLOAD_DATASET)

        if dataset is None or callable(dataset):
            pass

        elif isinstance(dataset, YZLiteDataset):
            ...
            try:
                ...
                prepend_exception_msg(
                    e, f'Exception while invoking yzlite_model.dataset.unload_dataset(): {dataset}')
                raise

        self.trigger_event(YZLiteModelEvent.AFTER_UNLOAD_DATASET)
    ...
    def summarize_dataset(self) -> str:
        ...
        if dataset is None or callable(dataset):
            ...
            try:
                ...
                prepend_exception_msg(
                    e, f'Exception while invoking yzlite_model.dataset.summarize_dataset(): {dataset}')
                raise

        class_counts = self.class_counts
        if not summary and class_counts:
            summary += YZLiteDataset.summarize_class_counts(class_counts)

        summary_dict = dict(value=summary)
        self.trigger_event(
            YZLiteModelEvent.SUMMARIZE_DATASET,
            summary=summary,
    ...
    def _register_attributes(self):
        self._attributes.register('dataset.dataset', dtype=(
            types.ModuleType, str, CallableType, object, YZLiteDataset))
        ...
        self._attributes.register(
            'dataset.class_weights', dtype=(str, dict, list))
...
class YZLiteDataset:
    ...
    Refer to :py:class:`yzlite.core.DatasetMixin` for more details.
    """
    ...
    def load_dataset(
        ...
        **kwargs
        ...
        Args:
            ...
            test: This is optional, it is used when invoking a training "dryrun", e.g.: yzlite train basic_example-test
