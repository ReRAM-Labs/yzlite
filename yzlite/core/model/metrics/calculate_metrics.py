...
def calculate_model_metrics(
    ...
    if not isinstance(model, (TfliteModel, KerasModel)):
        raise Exception(
            'Model must be an instance of a TfliteModel or KerasModel')
    ...
    def _process_model_layers(model):
        for model_layer in model.layers:
            ...
            if layer is None:
                logger.debug(
                    f'WARN: Model layer not included in metrics calculation, name: {model_layer.name}, type: {model_layer.__class__.__name__}')
                continue
