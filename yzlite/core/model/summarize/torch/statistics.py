...
from ._state_tree import StateTree, StateNode, report_format


def get_parent_node(root_node, stat_node_name):
    assert isinstance(root_node, StateNode)
...
def convert_leaf_modules_to_stat_tree(leaf_modules):
    ...
    create_index = 1
    root_node = StateNode(name='root', parent=None)
    for leaf_module_name, leaf_module in leaf_modules.items():
        ...
        for i in range(len(names)):
            ...
            node = StateNode(name=stat_node_name, parent=parent_node)
