...
def report_format(collected_nodes):
    ...
    for node in collected_nodes:
        ...
        data.append([
            ...
            inference_memory, Macs, duration, Flops, mread, mwrite
        ])
    df = pd.DataFrame(data)
    df.columns = [
        'module name', 'input shape', 'output shape', 'parameters',
        'memory(MB)', 'MACS', 'duration', 'FLOPS', 'MemRead(B)', 'MemWrite(B)'
    ]

    df['duration[%]'] = df['duration'] / (df['duration'].sum() + 1e-7)
    df['MemR+W(B)'] = df['MemRead(B)'] + df['MemWrite(B)']
    total_parameters_quantity = df['parameters'].sum()
    total_memory = df['memory(MB)'].sum()
    total_operation_quantity = df['MACS'].sum()
    ...
    total_df = pd.Series([
        ...
        total_flops, total_duration, mread, mwrite, total_memrw
                        ...
                        index=[
                            'parameters', 'memory(MB)', 'MACS', 'FLOPS',
