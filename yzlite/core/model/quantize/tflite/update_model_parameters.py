...
from yzlite.core.tflite.parameters import TfliteModelParameters, TFLITE_METADATA_TAG
from yzlite.core.yzlite import (
    load_yzlite_model, load_yzlite_model_with_path)
...
def update_model_parameters(
    ...
    if isinstance(model, TfliteModel) or (isinstance(model, str)
    ...
    elif isinstance(model, ) or isinstance(model, str):
        if isinstance(model, ):
            yzlite_model = model
            if not output:
        ...
        raise RuntimeError(
            'model argument must be model name, TfliteModel instance, or path to .tflite or .yzlite'
        )
    ...
    if output:
        ...
        # If the input was an  instance or name
    ...
    # If an  instance or name was given
    elif yzlite_model is not None:
