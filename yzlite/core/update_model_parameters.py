...
from .model import YZLiteModel, load_yzlite_model, load_yzlite_model_with_path
...
def update_model_parameters(
    ...
    Args:
        model: Either the name of a model
            ...
            the path to a `.tflite` model file or `.yzlite.zip` model archive
    ...
    if isinstance(model, TfliteModel) or (isinstance(model, str) and model.endswith('.tflite')):
        ...
        if isinstance(model, YZLiteModel):
        ...
        elif model.endswith('.yzlite.zip'):
            if params is None:
                raise Exception(
                    'Must provide "param" argument if "model" is a .yzlite.zip path')

            archive_path = model
            yzlite_model = load_yzlite_model_with_path(model, logger=logger)
            ...
            yzlite_model._attributes._entries['model_specification_path'].value = archive_path.replace(
                '.yzlite.zip', '.py')  # pylint: disable=protected-access
        ...
        raise RuntimeError(
            'model argument must be model name, YZLiteModel or TfliteModel instance, or path to .tflite or .yzlite.zip')
...
def add_default_parameters(
    ...
    for i in range(2):
        ...
        tflite_model.add_metadata(
            TFLITE_METADATA_TAG, serialized_model_parameters)
