...
from yzlite.utils.logger import get_logger, make_filelike
from yzlite.utils.python import append_exception_msg

from .model import (YZLiteModel, YZLiteModelEvent, load_tflite_model,
                    load_yzlite_model)
    ...
    **kwargs
    ...
    Args:
        model: The model to profile as either a :py:class:`yzlite.core.YZLiteModel` or :py:class:`yzlite.core.TfliteModel` instance,
            or the path to a `.tflite` or `.yzlite.zip` file
        ...
        build: If true, build the YZLITE Model as a .tflite before profiling
        ...
        post_process: This allows for post-processing the profiling results (e.g. uploading to a cloud) if supported by the given YZLiteModel
    ...
    profiling_model_results._model_name = (tflite_model.filename or 'my_model.tflite')[
        :-len('.tflite')]  # pylint: disable=protected-access
...
def profile_model_on_device(
    ...
    try:
        tflite_model_params = TfliteModelParameters.load_from_tflite_model(
            tflite_model)
        if 'runtime_memory_size' in tflite_model_params:
            # Ensure the memory size is -1 so it is calculated at runtime
            del tflite_model_params['runtime_memory_size']
    ...
    with SerialReader(
        ...
        if not serial_reader.read(timeout=60):
            raise TimeoutError(
                'Timed-out waiting for profiler on device to complete')

        # Check if the profiler failed
        if serial_reader.failed:
            raise RuntimeError(
                f'Profiler failed on device, err: {serial_reader.error_message}')
