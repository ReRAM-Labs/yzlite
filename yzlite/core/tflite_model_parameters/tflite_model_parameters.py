...
class TfliteModelParameters(FlatbufferDictionary):
    ...
    The model parameters are serialized using a `Flatbuffer schema <https://google.github.io/flatbuffers>`_.
    ...
    1. User Python script populates a :py:class:`~TfliteModelParameters` object
    2. Use :py:func:`~TfliteModelParameters.add_to_tflite_file` to serialize parameters and add to .tflite model metadata
    3. At a later time, use :py:func:`~TfliteModelParameters.load_from_tflite_file` to load parameters from .tflite model metadata
    4. Another user Python script accesses parameters in :py:class:`~TfliteModelParameters`

    .. note:: 
       The :py:class:`~TfliteModelParameters` object inherits :py:class:`~FlatbufferDictionary`
       which inherits the standard Python 'dict' class.
    ...
    def load_from_tflite_file(tflite_path: str):
        ...
        if not os.path.exists(tflite_path):
            raise FileNotFoundError(
                f'.tflite model file not found at: {tflite_path}')
    ...
    def load_from_tflite_model(tflite_model: TfliteModel):
        ...
        if parameters_flatbuffer is None:
            raise RuntimeError(
                f'Model parameters with tag {TFLITE_METADATA_TAG} not found in .tflite model file')
